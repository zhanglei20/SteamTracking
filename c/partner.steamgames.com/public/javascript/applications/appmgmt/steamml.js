(() => {
  (self.webpackChunkappmgmt_storeadmin =
    self.webpackChunkappmgmt_storeadmin || []).push([
    [4262],
    {
      chunkid: (module, module_exports, __webpack_require__) => {
        "use strict";
        __webpack_require__._(module_exports),
          __webpack_require__._(module_exports, {
            SteamMLRoutes: () => _,
            default: () => _,
          });
        var _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid"),
          _ = __webpack_require__._(_),
          _ = Object.defineProperty,
          _ = Object.getOwnPropertyDescriptor,
          _ = (_, _, _, _) => {
            for (
              var _ = _ > 1 ? void 0 : _ ? _(_, _) : _, _ = _.length - 1, _;
              _ >= 0;
              _--
            )
              (_ = _[_]) && (_ = (_ ? _(_, _, _) : _(_)) || _);
            return _ && _ && _(_, _, _), _;
          },
          _ = ((_) => (
            (_[(_.Unrequested = 0)] = "Unrequested"),
            (_[(_.Pending = 1)] = "Pending"),
            (_[(_.Valid = 2)] = "Valid"),
            (_[(_.Failed = 3)] = "Failed"),
            (_[(_.Rerequest = 4)] = "Rerequest"),
            _
          ))(_ || {});
        class _ {
          constructor() {
            (0, _._)(this);
          }
          getData(_, _, _) {
            const _ = this.eState == 1 || this.eState == 4;
            return (
              this.isExpired() &&
                !_ &&
                ((this.eState = 1),
                _()
                  .then((_) => {
                    if (this.eState == 4) {
                      this.expireData(), this.getData(_, _, _);
                      return;
                    }
                    _ ? (this.data = _(_)) : (this.data = _),
                      _
                        ? (this.delayNewData(_), (this.eState = 2))
                        : (this.eState = 3);
                  })
                  .catch((_) => {
                    (this.data = _(void 0)), (this.eState = 3);
                  })),
              this.data
            );
          }
          clearData() {
            (this.eState = 0), (this.data = null), (this.nExpirationTime = 0);
          }
          setDataPending() {
            this.eState = 1;
          }
          isDataPending() {
            return this.eState == 1;
          }
          setData(_, _) {
            (this.data = _), this.delayNewData(_), (this.eState = 2);
          }
          expireData() {
            this.nExpirationTime = 0;
          }
          isExpired() {
            const _ = Date.now() / 1e3;
            return this.nExpirationTime < _;
          }
          delayNewData(_) {
            if (_ <= 0) this.nExpirationTime = Number.MAX_SAFE_INTEGER;
            else {
              const _ = Date.now() / 1e3;
              this.nExpirationTime = _ + _;
            }
          }
          rerequestDataIfPending() {
            this.eState == 1 && (this.eState = 4);
          }
          getCachedData() {
            return this.data;
          }
          data = void 0;
          eState = 0;
          nExpirationTime = 0;
        }
        _([_._], _.prototype, "data", 2),
          _([_._], _.prototype, "nExpirationTime", 2);
        class _ {
          constructor() {
            (0, _._)(this);
          }
          getAsyncDataWrapper(_) {
            let _;
            return (
              this.m_mapKeyToDataWrapper.has(_)
                ? (_ = this.m_mapKeyToDataWrapper.get(_))
                : ((_ = new _()), this.m_mapKeyToDataWrapper.set(_, _)),
              _
            );
          }
          clearKey(_) {
            this.m_mapKeyToDataWrapper.has(_) &&
              this.m_mapKeyToDataWrapper.get(_).clearData(),
              this.m_mapKeyToDataWrapper.delete(_);
          }
          getAllElements() {
            return Array.from(this.m_mapKeyToDataWrapper.values()).map((_) =>
              _.getCachedData(),
            );
          }
          getAllElementsAndKeys() {
            let _ = [],
              _ = this.m_mapKeyToDataWrapper.entries(),
              _ = _.next();
            for (; !_.done; )
              _.push({
                key: _.value[0],
                data: _.value[1].getCachedData(),
              }),
                (_ = _.next());
            return _;
          }
          m_mapKeyToDataWrapper = new Map();
        }
        _([_._], _.prototype, "m_mapKeyToDataWrapper", 2);
        var _ = Object.defineProperty,
          _ = Object.getOwnPropertyDescriptor,
          _ = (_, _, _, _) => {
            for (
              var _ = _ > 1 ? void 0 : _ ? _(_, _) : _, _ = _.length - 1, _;
              _ >= 0;
              _--
            )
              (_ = _[_]) && (_ = (_ ? _(_, _, _) : _(_)) || _);
            return _ && _ && _(_, _, _), _;
          };
        const _ = class _ {
          constructor() {
            (0, _._)(this);
          }
          static s_SteamMLStore;
          m_asyncSchemaList = new _();
          m_asyncSchemaDetails = new _();
          m_asyncProblemList = new _();
          m_asyncProblemDetails = new _();
          static Get() {
            return (
              _.s_SteamMLStore ||
                ((_.s_SteamMLStore = new _()), _.s_SteamMLStore.Init()),
              _.s_SteamMLStore
            );
          }
          Init() {}
          GetSchemaList(_, _) {
            const _ = _ + "_" + _;
            return this.m_asyncSchemaList.getAsyncDataWrapper(_).getData(
              30,
              async () =>
                await _().get(
                  `${_._.PARTNER_BASE_URL}steamml/get_schemas?appid=${_}&includesteammlapp=${_}&sessionid=${(0, _._)()}`,
                ),
              (_) => _.data.schemas,
            );
          }
          GetSchemaDetails(_, _) {
            return this.m_asyncSchemaDetails.getAsyncDataWrapper(_).getData(
              30,
              async () =>
                await _().get(
                  `${_._.PARTNER_BASE_URL}steamml/get_schema_details?appid=${_}&schemaid=${_}&sessionid=${(0, _._)()}`,
                ),
              (_) => JSON.parse(_.data.schema_description_json),
            );
          }
          GetProblemList() {
            return this.m_asyncProblemList.getData(
              30,
              async () =>
                await _().get(
                  `${_._.PARTNER_BASE_URL}steamml/get_problems?sessionid=${(0, _._)()}`,
                ),
              (_) => _.data.problems,
            );
          }
          GetProblemDetails(_) {
            return this.m_asyncProblemDetails.getAsyncDataWrapper(_).getData(
              30,
              async () =>
                await _().get(
                  `${_._.PARTNER_BASE_URL}steamml/get_problem_details?problemid=${_}&sessionid=${(0, _._)()}`,
                ),
              (_) => _.data.problem,
            );
          }
          CreateProblem(_, _, _) {
            let _ = new FormData();
            return (
              _.append("sessionid", (0, _._)()),
              _.append("name", _),
              _.append("description", _),
              _.append("schemaids", _.join(",")),
              _()
                .post(`${_._.PARTNER_BASE_URL}steamml/create_problem`, _)
                .then(() => {
                  this.m_asyncProblemList.clearData();
                })
            );
          }
          EditProblemName(_, _) {
            let _ = new FormData();
            _.append("name", _), this.EditProblem(_, _);
          }
          EditProblemDesc(_, _) {
            let _ = new FormData();
            _.append("description", _), this.EditProblem(_, _);
          }
          EditProblemSchemas(_, _) {
            let _ = new FormData();
            _.append("schemaids", _.join(",")), this.EditProblem(_, _);
          }
          EditProblem(_, _) {
            return (
              _.append("sessionid", (0, _._)()),
              _.append("problemid", _.toString()),
              _()
                .post(`${_._.PARTNER_BASE_URL}steamml/edit_problem`, _)
                .then(() => {
                  this.m_asyncProblemDetails
                    .getAsyncDataWrapper(_)
                    .expireData();
                })
            );
          }
          DeleteProblem(_) {
            let _ = new FormData();
            return (
              _.append("sessionid", (0, _._)()),
              _.append("problemid", _.toString()),
              _()
                .post(`${_._.PARTNER_BASE_URL}steamml/delete_problem`, _)
                .then(() => {
                  this.m_asyncProblemList.clearData();
                })
            );
          }
        };
        _([_._], _.prototype, "m_asyncSchemaList", 2),
          _([_._], _.prototype, "m_asyncSchemaDetails", 2),
          _([_._], _.prototype, "m_asyncProblemList", 2),
          _([_._], _.prototype, "m_asyncProblemDetails", 2),
          _([_._], _.prototype, "Init", 1);
        let _ = _;
        var _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid"),
          _ = __webpack_require__("chunkid");
        const _ = (0, _._)((_) => {
          const _ = (0, _._)().schemaid == "0",
            _ = [
              {
                label: "816 - Dota Beta",
                value: "816",
              },
              {
                label: "570 - Dota",
                value: "570",
              },
              {
                label: "730 - CS:GO",
                value: "730",
              },
              {
                label: "1015410 - SteamML",
                value: "1015410",
              },
            ],
            [_, _] = (0, _.useState)(_[0].value);
          if (!_) return null;
          const _ = _.Get().GetSchemaList(parseInt(_), !0);
          if (!_) return null;
          let _ = [];
          for (const _ of _)
            _.push(
              (0, _.jsxs)(
                _._,
                {
                  _: _.SteamMLSchemas(_.appid, _.schemaid),
                  className: _.SchemaElement,
                  children: [
                    (0, _.jsxs)("div", {
                      className: _.HorizontalSection,
                      children: [
                        (0, _.jsx)("div", {
                          className: _.SchemaName,
                          children: _.name,
                        }),
                        (0, _.jsxs)("div", {
                          className: _.SchemaID,
                          children: [
                            (0, _.jsx)("span", {
                              className: _.Label,
                              children: "Schema ID",
                            }),
                            " ",
                            _.schemaid,
                          ],
                        }),
                      ],
                    }),
                    (0, _.jsxs)("div", {
                      className: _.AppID,
                      children: ["AppID ", _.appid],
                    }),
                    (0, _.jsxs)("div", {
                      className: _.HorizontalSection,
                      children: [
                        (0, _.jsxs)("div", {
                          className: _.RowCount,
                          children: [
                            (0, _.jsx)("span", {
                              className: _.Label,
                              children: "Rows:",
                            }),
                            " ",
                            _.row_count,
                          ],
                        }),
                        (0, _.jsxs)("div", {
                          className: _.KeepCount,
                          children: [
                            (0, _.jsx)("span", {
                              className: _.Label,
                              children: "Keep Count:",
                            }),
                            " ",
                            (0, _._)(_.keep_count),
                          ],
                        }),
                      ],
                    }),
                  ],
                },
                _.schemaid,
              ),
            );
          return (0, _.jsxs)("div", {
            className: _.SchemaList,
            children: [
              (0, _.jsx)("select", {
                className: _.SelectAppID,
                value: _,
                onChange: (_) => _(_.target.value),
                children: _.map((_) =>
                  (0, _.jsx)(
                    "option",
                    {
                      value: _.value,
                      children: _.label,
                    },
                    _.value,
                  ),
                ),
              }),
              _,
            ],
          });
        });
        function _(_) {
          switch (_) {
            case 1:
              return "int32";
            case 2:
              return "int64";
            case 3:
              return "bool";
            case 4:
              return "float";
            case 5:
              return "enum";
            case 6:
              return "timestamp";
          }
          return "";
        }
        function _(_) {
          switch (_) {
            case 1:
              return _.TypeInt32;
            case 2:
              return _.TypeInt64;
            case 3:
              return _.TypeBool;
            case 4:
              return _.TypeFloat;
            case 5:
              return _.TypeEnum;
            case 6:
              return _.TypeTimestamp;
          }
          return "";
        }
        function _(_, _, _) {
          let _;
          const _ = _.indexOf(_.name),
            _ = _ == -1 && !_.count;
          return (
            _.structure
              ? (_ = (0, _.jsxs)("div", {
                  className: (0, _._)(
                    _.SchemaDetailsElement,
                    _.SchemaDetailsStruct,
                  ),
                  children: [
                    !_.count &&
                      (0, _.jsxs)("div", {
                        className: _.SchemaDetailsStructRow,
                        onClick: () => {
                          _(
                            _
                              ? [..._, _.name]
                              : _.slice(0, _).concat(_.slice(_ + 1)),
                          );
                        },
                        children: [
                          (0, _.jsx)("div", {
                            className: (0, _._)(
                              _ && _.Collapsed,
                              _.CollapseIcon,
                            ),
                            children: "\u25BC",
                          }),
                          (0, _.jsxs)("div", {
                            className: _.SchemaDetailsElementBody,
                            children: [
                              (0, _.jsx)("div", {
                                className: _.ElementName,
                                children: _.name,
                              }),
                              (0, _.jsx)("div", {
                                className: _.TypeName,
                                children: "STRUCT",
                              }),
                            ],
                          }),
                        ],
                      }),
                    _.structure.member.map((_) =>
                      (0, _.jsx)(
                        "div",
                        {
                          className: (0, _._)(
                            _ && _.IndentCollapsed,
                            _.StructIndent,
                          ),
                          children: _(_, _, _),
                        },
                        _.name,
                      ),
                    ),
                  ],
                }))
              : _.array && _.array.primitive
                ? (_ = (0, _.jsx)("div", {
                    className: (0, _._)(
                      _.SchemaDetailsElement,
                      _.SchemaDetailsArray,
                    ),
                    children: (0, _.jsxs)("div", {
                      className: (0, _._)(
                        _.SchemaDetailsElementBody,
                        _(_.array.primitive.type_info.type),
                      ),
                      children: [
                        (0, _.jsx)("div", {
                          className: _.ElementName,
                          children: _.name,
                        }),
                        (0, _.jsxs)("div", {
                          className: _.TypeName,
                          children: [
                            _(_.array.primitive.type_info.type),
                            " [",
                            _.array.max_length,
                            "]",
                          ],
                        }),
                      ],
                    }),
                  }))
                : _.array && _.array.structure
                  ? (_ = (0, _.jsxs)("div", {
                      className: (0, _._)(
                        _.SchemaDetailsElement,
                        _.SchemaDetailsArray,
                      ),
                      children: [
                        !_.count &&
                          (0, _.jsxs)("div", {
                            className: _.SchemaDetailsStructRow,
                            onClick: () => {
                              _(
                                _
                                  ? [..._, _.name]
                                  : _.slice(0, _).concat(_.slice(_ + 1)),
                              );
                            },
                            children: [
                              (0, _.jsx)("div", {
                                className: (0, _._)(
                                  _ && _.Collapsed,
                                  _.CollapseIcon,
                                ),
                                children: "\u25BC",
                              }),
                              (0, _.jsxs)("div", {
                                className: _.SchemaDetailsElementBody,
                                children: [
                                  (0, _.jsx)("div", {
                                    className: _.ElementName,
                                    children: _.name,
                                  }),
                                  (0, _.jsxs)("div", {
                                    className: _.TypeName,
                                    children: [
                                      "STRUCT [",
                                      _.array.max_length,
                                      "]",
                                    ],
                                  }),
                                ],
                              }),
                            ],
                          }),
                        _.array.structure.member.map((_) =>
                          (0, _.jsx)(
                            "div",
                            {
                              className: (0, _._)(
                                _ && _.IndentCollapsed,
                                _.StructIndent,
                              ),
                              children: _(_, _, _),
                            },
                            _.name,
                          ),
                        ),
                      ],
                    }))
                  : _.primitive &&
                    (_ = (0, _.jsx)("div", {
                      className: (0, _._)(
                        _.SchemaDetailsElement,
                        _.SchemaDetailsArray,
                      ),
                      children: (0, _.jsxs)("div", {
                        className: (0, _._)(
                          _.SchemaDetailsElementBody,
                          _(_.primitive.type_info.type),
                        ),
                        children: [
                          (0, _.jsx)("div", {
                            className: _.ElementName,
                            children: _.name,
                          }),
                          (0, _.jsx)("div", {
                            className: _.TypeName,
                            children: _(_.primitive.type_info.type),
                          }),
                        ],
                      }),
                    })),
            _
          );
        }
        const _ = (0, _._)((_) => {
            const _ = (0, _._)(),
              _ = _.schemaid == "0",
              [_, _] = (0, _.useState)([]);
            if (_) return null;
            const _ = _.Get().GetSchemaDetails(parseInt(_.appid), _.schemaid);
            return _
              ? (console.log((0, _._)(_)),
                (0, _.jsxs)("div", {
                  className: _.SchemaDetails,
                  children: [
                    (0, _.jsx)(_._, {
                      _: _.SteamMLSchemas("0"),
                      className: _.Back,
                      children: "<< BACK",
                    }),
                    (0, _.jsx)("div", {
                      className: _.SchemaDetailsHeader,
                      children: (0, _.jsxs)("div", {
                        className: _.SchemaDetailsTitle,
                        children: [
                          (0, _.jsxs)("div", {
                            className: _.SchemaDetailsName,
                            children: ["SCHEMA ", _.name],
                          }),
                          (0, _.jsx)("div", {
                            className: _.SchemaDetailsSchemaID,
                            children: _.schemaid,
                          }),
                        ],
                      }),
                    }),
                    (0, _.jsx)("div", {
                      className: _.SchemaDetailsElements,
                      children: _(_, _, _),
                    }),
                  ],
                }))
              : null;
          }),
          _ = (0, _._)((_) =>
            (0, _.jsxs)("div", {
              className: _.SchemaPage,
              children: [(0, _.jsx)(_, {}), (0, _.jsx)(_, {})],
            }),
          ),
          _ = (_) =>
            (0, _.jsx)("div", {
              className: (0, _._)(
                _.SteamMLButton,
                _.acceptStyle && _.AcceptButton,
                _.cancelStyle && _.CancelButton,
                _.disabled && _.Disabled,
              ),
              style: {
                minWidth: _.minWidth,
              },
              onClick: (_) => {
                _.disabled || (_.onClick(), _.stopPropagation());
              },
              children: (0, _.jsx)("div", {
                className: _.Inner,
                children: _.children,
              }),
            }),
          _ = ({ problem: _ }) => (
            console.log((0, _._)(_)),
            (0, _.jsxs)(_._, {
              _: _.SteamMLProblems(_.problemid),
              className: _.ProblemEntry,
              children: [
                (0, _.jsx)("div", {
                  className: _.ProblemName,
                  children: _.name,
                }),
                !_.active &&
                  (0, _.jsx)("div", {
                    className: _.Inactive,
                    children: "Inactive",
                  }),
                (0, _.jsx)("div", {
                  className: _.ProblemDescription,
                  children: _.problem_description,
                }),
                (0, _.jsx)("div", {
                  className: _.ProblemID,
                  children: _.problemid,
                }),
                (0, _.jsxs)("div", {
                  className: _.Dates,
                  children: [
                    (0, _.jsxs)("div", {
                      className: _.CreatedDate,
                      children: [
                        "Created ",
                        new Date(_.create_time * 1e3).toLocaleDateString([
                          "en-us",
                        ]),
                      ],
                    }),
                    (0, _.jsxs)("div", {
                      className: _.UpdatedDate,
                      children: [
                        "Last modified ",
                        new Date(_.update_time * 1e3).toLocaleDateString([
                          "en-us",
                        ]),
                      ],
                    }),
                  ],
                }),
              ],
            })
          ),
          _ = (_) => {
            const [_, _] = (0, _.useState)(!1),
              [_, _] = (0, _.useState)(""),
              [_, _] = (0, _.useState)(""),
              [_, _] = (0, _.useState)([]),
              _ = _.Get()
                .GetSchemaList(0, !0)
                ?.sort((_, _) => (_.name < _.name ? -1 : 1))
                ?.filter((_, _, _) => _ == 0 || _.name != _[_ - 1].name),
              _ = _.length > 0 && _.length > 0 && _.length > 0,
              _ = () => {
                _.Get().CreateProblem(_, _, _);
              };
            return (0, _.jsxs)("div", {
              className: (0, _._)(
                _.AddNewProblem,
                !_ && _.IsPrompt,
                _ && _.IsHeader,
              ),
              onClick: () => _(!0),
              children: [
                (0, _.jsxs)("div", {
                  className: _.NewProblemHeader,
                  children: [
                    (0, _.jsx)("img", {
                      className: _.PlusSymbol,
                      src: `${_._.IMG_URL}webui/storeadmin/plus.png`,
                    }),
                    (0, _.jsx)("div", {
                      className: _.AddProblemHeader,
                      children: "Add New Problem" + (_ ? "" : "?"),
                    }),
                  ],
                }),
                (0, _.jsxs)("div", {
                  className: _.NewProblemDescription,
                  children: [
                    (0, _.jsxs)("div", {
                      className: _.LabelValue,
                      children: [
                        (0, _.jsx)("div", {
                          className: _.Label,
                          children: "Name",
                        }),
                        (0, _.jsx)("input", {
                          className: _.Value,
                          type: "text",
                          value: _,
                          onChange: (_) => _(_.target.value),
                        }),
                      ],
                    }),
                    (0, _.jsxs)("div", {
                      className: _.LabelValue,
                      children: [
                        (0, _.jsx)("div", {
                          className: _.Label,
                          children: "Description",
                        }),
                        (0, _.jsx)("input", {
                          className: _.Value,
                          type: "text",
                          value: _,
                          onChange: (_) => _(_.target.value),
                        }),
                      ],
                    }),
                    _ &&
                      (0, _.jsxs)("div", {
                        className: _.SchemaListContainer,
                        children: [
                          (0, _.jsx)("div", {
                            className: _.SchemaSelectListTitle,
                            children: "Select Schemas to Include",
                          }),
                          (0, _.jsx)("div", {
                            className: _.SchemaSelectList,
                            children: _.map((_) => {
                              const _ = _.indexOf(_.schemaid) != -1;
                              return (0, _.jsx)(
                                "div",
                                {
                                  className: (0, _._)(
                                    _.SchemaOption,
                                    _ && _.Selected,
                                  ),
                                  onClick: () =>
                                    _(
                                      _
                                        ? _.filter((_) => _ != _.schemaid)
                                        : _.concat(_.schemaid),
                                    ),
                                  children: _.name,
                                },
                                _.schemaid,
                              );
                            }),
                          }),
                        ],
                      }),
                    (0, _.jsxs)("div", {
                      className: _.ButtonContainer,
                      children: [
                        (0, _.jsx)(_, {
                          acceptStyle: !0,
                          minWidth: 100,
                          disabled: !_,
                          onClick: () => _ && _(),
                          children: "Create",
                        }),
                        (0, _.jsx)(_, {
                          cancelStyle: !0,
                          minWidth: 100,
                          onClick: () => _(!1),
                          children: "Cancel",
                        }),
                      ],
                    }),
                  ],
                }),
              ],
            });
          },
          _ = (0, _._)(({ schemaid: _ }) => {
            const _ = _.Get().GetSchemaList(0, !0);
            let _;
            if (_) {
              const _ = _.filter((_) => _.schemaid == _);
              _.length > 0 && (_ = _[0]);
            }
            return (0, _.jsx)("div", {
              className: _.SchemaElement,
              children:
                _ &&
                (0, _.jsx)("div", {
                  className: _.SchemaName,
                  children: _.name,
                }),
            });
          }),
          _ = (_) => {
            (0, _.useEffect)(() => {
              const _ = (_) => {
                _.keyCode === 27 && _();
              };
              return (
                window.addEventListener("keydown", _),
                () => {
                  window.removeEventListener("keydown", _);
                }
              );
            }, [_]);
          },
          _ = (0, _._)(({ problem: _ }) => {
            const [_, _] = (0, _.useState)(!1),
              [_, _] = (0, _.useState)(_.name),
              [_, _] = (0, _.useState)(!1),
              [_, _] = (0, _.useState)(_.problem_description),
              [_, _] = (0, _.useState)(!1),
              [_, _] = (0, _.useState)(_.schemaid),
              _ = (0, _._)(),
              _ = (0, _.useRef)(null),
              _ = (0, _.useRef)(null),
              _ = parseInt(_.problemid),
              _ = _.Get()
                .GetSchemaList(0, !0)
                ?.slice()
                .sort((_, _) => (_.name < _.name ? -1 : 1))
                ?.filter((_, _, _) => _ == 0 || _.name != _[_ - 1].name);
            _(() => {
              _(), _(), _(_.problem_description), _(!1);
            });
            const _ = () => {
                _(_.name), _(!1);
              },
              _ = () => {
                _(_.problem_description), _(!1);
              },
              _ = () => {
                _(_.schemaid), _(!1);
              },
              _ = () => {
                _(), _(), _(!0);
              },
              _ = () => {
                _(), _(), _(!0);
              },
              _ = () => {
                _(), _(), _(!0);
              },
              _ = () => {
                _.Get().EditProblemName(_, _), (_.name = _), _(!1);
              },
              _ = () => {
                _.Get().EditProblemDesc(_, _),
                  (_.problem_description = _),
                  _(!1);
              },
              _ = () => {
                _.Get().EditProblemSchemas(_, _), (_.schemaid = _), _(!1);
              },
              _ = () => {
                _.Get().DeleteProblem(_), _.push(_.SteamMLProblems(0));
              };
            return (
              _ && _.current.focus(),
              _ && _.current.focus(),
              (0, _.jsxs)("div", {
                className: _.ProblemDetails,
                children: [
                  (0, _.jsx)(_._, {
                    _: _.SteamMLProblems(0),
                    className: _.BackButton,
                    children: "<< BACK ",
                  }),
                  (0, _.jsxs)("div", {
                    className: _.ProblemDetailsHeaderContainer,
                    children: [
                      (0, _.jsx)("div", {
                        className: (0, _._)(_.ProblemName, _ && _.Hidden),
                        onClick: (_) => _(),
                        children: _.name,
                      }),
                      (0, _.jsx)("form", {
                        className: _ ? void 0 : _.Hidden,
                        onSubmit: (_) => {
                          _(), _.preventDefault();
                        },
                        children: (0, _.jsx)("input", {
                          ref: _,
                          className: _.ProblemNameEditing,
                          type: "text",
                          value: _,
                          onBlur: () => _(),
                          onFocus: (_) => _.target.select(),
                          onChange: (_) => _(_.target.value),
                        }),
                      }),
                      (0, _.jsx)("div", {
                        className: (0, _._)(
                          _.ProblemDescription,
                          _ && _.Hidden,
                        ),
                        onClick: (_) => _(),
                        children: _.problem_description,
                      }),
                      (0, _.jsx)("form", {
                        className: _ ? void 0 : _.Hidden,
                        onSubmit: (_) => {
                          _(), _.preventDefault();
                        },
                        children: (0, _.jsx)("input", {
                          ref: _,
                          className: _.ProblemDescEditing,
                          type: "text",
                          value: _,
                          onBlur: () => _(),
                          onFocus: (_) => _.target.select(),
                          onChange: (_) => _(_.target.value),
                        }),
                      }),
                      (0, _.jsx)("div", {
                        className: _.ProblemID,
                        children: _.problemid,
                      }),
                      (0, _.jsxs)("div", {
                        className: _.Dates,
                        children: [
                          (0, _.jsxs)("div", {
                            className: _.CreatedDate,
                            children: [
                              "Created ",
                              new Date(_.create_time * 1e3).toLocaleDateString([
                                "en-us",
                              ]),
                            ],
                          }),
                          (0, _.jsxs)("div", {
                            className: _.UpdatedDate,
                            children: [
                              "Last modified ",
                              new Date(_.update_time * 1e3).toLocaleDateString([
                                "en-us",
                              ]),
                            ],
                          }),
                        ],
                      }),
                    ],
                  }),
                  (0, _.jsxs)("div", {
                    className: _.SchemaListContainer,
                    children: [
                      (0, _.jsx)("div", {
                        className: _.SchemaListHeader,
                        children: "Schemas:",
                      }),
                      (0, _.jsx)("div", {
                        className: _.SchemaListEdit,
                        onClick: () => _(),
                        children: "Edit",
                      }),
                      (0, _.jsx)("div", {
                        className: (0, _._)(_.SchemaList, _ && _.Hidden),
                        children: _.schemaid.map((_) =>
                          (0, _.jsx)(
                            _,
                            {
                              schemaid: _,
                            },
                            _,
                          ),
                        ),
                      }),
                      (0, _.jsx)("div", {
                        className: (0, _._)(
                          _.SchemaSelectList,
                          (!_ || !_) && _.Hidden,
                        ),
                        children:
                          _ &&
                          _.map((_) => {
                            const _ = _.indexOf(_.schemaid) != -1;
                            return (0, _.jsx)(
                              "div",
                              {
                                className: (0, _._)(
                                  _.SchemaOption,
                                  _ && _.Selected,
                                ),
                                onClick: () =>
                                  _(
                                    _
                                      ? _.filter((_) => _ != _.schemaid)
                                      : _.concat(_.schemaid),
                                  ),
                                children: _.name,
                              },
                              _.schemaid,
                            );
                          }),
                      }),
                      _ &&
                        (0, _.jsxs)("div", {
                          className: _.ButtonContainer,
                          children: [
                            (0, _.jsx)(_, {
                              minWidth: 100,
                              acceptStyle: !0,
                              disabled: _.length == 0,
                              onClick: () => _(),
                              children: "Accept",
                            }),
                            (0, _.jsx)(_, {
                              minWidth: 100,
                              cancelStyle: !0,
                              onClick: () => _(),
                              children: "Cancel",
                            }),
                          ],
                        }),
                      (0, _.jsx)("div", {
                        className: (0, _._)(_.DeleteLink, _ && _.Hidden),
                        onClick: () => _(),
                        children: "Delete Problem",
                      }),
                    ],
                  }),
                ],
              })
            );
          }),
          _ = (0, _._)((_) => {
            const _ = (0, _._)(),
              _ = _.problemid == "0";
            let _, _;
            return (
              _
                ? (_ = _.Get().GetProblemList())
                : (_ = _.Get().GetProblemDetails(parseInt(_.problemid))),
              (0, _.jsxs)("div", {
                className: _.ProblemPage,
                children: [
                  _ &&
                    _ &&
                    (0, _.jsxs)("div", {
                      className: _.ProblemListContainer,
                      children: [
                        _.map((_) =>
                          (0, _.jsx)(
                            _,
                            {
                              problem: _,
                            },
                            _.problemid,
                          ),
                        ),
                        _.length == 0 &&
                          (0, _.jsx)("div", {
                            className: _.NoProblems,
                            children: "No Existing Problems",
                          }),
                        (0, _.jsx)(_, {}),
                      ],
                    }),
                  !_ &&
                    _ &&
                    (0, _.jsx)("div", {
                      className: _.ProblemDetails,
                      children: (0, _.jsx)(_, {
                        problem: _,
                      }),
                    }),
                ],
              })
            );
          }),
          _ = {
            SteamMLBase: () => "steamml",
            SteamMLSchemas: (_, _) => `/schemas/${_ ?? "0"}/${_ ?? "0"}`,
            SteamMLModels: () => "/models",
            SteamMLProblems: (_) => `/problems/${_ ?? "0"}`,
          };
        class _ extends _.Component {
          render() {
            return (0, _.jsx)("div", {
              className: _.Background,
              children: (0, _.jsxs)(_._, {
                basename: (0, _._)() + _.SteamMLBase(),
                children: [
                  (0, _.jsx)(_, {}),
                  (0, _.jsxs)(_._, {
                    children: [
                      (0, _.jsx)(_._, {
                        exact: !0,
                        path: _.SteamMLSchemas(":appid", ":schemaid"),
                        component: _,
                      }),
                      (0, _.jsx)(_._, {
                        exact: !0,
                        path: _.SteamMLModels(),
                        component: _,
                      }),
                      (0, _.jsx)(_._, {
                        exact: !0,
                        path: _.SteamMLProblems(":problemid"),
                        component: _,
                      }),
                      (0, _.jsx)(_._, {
                        exact: !0,
                        path: "/",
                        children: (0, _.jsx)(_._, {
                          _: _.SteamMLSchemas(),
                        }),
                      }),
                    ],
                  }),
                ],
              }),
            });
          }
        }
        const _ = (_) => {
            const _ = (0, _._)(),
              _ = (0, _._)(_.pathname, {
                path: _.SteamMLSchemas(":appid", ":schemaid"),
                exact: !0,
              }),
              _ = (0, _._)(_.pathname, {
                path: _.SteamMLModels(),
                exact: !0,
              }),
              _ = (0, _._)(_.pathname, {
                path: _.SteamMLProblems(":problemid"),
                exact: !0,
              });
            return (0, _.jsxs)("div", {
              className: _.Header,
              children: [
                (0, _.jsx)(_._, {
                  _: _.SteamMLSchemas(),
                  className: (0, _._)(_.HeaderOption, _ && _.Selected),
                  children: "SCHEMAS",
                }),
                (0, _.jsx)(_._, {
                  _: _.SteamMLModels(),
                  className: (0, _._)(_.HeaderOption, _ && _.Selected),
                  children: "MODELS",
                }),
                (0, _.jsx)(_._, {
                  _: _.SteamMLProblems(),
                  className: (0, _._)(_.HeaderOption, _ && _.Selected),
                  children: "PROBLEMS",
                }),
              ],
            });
          },
          _ = (_) =>
            (0, _.jsx)("div", {
              children: "MODEL PAGE",
            });
      },
      chunkid: (module) => {
        module.exports = {
          Background: "msjGfUYNm8rJ4bLS19n2g",
          SteamMLButton: "_2yReXdObU3Ij9YBkzvgYQ7",
          AcceptButton: "_2tY__PC6O-mUh8jRVOh_S7",
          Inner: "_1Zoc2tlDWolWjsI-W18Wvf",
          CancelButton: "_2BCE0thhjq41eNyAVsDAP4",
          Disabled: "_1W_vxrkrcK4BoMnWdzcIo2",
          FullSize: "_1-nKcLiXhx0iBBulrD55Xo",
          Header: "_35ajnHTsONJ6abrtNB0tiG",
          HeaderOption: "_3KFf6Xq-7-jJ29jn3bW4PL",
          Selected: "OilN-vRxEfOtVM-aw6H4Y",
          SchemaPage: "_36OV-RyYj5N9zIaubjlkDo",
          SchemaList: "_3r0tlX7BEcUjUC0nk5CW8i",
          SelectAppID: "_3nezXM-f76hluq1iA1sBte",
          Hidden: "_1VPtlX8LmlSvqpBchxC_7s",
          SchemaElement: "_1xjli_L5ZMmh-nvw9GnUWQ",
          Label: "_3uE4kPiSHlhS4R4wHqX-fN",
          SchemaName: "_36bwAwPhrWLdDlGBGrT1k1",
          AppID: "_3UaebM6BfC-B0yVyI-eoIJ",
          HorizontalSection: "_1rTEk-J6WyXu1fqOq9PjzY",
          RowCount: "gPsPXhyVjxvOBH82Veunm",
          KeepCount: "_2RxONnczAg5Y1_Xnnd7Grt",
          SchemaDetails: "_1gFQfWKXhOQbkaNjbGVU16",
          Back: "_2m9wkgXpcf3-99TaQKL0tE",
          SchemaDetailsHeader: "cUNhZfaxsGzKaFQAx4vvf",
          SchemaDetailsBack: "_15COwPUDCRCEDUMsYujCkU",
          SchemaDetailsTitle: "_9IFURqUFtrCAO5kbHAO_U",
          SchemaDetailsName: "CblW8QTasV2DO4cRW0prn",
          SchemaDetailsSchemaID: "_16qUhwW_c8Pbmu5Z6xinEO",
          SchemaDetailsElements: "_3DsdfooA5ZsBVzHlo0spou",
          SchemaDetailsElement: "_2_-ubEumKcrrljcqkDz-nd",
          SchemaDetailsElementBody: "_1lLqQDLnUIIdV6R57346No",
          TypeInt32: "_3q6FZ-EpaofO1UX84CRKPz",
          TypeName: "_11e6zJoDlODofUVbR3Lua3",
          TypeInt64: "_1sUna6-XvlT6BdFU-xDxr_",
          TypeBool: "_3QiDC_egBPUil1L8Lxw-E6",
          TypeFloat: "DfgR36rhRvbWATh2rkEU3",
          TypeEnum: "_3w9sP-z9NZw5JwDMJvGha_",
          TypeTimestamp: "_33yo73pTOoKCyF87NLP4Io",
          ElementName: "_1v_wKNlVbQ4nQF8_Zbhiea",
          SchemaDetailsStructRow: "lWQfKVb7uqk9UKbjyyGIM",
          CollapseIcon: "_1vRxM4k_iJiVi6F1fzS1e3",
          Collapsed: "_3qRbu04vYScSY1knrHbExY",
          StructIndent: "_1bga0T4ksdggWPn8sb1tmQ",
          IndentCollapsed: "_1OU7XyPtFGtCm92v_LX-be",
          ProblemPage: "_3F8_Lyio-BwNK1Z5Pbu5vv",
          ProblemListContainer: "_35Ll4m_-B9fWDt-no-d8xp",
          ProblemListHeader: "_21PbN-aJWj8YJP-4YCaaVU",
          NoProblems: "IRyHOENJwvmX3tAY16ZaS",
          ProblemEntry: "_13BKEPL1RjPkUJgyypi6LX",
          ProblemName: "_3Hgxj-6wNEi2QJdiNmSEq8",
          Inactive: "TW09gRIZG46PUd8KEpPJM",
          ProblemDescription: "_3qnlNmBMwzicL--uJDt0xW",
          ProblemID: "_2yjPq_vUBe5vX8477Nd6Lb",
          Dates: "_3jJSRWPIEMmq_zw-n_Vmuj",
          CreatedDate: "_3nOnI7HpOqBDP7GOYro3RB",
          UpdatedDate: "_3gM54QZsU-d3rfw7v88yoF",
          ProblemSchemaList: "_2k2xZ2WqcQSptTfC2u-1WK",
          AddNewProblem: "_1IaIKe5N5mtG4ReCT5Nfnk",
          IsPrompt: "_1F_iVaudtZtIBggLz8TgsQ",
          NewProblemHeader: "_2Uxe1WauLy_WSI8HBMDmpb",
          PlusSymbol: "_2zfyC9pxT6ynYwfsxjjIcc",
          NewProblemDescription: "_3vIDf6PJy59Oae8lfCLYRT",
          IsHeader: "_1vZgEO-rIh1azL1QsqnUkE",
          AddProblemHeader: "_3dXukBGeFSKrjNRapz-vdw",
          LabelValue: "_3wqW3cCIDXXgA0L0ry28IN",
          Value: "_3RWed3mcqH6MmHhBgbkBGD",
          SchemaListContainer: "_3TzDRcTE7EvonQr-6ImEtT",
          SchemaSelectListTitle: "_37bp6yYCYbU-Aaxc6idCSh",
          ButtonContainer: "Sp68rQoQKv4sPj4vE-rm6",
          SchemaSelectList: "_3spVxRuLYDKpX2IsKvxhXN",
          SchemaOption: "_2eV2TTGJXmq22-NxPpT3eI",
          ProblemDetails: "UGqiqHtL23JkoCU_MSnsq",
          BackButton: "_1Sb8R2jk0DatSLTFo4nULP",
          ProblemDetailsHeaderContainer: "_28zUxX41jCodxcCCVF55ZH",
          ProblemNameEditing: "_2268oRWsoMSsQ4JcKBxJlp",
          ProblemDescEditing: "VSvOsoK438LYv7WiV-r91",
          SchemaListHeader: "_3rvzlEx7gsz_CuUwlLbLeL",
          SchemaListEdit: "_1Ch8t8kvrdudLjhmNW6D86",
          DeleteLink: "_2lI41nfrJhWqIcuoYAfdcC",
        };
      },
    },
  ]);
})();
