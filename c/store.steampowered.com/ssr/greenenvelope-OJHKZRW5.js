...
function _(_, _, _, _) {
  ...
  if (_.Get().BIsStoreItemMissing(_, _)) return [void 0, _];
  if (!_.Get().BHasStoreItem(_, _, _)) return [void 0, _];
  let _ = _.Get().GetStoreItemWithLegacyVisibilityCheck(_, _);
  return _ ? [_, _] : [null, _];
}
function _(_, _, _) {
  return _(_, _.k_EStoreItemType_App, _, _);
}
function _(_, _, _) {
  let [_, _] = _(_, _, _),
    _;
  _?.GetStoreItemType() == _.k_EStoreItemType_Package &&
...
function _(_) {
  let {
    ...
    _ =
      _.package_id > 0 ? _.k_EStoreItemType_Package : _.k_EStoreItemType_Bundle,
    [_] = _(_, _, {
