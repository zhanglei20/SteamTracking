...
var _ = 60 * 1e3,
  _ = {
    ...
    EditItem: (_, _, _) =>
      `${_.COMMUNITY_BASE_URL}sharedfiles/edititem/?appid=${_}&filetype=${_}${_ ? `&id=` + _ : ``}`,
    VotingQueue: (_, _ = {}) =>
      _(`votingqueue`, {
        ..._,
        appid: _,
...
function _(_) {
  return _
    ? _.COMMUNITY_BASE_URL + `sharedfiles/itemdetails/?id=` + _
    : _.COMMUNITY_BASE_URL;
}
function _(_, _) {
  if (!_) return _.COMMUNITY_BASE_URL;
  let _ = _ && _ > 1 ? `&p=${_}` : ``;
  return `${_.COMMUNITY_BASE_URL}sharedfiles/itemdetails/changelog/?id=${_}${_}`;
}
function _(_) {
  return _
    ? `${_.COMMUNITY_BASE_URL}sharedfiles/itemdetails/stats/?id=${_}`
    : _.COMMUNITY_BASE_URL;
}
function _(_) {
  return _
    ? `${_.COMMUNITY_BASE_URL}sharedfiles/itemdetails/managepreviews/?id=${_}`
    : _.COMMUNITY_BASE_URL;
}
function _(_) {
  return _
    ? `${_.COMMUNITY_BASE_URL}sharedfiles/itemdetails/merchartwork/?id=${_}`
    : _.COMMUNITY_BASE_URL;
}
function _(_) {
  return _
    ? `${_.COMMUNITY_BASE_URL}sharedfiles/itemdetails/managechildren/?id=${_}`
    : _.COMMUNITY_BASE_URL;
}
function _(_) {
  return _(_);
...
function _(_) {
  return _({
    queryKey: _(_),
    queryFn: async () => _(`/workshop/actions`, `GetUGCContributorData`, _),
    staleTime: _,
  });
}
function _(_) {
