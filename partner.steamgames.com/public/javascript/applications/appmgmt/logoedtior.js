/**** (c) Valve Corporation. Use is governed by the terms of the Steam Subscriber Agreement http://store.steampowered.com/subscriber_agreement/.
 ****/
(self.webpackChunkappmgmt_storeadmin =
  self.webpackChunkappmgmt_storeadmin || []).push([
  [2995],
  {
    17221: (e) => {
      e.exports = {
        Container: "ImYU51-pS-0X9msMmxX5U",
        StickyHeader: "_2C2bsvp1XpB7NsNiirMAj6",
        Row: "_3i6Bc-bUSZZufhOjYDDRrI",
        ActionSection: "_18DZH0tsHhnLbHhjprfCRF",
        StatusAndStats: "_2dtS1KDBcIhTDkOnFTk7Ua",
        PermanentlyUnavailable: "dy473P5st43tTAJQsiyc",
        GameStatsSection: "_12DXxXXehZReMOx0cv1ua9",
        RightControls: "_18x3wwmqB0GFmNWajYzdXH",
        AppButtonsContainer: "_2UjH56C2fdwZd71OvxjJEd",
        GameInfoButton: "n3SLdJhMc8AbyBVKvtDlc",
        Arrow: "C0opAYh09LX9eIKNjbCoo",
        MenuButton: "_19jdYGZ9QxMVLi4FSegTK7",
        DetailsSection: "_3Ajg1f_C9nJ7a4if21xAgr",
        DetailsSectionStatus: "_2ZJjHct-OxGywoEu8l2mBP",
        DetailsSectionExtra: "_1wzuGRGHrCWzrbJ6qNQjKW",
        DetailsProgressContainer: "_1pcsxtyzCGpeC4LckvIlx1",
        DetailsProgressBar: "_2KOE1-80Tg9sYWzP6pOOyK",
        InvalidPlatform: "_31Ep4CsFJ5Lnp9G_oAKg6",
        Icons: "_3sb86ANNDt2jmea4cqXhen",
        Icon: "fJP-Y0gsxw4u2kJyP4P87",
        MiniAchievements: "omt3iZWkuU82eAga19_qR",
        AchievementLeft: "wiXRiLNn1CmlgeSNkPpsz",
        AchievementRight: "_2LYsH2qimRzqo3mYoLGYs1",
        AchievementProgressRow: "_2cJRwyNRpMI3ugyFyyRA-k",
        AchievementLabel: "_2ZFedrb3wNI-IeqX4Y7T9Z",
        AchievementCountLabel: "_1hPIvyPzs8wsHJgpb5keGa",
        Playtime: "_2JGJBdEjwfj_hBZTBW7J4D",
        PlaytimeLeft: "_2QY4pWRCZO2A7Ty6NOoj1a",
        PlaytimeRight: "_1LPuQL5qNQWgfbOJ8rasxe",
        PlaytimeLabel: "_3MSPYfEyMCKeRFKUQdqkOk",
        PlaytimeInfo: "_1SIEL9hMmB2l6wVjuRegg-",
        LastPlayed: "oWVFWCB2yNB-8wRIYTTJx",
        LastPlayedRight: "WRJ1Xt1UmbuaEZbR9_dnb",
        LastPlayedLabel: "_2-xkMAdfmS2CQmkYOWBGaq",
        LastPlayedInfo: "_1fGHkdu15mnItlF4reGbAB",
        RecentlyUpdated: "_1bxok-6eJWWLXSRsnza8Te",
        RecentlyUpdatedIcon: "aYj4F35HU2LPQaSLUWbsS",
        RecentlyUpdatedText: "_3rzHtELvn7kPM3aPi_6PL0",
        RecentlyUpdatedLink: "_3YyraL8CvXr7ispfZsVJD_",
        PlayButtonContainer: "_3dlYJa3Q0oFTATvDrSB6Rj",
        PlayButton: "_3MzeFk7QqBUU3-eahz5UWy",
        Green: "Y4siik6dD1l-cXer5nPxZ",
        Disabled: "o6V6tnMncxMeACPbqGtaV",
        NoAction: "GALfzJzn7Kgna0dhEzPpE",
        ThrobberContainer: "_2OBm7JJBuCZLh1HNG1eGxx",
        Throbber: "_1uJfKMWhYxjCM2KFfBM6mS",
        ButtonText: "_3PV-5LTASyzLs3qOdUAQBy",
      };
    },
    79058: (e) => {
      e.exports = {
        GameList: "_2Ke99VagjO6dSjAa2PNqxC",
        GameListEntry: "_14lr0njLPbO3nOvfNr1AzG",
        Selected: "_3QI8hmWvFHwAfrradTNDas",
        Updating: "yTNHkI0JwNFkQUrUwQqN7",
        Uninstalled: "_2hOgDeHebiGfan9FuHhgQd",
        GameIcon: "_2ROZT5MqP-gQIPzHhHBUo5",
        GameIconPlaceholder: "_2AyMQ524UTKZGtMnnRVlpV",
      };
    },
    41983: (e) => {
      e.exports = {
        LogoEditorContainer: "_3G8aWAJwLQa8Gis9zgJK3F",
        LogoEditorHeaderContainer: "_31GWsBgTN2gKGw6BvieS8V",
        LogoEditorDetailsContainer: "_1uN9ilsJ0eyEjXA2IV9FNJ",
        NoLogoImage: "_3HX66KemiUEZ3js_TEQAFO",
        Title: "_32fDUKf4cNVinbOTd9xBkG",
        LogoEditorGameListContainer: "_26dKgXG2_zNCw6SjXmJIeE",
        TitleSection: "_3EpYa33cJPKgNejOK1sHyV",
        Features: "_1XHDuTLpau5Y75yupaoJ7q",
        DetailsArea: "_1ToHZ-HcV8Y95iXoPS7OrW",
        LogoEditorPopup: "_3h6edcqYQAzGq2KaiIBpqF",
        SavingThrobberContainer: "_8igOrfTBDrZfqMDZzuMXW",
        SavingThrobber: "_1Oc14dzW12OCleTzcVOtw2",
        Saving: "QovVgMGM08zBuCOEZsvfS",
        LogoEditorSaveActions: "tWdDs8OaXUUQGnG1pjEBE",
        LogoEditorSavedMessage: "_1ZpFTgfevq-D5Ud9kYiorw",
        Saved: "_1ES2_9pHjkOqPCE4g7aTRL",
        LogoEditorButtons: "_2LdrCslr7wCaAZc_B2cmqj",
        LogoEditorButton: "_3XBe65d7s3nYEUha_3ifXO",
        CancelBtn: "_3xnGmB9d7nC7aTOSORqrzd",
        ListDivider: "_3JOU7NACBwpsdYzDlpSlPA",
        Instructions: "HQq3bQMQYEeuUBgBP4KPa",
      };
    },
    48077: (e) => {
      e.exports = { "duration-app-launch": "800ms" };
    },
    20521: (e) => {
      e.exports = {
        "duration-app-launch": "800ms",
        TopCapsule: "NZMJ6g2iVnFsOOp-lDmIP",
        NoArt: "_1amMH3zqcKV3WYkLKNId8R",
        ImgSrc: "HNbe3eZf6H7dtJ042x1vM",
        ImgContainer: "QlR9EFwTdUNm_J5vx54_Z",
        FallbackArt: "_1ZNHIcPSbQl11wXtomcqgH",
        BoxSizerEdge: "_253kKVMtNEOuJ2sC467VSH",
        Middle: "_193Yt09oJHb2WZtls8raUe",
        BoxSizerValidRegion: "_2aPcBP45fdgOK22RN0jbhm",
        Background: "_1ssCErQIw1aTDS36tCKZJy",
        FullscreenEnterStart: "_2g7gySnDtVCA9jauebIRjQ",
        FullscreenEnterActive: "pIQBPa0KJg8An6kGeeEsE",
        FullscreenExitActive: "_2VyorTjyllT8k6ZM6x3RZ4",
        FullscreenExitDone: "_1OMoDO9rXi2J4_33DBJPCk",
        FullscreenEnterDone: "_2ra4k5nFh53dvfWfZNqgir",
        FullscreenExitStart: "_-4NyJgPeOwUu148SipZ4t",
        TitleImageContainer: "_2DVdg_N1qLNDdnxJqN-RBX",
        UpperCenter: "_3b-fVlE1HfENx84jhklHeo",
        CenterCenter: "_27g5SXTsPA8g4MN9PxPjAD",
        BottomCenter: "_1nosIKD_xejGRDzGB0gy1v",
        TitleLogo: "_3NBxSLAZLbbbnul8KfDFjw",
        Loaded: "_2dzwXkCVAuZGFC-qKgo8XB",
        BottomLeft: "_2levHjhEzRAVzWSj6oMXzj",
        UpperLeft: "_2GZ34GrLXIVQect79u65lS",
        HeaderBackgroundImage: "_1IX7FPSY9Jb82KhBVBSkZa",
        ImgBlurBackdrop: "_3_IUVzR9tpG_JKEjhwXEAb",
        ImgBlur: "HSQWw9HUAP6jtA2OZjS-u",
        PinBox: "_2O6k6YchzA5nhfbrdAGHAn",
        BoxSizerContainer: "A14yd24JRjhFI-Q1ae6tN",
        BoxSizer: "_2Eh7Soh97QONu_grMi2m66",
        BoxSizerDragBox: "_3ICyuuBD3KZ7x_rORri0v-",
        EdgeDown: "_3BLqofLRCI5R3h0DMuTJWm",
        BoxSizerGridBox: "pnIGuR-v2DtHkCmOVZ_Uq",
        TopLeft: "_8zlhLCBgQIGfymwpvPHzT",
        Top: "_1Ou140h0MBUXFs81gXE0ev",
        TopRight: "_1cMfnzr3uBol-BSQXnZ5rw",
        Left: "_2ddbRtNsT9ZzhqCp31dLzq",
        BoxSizerDelete: "_3IBT6Z45wvXS85eeX2Yros",
        BoxSizerSettings: "_7bAuQkYr-ddYTnV1ggPdp",
        Right: "_16lytfxCiD4S0VEZYju8sq",
        Bottom: "_1Qse1FDSsobXaqkCaQSXC2",
        BottomRight: "_2MuQa9As5X-kSFHDLHw48q",
        AddBoxSizer: "_3Y_PJPCRvFji4gIiu3GUq8",
        SaveBoxSizer: "_3F0ncqFvrT3lUq0zGJk7Jp",
        BoxSizerButtonContainer: "_30Ci5EXeBOQ2rJlPY2nMkY",
        DialogButton: "_21DkRjmS_w3EZ_E5Ns0912",
        BoxSizerInfo: "_4fGFbPKOaxfKGxabnB6Hi",
        TextNameSpace: "_3sUYxfgWcbeGFzYMkx1YDG",
        TitleSection: "_30acA_E0q_GuOxBqxgDJj4",
        Features: "fqwOycZBL_zQfcUiVVWyz",
        SVGTitle: "_121fln1p_mlV8b9CUxpCDS",
        TopGradient: "_1IWSiRVmH1DlNlWSaBeUuZ",
      };
    },
    37703: (e, t, o) => {
      "use strict";
      o.r(t),
        o.d(t, {
          Init_LibraryLogoEditor: () => ne,
          LogoEditor: () => re,
          LogoEditorPopup: () => ie,
        });
      var s = o(34629),
        n = o(7850),
        i = o(90626),
        r = o(44844),
        a = o(41735),
        l = o.n(a),
        d = o(75844),
        h = o(80724),
        c = o(73745),
        p = o(3088),
        g = o(81393),
        m = o(52038),
        u = o(81363),
        L = o(6144);
      var P = o(20521),
        f = o.n(P),
        C = o(48077),
        x = o.n(C),
        _ = o(51272),
        B = o(97875),
        I = o(10050),
        v = o(25489);
      const S = {
          exit: f().FullscreenExitStart,
          exitActive: f().FullscreenExitActive,
          exitDone: f().FullscreenExitDone,
          enter: f().FullscreenEnterStart,
          enterDone: f().FullscreenEnterDone,
          enterActive: f().FullscreenEnterActive,
        },
        b = (0, _.i_)(x()["duration-app-launch"]),
        N = { pinnedPosition: "BottomLeft", nWidthPct: 50, nHeightPct: 50 },
        O = i.createContext({ bFullscreen: !1 });
      class R extends i.Component {
        m_refTopCapsule = i.createRef();
        constructor(e) {
          super(e), (this.state = { logoPosition: void 0 });
        }
        componentDidUpdate(e) {
          (e.appid == this.props.appid && e.editMode == this.props.editMode) ||
            this.setState({ logoPosition: void 0 });
        }
        get background_src() {
          return (
            this.m_refTopCapsule.current &&
            this.m_refTopCapsule.current.background_src
          );
        }
        get logo_src() {
          return (
            this.m_refTopCapsule.current &&
            this.m_refTopCapsule.current.logo_src
          );
        }
        GetLogoPosition() {
          return this.state.logoPosition || this.props.logoPosition || N;
        }
        SetPinnedPosition(e) {
          this.setState(
            { logoPosition: { ...this.GetLogoPosition(), pinnedPosition: e } },
            this.PostPositionChangeCallback,
          );
        }
        SetDimensions(e, t) {
          this.setState(
            {
              logoPosition: {
                ...this.GetLogoPosition(),
                nWidthPct: e,
                nHeightPct: t,
              },
            },
            this.PostPositionChangeCallback,
          );
        }
        OnPositionChanged(e) {
          this.SetDimensions(e.nWidthPct, e.nHeightPct);
        }
        PostPositionChangeCallback() {
          this.props.fnOnPositionChanged &&
            this.props.fnOnPositionChanged(this.GetLogoPosition());
        }
        render() {
          if (!this.props.editMode)
            return (0, n.jsx)(O.Provider, {
              value: { bFullscreen: this.props.bFullscreen },
              children: (0, n.jsx)(y, {
                ref: this.m_refTopCapsule,
                ...this.props,
              }),
            });
          const {
              children: e,
              logoPosition: t,
              editMode: o,
              ...s
            } = this.props,
            i = this.GetLogoPosition();
          return (0, n.jsx)(O.Provider, {
            value: { bFullscreen: this.props.bFullscreen },
            children: (0, n.jsxs)(y, {
              ref: this.m_refTopCapsule,
              ...s,
              editMode: !0,
              logoPosition: i,
              fnOnPositionChanged: this.OnPositionChanged,
              children: [
                (0, n.jsx)("div", {
                  className: `${f().PinBox} ${f().BottomLeft}`,
                  onClick: () => this.SetPinnedPosition("BottomLeft"),
                  title: "Pin to Bottom Left",
                }),
                (0, n.jsx)("div", {
                  className: `${f().PinBox} ${f().UpperCenter}`,
                  onClick: () => this.SetPinnedPosition("UpperCenter"),
                  title: "Pin to Top Center",
                }),
                (0, n.jsx)("div", {
                  className: `${f().PinBox} ${f().CenterCenter}`,
                  onClick: () => this.SetPinnedPosition("CenterCenter"),
                  title: "Pin to Center",
                }),
                (0, n.jsx)("div", {
                  className: `${f().PinBox} ${f().BottomCenter}`,
                  onClick: () => this.SetPinnedPosition("BottomCenter"),
                  title: "Pin to Bottom Center",
                }),
                e,
              ],
            }),
          });
        }
      }
      (0, s.Cg)([c.oI], R.prototype, "SetDimensions", null),
        (0, s.Cg)([c.oI], R.prototype, "OnPositionChanged", null),
        (0, s.Cg)([c.oI], R.prototype, "PostPositionChangeCallback", null);
      let y = class extends i.Component {
        m_refBackgroundImage = i.createRef();
        m_refLogoImage = i.createRef();
        constructor(e) {
          super(e),
            (0, g.wT)(
              !e.editMode || e.rgLogoImages.length <= 1,
              "Can't use multiple logo images in edit mode",
            ),
            (this.state = {
              bHasHeaderImage: e.rgHeaderImages.length > 0,
              bHasLogoImage: e.rgLogoImages.length > 0,
              bLogoLoaded: !1,
            });
        }
        componentDidUpdate(e) {
          let t = null;
          (JSON.stringify(e.rgHeaderImages) ==
            JSON.stringify(this.props.rgHeaderImages) &&
            JSON.stringify(e.rgBlurImages) ==
              JSON.stringify(this.props.rgBlurImages)) ||
            ((t = t || {}),
            (t.bHasHeaderImage = this.props.rgHeaderImages.length > 0)),
            JSON.stringify(e.rgLogoImages) !=
              JSON.stringify(this.props.rgLogoImages) &&
              ((t = t || {}),
              (t.bHasLogoImage = this.props.rgLogoImages.length > 0),
              (t.bLogoLoaded = !1)),
            t && this.setState(t);
        }
        OnHeaderError() {
          this.setState({ bHasHeaderImage: !1 }),
            this.props.fnOnLoaded && this.props.fnOnLoaded();
        }
        OnIncrementalLogoError(e, t, o) {
          this.props.fnReportLogoCacheMiss &&
            this.props.fnReportLogoCacheMiss(this.props.appid, t),
            this.props.fnOnLoaded && this.props.fnOnLoaded();
        }
        OnLogoError() {
          this.setState({ bHasLogoImage: !1 }, () => {
            this.props.fnOnLogoLoaded && this.props.fnOnLogoLoaded();
          });
        }
        OnLogoLoad() {
          this.setState({ bLogoLoaded: !0 }, () => {
            this.props.fnOnLogoLoaded && this.props.fnOnLogoLoaded();
          });
        }
        OnLoaded() {
          this.props.fnOnLoaded && this.props.fnOnLoaded();
        }
        get background_src() {
          return (
            this.state.bHasHeaderImage &&
            this.m_refBackgroundImage.current &&
            this.m_refBackgroundImage.current.src
          );
        }
        get logo_src() {
          return (
            this.state.bLogoLoaded &&
            this.m_refLogoImage.current &&
            this.m_refLogoImage.current.src
          );
        }
        render() {
          const {
              rgLogoImages: e,
              editMode: t,
              logoPosition: o,
              className: s,
              classNameNoLogo: i,
              fnOnPositionChanged: r,
              height: a,
            } = this.props,
            { bHasLogoImage: l } = this.state,
            d = 1 == this.props.rgHeaderImages.length,
            h = (0, m.A)(
              f().TopCapsule,
              s,
              !this.state.bHasHeaderImage && f().NoArt,
              (!this.props.hasHeroImage || d) && f().FallbackArt,
              !l && i,
            ),
            c = { "--header-height": null == a ? void 0 : a + "px" };
          return (0, n.jsx)(I.M, {
            timeout: b,
            appear: !0,
            in: this.props.bFullscreen,
            classNames: S,
            children: (s) =>
              (0, n.jsxs)("div", {
                ref: s,
                className: h,
                style: c,
                children: [
                  (0, n.jsx)(j, {
                    ref: this.m_refBackgroundImage,
                    bLowPerfMode: this.props.bLowPerfMode,
                    appid: this.props.appid,
                    rgHeaderImages: this.props.rgHeaderImages,
                    rgBlurImages: this.props.rgBlurImages,
                    onReportHeroImageMiss: this.props.fnReportHeroImageMiss,
                    onError: this.OnHeaderError,
                    onLoad: this.OnLoaded,
                  }),
                  l &&
                    null !== o &&
                    (0, n.jsx)(
                      E,
                      {
                        strLogoImageURL: e[0],
                        editMode: t,
                        logoPosition: o || N,
                        fnOnPositionChanged: t && r,
                        fullscreen: this.props.bFullscreen,
                        children: (0, n.jsx)(p.c, {
                          ref: this.m_refLogoImage,
                          className: (0, m.A)(
                            f().TitleLogo,
                            this.state.bLogoLoaded && f().Loaded,
                          ),
                          rgSources: e,
                          onLoad: this.OnLogoLoad,
                          onIncrementalError: this.OnIncrementalLogoError,
                          onError: this.OnLogoError,
                        }),
                      },
                      e[0],
                    ),
                  this.props.children,
                  (0, n.jsx)("div", { className: f().TopGradient }),
                ],
              }),
          });
        }
      };
      (0, s.Cg)([c.oI], y.prototype, "OnHeaderError", null),
        (0, s.Cg)([c.oI], y.prototype, "OnIncrementalLogoError", null),
        (0, s.Cg)([c.oI], y.prototype, "OnLogoError", null),
        (0, s.Cg)([c.oI], y.prototype, "OnLogoLoad", null),
        (0, s.Cg)([c.oI], y.prototype, "OnLoaded", null),
        (y = (0, s.Cg)([d.PA], y));
      class j extends i.Component {
        m_refBackgroundImage = i.createRef();
        m_refCanvasBlurImage;
        constructor(e) {
          super(e),
            (this.state = {
              bBackgroundLoaded: !1,
              nBlurImageIndex: 0,
              bUseCanvasBlur: !this.HasBlurImages(),
            });
        }
        componentDidMount() {
          !this.HasHeaderImages() && this.props.onLoad && this.props.onLoad();
        }
        componentDidUpdate(e) {
          (e.appid == this.props.appid &&
            JSON.stringify(e.rgHeaderImages) ==
              JSON.stringify(this.props.rgHeaderImages) &&
            JSON.stringify(e.rgBlurImages) ==
              JSON.stringify(this.props.rgBlurImages) &&
            this.props.bLowPerfMode == e.bLowPerfMode) ||
            (this.setState({
              bBackgroundLoaded: !1,
              nBlurImageIndex: 0,
              bUseCanvasBlur: !this.HasBlurImages(),
            }),
            !this.HasHeaderImages() &&
              this.props.onLoad &&
              this.props.onLoad());
        }
        get src() {
          return (
            this.m_refBackgroundImage.current &&
            this.m_refBackgroundImage.current.src
          );
        }
        OnIncrementalError(e, t, o) {
          this.props.onReportHeroImageMiss &&
            this.props.onReportHeroImageMiss(this.props.appid, t),
            this.HasBlurImages() &&
              this.setState({
                nBlurImageIndex: this.state.nBlurImageIndex + 1,
              });
        }
        OnHeaderLoad() {
          this.props.onLoad && this.props.onLoad(),
            (this.m_refCanvasBlurImage =
              this.m_refBackgroundImage.current.imgRef.current),
            (this.HasBlurImages() &&
              this.props.rgBlurImages[this.state.nBlurImageIndex]) ||
              this.setState({ bUseCanvasBlur: !0 }),
            this.setState({ bBackgroundLoaded: !0 });
        }
        OnBlurImageFailed() {
          this.setState({ bUseCanvasBlur: !0 });
        }
        HasHeaderImages() {
          let e = this.props.rgHeaderImages;
          return e && e.length > 0;
        }
        HasBlurImages() {
          let e = this.props.rgBlurImages;
          return e && e.length > 0;
        }
        render() {
          let e = null;
          return (
            (e = (0, n.jsxs)(i.Fragment, {
              children: [
                !this.state.bUseCanvasBlur &&
                  this.state.bBackgroundLoaded &&
                  this.props.rgBlurImages[this.state.nBlurImageIndex] &&
                  (0, n.jsx)("img", {
                    src: this.props.rgBlurImages[this.state.nBlurImageIndex],
                    className: (0, m.A)(
                      f().ImgSrc,
                      f().ImgBlur,
                      f().ImgBlurBackdrop,
                    ),
                    onError: this.OnBlurImageFailed,
                  }),
                this.state.bUseCanvasBlur &&
                  this.state.bBackgroundLoaded &&
                  !this.props.bLowPerfMode &&
                  (0, n.jsx)(u.m, {
                    className: (0, m.A)(
                      f().ImgSrc,
                      f().ImgBlur,
                      f().ImgBlurBackdrop,
                    ),
                    elementRef: this.m_refCanvasBlurImage,
                    updateRate: 0,
                    width: 192,
                    height: 62,
                    reductionFactor: 10,
                    blurAmount: 3,
                  }),
              ],
            })),
            (0, n.jsx)(O.Consumer, {
              children: (t) =>
                (0, n.jsx)(I.M, {
                  timeout: b,
                  appear: !0,
                  in: t.bFullscreen,
                  classNames: S,
                  children: (t) =>
                    (0, n.jsxs)("div", {
                      ref: t,
                      className: (0, m.A)(
                        f().HeaderBackgroundImage,
                        f().Glassy,
                      ),
                      children: [
                        !this.state.bUseCanvasBlur &&
                          this.state.bBackgroundLoaded &&
                          this.props.rgBlurImages[this.state.nBlurImageIndex] &&
                          (0, n.jsx)("img", {
                            src: this.props.rgBlurImages[
                              this.state.nBlurImageIndex
                            ],
                            className: (0, m.A)(f().ImgSrc, f().ImgBlur),
                            onError: this.OnBlurImageFailed,
                          }),
                        this.state.bUseCanvasBlur &&
                          this.state.bBackgroundLoaded &&
                          !this.props.bLowPerfMode &&
                          (0, n.jsx)(u.m, {
                            className: (0, m.A)(f().ImgSrc, f().ImgBlur),
                            elementRef: this.m_refCanvasBlurImage,
                            updateRate: 0,
                            width: 192,
                            height: 62,
                            reductionFactor: 10,
                            blurAmount: 3,
                          }),
                        (0, n.jsx)("div", {
                          className: f().ImgContainer,
                          children:
                            this.HasHeaderImages() &&
                            (0, n.jsx)(p.c, {
                              ref: this.m_refBackgroundImage,
                              rgSources: this.props.rgHeaderImages,
                              className: f().ImgSrc,
                              onLoad: this.OnHeaderLoad,
                              onIncrementalError: this.OnIncrementalError,
                              onError: this.props.onError,
                            }),
                        }),
                        e,
                      ],
                    }),
                }),
            })
          );
        }
      }
      function E(e) {
        const {
            logoPosition: t,
            strLogoImageURL: o,
            children: s,
            fnOnPositionChanged: r,
            fullscreen: a,
          } = e,
          {
            nBottomPct: l,
            nTopPct: d,
            nLeftPct: h,
            nRightPct: c,
          } = A(t.pinnedPosition, t.nWidthPct, t.nHeightPct),
          p =
            (i.useContext(O),
            {
              left: `${h}%`,
              top: `${d}%`,
              width: `${t.nWidthPct}%`,
              height: `${t.nHeightPct}%`,
            }),
          g = (0, m.A)(f().BoxSizer, f()[t.pinnedPosition]);
        return (0, n.jsx)("div", {
          className: f().BoxSizerContainer,
          children: (0, n.jsxs)("div", {
            className: f().BoxSizerValidRegion,
            children: [
              !e.editMode &&
                (0, n.jsx)("div", {
                  className: g,
                  style: p,
                  children: (0, n.jsx)(I.M, {
                    timeout: b,
                    appear: !0,
                    in: a,
                    classNames: S,
                    children: (e) =>
                      (0, n.jsx)("div", {
                        ref: e,
                        className: f().TitleImageContainer,
                        children: s,
                      }),
                  }),
                }),
              e.editMode &&
                (0, n.jsx)(M, {
                  id: o,
                  pinType: t.pinnedPosition,
                  index: 0,
                  widthPct: t.nWidthPct,
                  heightPct: t.nHeightPct,
                  fnOnPositionChanged: r,
                  children: s,
                }),
            ],
          }),
        });
      }
      function A(e, t, o) {
        let s, n, i, r;
        switch (e) {
          case "BottomLeft":
            (s = 0), (n = 100 - o), (i = 0), (r = 100 - t);
            break;
          case "UpperLeft":
            (s = 100 - o), (n = 0), (i = 0), (r = 100 - t);
            break;
          case "CenterCenter":
            (s = (100 - o) / 2),
              (n = (100 - o) / 2),
              (i = (100 - t) / 2),
              (r = (100 - t) / 2);
            break;
          case "UpperCenter":
            (s = 100 - o), (n = 0), (i = (100 - t) / 2), (r = (100 - t) / 2);
            break;
          case "BottomCenter":
            (s = 0), (n = 100 - o), (i = (100 - t) / 2), (r = (100 - t) / 2);
        }
        return { nBottomPct: s, nTopPct: n, nLeftPct: i, nRightPct: r };
      }
      var T;
      (0, s.Cg)([c.oI], j.prototype, "OnIncrementalError", null),
        (0, s.Cg)([c.oI], j.prototype, "OnHeaderLoad", null),
        (0, s.Cg)([c.oI], j.prototype, "OnBlurImageFailed", null),
        (function (e) {
          (e.topleft = "Topleft"),
            (e.top = "Top"),
            (e.topright = "TopRight"),
            (e.left = "Left"),
            (e.middle = "Middle"),
            (e.right = "Right"),
            (e.bottomleft = "BottomLeft"),
            (e.bottom = "Bottom"),
            (e.bottomright = "BottomRight");
        })(T || (T = {}));
      class M extends i.Component {
        m_rectLinkRegion;
        m_elLinkRegionBox;
        m_nLocalOffsetXPct;
        m_nLocalOffsetYPct;
        m_fnMouseUp = null;
        m_fnMouseMove = null;
        m_listeners = new L.Ji();
        m_pinType;
        constructor(e) {
          super(e), (this.state = {});
        }
        componentWillUnmount() {
          this.m_listeners.Unregister();
        }
        componentDidUpdate() {
          this.props.pinType != this.state.pinType &&
            ((this.m_pinType = this.props.pinType),
            this.setState({ pinType: this.props.pinType }),
            this.UpdateBoxPosition());
        }
        static getDerivedStateFromProps(e, t) {
          const { pinType: o, widthPct: s, heightPct: n, id: i } = e;
          if (t && t.id == i) return null;
          const {
            nBottomPct: r,
            nTopPct: a,
            nLeftPct: l,
            nRightPct: d,
          } = A(o, s, n);
          return {
            id: i,
            curBottomPosPct: r,
            curTopPosPct: a,
            curLeftPosPct: l,
            curRightPosPct: d,
            curWidthPct: s,
            curHeightPct: n,
            EdgeDown: null,
            pinType: o,
          };
        }
        LinkRegionBoxRef(e) {
          this.m_elLinkRegionBox = e;
        }
        OnMouseDown(e, t) {
          (this.m_fnMouseUp = (e) => {
            this.OnMouseUp(e, t);
          }),
            (this.m_fnMouseMove = (e) => {
              this.OnMouseMove(e, t);
            }),
            this.setState({ EdgeDown: t }),
            (this.m_rectLinkRegion =
              this.m_elLinkRegionBox.parentElement.getBoundingClientRect()),
            (this.m_nLocalOffsetXPct =
              ((e.clientX - this.m_rectLinkRegion.left) /
                (this.m_rectLinkRegion.right - this.m_rectLinkRegion.left)) *
                100 -
              this.state.curLeftPosPct),
            (this.m_nLocalOffsetYPct =
              ((e.clientY - this.m_rectLinkRegion.top) /
                (this.m_rectLinkRegion.bottom - this.m_rectLinkRegion.top)) *
                100 -
              this.state.curTopPosPct),
            this.m_listeners.AddEventListener(
              this.m_elLinkRegionBox.ownerDocument.defaultView,
              "mousemove",
              this.m_fnMouseMove,
            ),
            this.m_listeners.AddEventListener(
              this.m_elLinkRegionBox.ownerDocument.defaultView,
              "mouseup",
              this.m_fnMouseUp,
            ),
            e.preventDefault(),
            e.stopPropagation();
        }
        UpdateBoxPosition() {
          const {
            nBottomPct: e,
            nTopPct: t,
            nLeftPct: o,
            nRightPct: s,
          } = A(
            this.m_pinType,
            this.state.curWidthPct,
            this.state.curHeightPct,
          );
          this.setState({
            curBottomPosPct: e,
            curTopPosPct: t,
            curLeftPosPct: o,
            curRightPosPct: s,
          });
        }
        OnMouseMove(e, t) {
          if (void 0 === this.state.EdgeDown) return;
          e.shiftKey && this.m_fnMouseUp();
          let {
            curTopPosPct: o,
            curRightPosPct: s,
            curBottomPosPct: n,
            curLeftPosPct: i,
          } = this.state;
          const r = (e) => {
              let t =
                ((e - this.m_rectLinkRegion.left) /
                  (this.m_rectLinkRegion.right - this.m_rectLinkRegion.left)) *
                  100 -
                this.m_nLocalOffsetXPct;
              if (
                "UpperLeft" == this.props.pinType ||
                "BottomLeft" == this.props.pinType
              )
                return 0;
              if (
                "CenterCenter" == this.props.pinType ||
                "BottomCenter" == this.props.pinType ||
                "UpperCenter" == this.props.pinType
              ) {
                let e = Math.min(Math.max(t, 0), 45);
                return (s = e), e;
              }
              return t;
            },
            a = (e) => {
              let t =
                100 -
                (((e - this.m_rectLinkRegion.left) /
                  (this.m_rectLinkRegion.right - this.m_rectLinkRegion.left)) *
                  100 +
                  (this.state.curWidthPct - this.m_nLocalOffsetXPct));
              if (
                "CenterCenter" == this.props.pinType ||
                "BottomCenter" == this.props.pinType ||
                "UpperCenter" == this.props.pinType
              ) {
                let e = Math.min(Math.max(t, 0), 45);
                return (i = e), e;
              }
              return t;
            },
            l = (e) => {
              let t =
                ((e - this.m_rectLinkRegion.top) /
                  (this.m_rectLinkRegion.bottom - this.m_rectLinkRegion.top)) *
                  100 -
                this.m_nLocalOffsetYPct;
              if (
                "UpperCenter" == this.props.pinType ||
                "UpperLeft" == this.props.pinType
              )
                return 0;
              if ("CenterCenter" == this.props.pinType) {
                let e = Math.min(Math.max(t, 0), 45);
                return (n = e), e;
              }
              return t;
            },
            d = (e) => {
              let t =
                100 -
                (((e - this.m_rectLinkRegion.top) /
                  (this.m_rectLinkRegion.bottom - this.m_rectLinkRegion.top)) *
                  100 +
                  (this.state.curHeightPct - this.m_nLocalOffsetYPct));
              if (
                "BottomLeft" == this.props.pinType ||
                "BottomCenter" == this.props.pinType
              )
                return 0;
              if ("CenterCenter" == this.props.pinType) {
                let e = Math.min(Math.max(t, 0), 45);
                return (o = e), e;
              }
              return t;
            };
          function h() {
            n = Math.min(n, 98 - o);
          }
          function c() {
            s = Math.min(s, 99 - i);
          }
          function p() {
            o = Math.min(o, 98 - n);
          }
          function g() {
            i = Math.min(i, 99 - s);
          }
          switch (t) {
            case T.left:
              (i = r(e.clientX)), g();
              break;
            case T.right:
              (s = a(e.clientX)), c();
              break;
            case T.top:
              (o = l(e.clientY)), p();
              break;
            case T.bottom:
              (n = d(e.clientY)), h();
              break;
            case T.topleft:
              (o = l(e.clientY)), (i = r(e.clientX)), p(), g();
              break;
            case T.topright:
              (o = l(e.clientY)), (s = a(e.clientX)), p(), c();
              break;
            case T.bottomleft:
              (n = d(e.clientY)), (i = r(e.clientX)), h(), g();
              break;
            case T.bottomright:
              (n = d(e.clientY)), (s = a(e.clientX)), h(), c();
              break;
            case T.middle:
              (s = a(e.clientX)),
                (n = d(e.clientY)),
                "CenterCenter" != this.state.pinType && (o = l(e.clientY)),
                "CenterCenter" != this.state.pinType &&
                  "BottomCenter" != this.state.pinType &&
                  "UpperCenter" != this.state.pinType &&
                  (i = r(e.clientX)),
                p(),
                h(),
                g(),
                c();
          }
          (o = (0, v.OQ)(o, 0, 98)),
            (n = (0, v.OQ)(n, 0, 98)),
            (i = (0, v.OQ)(i, 0, 99)),
            (s = (0, v.OQ)(s, 0, 99)),
            this.setState({
              curTopPosPct: o,
              curRightPosPct: s,
              curBottomPosPct: n,
              curLeftPosPct: i,
            }),
            e.preventDefault(),
            e.stopPropagation();
        }
        OnMouseUp(e, t) {
          this.setState(
            {
              curWidthPct:
                100 - this.state.curRightPosPct - this.state.curLeftPosPct,
              curHeightPct:
                100 - this.state.curBottomPosPct - this.state.curTopPosPct,
            },
            this.OnResizeComplete,
          ),
            this.setState({ EdgeDown: void 0 }),
            this.m_listeners.Unregister();
        }
        OnResizeComplete() {
          this.props.fnOnPositionChanged &&
            this.props.fnOnPositionChanged({
              pinnedPosition: this.state.pinType,
              nWidthPct: this.state.curWidthPct,
              nHeightPct: this.state.curHeightPct,
            });
        }
        render() {
          let e = {
              left: this.state.curLeftPosPct + "%",
              top: this.state.curTopPosPct + "%",
              right: this.state.curRightPosPct + "%",
              bottom: this.state.curBottomPosPct + "%",
            },
            t = (0, m.A)(
              f().BoxSizerDragBox,
              this.state.EdgeDown &&
                (0, m.A)(f().EdgeDown, f()[this.state.EdgeDown]),
              f()[this.props.pinType],
            );
          return (0, n.jsx)("div", {
            className: t,
            style: e,
            ref: this.LinkRegionBoxRef,
            draggable: !1,
            children: (0, n.jsxs)("div", {
              className: f().BoxSizerGridBox,
              children: [
                (0, n.jsx)("div", {
                  className: `${f().BoxSizerEdge} ${f().TopLeft}`,
                  onMouseDown: (e) => {
                    this.OnMouseDown(e, T.topleft);
                  },
                  draggable: !1,
                }),
                (0, n.jsx)("div", {
                  className: `${f().BoxSizerEdge} ${f().Top}`,
                  onMouseDown: (e) => {
                    this.OnMouseDown(e, T.top);
                  },
                }),
                (0, n.jsx)("div", {
                  className: `${f().BoxSizerEdge} ${f().TopRight}`,
                  onMouseDown: (e) => {
                    this.OnMouseDown(e, T.topright);
                  },
                  draggable: !1,
                }),
                (0, n.jsx)("div", {
                  className: `${f().BoxSizerEdge} ${f().Left}`,
                  onMouseDown: (e) => {
                    this.OnMouseDown(e, T.left);
                  },
                  draggable: !1,
                }),
                (0, n.jsx)("div", {
                  className: `${f().BoxSizerEdge} ${f().Middle}`,
                  onMouseDown: (e) => {
                    this.OnMouseDown(e, T.middle);
                  },
                  draggable: !1,
                  children: (0, n.jsx)("div", {
                    className: f().TitleImageContainer,
                    children: this.props.children,
                  }),
                }),
                (0, n.jsx)("div", {
                  className: `${f().BoxSizerEdge} ${f().Right}`,
                  onMouseDown: (e) => {
                    this.OnMouseDown(e, T.right);
                  },
                  draggable: !1,
                }),
                (0, n.jsx)("div", {
                  className: `${f().BoxSizerEdge} ${f().BottomLeft}`,
                  onMouseDown: (e) => {
                    this.OnMouseDown(e, T.bottomleft);
                  },
                  draggable: !1,
                }),
                (0, n.jsx)("div", {
                  className: `${f().BoxSizerEdge} ${f().Bottom}`,
                  onMouseDown: (e) => {
                    this.OnMouseDown(e, T.bottom);
                  },
                  draggable: !1,
                }),
                (0, n.jsx)("div", {
                  className: `${f().BoxSizerEdge} ${f().BottomRight}`,
                  onMouseDown: (e) => {
                    this.OnMouseDown(e, T.bottomright);
                  },
                  draggable: !1,
                }),
              ],
            }),
          });
        }
      }
      (0, s.Cg)([c.oI], M.prototype, "LinkRegionBoxRef", null),
        (0, s.Cg)([c.oI], M.prototype, "OnMouseDown", null),
        (0, s.Cg)([c.oI], M.prototype, "UpdateBoxPosition", null),
        (0, s.Cg)([c.oI], M.prototype, "OnMouseMove", null),
        (0, s.Cg)([c.oI], M.prototype, "OnMouseUp", null),
        (0, s.Cg)([c.oI], M.prototype, "OnResizeComplete", null);
      const D = (e) => {
        const { title: t, className: o } = e;
        let s = 26;
        t.length > 8 && (s = Math.max(s - (t.length - 5) / 2, 5.8));
        const [i, r] = (0, B.l)();
        return (0, n.jsxs)("svg", {
          className: (0, m.A)(f().SVGTitle, o),
          viewBox: "0 0 300 14",
          children: [
            (0, n.jsx)("defs", {
              children: (0, n.jsxs)("linearGradient", {
                id: i,
                x1: "0",
                x2: "0",
                y1: "0",
                y2: "100%",
                gradientUnits: "userSpaceOnUse",
                children: [
                  (0, n.jsx)("stop", { stopColor: "#fff", offset: "0%" }),
                  (0, n.jsx)("stop", { stopColor: "#fff", offset: "20%" }),
                  (0, n.jsx)("stop", { stopColor: "#fff", offset: "40%" }),
                  (0, n.jsx)("stop", { stopColor: "#eee", offset: "60%" }),
                  (0, n.jsx)("stop", { stopColor: "#ddd", offset: "80%" }),
                  (0, n.jsx)("stop", { stopColor: "#ccc", offset: "100%" }),
                ],
              }),
            }),
            (0, n.jsx)("text", {
              x: "-1",
              y: 14,
              fontSize: s,
              textAnchor: "bottom",
              fontWeight: "200",
              fill: r,
              children: t,
            }),
          ],
        });
      };
      function w(e) {
        const { title: t, children: o } = e,
          s = i.useContext(O),
          r = i.useRef(null);
        return (0, n.jsx)(h.A, {
          nodeRef: r,
          timeout: b,
          appear: !0,
          in: s.bFullscreen,
          classNames: S,
          children: (0, n.jsxs)("div", {
            ref: r,
            className: (0, m.A)(f().TitleSection, t ? f().NoLogo : ""),
            children: [
              (0, n.jsx)("div", {
                className: f().TextNameSpace,
                children: !!t && (0, n.jsx)(D, { title: t }),
              }),
              (0, n.jsx)("div", { className: f().Features, children: o }),
            ],
          }),
        });
      }
      var H = o(14947);
      class k {
        m_strLibraryLogoURL;
        m_strLibraryHeroURL;
        m_unAppID;
        m_strAppName;
        m_logoPosition = void 0;
        m_strSaveURL;
        constructor(e) {
          (0, H.Gn)(this),
            (this.m_strLibraryHeroURL = e.strLibraryHeroURL),
            (this.m_strLibraryLogoURL = e.strLibraryLogoURL),
            (this.m_unAppID = e.unAppID),
            (this.m_strAppName = e.strAppName),
            (this.m_logoPosition = e.logoPosition || N),
            (this.m_strSaveURL = e.strSaveURL || null),
            this.m_logoPosition.pinnedPosition ||
              (this.m_logoPosition.pinnedPosition = N.pinnedPosition),
            this.m_logoPosition.nHeightPct ||
              (this.m_logoPosition.nHeightPct = N.nHeightPct),
            this.m_logoPosition.nWidthPct ||
              (this.m_logoPosition.nWidthPct = N.nWidthPct);
        }
        GetHeroURL() {
          return this.m_strLibraryHeroURL;
        }
        GetLogoURL() {
          return this.m_strLibraryLogoURL;
        }
        GetAppID() {
          return this.m_unAppID;
        }
        GetAppName() {
          return this.m_strAppName;
        }
        GetLogoPosition() {
          return this.m_logoPosition;
        }
        SetLogoPosition(e) {
          this.m_logoPosition = e;
        }
        GetSaveURL() {
          return this.m_strSaveURL;
        }
      }
      (0, s.Cg)([H.sH], k.prototype, "m_logoPosition", void 0);
      var U = o(41983),
        G = o.n(U),
        F = o(17221),
        z = o.n(F),
        W = o(12155);
      function J() {
        return (0, n.jsxs)(i.Fragment, {
          children: [
            (0, n.jsx)("span", { style: { fontSize: 0 } }),
            (0, n.jsx)("div", {
              className: (0, m.A)(z().Container),
              children: (0, n.jsxs)("div", {
                className: z().Row,
                children: [
                  (0, n.jsx)(K, {}),
                  (0, n.jsx)(Y, {}),
                  (0, n.jsx)("div", {
                    className: z().RightControls,
                    children: (0, n.jsxs)("div", {
                      className: z().AppButtonsContainer,
                      children: [
                        (0, n.jsx)("div", {
                          className: z().MenuButton,
                          children: (0, n.jsx)(W.wB_, {}),
                        }),
                        (0, n.jsxs)("div", {
                          className: z().GameInfoButton,
                          children: [
                            "Game Info",
                            (0, n.jsx)("div", {
                              className: z().Arrow,
                              children: (0, n.jsx)(W.V5W, { angle: 180 }),
                            }),
                          ],
                        }),
                      ],
                    }),
                  }),
                ],
              }),
            }),
          ],
        });
      }
      function Y() {
        return (0, n.jsx)("div", {
          className: (0, m.A)(z().StatusAndStats),
          children: (0, n.jsx)(X, {}),
        });
      }
      function X() {
        return (0, n.jsxs)("div", {
          className: z().GameStatsSection,
          children: [
            (0, n.jsx)("div", {
              className: z().LastPlayed,
              children: (0, n.jsxs)("div", {
                className: z().LastPlayedRight,
                children: [
                  (0, n.jsx)("div", {
                    className: z().LastPlayedLabel,
                    children: "LAST PLAYED",
                  }),
                  (0, n.jsx)("div", {
                    className: z().LastPlayedInfo,
                    children: "Mar 24",
                  }),
                ],
              }),
            }),
            (0, n.jsxs)("div", {
              className: z().Playtime,
              children: [
                (0, n.jsx)("div", { className: z().PlaytimeLeft }),
                (0, n.jsxs)("div", {
                  className: z().PlaytimeRight,
                  children: [
                    (0, n.jsx)("div", {
                      className: z().PlaytimeLabel,
                      children: "PLAY TIME",
                    }),
                    (0, n.jsx)("div", {
                      className: z().PlaytimeInfo,
                      children: "37 hours",
                    }),
                  ],
                }),
              ],
            }),
            (0, n.jsxs)("div", {
              className: z().MiniAchievements,
              children: [
                (0, n.jsx)("div", { className: z().AchievementLeft }),
                (0, n.jsxs)("div", {
                  className: z().AchievementRight,
                  children: [
                    (0, n.jsx)("div", {
                      className: z().AchievementLabel,
                      children: "ACHIEVEMENTS",
                    }),
                    (0, n.jsxs)("div", {
                      className: z().AchievementProgressRow,
                      children: [
                        (0, n.jsx)("div", {
                          className: z().AchievementCountLabel,
                          children: "30/47",
                        }),
                        (0, n.jsx)(V, { progressPct: 3e3 / 47 }),
                      ],
                    }),
                  ],
                }),
              ],
            }),
          ],
        });
      }
      class V extends i.Component {
        render() {
          return (0, n.jsx)("div", {
            className: z().DetailsProgressContainer,
            children: (0, n.jsx)("div", {
              className: z().DetailsProgressBar,
              style: { width: this.props.progressPct + "%" },
            }),
          });
        }
      }
      function K() {
        return (0, n.jsx)("div", {
          className: z().ActionSection,
          children: (0, n.jsx)("div", {
            className: z().PlayButtonContainer,
            children: (0, n.jsx)("div", {
              className: (0, m.A)(z().PlayButton, z().Green),
              children: (0, n.jsx)("div", {
                className: z().ButtonText,
                children: "PLAY",
              }),
            }),
          }),
        });
      }
      var Q = o(79058),
        Z = o.n(Q);
      const q = JSON.parse(
        '["Apex Legends™","Baldur\'s Gate 3","Battlefield™ 6","Black Desert","Black Myth: Wukong","Borderlands 4","Clair Obscur: Expedition 33","Counter-Strike 2","Cronos: The New Dawn","Crusader Kings III","Cyberpunk 2077","Dead by Daylight","Delta Force","Destiny 2","Dota 2","Dune: Awakening","Dying Light: The Beast","EA SPORTS FC™ 26","ELDEN RING","ELDEN RING NIGHTREIGN","Euro Truck Simulator 2","F1® 25","FINAL FANTASY XIV Online","Forza Horizon 5","Grand Theft Auto V Enhanced","Grounded 2","HELLDIVERS™ 2","Hollow Knight","Hollow Knight: Silksong","Killing Floor 3","Kingdom Come: Deliverance II","Limbus Company","Lost Ark","METAL GEAR SOLID Δ: SNAKE EATER","Madden NFL 26","Mafia: The Old Country","Mage Arena","Magic: The Gathering Arena","Marvel Rivals","NARAKA: BLADEPOINT","NBA 2K26","No Man\'s Sky","Once Human","Overwatch® 2","PEAK","PUBG: BATTLEGROUNDS","Path of Exile 2","R.E.P.O.","Ready or Not","Red Dead Redemption 2","RimWorld","Rust","Schedule I","Shape of Dreams","Split Fiction","Stardew Valley","Street Fighter™ 6","THE FINALS","THRONE AND LIBERTY","Team Fortress 2","The Elder Scrolls® Online","The First Descendant","The Sims™ 4","Titan Quest II","Tom Clancy\'s Rainbow Six® Siege X","Umamusume: Pretty Derby","WUCHANG: Fallen Feathers","Wallpaper Engine","War Thunder","Warframe","Warhammer 40,000: Dawn of War - Definitive Edition","Warhammer 40,000: Space Marine 2","Wuthering Waves","Yu-Gi-Oh! Master Duel","雀魂麻將(MahjongSoul)"]',
      );
      function $(e) {
        const { highlightedName: t } = e,
          o = i.useCallback(
            (e) => e && e.scrollIntoView({ block: "center" }),
            [],
          ),
          s = i.useMemo(() => {
            if (-1 !== q.indexOf(t)) return q;
            {
              const e = [...q, t];
              return e.sort(), e;
            }
          }, [t]);
        return (0, n.jsx)("div", {
          className: Z().GameList,
          children: s.map((e) => {
            let s = t === e;
            return (0, n.jsx)(
              "div",
              {
                className: (0, m.A)(
                  Z().GameListEntry,
                  s ? Z().Selected : Z().Uninstalled,
                ),
                ref: s ? o : void 0,
                children: e,
              },
              e,
            );
          }),
        });
      }
      var ee = o(61859),
        te = o(78327),
        oe = o(22797),
        se = o(16676);
      function ne(e, t) {
        const o = (0, te.Tc)("editorconfig", t);
        let s = new k(o);
        r.createRoot(t).render(
          i.createElement(e ? ie : re, { LogoEditorStore: s }),
        );
      }
      class ie extends i.Component {
        constructor(e) {
          super(e),
            (this.state = { saving: !1, saved: !1, unsavedChanges: !1 });
        }
        componentDidMount() {
          window.addEventListener("beforeunload", this.OnBeforeUnload);
        }
        componentWillUnmount() {
          window.removeEventListener("beforeunload", this.OnBeforeUnload);
        }
        OnBeforeUnload(e) {
          this.state.unsavedChanges &&
            (e.preventDefault(),
            (e.returnValue =
              "You have unsaved changes.  Are you sure you want to close this window?"));
        }
        async OnOK() {
          if (this.state.saving) return;
          const { LogoEditorStore: e } = this.props;
          this.setState({ saving: !0 }, async () => {
            if (e.GetSaveURL()) {
              let t = e.GetLogoPosition();
              try {
                let o = new FormData();
                o.append("json", "1"),
                  o.append("sessionid", te.TS.SESSIONID),
                  o.append(
                    "app[assets][library_logo][logo_position][pinned_position]",
                    t.pinnedPosition,
                  ),
                  o.append(
                    "app[assets][library_logo][logo_position][width_pct]",
                    "" + t.nWidthPct,
                  ),
                  o.append(
                    "app[assets][library_logo][logo_position][height_pct]",
                    "" + t.nHeightPct,
                  );
                await l().post(e.GetSaveURL(), o);
                this.setState({ saving: !1, saved: !0, unsavedChanges: !1 });
              } catch (e) {
                console.error(e),
                  alert("There was a problem saving changes"),
                  this.setState({ saving: !1 });
              }
            } else
              window.opener &&
                window.opener.postMessage(
                  { appid: e.GetAppID(), ...(0, H.HO)(e.GetLogoPosition()) },
                  "*",
                ),
                this.OnCancel();
          });
        }
        OnCancel() {
          window.removeEventListener("beforeunload", this.OnBeforeUnload),
            window.close();
        }
        OnPositionChanged() {
          this.state.unsavedChanges ||
            this.setState({ unsavedChanges: !0, saved: !1 });
        }
        render() {
          return (0, n.jsx)(i.Fragment, {
            children: (0, n.jsxs)("div", {
              className: (0, m.A)(
                G().LogoEditorPopup,
                this.state.saving && G().Saving,
              ),
              children: [
                (0, n.jsxs)("div", {
                  className: G().LogoEditorSaveActions,
                  children: [
                    (0, n.jsx)("div", {
                      className: (0, m.A)(
                        G().LogoEditorSavedMessage,
                        this.state.saved && G().Saved,
                      ),
                      children: (0, ee.we)(
                        "#StoreAdmin_LibraryPlacementTool_ChangesSaved",
                      ),
                    }),
                    (0, n.jsxs)("div", {
                      className: G().LogoEditorButtons,
                      children: [
                        (0, n.jsx)(se.jn, {
                          onClick: this.OnOK,
                          className: G().LogoEditorButton,
                          children: (0, ee.we)("#Button_Save"),
                        }),
                        (0, n.jsx)(se.$n, {
                          onClick: this.OnCancel,
                          className: G().LogoEditorButton,
                          children: this.state.unsavedChanges
                            ? (0, ee.we)("#Button_Cancel")
                            : (0, ee.we)("#Button_Close"),
                        }),
                      ],
                    }),
                  ],
                }),
                (0, n.jsx)("div", {
                  className: G().SavingThrobberContainer,
                  children: (0, n.jsx)(oe.t, {
                    size: "xlarge",
                    className: G().SavingThrobber,
                  }),
                }),
                (0, n.jsx)(re, {
                  ...this.props,
                  fnOnPositionChanged: this.OnPositionChanged,
                }),
              ],
            }),
          });
        }
      }
      (0, s.Cg)([c.oI], ie.prototype, "OnBeforeUnload", null),
        (0, s.Cg)([c.oI], ie.prototype, "OnOK", null),
        (0, s.Cg)([c.oI], ie.prototype, "OnCancel", null),
        (0, s.Cg)([c.oI], ie.prototype, "OnPositionChanged", null);
      class re extends i.Component {
        constructor(e) {
          super(e), (this.state = { bEditModeEnabled: !0 });
        }
        OnPositionChanged(e) {
          const { LogoEditorStore: t, fnOnPositionChanged: o } = this.props;
          t.SetLogoPosition(e), o && o(e);
        }
        render() {
          const { LogoEditorStore: e } = this.props;
          let t = !!e.GetLogoURL();
          return (0, n.jsxs)("div", {
            className: G().LogoEditorContainer,
            children: [
              (0, n.jsx)("div", {
                className: G().LogoEditorGameListContainer,
                children: (0, n.jsx)($, { highlightedName: e.GetAppName() }),
              }),
              (0, n.jsx)("div", { className: G().ListDivider }),
              (0, n.jsxs)("div", {
                className: G().LogoEditorDetailsContainer,
                children: [
                  (0, n.jsx)(ae, { LogoEditorStore: e }),
                  (0, n.jsx)("div", {
                    className: G().LogoEditorHeaderContainer,
                    children: (0, n.jsx)(R, {
                      editMode: t && this.state.bEditModeEnabled,
                      hasHeroImage: !0,
                      rgHeaderImages: [e.GetHeroURL()],
                      rgLogoImages: t ? [e.GetLogoURL()] : [],
                      classNameNoLogo: G().NoLogoImage,
                      fnOnPositionChanged: this.OnPositionChanged,
                      logoPosition: e.GetLogoPosition(),
                      children: (0, n.jsx)(w, {
                        title: e.GetLogoURL() ? "" : e.GetAppName(),
                      }),
                    }),
                  }),
                  (0, n.jsx)(J, {}),
                  (0, n.jsxs)("div", {
                    className: G().DetailsArea,
                    children: [
                      (0, n.jsx)("br", {}),
                      (0, n.jsx)("br", {}),
                      (0, n.jsx)("br", {}),
                      (0, n.jsx)("br", {}),
                      (0, n.jsx)("br", {}),
                    ],
                  }),
                ],
              }),
            ],
          });
        }
      }
      (0, s.Cg)([c.oI], re.prototype, "OnPositionChanged", null);
      const ae = (0, d.PA)((e) => {
        const t = e.LogoEditorStore.GetLogoPosition();
        return (0, n.jsxs)(i.Fragment, {
          children: [
            (0, n.jsx)("input", {
              type: "hidden",
              name: "app[assets][library_logo][logo_position][pinned_position]",
              value: t.pinnedPosition,
            }),
            (0, n.jsx)("input", {
              type: "hidden",
              name: "app[assets][library_logo][logo_position][width_pct]",
              value: t.nWidthPct,
            }),
            (0, n.jsx)("input", {
              type: "hidden",
              name: "app[assets][library_logo][logo_position][height_pct]",
              value: t.nHeightPct,
            }),
          ],
        });
      });
    },
    81363: (e, t, o) => {
      "use strict";
      o.d(t, { m: () => l });
      var s = o(34629),
        n = o(7850),
        i = o(90626),
        r = o(73745),
        a = o(6144);
      class l extends i.Component {
        m_elCanvas;
        m_Context;
        m_schUpdate = new a.LU();
        m_bSetupComplete = !1;
        componentDidMount() {
          0 == this.props.updateRate && this.updateCanvas();
        }
        componentWillUnmount() {
          this.m_schUpdate.Cancel();
        }
        componentDidUpdate() {
          this.updateCanvas();
        }
        BindCanvasRef(e) {
          this.m_elCanvas = e;
        }
        updateCanvas() {
          if (
            null == this.props.elementRef ||
            null == this.m_elCanvas ||
            this.m_bSetupComplete
          )
            return;
          let e = this.props.scaleFactor || [1, 1],
            t = this.props.elementRef,
            o = this.props.updateRate;
          this.m_Context = this.m_elCanvas.getContext("2d");
          let s = Math.floor(
              this.m_elCanvas.clientWidth / this.props.reductionFactor,
            ),
            n = Math.floor(
              this.m_elCanvas.clientHeight / this.props.reductionFactor,
            );
          (this.m_elCanvas.width = s),
            (this.m_elCanvas.height = n),
            this.props.blurAmount > 0 &&
              (this.m_Context.filter = "blur(" + this.props.blurAmount + "px)");
          let i = () => {
            this.m_Context.drawImage(t, 0, 0, s * e[0], n * e[1]),
              o > 0 && this.m_schUpdate.Schedule(o, i);
          };
          i(), (this.m_bSetupComplete = !0);
        }
        render() {
          return (0, n.jsx)("canvas", {
            id: this.props.id,
            className: this.props.className,
            ref: this.BindCanvasRef,
            width: this.props.width,
            height: this.props.height,
          });
        }
      }
      (0, s.Cg)([r.oI], l.prototype, "BindCanvasRef", null),
        (0, s.Cg)([r.oI], l.prototype, "updateCanvas", null);
    },
    10050: (e, t, o) => {
      "use strict";
      o.d(t, { M: () => r });
      var s = o(7850),
        n = o(90626),
        i = o(80724);
      function r(e) {
        const { children: t, ...o } = e,
          r = n.useRef(null);
        return (0, s.jsx)(i.A, { nodeRef: r, ...o, children: e.children(r) });
      }
    },
    3088: (e, t, o) => {
      "use strict";
      o.d(t, { c: () => a });
      var s = o(34629),
        n = o(7850),
        i = o(90626),
        r = o(73745);
      class a extends i.Component {
        m_refImage = i.createRef();
        constructor(e) {
          super(e), (this.state = { nImage: 0, nPropChangeCounter: 0 });
        }
        componentDidUpdate(e, t) {
          JSON.stringify(this.props.rgSources) != JSON.stringify(e.rgSources) &&
            this.setState({
              nImage: 0,
              nPropChangeCounter: t.nPropChangeCounter + 1,
            });
        }
        get src() {
          let e = "";
          return (
            this.props.rgSources &&
              this.props.rgSources.length > this.state.nImage &&
              (e = this.props.rgSources[this.state.nImage]),
            e ||
              (console.warn(
                "MultiSourceImage created with no image src",
                this.props,
                this.state.nImage,
              ),
              (e =
                "data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=")),
            e
          );
        }
        get imgRef() {
          return this.m_refImage;
        }
        OnImageError(e) {
          this.props.onIncrementalError &&
            this.props.onIncrementalError(
              e,
              this.props.rgSources[this.state.nImage],
              this.state.nImage,
            );
          let t = this.state.nImage + 1;
          t >= this.props.rgSources.length &&
            this.props.onError &&
            this.props.onError(e),
            t < this.props.rgSources.length && this.setState({ nImage: t });
        }
        render() {
          const {
              rgSources: e,
              onIncrementalError: t,
              onError: o,
              strAltText: s,
              ...i
            } = this.props,
            r = this.src,
            a = this.state.nPropChangeCounter;
          return (0, n.jsx)(
            "img",
            {
              ref: this.m_refImage,
              ...i,
              src: r,
              onError: this.OnImageError,
              alt: s,
            },
            a,
          );
        }
      }
      (0, s.Cg)([r.oI], a.prototype, "OnImageError", null);
    },
  },
]);
