/**** (c) Valve Corporation. Use is governed by the terms of the Steam Subscriber Agreement http://store.steampowered.com/subscriber_agreement/.
 ****/
(self.webpackChunkstore = self.webpackChunkstore || []).push([
  [23027],
  {
    33924: (e) => {
      e.exports = {
        OtherEventsCtn: "_9H6b5yfaxlmcnHvkqtwDK",
        OtherEvents_MainImageCtn: "_2qyLPxO8_nkczRvFiaju8N",
        OtherEvents: "_16DzRvjcqFcYr0NYcWmTrg",
        EventSizer: "_2JC5DEuXUeE50kjpb7Eeau",
        OtherEvents_EventCtn: "_1MwNf8slOG9lOvAeOshmuu",
        EventSummaryText: "ENbI1gFgvIca6HSKAbfiJ",
        ShowInWideMode: "RLbLb742gN095uDUITtIB",
        EventSummaryContainer: "_2GYp44BuZLfKRQdeILTDC3",
        HideInWideMode: "_3itHivPkrgI7TWENi1yxjI",
        OtherEvents_ContentCtn: "_22jEpNTfml-w_aRJV-fKDm",
        HoversEnabled: "_3o6M87A6T172WsUE6MNvdW",
        OtherEvents_TextTitle: "_2jc1DpJ_WzFtigRh5qDWce",
        OtherEvents_MainImage: "_3_wKbXvT7_y5YkrtadL0I6",
        PartnerEventRowCapsule_MainImage: "bC2Zkx7FlANno4SW8FwB-",
        EventSummaryType: "_11JXznGoylLSEmZXZbgcsq",
        OtherEvents_BGImage: "_2pPj9UWoWM6h318uBN0-8X",
        MaskImages: "_1kFdtNfhXozP4yI_qOv2H-",
        OtherEvents_TextCtn: "_3-EtNa1Nr_737K0kglkT9C",
        UpcomingCtn: "_2CXrGPtlQh-j3aSa6XsQDI",
        OtherEvents_SubTitle: "_1Swox5XYdeesack-J7fNLH",
        EventType: "_2BWwVF5N-3fDuJRblB6gHb",
        AppCapsuleImage: "_3OzV3h4jW1bkLmB6TqbYmo",
        CapsuleShadow: "_2rjkJQtvus70aLmbfGoneD",
        AppCapsuleCtn: "_16au-uWHggl6G731aw_eHt",
        AppCapsuleImageHover: "IeC3X0McKdGC79BsC3VvM",
        AppCapsulePrice: "_2-l2M5GPuxKFwV8h1tc_fH",
      };
    },
    28124: () => {},
    85415: (e) => {
      e.exports = {
        narrowWidth: "500px",
        "duration-app-launch": "800ms",
        NominationsPageContent: "w8AKcU3i8ClK1UzQzqzI",
        SectionContent: "_20IkdxpCDwL8VLda92dq2Y",
        SteamAwardCategories: "_192O0SfNHIDsbHXq5iRgCM",
        NominationCategory: "_2G4S0SFOCuBF1c4Bx-dELf",
        BackgroundDark: "_1tIgRv45QmeUOQXrV9mdMq",
        NominationsHeaderCtn: "_1TnKc86XFqFWZThfWLgaRw",
        FAQHeaderArea: "_2Mwx1ICH6jNzJoeIPhhFDv",
        FAQHeaderCtn: "_3Jx_3njrOqCGvmBb5b53I6",
        FriendsHeader: "_27LlNtlWucjFwSNWY-Sbyb",
        FAQSaletitle: "_3s_6VHcgaXEyyUZ16zweVp",
        FAQComingsoon: "_1cG2WETjKlQ8Z5EC0lw0i9",
        NominationsFAQCtn: "_296AV5WTzSw4KsXXNtA2_x",
        FaqSectionTitle: "_1XIaoVZxTy-yIN4iv4QqL1",
        LeftCol: "Swdwp186ooghtuQCkEqml",
        RightCol: "_2RbOCvYEtWD-qpyXmp3Ye7",
        FaqEntry: "_3QrF1mai6HX6dr7jJijnTT",
        FAQ_Q: "_3Mou7FjFlqrR-kKuTt9N8g",
        FAQ_A: "-K9ED0JWohLoxFufaRmWt",
        SteamAwardCategory: "_3i1u-y7pjl-qc98gufdZUu",
        PickerOpen: "_1rSWia4DrhNXjIAW9m_H7R",
        SteamAwardCategoryBackground: "_35LJC3vUDhyY8YWBfRTzRB",
        Nominated: "_4GIBs4zFQvYzih0mbwMoB",
        SteamAwardCategoryBlurryBackground: "_3obFcpxM1lY5DMX8dIuroo",
        CategoryRow: "_1n-khJ_oWP9-ADtdauxmaP",
        LeftColumn: "_3Od-8EtYzDWL-rqzXdco_3",
        CategoryTitleRow: "_21wHFSOia958bDDSKRAnuz",
        Checkbox: "_33AlTMfX070fpO5wsAbYCj",
        CategoryTitle: "_3nVYk2PImiZU57xx8MxH-X",
        CategoryDescription: "_3mFT207sfDAaxfn-HB15sj",
        RightColumn: "_1g7X8N5RaLG09kZx1u2fbV",
        NominateBtnCtn: "_1HYrGX2M4CebVc7ClAyqi5",
        NominateButton: "_3yYqk-nDDM0DcyB3Id8NjD",
        PickerShown: "_2O-paI1bSbPTq6FALASFAe",
        ActionLogin: "_3s5xsmskAdOpXRyYWej0pj",
        ActionEdit: "_2p6oT45t7Kjk11DKng_WDR",
        CapsuleContainer: "_1fSqpH3qLOnMPsLvsocjDD",
        CapsuleBlurryContainer: "_1NTFi-9ML6rmzzrsR1dDt4",
        SectionTitle: "_2pyDiHU7ljm9ZRO_MWsJwr",
        BadgeSectionCtn: "_2lMcC-ffwNaKEZXfHcIZ2R",
        BadgeTasksCtn: "lVHOaGMQxyd0Pv3gC9QUj",
        BadgeTask: "_3nxfLmk626DE9RgzkJKLsD",
        TaskTitle: "uZMkLwiiLuPCt8EGWV29h",
        BadgeStatusCtn: "_2y8n9AG3gE8R7p0vnCu11y",
        BadgeStatusTitle: "_2sMYOvGVozOqLO6WXFCAMk",
        BadgeStatusDesc: "_3L4e0hBb8fmEtnQ8yFIoZe",
        BadgeImageRow: "_2MLgogShQSsdXcjJVnCrs_",
        BadgeItem: "PJRtCS56eG2Y0_F_VSsx7",
        BadgeImage: "_2sB2cTqit-_ethQFZRSqSN",
        Active: "_20Ol8bIY8KcNPBctmmB8zn",
        NominationPickerCtn: "_3I1Ga7kMZPDXXbOfgfBTDj",
        CarouselView: "_3mKDmumAiG1vMlU0jWpTe0",
        BottomRow: "a3_pmPQJhnwuVtzHgSyu8",
        TopRow: "_1ARPXpnwKMwds0ED6n65hh",
        CloseButton: "_2kep5HQu4ssXfj5IcG2_Zk",
        TopBarText: "_1CNpmoVrp_326dsK0CmqNS",
        SearchBarCtn: "_2sWS-29Gf-VMMR6kq1I0bN",
        RecommendationRow: "_3Xwn8eFK1QXh1ckrOXov07",
        RecommendationRowTitle: "_2Q0KQMyl8fMBpSMgOn0jsf",
        RecommendationRowSubtitle: "xKXQL9kh2IaXx62pUvaM8",
        Events: "kigvxizh8JmTyh5cyzSEA",
        Games: "IpzhVl6SWsWj5Vcmktozv",
        NoEligibleGamesCtn: "_2AqJTwluya2l8sHRS0Nrne",
        SearchContainer: "KBqkfDknFrTcr7ETAE2OW",
        SearchThrobber: "i4KlSZuYm2iFdxbjUegqJ",
        SearchResultsContainer: "_3M2mdkv-ZvUKBkLPH46U8S",
        SearchResultApp: "_3cbS4zsH_qv-1ZcwaA-4Tt",
        NominateGameButton: "CEmahjyiRmgWZlXL6XEpv",
        GameCarouselItemCtn: "_1Em3_QE0y0zfLhSA7n0NDm",
        PlaytimeIndicator: "_3t0T2BFkpBYduyOOwxnBI2",
        BadgeSectionTitle: "_3G1iNhfSRJ9NLuaxjN8MRA",
        White: "_17enANXtRLMRK09o9OKDdF",
        NoResultsCtn: "_2IlW9sfsAiruuo0GxHGgwl",
        ShareLinkCtn: "_15-jFxRvrZDrlMsnc5flU3",
        FriendsHeaderCtn: "_1T9FWlr19WP72HpE5wYup8",
        FriendsTitleCtn: "_3we3-2_E8Qgoj9vpaFu9JG",
        FriendsTitle: "oaA8CcFGe_Kz-3D-qR28Z",
        ProgressAndShareCtn: "_3FdxnzdLCLEUpdpoJJThYZ",
        ProgressTitle: "_3x6HhTNw3LbAwygEgTzwDr",
        ShareBtn: "_1t4jW207kK9lq0dm7UwBIq",
        HeaderButtonCtn: "WSKzFdd9sj_fIAKnqmiOT",
        Gold: "_3f-rIyt2OF08sZWkdxcuzG",
        EventCarousel: "_1ggzska8h4zQPvkSiT3642",
        ShareModalBody: "_3gj5bE9dm_7GXn87PPWJ7q",
        ShareModalText: "_1xonFOaYYv3jpu4vluDcs6",
        IntroText: "_3_Vb7QOOCe07PD_0t4UL0",
        Button: "_2Ynf3ZGFeViNrevWHYC6a-",
        UrlContainer: "_3CXjxVNiKxUH_xCflrU0fv",
        Url: "_3YxsXALKJ70zL3MTTWqibh",
        GenerateShareLinkBtn: "_23i3vxhJO5yZFZ8UkYUHfq",
        BackgroundAnimation: "_2LOnSlelExzMt5V4a0dNMM",
        "ItemFocusAnim-darkerGrey-nocolor": "_1HvhiBok8gSNZxjvKJodk_",
        "ItemFocusAnim-darkerGrey": "_27N4Tx9ZkLoouSi6u2L0Rj",
        "ItemFocusAnim-darkGreySettings": "_3BuduqT5jtwJUrO_Rlx9pN",
        "ItemFocusAnim-darkGrey": "_1t5LnzcxRDUwMaXczWLvT9",
        "ItemFocusAnim-grey": "UvYtyIHWLaxtSAiaOZoaD",
        "ItemFocusAnim-translucent-white-10": "_2Ze0Xg_Rmw0Fw4dtnjqu06",
        "ItemFocusAnim-translucent-white-20": "_6KKdX8p_ia-DW2BgO6XxL",
        "ItemFocusAnimBorder-darkGrey": "_2VHjMLLOWNhhfZTRb8KGKl",
        "ItemFocusAnim-green": "_3qPaUa-qxMzbOWeav6OXLP",
        focusAnimation: "_2YJq7kaNhJ2mQ-Kc9fGZdd",
        hoverAnimation: "_1Ic0fzk_zPsp6jbxsdCaeZ",
      };
    },
    26019: (e) => {
      e.exports = {
        HeaderCtn: "_1oXTW_jpUID161hkqtu59M",
        EventTitle: "_3xLoBdLse_J3sDGG1p-yXS",
        InfoText: "_2ajjx_6sFyPgZhqB5H3ZnC",
        Large: "TDUFDP_Bl5TP_b5lgQUzr",
        FAQCtn: "_21i-Qc2WCMVQJPd0uWk5t4",
        FAQ_Q: "dwrawv-PnUTEQ1WVRwb1A",
        FAQ_A: "_2EmOv2BukDb5pXZiDipwMd",
        ProgressCtn: "_10nGaDuKJPA3cGdfbd7NPA",
        CategoryStickerCtn: "bI10T2_lqnfiIng7NCXzs",
        CategoryStickerHover: "_10SrgdoAjwiz9kvKG9lBKZ",
        FaqSectionTitle: "_3iDsXG7lNe9DQrxH1NNhwu",
      };
    },
    79870: (e) => {
      e.exports = {
        HeaderCtn: "_1GMPkxVZv0yXnOWg0EPh6C",
        EventTitle: "_2tLWKoQISyrtT5cRa7Bthz",
        InfoText: "_3FtbGRpd5Nc_3f0NWPSmPD",
        Large: "_3lWFGFpuyI0hO8I3yb_rNn",
        FAQCtn: "_3VrYL1Qsqq2VRPBJkD_HaR",
        FAQ_Q: "_3RNM6O7oImIghEzoU__8Xv",
        FAQ_A: "Eq7UcjWcQdQ6tRTSkwOAX",
        ProgressCtn: "_33gp2xeb7nySC065zyyBA4",
        CategoryStickerCtn: "BLVGwcLwZotI2A0JYeAmO",
        CategoryStickerHover: "_3gQFhFaB86zoAkrwBD3DlC",
        FaqSectionTitle: "_1M-Tp2eIsHzDqKVdeyNwEE",
      };
    },
    66701: (e) => {
      e.exports = {
        HeaderCtn: "_1LNVToRGS4KgtDmwChCdO-",
        EventTitle: "SHcWFaCZK_WUravI9p1Lm",
        InfoText: "_3Rnea2kVxFT3chmOmqisne",
        Large: "_2jbNvt46h0RqPVOeAwcb8o",
        FAQCtn: "_3UcPLvxpWKhkpPp5vuNZ4T",
        FaqEntry: "_8B-oP4jghkxueOwLkY5FR",
        FAQ_Q: "_3PJmH1yq33fNNGz3U9m-G0",
        FAQ_A: "_2CZcny1e2NwmV-B3klLJiN",
        ProgressCtn: "_3NRXvKouZmkfv0SJaRwm3z",
        CategoryStickerCtn: "_3aAMtCHlbBv9NIEZdV0q4E",
        CategoryStickerHover: "MofbE6cz8noD7aLG-RuYg",
        FaqSectionTitle: "_1KImUETUAUTMVe-r0tBhbv",
      };
    },
    4775: (e) => {
      e.exports = {
        narrowWidth: "500px",
        "duration-app-launch": "800ms",
        VotingPageContent: "_1HBQ3phQnNEooQ3rlXJWEU",
        SectionContent: "zKwKtEP3BYYZy9vgcUrYt",
        HeaderCtn: "_13RFBrxHyoARje0nMxX8tH",
        HeaderContent: "_1o6dhi3wA_BGCQuKqQs9Ms",
        TextColumn: "PsKAPvFalwTAqd-O5tpBs",
        FriendsHeader: "_2pdEK8BhZO9Fsk7gBzJcYG",
        EventTitle: "_2mq56csinWHiVtUbhcbIOM",
        InfoText: "_1fILAMegJPwehj7zLNiCnz",
        Large: "_2mg9K9HjkOCk8oaDZr8nkp",
        FaqSectionTitle: "_1EC9jX-4aGWknZdkL2jXz_",
        FAQWrapper: "_3k0Nep9QQO6OOkrJKij0rN",
        FAQCtn: "_2771UKLz1V9nHv-LmzpMxN",
        FaqEntry: "_2ajVjokAhuY_AzHO1fxS-N",
        FAQ_Q: "_1HaJqKTqlf4p0CRH0hJkFP",
        FAQ_A: "_3471NyjBfuVEY6GEgzQvTL",
        CategoryList: "_1XPN-o00qToVhhqCl0uCTs",
        SteamAwardCategory: "_1XBoC_51pQmCRN5avridVk",
        Anchor: "VcuHpIeUsOyyyT_KdUjAm",
        Active: "_1O8sHGuQHPL5zH5Sb46LPO",
        SteamAwardCategoryBackground: "_1ZcFSopSc0cwMRbi5HSVry",
        SteamAwardCategoryBlurryBackground: "L4OcSCmIVcuUl12bXB0TX",
        CategoryVoted: "_1t5sSouAYZGCwMwKgvAnnK",
        FinalistGameCtn: "_1BgO7N4S-tDGDCVnQbvKkq",
        MyVote: "_280DKBVL06EUXmV8oW_Stf",
        FinalistsRow: "gyb_bFa1822peSfG3BTXg",
        CurrentlyFeatured: "_26AMuDHs36difPA2OiqXtX",
        CapsuleLink: "_2jKUwrwbrF2t4kX3B82jyy",
        Highlight: "_2U0iDjf7-xhJmnwL85CgAL",
        CategoryRow: "_2coYuktGYgHFSf2bVXdpq_",
        LeftColumn: "_3Jr5t3bKaU_ex0uuo649cF",
        CategoryTitleRow: "xuEPVrSwJC-vQEtreQvnm",
        CategoryYear: "mRgEVFiqQRKYg192KYX8p",
        CategoryTitle: "_3Ly3DC2P8CDiJ7Xasn2ebQ",
        CategoryDescription: "JP2ZzubSDTJPbrIuVD2dj",
        RightColumn: "p_hWPrN8iGC3gBczTo-6t",
        HideShowBtn: "_15hKKH8LRiReglaS1WuEv3",
        CapsuleContainer: "_1hsDa2rFPpjzMu9rjU82U2",
        MicrotrailerVideo: "_3vYc4xomNAmkqKxT6BWe7-",
        CurrentVoteBanner: "_1jM-nBkKTkN_b8SE_j6ZlE",
        FinalistsCtn: "e6QP7VDRKVVFuD2RnEOjR",
        Enter: "_2bR4vh-7XndeYuAbZ9m3mi",
        EnterActive: "HzogqjfTp-YZfCxAYW1fk",
        "open-finalists": "_28Ukz2J5tkhC8hlAaMrh4a",
        Exit: "_1ihgKIWB0hGadYHxkDnfkc",
        ExitActive: "_2PHm0gzC_ah6_cbZ3Oy37F",
        "close-finalists": "_16yj_nglGTyrUl6zzC-jbj",
        FinalistsIntro: "_1tf4a4qNCkQo4ge5yBdOKV",
        FinalistsLine: "_1Se69bGQiI6RkGrQPuVBAa",
        Capsule: "bfZUvnddL__eLX2GmRP-F",
        ActionButton: "_38mJcOp4-kSzGLzkPdUCp",
        YourNomination: "_2KubnqoAbzyD4ZXRpsMPHy",
        WinnerInfoCtn: "_33Gx5kCJIh6ENRNdv04f9b",
        WinnerText: "joZ0rl06vZjm-lcXLbuSO",
        WinnerName: "_3ycfW5X-9UvOmi_kWZgnNU",
        WinnerCapsule: "_3ELJccHCME0WPJAkoGrxJt",
        VotingArea: "n6ZgKyivXpFdb4SqlA64M",
        ProgressCtn: "_2vUweIYz36Cfi6nm57qY0E",
        Title: "_3BVrSWCKUqUJI-Y9JHyPzu",
        StickerRow: "_1zyzM5BB2snSZ78ORI-3no",
        CategoryStickerCtn: "_1-Z8yTrwnOFgH8CFXjAh50",
        CategorySticker: "_1draD7X6gu1HpgfRr0k3bO",
        Inactive: "_1N55FNJtt9fgD3oEKh1ulo",
        CategoryStickerHover: "_16qkH83hzNBjI3OfNA2yPZ",
        BackgroundAnimation: "_1seXoMt6Gw5ShAn1fMGCk1",
        "ItemFocusAnim-darkerGrey-nocolor": "_2Pfbc5UJLub15f3GMuUOpu",
        "ItemFocusAnim-darkerGrey": "_1zD-uKidolXqoKdiqmbZmN",
        "ItemFocusAnim-darkGreySettings": "_1CaH5L10wLn4wyijOgOTH6",
        "ItemFocusAnim-darkGrey": "_3Hclo3bSXjWjLg7ak63JZU",
        "ItemFocusAnim-grey": "_276KuR-DDgs7rfFECDuv2Q",
        "ItemFocusAnim-translucent-white-10": "_1yYdEMizQD0Hu99aKPdShy",
        "ItemFocusAnim-translucent-white-20": "_8CBvtVkjTgCz2X_r9V8wT",
        "ItemFocusAnimBorder-darkGrey": "_3Buhs9OTFb_CFYf4_t6djI",
        "ItemFocusAnim-green": "_436DEQij8XRuUbarXlQ7H",
        focusAnimation: "_2oswwg33QBrwNVBaVy2Cz0",
        hoverAnimation: "ux33-vIaysS2_ZFy8EmtK",
      };
    },
    50433: (e, a, t) => {
      "use strict";
      t.d(a, { u: () => Q, z: () => B });
      var n = t(7850),
        s = t(7221),
        i = t(22837),
        r = t(76217),
        o = t(46107),
        l = t(32803),
        d = t(66418),
        c = t(29008),
        m = t(42834),
        _ = t(84518),
        u = t(14987),
        h = t(39777),
        g = t(60014),
        p = t(38535),
        w = t(65946),
        A = t(83392),
        x = t(90626),
        S = t(62641),
        v = t(33924),
        j = t.n(v),
        N = t(48593),
        C = t(18654),
        f = t.n(C),
        F = t(72860),
        y = t(76684),
        I = t(3088),
        T = t(12155),
        b = t(52038),
        k = t(78327);
      const E = 30;
      function Q(e) {
        const {
            event: a,
            imageURLOverride: t,
            bShowAssociatedApp: r,
            langOverride: c,
            onClick: _,
            eEventRount: g,
            bHidePrices: v,
            nSummaryMaxLength: C,
          } = e,
          f = (0, p.Zj)(a.appid),
          F = (0, N.n)(),
          T = c || (0, i.sfN)(d.TS.LANGUAGE),
          k =
            (0, o.m0)(
              void 0 !== t ? void 0 : a,
              "capsule",
              T,
              s.wI.capsule_main,
            ) ?? t,
          Q =
            (0, o.m0)(void 0 !== t ? void 0 : a, "capsule", T, s.wI.full) ?? t,
          [D, L, R, P] = (0, w.q3)(() => [
            a.GetNameWithFallback(T) || "",
            a.GetCategoryAsString(),
            a.GetSummaryWithFallback(T, C),
            a.GetSubTitleWithLanguageFallback(T) || "",
          ]),
          O = (0, u.$5)(a.appid),
          { data: V } = (0, h.lv)(O),
          H = [];
        if ((k && H.push(k), Q && Q !== k && H.push(Q), V)) {
          const e = (0, m.b0)(V, "main_capsule");
          e && H.push(e);
        }
        const [M, G] = (0, x.useState)(k);
        if (!a)
          return (0, n.jsx)("div", { className: j().OtherEvents_EventCtn });
        const W = a ? a.GetStartTimeAndDateUnixSeconds() : 0;
        let U = P;
        return (
          P && (P.length > E || D.length > E) && (U = void 0),
          (0, n.jsxs)("div", {
            className: j().EventSizer,
            children: [
              (0, n.jsxs)(l.tj, {
                className: (0, b.A)(
                  j().OtherEvents_EventCtn,
                  "OtherEvents_EventCtn",
                  j().HoversEnabled,
                ),
                eventModel: a,
                route: g || l.PH.k_eView,
                onClick: _,
                preferredFocus: !0,
                children: [
                  (0, n.jsxs)("div", {
                    className: (0, b.A)(
                      j().EventSummaryContainer,
                      j().HideInWideMode,
                    ),
                    children: [
                      (0, n.jsx)("div", {
                        className: j().EventSummaryType,
                        children: L,
                      }),
                      (0, n.jsx)("div", {
                        className: j().EventSummaryText,
                        children: R,
                      }),
                    ],
                  }),
                  (0, n.jsx)("div", {
                    className: j().OtherEvents_BGImage,
                    style: {
                      backgroundColor: "#ffffff",
                      backgroundImage: M ? `url(${(0, S.j3)(M)})` : "none",
                    },
                  }),
                  (0, n.jsxs)("div", {
                    className: j().OtherEvents_ContentCtn,
                    children: [
                      (0, n.jsx)("div", {
                        className: (0, b.A)(
                          j().OtherEvents_MainImageCtn,
                          f && j().MaskImages,
                        ),
                        children: (0, n.jsx)(I.c, {
                          rgSources: H,
                          onIncrementalError: (e, a, t) => {
                            t >= H.length && G(void 0), G(H[t + 1]);
                          },
                          className: j().OtherEvents_MainImage,
                          alt: "",
                        }),
                      }),
                      (0, n.jsxs)("div", {
                        className: j().OtherEvents_TextCtn,
                        children: [
                          (0, n.jsx)("div", {
                            className: j().OtherEvents_TextTitle,
                            children: D,
                          }),
                          Boolean(U) &&
                            (0, n.jsx)("div", {
                              className: j().OtherEvents_SubTitle,
                              children: U,
                            }),
                          (0, n.jsxs)(A.s, {
                            direction: "row",
                            gap: "3",
                            align: "center",
                            children: [
                              (0, n.jsx)("div", {
                                className: (0, b.A)(
                                  j().EventType,
                                  j().ShowInWideMode,
                                ),
                                children: L,
                              }),
                              Boolean(W > F)
                                ? (0, n.jsx)("div", {
                                    className: (0, b.A)(
                                      j().UpcomingCtn,
                                      "UpcomingCtn",
                                    ),
                                    children: (0, n.jsx)(y.K4, {
                                      bSingleLine: !0,
                                      dateAndTime:
                                        a.GetStartTimeAndDateUnixSeconds(),
                                    }),
                                  })
                                : (0, n.jsx)(y.K4, {
                                    bSingleLine: !0,
                                    bOnlyDate: !0,
                                    dateAndTime:
                                      a.GetStartTimeAndDateUnixSeconds(),
                                  }),
                            ],
                          }),
                          (0, n.jsx)("div", {
                            className: (0, b.A)(
                              j().EventSummaryText,
                              j().ShowInWideMode,
                            ),
                            children: R,
                          }),
                        ],
                      }),
                    ],
                  }),
                ],
              }),
              Boolean(r && a.appid) &&
                (0, n.jsx)(B, { appid: a.appid, bHidePrice: v }),
            ],
          })
        );
      }
      function B(e) {
        const { appid: a, bHidePrice: t } = e,
          s = (0, u.$5)(a),
          { data: i } = (0, h.J$)(s),
          { data: o } = (0, h.lv)(s),
          { data: l } = (0, h.Q_)(s),
          d = (0, g.n9)(),
          p = (0, k.Qn)();
        if (!o || !i) return null;
        const w = l && l.hide_discount_pct_for_compliance;
        return (0, n.jsx)(_.A, {
          appID: a,
          children: (0, n.jsxs)(r.Z, {
            className: (0, b.A)(j().AppCapsuleCtn, "AppCapsuleCtn"),
            ...(0, F.S)(i, d, p, !1),
            children: [
              (0, n.jsx)(c.Q, {
                id: s,
                hoverProps: {
                  direction: "overlay",
                  style: { minWidth: "320px" },
                },
                children: (0, n.jsx)("img", {
                  className: (0, b.A)(j().AppCapsuleImage, j().CapsuleShadow),
                  src: (0, m.b0)(o, "small_capsule"),
                  alt: i.name,
                }),
              }),
              Boolean(!t && !i.is_free) &&
                (0, n.jsxs)("span", {
                  className: (0, b.A)(
                    j().AppCapsulePrice,
                    Boolean(l?.discount_pct) ? f().Discounted : "",
                  ),
                  children: [
                    Boolean(l?.discount_pct && w) &&
                      (0, n.jsx)("div", {
                        className: f().DiscountIconCtn,
                        children: (0, n.jsx)(T.XH_, {}),
                      }),
                    Boolean(l?.discount_pct && !w) &&
                      (0, n.jsx)("span", {
                        className: f().StoreSaleDiscountBox,
                        children: `-${l?.discount_pct}%`,
                      }),
                    l &&
                      l.final_price_in_cents &&
                      (0, n.jsx)("span", {
                        className: f().StoreSalePriceBox,
                        children: l.formatted_final_price,
                      }),
                  ],
                }),
            ],
          }),
        });
      }
    },
    20433: (e, a, t) => {
      "use strict";
      t.d(a, { j: () => u, u: () => h });
      var n = t(7850),
        s = t(90626),
        i = t(45699),
        r = t(55963),
        o = t(60014),
        l = t(49411),
        d = t(61336),
        c = t(52541),
        m = t(66418);
      t(78327);
      function _(e) {
        if (e) {
          if ("appid" in e) return "app";
          if ("bundleid" in e) return "bundle";
          if ("packageid" in e) return "sub";
        }
      }
      function u(e) {
        const {
            id: a,
            hoverClassName: t,
            fnGetIDOverride: i,
            fnHoverState: r,
            disableScreenshots: o,
            children: l,
          } = e,
          d = s.useRef(null),
          m = s.useCallback(
            (e) => {
              const t = _(a);
              t &&
                (r && r(!0),
                window.GameHover &&
                  (d.current &&
                    o &&
                    (d.current.dataset.hoverDisableScreenshots = "true"),
                  window.GameHover(i ? i() : d.current, e, "global_hover", {
                    type: t,
                    id: (0, c.G$)(a).id,
                    v6: 1,
                  })));
            },
            [r, i, o, a],
          ),
          u = s.useCallback(
            (e) => {
              _(a) &&
                (r && e.relatedTarget && r(!1),
                window.HideGameHover &&
                  window.HideGameHover(i ? i() : d.current, e, "global_hover"));
            },
            [a, r, i],
          );
        return (0, n.jsx)("div", {
          ref: d,
          className: t,
          onMouseEnter: m,
          onMouseLeave: u,
          onFocus: m,
          onBlur: u,
          children: l,
        });
      }
      function h(e) {
        const {
            id: a,
            strExtraParams: t,
            fnOnClickOverride: s,
            strOverrideURL: h,
          } = e,
          g = (0, o.n9)(),
          p = (0, l.w)(),
          w = (0, d.NT)(
            h ||
              (a && "creatorid" in a
                ? (0, r.It)(
                    `${m.TS.STORE_BASE_URL}curator/${((0, c.G$))(a).id}${t ? `?${t}` : ""}`,
                    g,
                    p,
                  )
                : (0, r.It)(
                    `${m.TS.STORE_BASE_URL}${_(a)}/${((0, c.G$))(a).id}${t ? `?${t}` : ""}`,
                    g,
                    p,
                  )),
          );
        return (0, n.jsx)(u, {
          ...e,
          children: (0, n.jsx)(i.Ii, {
            className: e.className,
            href: s ? void 0 : w,
            target: m.TS.IN_CLIENT || s ? void 0 : "_blank",
            rel: "noopener noreferrer",
            onClick: s,
            children: e.children,
          }),
        });
      }
    },
    82477: (e, a, t) => {
      "use strict";
      t.d(a, { Cg: () => h, pZ: () => p, vg: () => g });
      var n = t(7850),
        s = t(90626),
        i = t(738),
        r = t(61859),
        o = t(78327),
        l = t(92120),
        d = t(51883),
        c = t(72034),
        m = t(28240),
        _ = t(74568);
      function u(e) {
        return (0, n.jsx)(i.x_, {
          onEscKeypress: e.closeModal,
          bDisableBackgroundDismiss: !0,
          children: (0, n.jsx)(w, {
            redirectURL: e.redirectURL,
            guestOption: e.guestOption,
          }),
        });
      }
      function h(e) {
        const { redirectURL: a = window.location.href } = e;
        return (0, n.jsx)(_.EN, {
          active: !0,
          children: (0, n.jsx)(u, { redirectURL: a }),
        });
      }
      function g() {
        (0, i.pg)(
          (0, n.jsx)(u, {
            ownerWin: window,
            redirectURL: window.location.href,
          }),
          window,
          { strTitle: (0, r.we)("#Login_SignInTitle") },
        );
      }
      function p(e, a) {
        (0, i.pg)(
          (0, n.jsx)(u, { ownerWin: window, redirectURL: e, guestOption: a }),
          window,
          { strTitle: (0, r.we)("#Login_SignInTitle") },
        );
      }
      function w(e) {
        const { redirectURL: a, guestOption: t } = e,
          [i] = (0, s.useState)(
            new c.D(o.TS.WEBAPI_BASE_URL).GetAnonymousServiceTransport(),
          ),
          [r, _] = (0, s.useState)(!1);
        return (0, n.jsx)("div", {
          children: r
            ? (0, n.jsx)(l.Fn, {})
            : (0, n.jsx)(l.YN, {
                autoFocus: !0,
                transport: i,
                platform: d.SS.tS,
                onComplete: (e) => {
                  e == m.wI.k_PrimaryDomainFail
                    ? _(!0)
                    : window.location.assign(a);
                },
                redirectUrl: a,
                theme: "modal",
                children: t && (0, n.jsx)(l.Mk, { redirectURL: a }),
              }),
        });
      }
    },
    14326: (e, a, t) => {
      "use strict";
      t.d(a, { _: () => N, r: () => j });
      var n = t(7850),
        s = t(95578),
        i = t(14987),
        r = t(39777),
        o = t(60014),
        l = t(58918),
        d = t(17376),
        c = t(79969),
        m = t(90626),
        _ = t(55963),
        u = t(18654),
        h = t.n(u),
        g = t(39700),
        p = t(12155),
        w = t(32754),
        A = t(52038),
        x = t(61859),
        S = t(78327),
        v = t(84547);
      function j(e) {
        const { appid: a, className: t, bTextMode: s } = e,
          o = (0, i.$5)(a),
          { data: l } = (0, r.J$)(o),
          { data: d } = (0, r.by)(o);
        return (0, n.jsx)(N, {
          appid: a,
          bIsFree: Boolean(l?.is_free),
          bIsComingSoon: Boolean(d?.is_coming_soon),
          bTextMode: s,
          className: t,
        });
      }
      function N(e) {
        const [a, t] = m.useState(!1),
          s = (0, o.n9)(),
          {
            appid: r,
            bIsFree: u,
            bIsComingSoon: j,
            className: N,
            bTextMode: f,
          } = e,
          F = (0, i.$5)(r),
          { bIsOwned: y } = (0, l.ZJ)(F),
          I = (0, d.bB)(r),
          { mutateAsync: T } = (0, c.s)(r, !I, (0, _.L3)(s)),
          { elDialogElement: b, fnShowLogonDialog: k } = (0, v.l)();
        if (y || (!j && u))
          return u ? (0, n.jsx)(C, { possibleDemoAppID: r }) : null;
        let E = null;
        return (
          a && !f
            ? (E = (0, n.jsx)(g.k, { size: 18 }))
            : I
              ? I && (E = f ? (0, x.we)("#OnWishlist") : (0, n.jsx)(p.qnF, {}))
              : (E = f
                  ? (0, x.we)("#wishlist_add_to_wishlist")
                  : (0, n.jsx)(p.T4m, {})),
          (0, n.jsxs)(n.Fragment, {
            children: [
              (0, n.jsx)(w.he, {
                toolTipContent: (0, x.we)("#AddToWishlist_ttip"),
                children: (0, n.jsx)("div", {
                  className: (0, A.A)(h().WishList, N),
                  onClick: async () => {
                    S.iA.logged_in ? a || (t(!0), await T(), t(!1)) : k();
                  },
                  children: E,
                }),
              }),
              b,
            ],
          })
        );
      }
      function C(e) {
        const { possibleDemoAppID: a, className: t } = e,
          o = (0, i.$5)(a),
          { data: l } = (0, r.J$)(o);
        return l
          ? (l.type != s.uE.ue && l.type != s.uE.Vi) ||
            !l.related_items?.parent_appid
            ? null
            : (0, n.jsx)(f, {
                parentAppID: l.related_items?.parent_appid,
                className: t,
              })
          : null;
      }
      function f(e) {
        const { parentAppID: a, className: t } = e,
          s = (0, i.$5)(a),
          { data: o } = (0, r.J$)(s),
          { data: l } = (0, r.by)(s);
        return o && l
          ? (0, n.jsx)(N, {
              appid: a,
              bIsComingSoon: Boolean(l.is_coming_soon),
              bIsFree: Boolean(o.is_free),
              className: t,
            })
          : null;
      }
    },
    5136: (e, a, t) => {
      "use strict";
      t.r(a), t.d(a, { default: () => ta });
      var n = t(7850),
        s = t(43527),
        i = t(92757),
        r = t(90626),
        o = t(85415),
        l = t.n(o),
        d = t(45699),
        c = t(76217),
        m = t(20433),
        _ = t(71322),
        u = t(30470),
        h = t(56011),
        g = t(78327),
        p = t(56283),
        w = t(12155),
        A = t(61859),
        x = t(52038),
        S = t(74568),
        v = t(738),
        j = t(42457),
        N = t(82477),
        C = t(75682),
        f = t(22797),
        F = t(55263),
        y = t(37085),
        I = t(32630),
        T = t(75933),
        b = t(43568),
        k = t(91254),
        E = t(17720),
        Q = t(87652),
        B = t(50433),
        D = t(16021),
        L = t(34214),
        R = t(81301),
        P = t(84933),
        O = t(13871),
        V = t(28124),
        H = t(93826),
        M = t(14987),
        G = t(39777),
        W = t(42834);
      const U = {
        include_basic_info: !0,
        include_assets_without_overrides: !0,
      };
      function q(e) {
        const { category: a, fnShowPicker: t, rgPrevLaborOfLoveWinners: s } = e,
          [i, o] = r.useState("");
        if (!(0, _.jT)(a.voteid).data)
          return (0, n.jsx)(f.t, {
            size: "medium",
            position: "center",
            msDelayAppear: 200,
          });
        let c = (0, A.we)("#Steamawards_Nominate_ThisYear");
        return (
          a.flag == C.Xs.bV &&
            (c = (0, A.we)("#Steamawards_Nominate_PastYear")),
          (0, n.jsxs)("div", {
            className: l().NominationPickerCtn,
            children: [
              (0, n.jsxs)("div", {
                className: l().TopRow,
                children: [
                  (0, n.jsx)("div", { className: l().TopBarText, children: c }),
                  (0, n.jsx)("div", {
                    className: l().SearchBarCtn,
                    children: (0, n.jsx)(p.pd, {
                      focusOnMount: !0,
                      onChange: (e) =>
                        o(e.currentTarget.value.toLocaleLowerCase()),
                      value: i,
                      className: l().SearchBar,
                      placeholder: (0, A.we)("#Steamawards_Nominate_Search"),
                      bShowClearAction: !0,
                    }),
                  }),
                  (0, n.jsx)(d.fu, {
                    className: l().CloseButton,
                    onClick: () => t(!1),
                    children: (0, A.we)("#Button_Close"),
                  }),
                ],
              }),
              i.trim().length > 0
                ? (0, n.jsx)(X, {
                    strSearch: i,
                    category: a,
                    rgPrevLaborOfLoveWinners: s,
                  })
                : (0, n.jsx)(z, { category: a, fnShowPicker: t }),
              (0, n.jsx)("div", {
                className: l().BottomRow,
                children: (0, n.jsx)(ee, {
                  unAppID: _.Fq,
                  eSteamAwardCategoryID: a.voteid,
                  eNominatonSource: C.Ji.HW,
                  fnShowPicker: t,
                }),
              }),
            ],
          })
        );
      }
      function z(e) {
        const { category: a, fnShowPicker: t } = e,
          s = (0, _.jT)(a.voteid),
          i = (0, _.cO)(),
          o = (0, g.Qn)(),
          d = r.useMemo(() => {
            let e = [];
            return s.data.played_app
              .map((e) => ({ appID: e.appid, nPlaytime: e.playtime }))
              .filter((t) => {
                const n = i.data?.some(
                  (e) => e.appid == t.appID && e.category_id != a.voteid,
                );
                return !n || (e.push(t), !1);
              })
              .concat(e);
          }, [a.voteid, i.data, s.data.played_app]);
        return (0, n.jsxs)("div", {
          className: l().CarouselView,
          children: [
            s.data?.played_app?.length
              ? (0, n.jsxs)("div", {
                  className: (0, x.A)(l().RecommendationRow, l().Games),
                  children: [
                    (0, n.jsx)("div", {
                      className: l().RecommendationRowTitle,
                      children: (0, A.we)(
                        "#Steamawards_Nominate_GamesYouPlayed",
                      ),
                    }),
                    (0, n.jsx)(J, {
                      eSteamAwardCategoryID: a.voteid,
                      eNominatonSource: C.Ji.MU,
                      rgGameCarouselItems: d,
                    }),
                  ],
                })
              : (0, n.jsx)(Y, {
                  eSteamAwardCategoryID: a.voteid,
                  fnShowPicker: t,
                }),
            Boolean(!o) &&
              s.data?.suggested_events?.length > 0 &&
              (0, n.jsxs)("div", {
                className: (0, x.A)(l().RecommendationRow, l().Events),
                children: [
                  (0, n.jsx)("div", {
                    className: l().RecommendationRowTitle,
                    children: (0, A.we)("#Steamawards_Nominate_Events"),
                  }),
                  (0, n.jsx)(ae, { rgEvents: s.data.suggested_events }),
                ],
              }),
            (0, n.jsxs)("div", {
              className: (0, x.A)(l().RecommendationRow, l().Games),
              children: [
                (0, n.jsx)("div", {
                  className: l().RecommendationRowTitle,
                  children: (0, A.we)("#Steamawards_Nominate_Recommended"),
                }),
                s.data?.suggested_apps &&
                  (0, n.jsx)(J, {
                    eSteamAwardCategoryID: a.voteid,
                    eNominatonSource: C.Ji.qP,
                    rgGameCarouselItems: s.data.suggested_apps.map((e) => ({
                      appID: e.appid,
                    })),
                  }),
              ],
            }),
          ],
        });
      }
      function Y(e) {
        const { eSteamAwardCategoryID: a, fnShowPicker: t } = e;
        return (0, n.jsxs)("div", {
          className: (0, x.A)(l().RecommendationRow, l().NoEligibleGamesCtn),
          children: [
            (0, n.jsx)("div", {
              className: l().RecommendationRowTitle,
              children: (0, A.we)("#Steamawards_Nominate_NoEligibleGames"),
            }),
            (0, n.jsx)("div", {
              className: l().RecommendationRowSubtitle,
              children: (0, A.we)("#Steamawards_Nominate_NoEligibleGames_cont"),
            }),
            (0, n.jsx)(ee, {
              unAppID: _.Fq,
              eSteamAwardCategoryID: a,
              eNominatonSource: C.Ji.HW,
              fnShowPicker: t,
            }),
          ],
        });
      }
      function X(e) {
        const { strSearch: a, category: t, rgPrevLaborOfLoveWinners: s } = e,
          i = (0, _.lE)(a, t, s),
          o = r.useRef(void 0),
          d = (0, g.Qn)();
        return (
          r.useEffect(() => {
            o?.current && d && o.current.scrollIntoView();
          }, [i, d]),
          (0, n.jsx)("div", {
            className: l().SearchContainer,
            ref: o,
            children: Boolean(i.isLoading)
              ? (0, n.jsx)(f.t, {
                  className: l().SearchThrobber,
                  size: "large",
                  position: "center",
                  msDelayAppear: 200,
                })
              : (0, n.jsx)(n.Fragment, {
                  children: Boolean(i.data?.length > 0)
                    ? (0, n.jsx)(c.Z, {
                        className: l().SearchResultsContainer,
                        children: i.data.map((e) =>
                          (0, n.jsx)(
                            Z,
                            {
                              eSteamAwardCategoryID: t.voteid,
                              eNominatonSource: C.Ji.RU,
                              appSuggestion: e,
                            },
                            e.id,
                          ),
                        ),
                      })
                    : (0, n.jsx)("div", {
                        className: l().NoResultsCtn,
                        children: (0, A.we)("#Steamawards_Search_NoResults"),
                      }),
                }),
          })
        );
      }
      function Z(e) {
        const {
            appSuggestion: a,
            eSteamAwardCategoryID: t,
            eNominatonSource: s,
          } = e,
          i = parseInt(a.id),
          r = (0, M.$5)(i);
        return (0, n.jsxs)(c.Z, {
          className: l().SearchResultApp,
          children: [
            (0, n.jsx)(m.u, {
              id: r,
              children: (0, n.jsx)("img", { src: a.small_cap }),
            }),
            (0, n.jsx)(ee, {
              eSteamAwardCategoryID: t,
              eNominatonSource: s,
              unAppID: i,
            }),
          ],
        });
      }
      function K(e) {
        let a = 1;
        return (
          e.innerWidth >= 1080
            ? (a = 4)
            : e.innerWidth >= 800
              ? (a = 3)
              : e.innerWidth >= 600 && (a = 2),
          a
        );
      }
      function J(e) {
        const {
            rgGameCarouselItems: a,
            eSteamAwardCategoryID: t,
            eNominatonSource: s,
          } = e,
          i = (0, F.zX)(
            a?.map((e) => e.appID),
            U,
          ),
          o = (0, O.R7)(),
          d = o?.ownerWindow || window,
          [c, m] = r.useState(() => K(d)),
          _ = (0, g.Qn)(),
          u = r.useCallback(
            (e) => {
              m(K(d));
            },
            [d],
          ),
          h = (0, P.wY)(u);
        if (i == F.Sq) return null;
        const p = a.filter((e) => D.A.Get().BHasApp(e.appID));
        return (0, n.jsx)("div", {
          ref: h,
          className: l().SuggestionCarousel,
          children: (0, n.jsx)(I.Ay, {
            feature: "steamawards_nominate",
            children: (0, n.jsx)(b.F, {
              gap: 12,
              hideArrows: !(0, T.rp)(),
              visibleElements: c,
              useTestScrollbar: !0,
              bLazyRenderChildren: !0,
              hidePips: _,
              screenIsWide: (0, T.rp)(),
              children: p.map((e) =>
                (0, n.jsx)(
                  $,
                  {
                    eNominatonSource: s,
                    eSteamAwardCategoryID: t,
                    appID: e.appID,
                    nPlaytime: e.nPlaytime,
                  },
                  e.appID,
                ),
              ),
            }),
          }),
        });
      }
      function $(e) {
        const {
            appID: a,
            eSteamAwardCategoryID: t,
            eNominatonSource: s,
            nPlaytime: i,
          } = e,
          r = (0, M.$5)(a),
          { data: o } = (0, G.J$)(r),
          { data: d } = (0, G.gy)(r);
        if (!o) return null;
        let c = null;
        return (
          i && (c = (i / 60).toFixed(1)),
          (0, n.jsxs)("div", {
            className: l().GameCarouselItemCtn,
            children: [
              c &&
                (0, n.jsx)("div", {
                  className: l().PlaytimeIndicator,
                  children: (0, A.we)("#Steamawards_Playtime_Hours", c),
                }),
              (0, n.jsx)(m.u, {
                id: r,
                children: (0, n.jsx)("img", {
                  className: V.AppCapsuleImage,
                  src: (0, W.b0)(d, "small_capsule"),
                  alt: o.name || "",
                }),
              }),
              (0, n.jsx)(ee, {
                unAppID: a,
                eNominatonSource: s,
                eSteamAwardCategoryID: t,
              }),
            ],
          })
        );
      }
      function ee(e) {
        const {
            unAppID: a,
            eSteamAwardCategoryID: t,
            eNominatonSource: s,
            fnShowPicker: i,
          } = e,
          o = (0, _.cO)(),
          d = (0, O.R7)(),
          c = d?.ownerWindow || window,
          m = a === _.Fq,
          u = r.useMemo(() => {
            const e = o.data?.find((e) => e.category_id == t);
            return e?.appid === a;
          }, [t, o.data, a]),
          h = r.useCallback(
            (e) => {
              let t = (0, A.we)(
                "#Steamawards_Nominate_Error_Generic",
                a,
                e ?? "Unknown",
              );
              e == y.p &&
                (t = (0, A.we)("#Steamawards_Nominate_Error_NoMatch", a)),
                (0, S.pY)(t, c);
            },
            [c, a],
          ),
          g = r.useCallback(() => {
            a === _.Fq && i && i(!1);
          }, [i, a]),
          v = (0, _.$d)(a, t, s, h, g);
        let j = (0, A.we)("#Steamawards_Nominate"),
          N = null;
        return (
          m
            ? ((j = (0, A.we)("#Steamawards_Skip_Btn")),
              (N = (0, n.jsx)(w.MOk, {})))
            : u && (j = (0, A.we)("#Steamawards_Nominated")),
          (0, n.jsxs)(p.$n, {
            onClick: (e) => {
              u
                ? e.preventDefault()
                : (0, H.UserEligibleToNominateOrVote)(!1)
                  ? v.mutate()
                  : console.log(
                      "EventDisplaySteamAwardNomination: UserEligibleToNominateOrVote failed",
                    );
            },
            className: (0, x.A)(l().NominateGameButton, u && l().Nominated),
            children: [N, j],
          })
        );
      }
      function ae(e) {
        const { rgEvents: a } = e,
          [t, s] = r.useState(!1),
          i = (0, O.R7)(),
          o = i?.ownerWindow || window,
          [d, c] = r.useState(4),
          m = r.useCallback(
            (e) => {
              let a = 1;
              o.innerWidth >= 1080
                ? (a = 4)
                : o.innerWidth >= 920
                  ? (a = 3)
                  : o.innerWidth >= 600 && (a = 2),
                c(a);
            },
            [o],
          ),
          _ = (0, P.wY)(m);
        return (
          r.useEffect(() => {
            if (t) return;
            (async () => {
              a.forEach((e) => {
                k.O3.QueueLoadPartnerEvent(e.clanid, e.event_gid, !1);
              });
              const e = a.map((e) =>
                k.O3.LoadPartnerEventFromClanEventGIDAndClanSteamID(
                  E.b.InitFromClanID(e.clanid),
                  e.event_gid,
                  0,
                  !1,
                ),
              );
              await Promise.all(e), s(!0);
            })();
          }, [a, t]),
          a.length
            ? t
              ? (0, n.jsx)("div", {
                  ref: _,
                  className: l().EventCarousel,
                  children: (0, n.jsx)(I.Ay, {
                    feature: "steamawards_event",
                    children: (0, n.jsx)(b.F, {
                      gap: 12,
                      hideArrows: !(0, T.rp)(),
                      visibleElements: d,
                      useTestScrollbar: !0,
                      bLazyRenderChildren: !0,
                      className: l().GameCarousel,
                      screenIsWide: (0, T.rp)(),
                      children: a.map((e) =>
                        (0, n.jsx)(te, { gidEvent: e.event_gid }, e.event_gid),
                      ),
                    }),
                  }),
                })
              : (0, n.jsx)(f.t, {
                  className: l().EventCarousel,
                  size: "xlarge",
                  position: "center",
                })
            : null
        );
      }
      function te(e) {
        const { gidEvent: a } = e,
          t = k.O3.GetClanEventModel(a),
          s = (0, Q.Y)();
        if (!t) return null;
        return (0, n.jsx)(B.u, {
          event: t,
          bShowAssociatedApp: !0,
          bHidePrices: !0,
          onClick: (e) => {
            s.RecordEventRead(t, L.Tc.HX),
              (0, R.Y)(t, h.uX(e)),
              e.stopPropagation(),
              e.preventDefault();
          },
        });
      }
      var ne = t(95034),
        se = t(41471),
        ie = t(19367),
        re = t.n(ie),
        oe = t(44165);
      function le(e) {
        const { steamID: a, nYear: t } = e,
          [s] = (0, ne.QD)("k", null),
          i = Boolean((a && a != u.iA.steamid) || s),
          o = re()("2025-12-01T10:00:00-08:00").unix(),
          d = oe.HD.GetTimeNowWithOverride(),
          m = !i && d <= o;
        return (
          r.useEffect(() => {
            k.O3.Init();
          }, []),
          (0, n.jsx)(I.Ay, {
            method: "nominations",
            children: (0, n.jsxs)(c.Z, {
              className: l().NominationsPageContent,
              children: [
                i
                  ? (0, n.jsx)(ce, { bEnableNominating: m, steamid: a })
                  : (0, n.jsx)(de, { year: t }),
                !i &&
                  (0, n.jsxs)("div", {
                    className: (0, x.A)(
                      l().SectionContent,
                      l().ProgressAndShareCtn,
                    ),
                    children: [
                      (0, n.jsx)(me, {}),
                      (0, n.jsx)(Ne, { nYear: t }),
                    ],
                  }),
                (0, n.jsx)(_e, { bEnableNominating: m }),
                (0, n.jsxs)("div", {
                  className: l().BackgroundDark,
                  children: [!i && (0, n.jsx)(we, {}), (0, n.jsx)(ve, {})],
                }),
              ],
            }),
          })
        );
      }
      function de(e) {
        return (0, n.jsx)("div", {
          className: l().NominationsHeaderCtn,
          children: (0, n.jsx)("div", {
            className: l().FAQHeaderArea,
            children: (0, n.jsxs)("div", {
              className: l().FAQHeaderCtn,
              children: [
                (0, n.jsx)("div", {
                  className: l().FAQSaletitle,
                  children: (0, A.PP)(
                    "#Steamawards_Title",
                    (0, n.jsx)("br", {}),
                    (0, n.jsx)("br", {}),
                  ),
                }),
                (0, n.jsx)("div", {
                  className: l().FAQComingsoon,
                  children: (0, A.we)("#Steamawards_NominateNow"),
                }),
                (0, n.jsx)("div", {
                  className: l().FAQComingsoon,
                  children: (0, A.we)("#Steamawards_LevelUpNow", e.year),
                }),
                (0, n.jsx)("div", {
                  className: l().FAQComingsoon,
                  children: (0, A.we)("#Steamawards_VoteWinter"),
                }),
                (0, n.jsx)("div", {
                  className: l().FAQComingsoon,
                  children: (0, A.we)("#Steamawards_WinnersAnnounced"),
                }),
              ],
            }),
          }),
        });
      }
      function ce(e) {
        const { steamid: a, bEnableNominating: t } = e,
          s = (0, se.js)(a);
        return (0, n.jsx)("div", {
          className: l().NominationsHeaderCtn,
          children: (0, n.jsxs)("div", {
            className: (0, x.A)(l().FAQHeaderArea, l().FriendsHeader),
            children: [
              (0, n.jsxs)("div", {
                className: l().FriendsHeaderCtn,
                children: [
                  (0, n.jsx)("img", { src: s.data?.avatar_url_full }),
                  (0, n.jsxs)("div", {
                    className: l().FriendsTitleCtn,
                    children: [
                      (0, n.jsx)("div", {
                        className: (0, x.A)(l().FriendsTitle, l().Gold),
                        children: (0, A.we)("#Steamawards_TheSteamAwards"),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FriendsTitle,
                        children: (0, A.we)(
                          "#Steamawards_FriendsNominations",
                          s.data?.m_strPlayerName,
                        ),
                      }),
                    ],
                  }),
                ],
              }),
              t &&
                (0, n.jsx)("div", {
                  className: l().HeaderButtonCtn,
                  children: (0, n.jsx)("a", {
                    href: `${g.TS.STORE_BASE_URL}steamawards/nominations`,
                    className: (0, x.A)(l().NominateGameButton, l().White),
                    children: (0, A.we)(
                      "#Steamawards_MakeYourOwnNominations_Btn",
                    ),
                  }),
                }),
            ],
          }),
        });
      }
      function me(e) {
        const a = (0, _.cO)(),
          t = (0, _.Jo)(_.Ri);
        if (!t.data?.votes) return null;
        const s = a.data && 0 != a.data.length ? "" + a.data.length : "0";
        return (0, n.jsx)(c.Z, {
          className: (0, x.A)(l().NominationProgressCtn),
          children: (0, n.jsx)("div", {
            className: l().ProgressTitle,
            children: (0, A.PP)(
              "#Steamawards_TotalNominations",
              s,
              t.data?.votes?.length,
            ),
          }),
        });
      }
      function _e(e) {
        const { bEnableNominating: a } = e,
          t = (0, _.Jo)(_.Ri);
        if (!t.data) return null;
        const s = t.data.votes.map((e) =>
          (0, n.jsx)(
            ue,
            {
              bEnableNominating: a,
              category: e,
              rgPrevLaborOfLoveWinners: t.data.labor_of_love_winners,
            },
            e.voteid,
          ),
        );
        return (0, n.jsx)(c.Z, {
          className: (0, x.A)(l().SectionContent, l().SteamAwardCategories),
          children: s,
        });
      }
      function ue(e) {
        const {
            category: a,
            bEnableNominating: t,
            rgPrevLaborOfLoveWinners: s,
          } = e,
          { currentNomination: i } = (0, _.Vz)(a.voteid),
          [o, d] = r.useState(!1),
          m = a.internal_name,
          u =
            g.TS.BASE_URL_STORE_CDN_ASSETS +
            "promo/steamawards2024/backgrounds/" +
            m +
            ".jpg?v=3";
        return (0, n.jsxs)(c.Z, {
          className: (0, x.A)(
            l().SteamAwardCategory,
            i && l().Nominated,
            o && l().PickerOpen,
          ),
          children: [
            (0, n.jsx)("div", {
              className: (0, x.A)(l().SteamAwardCategoryBackground),
              style: { backgroundImage: `url( ${u} )` },
            }),
            (0, n.jsx)("div", {
              className: (0, x.A)(l().SteamAwardCategoryBlurryBackground),
              style: { backgroundImage: `url( ${u} )` },
            }),
            (0, n.jsxs)("div", {
              className: l().CategoryRow,
              children: [
                (0, n.jsxs)("div", {
                  className: l().LeftColumn,
                  children: [
                    (0, n.jsxs)("div", {
                      className: l().CategoryTitleRow,
                      children: [
                        t &&
                          (0, n.jsx)("div", {
                            className: l().Checkbox,
                            children:
                              i &&
                              (0, n.jsx)(w.X4B, {
                                color: "#ffffff",
                                highlightColor: "#ffffff",
                              }),
                          }),
                        (0, n.jsx)("div", {
                          className: l().CategoryTitle,
                          children: a.localization.title_award,
                        }),
                      ],
                    }),
                    (0, n.jsx)("div", {
                      className: l().CategoryDescription,
                      children: a.localization.award_description,
                    }),
                  ],
                }),
                (0, n.jsxs)("div", {
                  className: (0, x.A)(l().RightColumn, o && l().PickerOpen),
                  children: [
                    (0, n.jsx)("div", {
                      className: l().CapsuleBlurryContainer,
                      children: Boolean(i)
                        ? (0, n.jsx)(he, { nomination: i, bBlurry: !0 })
                        : (0, n.jsx)(ge, {}),
                    }),
                    (0, n.jsx)("div", {
                      className: l().CapsuleContainer,
                      children: Boolean(i)
                        ? (0, n.jsx)(he, { nomination: i, bBlurry: !1 })
                        : (0, n.jsx)(ge, {}),
                    }),
                    t &&
                      (0, n.jsx)(pe, {
                        fnShowPicker: d,
                        has_nomination: Boolean(i),
                      }),
                  ],
                }),
              ],
            }),
            o &&
              (0, n.jsx)(q, {
                fnShowPicker: d,
                category: a,
                rgPrevLaborOfLoveWinners: s,
              }),
          ],
        });
      }
      function he(e) {
        const { nomination: a, bBlurry: t } = e,
          s = (0, M.$5)(a.appid),
          { data: i } = (0, G.J$)(s),
          { data: r } = (0, G.gy)(s);
        return i
          ? t
            ? (0, n.jsx)("img", { src: (0, W.b0)(r, "header"), alt: i.name })
            : (0, n.jsx)(m.u, {
                className: l().NominatedGameCapsule,
                id: s,
                children: (0, n.jsx)("img", {
                  src: (0, W.b0)(r, "header"),
                  alt: i.name,
                }),
              })
          : null;
      }
      function ge() {
        return (0, n.jsx)("div", { className: l().NominatedGameCapsule });
      }
      function pe(e) {
        const { has_nomination: a, fnShowPicker: t } = e;
        let s = (0, A.we)("#Steamawards_Nominate_Btn"),
          i = l().ActionNominate,
          r = () => t(!0);
        return (
          u.iA.logged_in
            ? a &&
              ((s = (0, A.we)("#Steamawards_Edit_Btn")), (i = l().ActionEdit))
            : ((s = (0, A.we)("#Steamawards_Login_Btn")),
              (i = l().ActionLogin),
              (r = () => (0, N.vg)())),
          (0, n.jsx)("div", {
            className: (0, x.A)(l().NominateBtnCtn, i),
            children: (0, n.jsx)(p.$n, {
              onClick: r,
              className: (0, x.A)(l().NominateButton),
              children: s,
            }),
          })
        );
      }
      function we() {
        const e = (0, _.ed)(j.GPz.Mt);
        let a = 0;
        const t = e.data?.quests?.map(
          (e) => (
            e.completed && a++,
            (0, n.jsx)(
              Se,
              { eStoreQuestID: e.questid, completed: e.completed },
              e.questid,
            )
          ),
        );
        let s = [];
        for (let e = 1; e < 5; e++)
          s.push(
            (0, n.jsx)(Ae, { nBadgeLevel: e, bCompleted: Boolean(a >= e) }, e),
          );
        return (0, n.jsxs)("div", {
          className: (0, x.A)(l().BadgeSectionCtn, l().SectionContent),
          children: [
            u.iA.logged_in &&
              (0, n.jsxs)(n.Fragment, {
                children: [
                  (0, n.jsx)("div", {
                    className: l().BadgeSectionTitle,
                    children: (0, A.we)("#Steamawards_Badge_SectionTitle"),
                  }),
                  (0, n.jsx)("div", {
                    className: l().BadgeTasksCtn,
                    children: t,
                  }),
                ],
              }),
            (0, n.jsxs)("div", {
              className: l().BadgeStatusCtn,
              children: [
                (0, n.jsx)("div", {
                  className: l().BadgeStatusTitle,
                  children: (0, A.we)("#Steamawards_Badge_BadgeTitle"),
                }),
                (0, n.jsx)("div", {
                  className: l().BadgeStatusDesc,
                  children: (0, A.we)("#Steamawards_Badge_BadgeDesc"),
                }),
                (0, n.jsx)("div", {
                  className: l().BadgeImageRow,
                  children: s,
                }),
              ],
            }),
          ],
        });
      }
      function Ae(e) {
        const { nBadgeLevel: a, bCompleted: t } = e;
        return (0, n.jsxs)("div", {
          className: (0, x.A)(l().BadgeItem, t && l().Active),
          children: [
            (0, n.jsx)("div", {
              className: l().BadgeImage,
              children: (0, n.jsx)("img", {
                src:
                  g.TS.BASE_URL_STORE_CDN_ASSETS +
                  `promo/steamawards2025/level_0${a}.webp`,
              }),
            }),
            (0, n.jsx)("div", {
              className: l().BadgeDesc,
              children: (0, A.we)(`#Steamawards_Badge_BadgeTask${a}`),
            }),
          ],
        });
      }
      function xe(e) {
        switch (e) {
          case 610:
            return (0, A.we)("#Steamawards_Task1");
          case 611:
            return (0, A.we)("#Steamawards_Task2");
          case 612:
            return (0, A.we)("#Steamawards_Task3");
          case 613:
            return (0, A.we)("#Steamawards_Task4");
          default:
            return "Unknown Task";
        }
      }
      function Se(e) {
        const { eStoreQuestID: a, completed: t } = e;
        return (0, n.jsxs)("div", {
          className: l().BadgeTask,
          children: [
            (0, n.jsx)("div", {
              className: l().Checkbox,
              children: t ? (0, n.jsx)(w.Jlk, {}) : null,
            }),
            (0, n.jsx)("div", { className: l().TaskTitle, children: xe(a) }),
          ],
        });
      }
      function ve() {
        const e = g.TS.COMMUNITY_BASE_URL + "my/badges/";
        return (0, n.jsx)("div", {
          className: (0, x.A)(l().NominationsFAQ, l().SectionContent),
          children: (0, n.jsxs)("div", {
            className: l().NominationsFAQCtn,
            children: [
              (0, n.jsxs)("div", {
                className: l().LeftCol,
                children: [
                  (0, n.jsx)("h3", {
                    className: l().FaqSectionTitle,
                    children: (0, A.we)("#Steamawards_FAQ_Title_Badges"),
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)("#Steamawards_FAQ_XP_Q"),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)("#Steamawards_FAQ_XP_A"),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)("#Steamawards_FAQ_BadgesAll_Q"),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.PP)(
                          "#Steamawards_FAQ_BadgesAll_A_wLink",
                          (0, n.jsx)("a", {
                            href: e,
                            children: (0, A.we)(
                              "#Steamawards_FAQ_BadgesAll_A_YourBadges",
                            ),
                          }),
                        ),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)("#Steamawards_FAQ_ReviewsPrev_Q"),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)("#Steamawards_FAQ_ReviewsPrev_A"),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)("#Steamawards_FAQ_BadgeSkipping_Q"),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_BadgeSkipping_A1",
                        ),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_BadgeSkipping_A2",
                        ),
                      }),
                    ],
                  }),
                ],
              }),
              (0, n.jsxs)("div", {
                className: l().RightCol,
                children: [
                  (0, n.jsx)("h3", {
                    className: l().FaqSectionTitle,
                    children: (0, A.we)("#Steamawards_FAQ_Title_Nominations"),
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_WhoCanNominate_Q",
                        ),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_WhoCanNominate_A",
                        ),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)("#Steamawards_FAQ_HowToNominate_Q"),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)("#Steamawards_FAQ_HowToNominate_A"),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)("#Steamawards_FAQ_WhichGames_Q"),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)("#Steamawards_FAQ_WhichGames_A1"),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)("#Steamawards_FAQ_WhichGames_A2"),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)("#Steamawards_FAQ_WhichGames_A3"),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_NominateMultiple_Q",
                        ),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_NominateMultiple_A",
                        ),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_EditNominations_Q",
                        ),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_EditNominations_A",
                        ),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_HowShareFriends_Q",
                        ),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_HowShareFriends_A",
                        ),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_WinnersSelected_Q",
                        ),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_WinnersSelected_A",
                        ),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: l().FaqEntry,
                    children: [
                      (0, n.jsx)("div", {
                        className: l().FAQ_Q,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_WhyParticipate_Q",
                        ),
                      }),
                      (0, n.jsx)("div", {
                        className: l().FAQ_A,
                        children: (0, A.we)(
                          "#Steamawards_FAQ_WhyParticipate_A",
                        ),
                      }),
                    ],
                  }),
                ],
              }),
            ],
          }),
        });
      }
      function je(e) {
        const { closeModal: a, nYear: t } = e,
          s = (0, _.np)(),
          i = (0, _._C)(),
          [o, d] = (0, r.useState)(!1);
        if (!s.data) return null;
        const [m, h] = s.data;
        let w = "";
        h.code &&
          (w =
            g.TS.STORE_BASE_URL +
            `steamawards/nominations/${t}/` +
            u.iA.steamid +
            "?k=" +
            h.code);
        return (0, n.jsx)(S.o0, {
          closeModal: a,
          bAlertDialog: !0,
          strCancelButtonText: (0, A.we)("#Steamawards_Close_Btn"),
          strTitle: (0, A.we)("#Steamawards_ShareLink_Title"),
          children: (0, n.jsxs)(c.Z, {
            className: l().ShareModalBody,
            "flow-children": "column",
            children: [
              (0, n.jsx)("p", {
                className: (0, x.A)(l().ShareModalText, l().IntroText),
                children: (0, A.we)("#Steamawards_ShareModal_Description"),
              }),
              w &&
                (0, n.jsxs)(n.Fragment, {
                  children: [
                    (0, n.jsx)(p.JU, {
                      children: (0, A.we)("#YIR_ShareModal_YourLink"),
                    }),
                    (0, n.jsxs)("div", {
                      className: l().UrlContainer,
                      children: [
                        (0, n.jsx)("div", { className: l().Url, children: w }),
                        (0, n.jsx)(p.jn, {
                          className: l().Button,
                          onClick: () => {
                            navigator.clipboard.writeText(w), d(!0);
                          },
                          children: (0, A.we)(
                            o
                              ? "#YIR_ShareModal_CopyLink_Success"
                              : "#YIR_ShareModal_CopyLink",
                          ),
                        }),
                      ],
                    }),
                    (0, n.jsx)("p", {
                      className: l().ShareModalText,
                      children: (0, A.we)(
                        "#Steamawards_ShareModal_Description2",
                      ),
                    }),
                  ],
                }),
              (0, n.jsx)(p.$n, {
                className: l().GenerateShareLinkBtn,
                onClick: () => i.mutate(),
                children: (0, A.we)(
                  w
                    ? "#Steamawards_GenerateLink_Btn_Renew"
                    : "#Steamawards_GenerateLink_Btn",
                ),
              }),
            ],
          }),
        });
      }
      function Ne(e) {
        const { nYear: a } = e,
          t = (0, _.cO)();
        return u.iA.logged_in && t.data && 0 != t.data.length
          ? (0, n.jsx)("div", {
              className: l().ShareLinkCtn,
              children: (0, n.jsxs)(d.Ii, {
                className: l().ShareBtn,
                onClick: (e) => {
                  e.preventDefault(),
                    e.stopPropagation(),
                    (function (e, a) {
                      (0, v.pg)((0, n.jsx)(je, { nYear: e }), a);
                    })(a, (0, h.uX)(e));
                },
                children: [
                  (0, n.jsx)(w.SYj, {}),
                  (0, n.jsx)("span", {
                    children: (0, A.we)("#Steamawards_ShareLink_Btn"),
                  }),
                ],
              }),
            })
          : (0, n.jsx)("div", {
              className: l().ShareLinkCtn,
              children: (0, n.jsx)("div", {
                className: l().ProgressTitle,
                children: (0, A.we)("#Steamawards_GenerateLink_Fallback"),
              }),
            });
      }
      var Ce = t(4775),
        fe = t.n(Ce),
        Fe = t(60383),
        ye = t(60014),
        Ie = t(84811),
        Te = t(49419),
        be = t(10050),
        ke = t(38535);
      const Ee = {
        include_basic_info: !0,
        include_assets_without_overrides: !0,
        include_trailers: !0,
      };
      function Qe() {
        const e = r.useContext(_.AD);
        return e
          ? (0, n.jsxs)(c.Z, {
              className: fe().VotingPageContent,
              children: [
                (0, n.jsx)(Be, {
                  bIsVotingOpen: e.bVotingOpen,
                  bIsVotingPast: e.bVotingPast,
                }),
                (0, n.jsxs)("div", {
                  className: fe().VotingArea,
                  children: [
                    e.bHasStickerRewards &&
                      (0, n.jsx)(De, {
                        unSaleAppID: e.config.appid,
                        bIsVotingOpen: e.bVotingOpen,
                        bIsVotingPast: e.bVotingPast,
                      }),
                    (0, n.jsx)(Re, {
                      bIsVotingOpen: e.bVotingOpen,
                      bIsVotingPast: e.bVotingPast,
                      unSaleAppID: e.config.appid,
                    }),
                  ],
                }),
                (0, n.jsx)(Ge, {}),
              ],
            })
          : null;
      }
      function Be(e) {
        const { bIsVotingOpen: a, bIsVotingPast: t } = e,
          s = r.useContext(_.AD),
          i = (0, _.CF)();
        let o;
        return (
          (o = a
            ? (0, A.we)("#Steamawards_Voting_Header_VoteNow")
            : t
              ? (0, A.we)("#Steamawards_Voting_Header_WinnersUp")
              : (0, A.we)(
                  "#Steamawards_Voting_Header_VoteSoon_New",
                  Ue(s.rtVoteStart),
                )),
          (0, n.jsx)(c.Z, {
            children: (0, n.jsx)("div", {
              className: (0, x.A)(fe().HeaderCtn, i.HeaderCtn),
              children: (0, n.jsx)("div", {
                className: fe().HeaderContent,
                children: (0, n.jsxs)("div", {
                  className: fe().TextColumn,
                  children: [
                    (0, n.jsx)("div", {
                      className: (0, x.A)(fe().EventTitle, i.EventTitle),
                      children: (0, A.PP)(
                        "#Steamawards_Title_WithYear",
                        s.nYear,
                        (0, n.jsx)("br", {}),
                      ),
                    }),
                    (0, n.jsx)("div", {
                      className: (0, x.A)(
                        fe().InfoText,
                        fe().Large,
                        i.InfoText,
                        i.Large,
                      ),
                      children: o,
                    }),
                    t
                      ? (0, n.jsx)(n.Fragment, {
                          children: (0, n.jsx)("div", {
                            className: (0, x.A)(fe().InfoText, i.InfoText),
                            children: (0, A.PP)(
                              "#Steamawards_Voting_Header_Winners",
                              s.nYear,
                            ),
                          }),
                        })
                      : (0, n.jsxs)(n.Fragment, {
                          children: [
                            (0, n.jsx)("div", {
                              className: (0, x.A)(fe().InfoText, i.InfoText),
                              children: (0, A.we)(
                                "#Steamawards_Voting_Header_Finalists",
                              ),
                            }),
                            (0, n.jsx)("div", {
                              className: (0, x.A)(fe().InfoText, i.InfoText),
                              children: (0, A.we)(
                                "#Steamawards_Voting_Header_HowTo",
                                Ue(s.rtVoteEnd),
                              ),
                            }),
                            (0, n.jsx)("div", {
                              className: (0, x.A)(fe().InfoText, i.InfoText),
                              children: (0, A.we)(
                                "#Steamawards_Voting_Header_Dates_New",
                                Ue(s.rtVoteEnd),
                              ),
                            }),
                          ],
                        }),
                  ],
                }),
              }),
            }),
          })
        );
      }
      function De(e) {
        const { unSaleAppID: a, bIsVotingOpen: t, bIsVotingPast: s } = e,
          i = (0, _.Jo)(a),
          r = (0, _.a8)(a),
          o = (0, _.CF)();
        if (!i.data) return null;
        const l = i.data.votes.map((e) =>
          (0, n.jsx)(Le, { unSaleAppID: a, definition: e }, e.voteid),
        );
        let d = null;
        return (
          (d = s
            ? (0, A.PP)(
                "#Steamawards_Progress_Title_Past",
                r.data?.length ?? 0,
                i.data.votes.length,
              )
            : t
              ? g.iA.logged_in
                ? r.data?.length > 0
                  ? (0, A.PP)(
                      "#Steamawards_Progress_Title",
                      r.data?.length,
                      i.data.votes.length,
                    )
                  : (0, A.PP)(
                      "#Steamawards_Progress_Title_None",
                      i.data.votes.length,
                    )
                : (0, A.we)("#Steamawards_Progress_Title_LoggedOut")
              : (0, A.PP)(
                  "#Steamawards_Progress_Title_Soon",
                  i.data.votes.length,
                )),
          (0, n.jsxs)(c.Z, {
            className: (0, x.A)(
              fe().ProgressCtn,
              fe().SectionContent,
              o.ProgressCtn,
            ),
            children: [
              (0, n.jsx)("div", { className: fe().Title, children: d }),
              (0, n.jsx)("div", { className: fe().StickerRow, children: l }),
            ],
          })
        );
      }
      function Le(e) {
        const { definition: a, unSaleAppID: t } = e,
          s = (0, _.Mn)(t, a.voteid),
          i = (0, _.PV)(t, a.voteid),
          r = (0, _.CF)(),
          o = `${g.TS.MEDIA_CDN_COMMUNITY_URL}images/items/${t}/${s ? i?.item_image_small : i?.item_image_large}`,
          l = `${g.TS.MEDIA_CDN_COMMUNITY_URL}images/items/${t}/${i?.item_image_small}`;
        return (0, n.jsxs)("div", {
          className: (0, x.A)(fe().CategoryStickerCtn, r.CategoryStickerCtn),
          children: [
            (0, n.jsx)("img", {
              className: (0, x.A)(
                fe().CategoryStickerHover,
                r.CategoryStickerHover,
              ),
              src: l,
            }),
            (0, n.jsx)(c.Z, {
              className: (0, x.A)(fe().CategorySticker, !s && fe().Inactive),
              onActivate: () =>
                (window.location.href =
                  "#" + a.localization.title.replace(/\s/g, "")),
              style: { backgroundImage: `url( '${o}' )` },
            }),
          ],
        });
      }
      function Re(e) {
        const { unSaleAppID: a, bIsVotingOpen: t, bIsVotingPast: s } = e,
          i = (0, _.Jo)(a).data.votes.map((e) =>
            (0, n.jsx)(
              Pe,
              {
                bIsVotingOpen: t,
                bIsVotingPast: s,
                unSaleAppID: a,
                definition: e,
              },
              e.voteid,
            ),
          );
        return (0, n.jsx)(c.Z, {
          id: "Categories",
          className: (0, x.A)(fe().CategoryList, fe().SectionContent),
          children: i,
        });
      }
      function Pe(e) {
        const {
            definition: a,
            unSaleAppID: t,
            bIsVotingOpen: s,
            bIsVotingPast: i,
          } = e,
          o = (0, F.zX)(
            a.app_discounts.map((e) => e.appid),
            Ee,
          ),
          l = (0, _.Mn)(t, a.voteid),
          d = r.useRef(0),
          m = (0, O.R7)(),
          u = r.useContext(_.AD),
          [h, w] = r.useState([]),
          [S, v] = r.useState(0),
          [j, N] = r.useState(!1),
          [C, y] = r.useState(0),
          [I, T] = r.useState(!l && !i),
          [b, k] = r.useState(),
          [E, Q] = r.useState(669),
          B =
            g.TS.BASE_URL_STORE_CDN_ASSETS +
            "promo/steamawards2024/backgrounds/" +
            a.internal_name +
            ".jpg?v=1",
          D = g.TS.IMG_URL + "promo/steamawards2023/placeholder_main.png",
          L = m?.ownerWindow || window,
          R = r.useCallback((e) => {
            Q(e.contentRect.height);
          }, []),
          V = (0, P.wY)(R),
          H = r.useCallback(() => {
            let e = "-20% 0px -50% 0px";
            if (L.innerHeight <= E) e = "0px 0px 0px 0px";
            else {
              const a = 40,
                t = L.innerHeight / E,
                n = Math.min(t * a + t * E - (E + a), 0.65 * L.innerHeight);
              e = `-${Math.min(0.4 * E, 0.1 * L.innerHeight)}px 0px -${n}px 0px`;
            }
            k(e);
          }, [L.innerHeight, E]);
        r.useEffect(
          () => (
            window.addEventListener("resize", H),
            () => window.removeEventListener("resize", H)
          ),
        ),
          r.useEffect(() => {
            h.length ||
              (async () => {
                let e = [];
                if (g.iA.logged_in) {
                  const t = await Promise.all(
                    a.app_discounts?.map(async (e) => {
                      const a = [e.appid, g.iA.accountid].toString(),
                        t = await (0, Te.sx)(a);
                      return { appid: e.appid, hash: t };
                    }),
                  );
                  t.sort((e, a) => (e.hash > a.hash ? 1 : -1)),
                    (e = t.map((e) => e.appid));
                } else e = a.app_discounts.map((e) => e.appid);
                w(e), v(e[d.current]), H();
              })();
          }, [a.app_discounts, H, h]);
        const M = r.useCallback(() => {
            if (!C) {
              let e = d.current + 1;
              e >= h.length && (e = 0), (d.current = e), v(h[d.current]);
            }
          }, [h, C]),
          G = r.useCallback((e) => {
            N(e);
          }, []),
          W = r.useCallback((e) => {
            y(e), v(e);
          }, []),
          U = r.useCallback(() => {
            y(0), v(h[d.current]);
          }, [h]);
        if (o == F.Sq)
          return (0, n.jsx)(f.t, {
            position: "center",
            size: "large",
            msDelayAppear: 300,
          });
        const q = h?.map((e) =>
            (0, n.jsx)(
              Me,
              {
                eCategory: a.voteid,
                unSaleAppID: t,
                bCurrentlyActive: S === e,
                unAppID: e,
                bIsVotingOpen: s,
                bIsCurrentVoteApp: l == e,
                fnOnMouseLeaveApp: U,
                fnOnMouseEnterApp: W,
              },
              e,
            ),
          ),
          z = h?.map((e) =>
            (0, n.jsx)(
              He,
              {
                bHoveringApp: e == C,
                unAppID: e,
                bPlayMicrotrailers: j,
                bCurrentlyActive: S === e,
                fnOnVideoEnd: M,
              },
              e,
            ),
          );
        let Y = (0, n.jsx)(n.Fragment, { children: z });
        return (
          a.winner_appid && i
            ? (Y = (0, n.jsx)(Ve, { unAppID: a.winner_appid }))
            : l && (Y = (0, n.jsx)(Oe, { unAppID: l })),
          (0, n.jsx)(Ie.tH, {
            children: (0, n.jsx)(Fe.J, {
              thresholds: [0.4],
              rootMargin: b,
              trigger: "repeated",
              onVisibilityChange: G,
              children: (0, n.jsxs)(c.Z, {
                ref: V,
                className: (0, x.A)(
                  fe().SteamAwardCategory,
                  l && fe().CategoryVoted,
                  j && fe().Active,
                ),
                children: [
                  (0, n.jsx)("a", {
                    id: a.localization.title.replace(/\s/g, ""),
                    className: fe().Anchor,
                  }),
                  (0, n.jsx)("div", {
                    className: (0, x.A)(fe().SteamAwardCategoryBackground),
                    style: { backgroundImage: `url( ${B} )` },
                  }),
                  (0, n.jsx)("div", {
                    className: (0, x.A)(
                      fe().SteamAwardCategoryBlurryBackground,
                    ),
                    style: { backgroundImage: `url( ${B} )` },
                  }),
                  (0, n.jsxs)("div", {
                    className: fe().CategoryRow,
                    children: [
                      (0, n.jsxs)("div", {
                        className: fe().LeftColumn,
                        children: [
                          (0, n.jsxs)("div", {
                            className: fe().CategoryTitleRow,
                            children: [
                              (0, n.jsx)("div", {
                                className: fe().CategoryYear,
                                children: (0, A.PP)(
                                  "#Steamawards_Title_WithYear_NoBreaks",
                                  u.nYear,
                                ),
                              }),
                              (0, n.jsx)("div", {
                                className: fe().CategoryTitle,
                                children: a.localization.title_award,
                              }),
                            ],
                          }),
                          (0, n.jsx)("div", {
                            className: fe().CategoryDescription,
                            children: a.localization.award_description,
                          }),
                          (l || i) &&
                            (0, n.jsx)(p.$n, {
                              className: fe().HideShowBtn,
                              onClick: () => T(!I),
                              children: I
                                ? (0, A.we)("#Steamawards_HideFinalists_Btn")
                                : (0, A.we)("#Steamawards_ShowFinalists_Btn"),
                            }),
                        ],
                      }),
                      (0, n.jsx)("div", {
                        className: (0, x.A)(fe().RightColumn),
                        children: (0, n.jsxs)("div", {
                          className: fe().CapsuleContainer,
                          children: [Y, (0, n.jsx)("img", { src: D })],
                        }),
                      }),
                    ],
                  }),
                  (0, n.jsx)(be.M, {
                    timeout: 500,
                    unmountOnExit: !0,
                    mountOnEnter: !0,
                    in: I,
                    classNames: {
                      enter: fe().Enter,
                      enterActive: fe().EnterActive,
                      exit: fe().Exit,
                      exitActive: fe().ExitActive,
                    },
                    children: (e) =>
                      (0, n.jsxs)("div", {
                        ref: e,
                        className: fe().FinalistsCtn,
                        children: [
                          (0, n.jsxs)("div", {
                            className: fe().FinalistsIntro,
                            children: [
                              (0, n.jsx)("div", {
                                children: i
                                  ? (0, A.we)(
                                      "#Steamawards_Voting_Finalists_Past",
                                    )
                                  : (0, A.we)("#Steamawards_Voting_Finalists"),
                              }),
                              (0, n.jsx)("div", {
                                className: fe().FinalistsLine,
                              }),
                            ],
                          }),
                          (0, n.jsx)("div", {
                            className: fe().FinalistsRow,
                            children: q,
                          }),
                        ],
                      }),
                  }),
                ],
              }),
            }),
          })
        );
      }
      function Oe(e) {
        const { unAppID: a } = e,
          [t] = (0, F.t7)(a, Ee),
          s = (0, ye.aL)(t?.GetStorePageURL(), "nominee_capsule");
        return t
          ? (0, n.jsxs)(n.Fragment, {
              children: [
                (0, n.jsx)("div", {
                  className: fe().CurrentVoteBanner,
                  children: (0, A.we)("#Steamawards_Voting_YourVote"),
                }),
                (0, n.jsx)("a", {
                  href: s,
                  children: (0, n.jsx)("img", {
                    src: t.GetAssetsWithoutOverrides().GetMainCapsuleURL(),
                  }),
                }),
              ],
            })
          : null;
      }
      function Ve(e) {
        const { unAppID: a } = e,
          [t] = (0, F.t7)(a, Ee),
          s = (0, ye.aL)(t?.GetStorePageURL(), "winner_capsule");
        return t
          ? (0, n.jsxs)(n.Fragment, {
              children: [
                (0, n.jsx)("div", {
                  className: fe().CurrentVoteBanner,
                  children: (0, A.we)("#Steamawards_Voting_Winner"),
                }),
                (0, n.jsx)("a", {
                  className: fe().WinnerCapsule,
                  href: s,
                  children: (0, n.jsx)("img", {
                    src: t.GetAssetsWithoutOverrides().GetMainCapsuleURL(),
                  }),
                }),
              ],
            })
          : null;
      }
      function He(e) {
        const {
            unAppID: a,
            bCurrentlyActive: t,
            fnOnVideoEnd: s,
            bPlayMicrotrailers: i,
            bHoveringApp: o,
          } = e,
          [l] = (0, F.t7)(a, Ee),
          d = r.useRef(void 0),
          c = (0, ke.dy)();
        r.useEffect(() => {
          d.current && (t && i ? d.current.play() : d.current.pause());
        }, [t, i]);
        const m = l?.GetMicroTrailer(c);
        return l && m?.strWebMURL
          ? (0, n.jsxs)("video", {
              className: (0, x.A)(fe().MicrotrailerVideo, t && fe().Active),
              poster: l.GetAssetsWithoutOverrides().GetMainCapsuleURL(),
              onEnded: s,
              ref: d,
              preload: "auto",
              loop: o,
              playsInline: !0,
              muted: !0,
              children: [
                (0, n.jsx)("source", { src: m.strWebMURL, type: "video/webm" }),
                Boolean(!g.TS.IN_CLIENT) &&
                  (0, n.jsx)("source", { src: m.strMP4URL, type: "video/mp4" }),
              ],
            })
          : null;
      }
      function Me(e) {
        const {
            unAppID: a,
            unSaleAppID: t,
            eCategory: s,
            bCurrentlyActive: i,
            fnOnMouseEnterApp: o,
            fnOnMouseLeaveApp: l,
            bIsVotingOpen: d,
            bIsCurrentVoteApp: m,
          } = e,
          [u] = (0, F.t7)(a, Ee),
          h = (0, _.ZB)(a, s, t),
          g = (0, ye.aL)(u?.GetStorePageURL(), "nominee_capsule"),
          w = (0, _.Vz)(s),
          S = w && w.currentNomination?.appid === a,
          v = r.useCallback(() => {
            (0, H.UserEligibleToNominateOrVote)(!0)
              ? m || h.mutate()
              : console.log(
                  "EventDisplaySteamAwardNomination: UserEligibleToNominateOrVote failed",
                );
          }, [m, h]);
        if (!u) return null;
        const j = m
          ? (0, A.we)("#Steamawards_Voting_Action_Voted")
          : (0, A.we)("#Steamawards_Voting_Action_Vote");
        return (0, n.jsxs)(c.Z, {
          className: (0, x.A)(
            fe().FinalistGameCtn,
            i ? fe().CurrentlyFeatured : "",
            m ? fe().MyVote : "",
          ),
          onMouseEnter: () => o(a),
          onBlur: l,
          onFocus: () => o(a),
          onMouseLeave: l,
          children: [
            (0, n.jsxs)("a", {
              href: g,
              className: fe().CapsuleLink,
              children: [
                (0, n.jsx)("img", {
                  src: u.GetAssetsWithoutOverrides().GetHeroCapsuleURL(),
                  className: fe().Capsule,
                }),
                (0, n.jsx)("div", { className: fe().Highlight, children: " " }),
              ],
            }),
            d &&
              (0, n.jsx)(p.$n, {
                onClick: v,
                className: fe().ActionButton,
                children: j,
              }),
            S &&
              (0, n.jsx)("div", {
                className: fe().YourNomination,
                children: (0, A.we)("#Steamawards_Voting_Action_YourNominee"),
              }),
          ],
        });
      }
      function Ge() {
        const e = r.useContext(_.AD),
          a = (0, _.CF)(),
          t = g.TS.HELP_BASE_URL + "faqs/view/71D3-35C2-AD96-AA3A",
          s = "#Steamawards_Voting_FAQ_6_Q" + (e.nYear >= 2024 ? "_2024" : ""),
          i = "#Steamawards_Voting_FAQ_6_A" + (e.nYear >= 2024 ? "_2024" : "");
        let o = [
          (0, n.jsx)(
            We,
            {
              strQuestion: (0, A.we)("#Steamawards_Voting_FAQ_1_Q"),
              strAnswer: (0, A.oW)(
                "#Steamawards_Voting_FAQ_1_A",
                (0, n.jsx)("a", { href: t, children: ", " }),
              ),
            },
            "FAQ_1",
          ),
        ];
        return (
          e.bHasStickerRewards &&
            o.push(
              (0, n.jsx)(
                We,
                {
                  strQuestion: (0, A.we)("#Steamawards_Voting_FAQ_2_Q"),
                  strAnswer: (0, A.we)("#Steamawards_Voting_FAQ_2_A"),
                },
                "FAQ_2",
              ),
            ),
          (o = [
            ...o,
            (0, n.jsx)(
              We,
              {
                strQuestion: (0, A.we)("#Steamawards_Voting_FAQ_3_Q"),
                strAnswer: (0, A.we)(
                  "#Steamawards_Voting_FAQ_3_A_New",
                  Ue(e.rtVoteStart),
                  Ue(e.rtVoteEnd),
                ),
              },
              "FAQ_3",
            ),
            (0, n.jsx)(
              We,
              {
                strQuestion: (0, A.we)("#Steamawards_Voting_FAQ_7_Q"),
                strAnswer: (0, A.we)("#Steamawards_Voting_FAQ_7_A"),
              },
              "FAQ_4",
            ),
            (0, n.jsx)(
              We,
              {
                strQuestion: (0, A.we)("#Steamawards_Voting_FAQ_8_Q"),
                strAnswer: (0, A.we)("#Steamawards_Voting_FAQ_8_A"),
              },
              "FAQ_5",
            ),
            (0, n.jsx)(
              We,
              {
                strQuestion: (0, A.we)("#Steamawards_Voting_FAQ_4_Q"),
                strAnswer: (0, A.we)(
                  "#Steamawards_Voting_FAQ_4_A_New",
                  Ue(e.rtVoteEnd),
                ),
              },
              "FAQ_6",
            ),
            (0, n.jsx)(
              We,
              {
                strQuestion: (0, A.we)("#Steamawards_Voting_FAQ_5_Q"),
                strAnswer: (0, A.we)("#Steamawards_Voting_FAQ_5_A"),
              },
              "FAQ_7",
            ),
            (0, n.jsx)(
              We,
              { strQuestion: (0, A.we)(s), strAnswer: (0, A.we)(i) },
              "FAQ_8",
            ),
          ]),
          (0, n.jsxs)("div", {
            className: (0, x.A)(fe().FAQWrapper, fe().SectionContent),
            children: [
              (0, n.jsx)("div", {
                className: (0, x.A)(fe().FaqSectionTitle, a.FaqSectionTitle),
                children: (0, A.we)("#Steamawards_Voting_FAQ_Title"),
              }),
              (0, n.jsx)("div", {
                className: (0, x.A)(fe().FAQCtn, a.FAQCtn),
                children: o,
              }),
            ],
          })
        );
      }
      function We(e) {
        const { strQuestion: a, strAnswer: t } = e,
          s = (0, _.CF)();
        return (0, n.jsxs)(c.Z, {
          className: (0, x.A)(fe().FaqEntry, s.FaqEntry),
          children: [
            (0, n.jsx)("div", {
              className: (0, x.A)(fe().FAQ_Q, s.FAQ_Q),
              children: a,
            }),
            (0, n.jsx)("div", {
              className: (0, x.A)(fe().FAQ_A, s.FAQ_A),
              children: t,
            }),
          ],
        });
      }
      function Ue(e) {
        return new Date(1e3 * e).toLocaleString(A.pf.GetPreferredLocales(), {
          day: "numeric",
          month: "short",
          hour: "numeric",
          minute: "numeric",
          timeZoneName: "short",
        });
      }
      var qe = t(26019),
        ze = t.n(qe),
        Ye = t(79870),
        Xe = t.n(Ye),
        Ze = t(66701),
        Ke = t.n(Ze);
      function Je(e) {
        const { nYear: a } = e,
          t = (0, oe.f1)(),
          s = (0, _.RE)(),
          i = (function (e) {
            const [a, t] = r.useState({});
            return (
              r.useEffect(() => {
                let a = $e[e];
                a || (a = $e[aa]), t({ ...ea, ...a });
              }, [e]),
              a
            );
          })(a),
          o = r.useMemo(() => {
            if (!s || !s.definitions?.votes?.length) return null;
            const e = s.definitions.votes[0].start_time,
              n = s.definitions.votes[0].end_time,
              r = t >= e && t < n,
              o = t >= n,
              l = s.definitions.votes.some((e) => Boolean(e.item_type));
            return {
              config: s,
              bVotingOpen: r,
              bVotingPast: o,
              yearStyles: i,
              nYear: a,
              rtVoteStart: e,
              rtVoteEnd: n,
              bHasStickerRewards: l,
            };
          }, [a, t, s, i]);
        return o
          ? (0, n.jsx)(I.Ay, {
              method: "steamawards",
              children: (0, n.jsx)(_.AD.Provider, {
                value: o,
                children: (0, n.jsx)(Qe, {}),
              }),
            })
          : null;
      }
      const $e = { 2023: ze(), 2024: Xe(), 2025: Ke() },
        ea = Object.values($e).reduce((e, a) => ({ ...e, ...a }), {}),
        aa = 2023;
      const ta = () =>
        (0, n.jsx)(I.Ay, {
          controller: "steamawards",
          children: (0, n.jsxs)(i.dO, {
            children: [
              (0, n.jsx)(i.qh, {
                path: s.B.SteamAwardNominations(),
                render: (e) =>
                  (0, n.jsx)(le, {
                    nYear: parseInt(e.match.params.year),
                    steamID: e.match.params.steamid,
                    ...e,
                  }),
              }),
              (0, n.jsx)(i.qh, {
                path: s.B.SteamAwards(),
                render: (e) =>
                  (0, n.jsx)(Je, { nYear: parseInt(e.match.params.year) }),
              }),
            ],
          }),
        });
    },
  },
]);
