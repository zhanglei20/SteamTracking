/**** (c) Valve Corporation. Use is governed by the terms of the Steam Subscriber Agreement http://store.steampowered.com/subscriber_agreement/.
 ****/
(self.webpackChunkstore = self.webpackChunkstore || []).push([
  [5871],
  {
    91970: (e) => {
      e.exports = {
        "duration-app-launch": "800ms",
        strMediumWidth: "800px",
        strMaxMobileWidth: "600px",
        MediaContainer: "-ap7mXqcxyhrxn5-5EaM8",
        MediaContainerMM: "_1XVJzaNLt0tgl6Pq5QI2tK",
        ScreenshotThumbnailRow: "COp9MkmgXCc5Y3ytJM70W",
        HilightGrid: "_28DkEkOjvTWfq0sU8m_i32",
        MainMediaCtn: "FATLceF4ncP8L8QnrQTXV",
        VideoThumbnail: "h0pphNa_Fdu4JUevL5V9y",
        ScreenshotDisplayCtn: "_20YpaGv3V_QArQyy15zAJc",
        MainCapsuleWithHover: "_30BxsjSFrUWCBKntWK9hPg",
        MainCapsule: "_2UuNsdHxhT9la27J_j3BFs",
        AppDetails: "ETHkFxrlPF1znTFXFtjtK",
        GameName: "udhKC-bUdlotRKphK3Fn2",
        ShortDesc: "_2e5PhNJqb68XISttHxdVF2",
        ThumbnialClickable: "_3B8X2c5pBF96hapgz03tC4",
        ThumbnailCtn: "rE40TQ86G7cjVETvUYZm3",
        videoPlaying: "_3D99vg3-OLr6XsvxrtzhZa",
        VideoPlayButton: "_2sJlIVAVWZ2I2iA6IO4pOv",
        VideoLargeContainer: "_3mzKCP4FLx7fUVeKrhZ1nn",
        VideoPopupContainers: "_2LqAoKcZgpOtpoj5_URVNJ",
        VideoLarge: "_3zPl6O67BYIiFOlq7FxlJP",
        BackgroundAnimation: "TqTKL7i3GSFlH_ZgPVf1z",
        "ItemFocusAnim-darkerGrey-nocolor": "cm_5koVmrBLuCZgPBCMie",
        "ItemFocusAnim-darkerGrey": "PnVMlqvX5SCAvSN6U3hb9",
        "ItemFocusAnim-darkGreySettings": "_3WN-nRn7q-FIwR4QBoVI_u",
        "ItemFocusAnim-darkGrey": "_3y6HrP9DK-Ttsri2UnKeJ4",
        "ItemFocusAnim-grey": "_3adXp5-v5skt9f-KA1WB_8",
        "ItemFocusAnim-translucent-white-10": "_1981I-jlGsaAi65IqAxEdm",
        "ItemFocusAnim-translucent-white-20": "_1ukTwVbofcJj1v0AVkiuc3",
        "ItemFocusAnimBorder-darkGrey": "_2LVjWAaAuOqf5tKQMJ99XC",
        "ItemFocusAnim-green": "_1VtcP5LWUyNvo0_5bYQYEx",
        focusAnimation: "n40ksCk7VHLZG8f1e517V",
        hoverAnimation: "YiXS6irpqMezE03xkO2Un",
      };
    },
    84121: (e) => {
      e.exports = {
        MarketingMessagePage: "_1HVoKfdcaouK3kHKX2kH5t",
        MessageListPage: "_1N7O3VXbkpN2z55HsSDZsi",
        MessageListScroll: "_2RW7G8Bi-8k-29anQh8Ie8",
        CarouselBar: "O7VJKyPtoS7TXCr6mrwCr",
        Content: "_1qtg3ASXX4ClYiTHPq9Tkl",
        LeftArrow: "_3ZyZUkBq73dLvZ6zKdM9PQ",
        RightArrow: "_2WuPusSmjw_B4rfkgb-NV8",
        Active: "cwA5j4AsP5OB7WjWuOlFv",
        Spacer: "COvXCP3wZxk_s1ho7e3WO",
        PipContainer: "_3ASpk2zuTuDhIXKorFGF_L",
        Pips: "_1rpuGJtVR-xrddd9ui6IWu",
        Pip: "ZQrtMhZB1tpkd2bgBXyIF",
        MessageListContainer: "_33cf4TnmCK5XdTjanW9bVf",
        Loading: "_1xSKZdDmIvXYCmFif6fSAq",
        MessageWrapper: "_152h7KWYvm_9hIyzRzh8kl",
        Enter: "DZICERQkVmyCBP2E-PwlH",
        EnterActive: "RU-xm1VaMHf9H7o1TFkVO",
        EnterDone: "_3AP2YQ_mKZF-H78sMt3rKC",
        Exit: "_2xmDIRYN5Pu_eHRMjWop63",
        ExitActive: "_32DFHj9NmyNtEK-3K3nKuX",
        ExitDone: "_3cFXRpyI9jrZ2qCySg3fiT",
      };
    },
    92609: (e) => {
      e.exports = {
        "duration-app-launch": "800ms",
        AutoRenderContents: "_1H1L2spV-tto6AJmQMSfsA",
        TitleContainer: "_2iA76-Eh1ID-HXFGGR5BoH",
        TypeTitle: "klzItvORTtJ0Eeg8ArDrN",
        TypeSubTitle: "QPZq6bu7lf50YpRKZgPCc",
        BackgroundAnimation: "_13qwvch-5kbKbOaRivhMXc",
        "ItemFocusAnim-darkerGrey-nocolor": "_3DNhNNrE4iKRi60PdXWepo",
        "ItemFocusAnim-darkerGrey": "_15lW83rYVxUKmbx_X2Huiw",
        "ItemFocusAnim-darkGreySettings": "_2-gDTKZjm9Ajq5sJ9tSPUR",
        "ItemFocusAnim-darkGrey": "_3HGF0obZAs6s2bNbYpM7qW",
        "ItemFocusAnim-grey": "_3YpbyrBpBdoe0gBRw1Hbqx",
        "ItemFocusAnim-translucent-white-10": "_30y8LayXh8jUHdaxQlL0ZS",
        "ItemFocusAnim-translucent-white-20": "_27U6YkfThQDFTGvax-gmlu",
        "ItemFocusAnimBorder-darkGrey": "_2JbY6SYVtLg0XfAcDsFaMR",
        "ItemFocusAnim-green": "_3976kBFFMdgKDnNELLMM6G",
        focusAnimation: "SzNeb_49-6eaLunS7L3C",
        hoverAnimation: "_1ji7N9JSTyeK_qtueFlYQi",
      };
    },
    720: (e) => {
      e.exports = {
        "duration-app-launch": "800ms",
        UpdateEventCtn: "_3ICxVDu3-Bkx9aqyLehGGP",
        EventType: "fRZmXDVszenGo5WnHprTJ",
        EventTitle: "_3ceduHPK-lBiZ5fsmCstLP",
        EventImage: "zDfA0bzAjCUkfaQx_KvS7",
        BackgroundAnimation: "OTs9Tg4QxhQ0Y_QNGDDt9",
        "ItemFocusAnim-darkerGrey-nocolor": "_1NdRyUGS8v6vgPO4so1xTw",
        "ItemFocusAnim-darkerGrey": "_2pygTjxugmVCyf_AACiIV9",
        "ItemFocusAnim-darkGreySettings": "_2k7FRzOjQym6IuES_SDcJs",
        "ItemFocusAnim-darkGrey": "_3znWC1DynbIfoAl5wwBaoo",
        "ItemFocusAnim-grey": "_2oa-LIlworjV0-bg_CQfoy",
        "ItemFocusAnim-translucent-white-10": "i9BFCko5sTUsKunkoSgIG",
        "ItemFocusAnim-translucent-white-20": "_2XDvVDTKofCXGIqRPg15Bt",
        "ItemFocusAnimBorder-darkGrey": "_3rZ3pu0VIsvlLePQQSHFLd",
        "ItemFocusAnim-green": "_2vAKcsXEpb9snlDyeK82te",
        focusAnimation: "_2I7fW0qyN3vk4sB9-UXHBH",
        hoverAnimation: "_1TlYxkxN-WZNFNjoM2t3Ha",
      };
    },
    16205: (e) => {
      e.exports = {
        "duration-app-launch": "800ms",
        BackgroundImage: "_3vQ5a1hrg4SbTU1Uw3351z",
        DlcCtn: "_3H7i0ChQjoGWieNRcKmYea",
        Type: "HmoaKPBZvHxysTOtEt6I_",
        DealDesc: "_3fPQtBXFQwL3xrK4zmpmyq",
        OneItemRow: "_6a9d-uI8VyQTmWHS3vFI-",
        OneBigItem: "_2lKQioAzojICHeg5SX6fhc",
        TwoCapsuleRow: "_1s9-9e_1KjO0xTTJNj-KuT",
        BackgroundAnimation: "MAJyK9QcMypCZbMYmoLHu",
        "ItemFocusAnim-darkerGrey-nocolor": "_2IE90ljBHRp1SITs86XIYT",
        "ItemFocusAnim-darkerGrey": "_3Izi4xp4tar8pwj5nAAAJx",
        "ItemFocusAnim-darkGreySettings": "_1AXb9HQ7EwT6k-c4TWuD6b",
        "ItemFocusAnim-darkGrey": "_17XlJo9wQvFGnl_mC2sDDP",
        "ItemFocusAnim-grey": "_2_GD3bdKzrHkm2Ldb6tVut",
        "ItemFocusAnim-translucent-white-10": "_31VDSRuwtIPGNRsMsAiiuS",
        "ItemFocusAnim-translucent-white-20": "RpA2APZ8BdkDDu88M1t99",
        "ItemFocusAnimBorder-darkGrey": "_1JoYgnM2uNiTB2PbeAfe-S",
        "ItemFocusAnim-green": "_2cI4C5AH9C-VNLXQyhLoZE",
        focusAnimation: "_1kz2gc1Q6nZYfgugTIMxpo",
        hoverAnimation: "_2Kc7YeIDyVeYMtcdglCErh",
      };
    },
    31343: (e) => {
      e.exports = {
        "duration-app-launch": "800ms",
        PosterCtn: "_24bJJZ8-xr8Et4DVUrr17J",
        Poster: "_3LbyxomxMvYMy7cLThkkIf",
        Video: "_2bH_Jxh-T2YFCPBCW_pvQB",
        BackgroundAnimation: "fmJwkjLpps2lTYiDhuZaS",
        "ItemFocusAnim-darkerGrey-nocolor": "_3P7eXvQeL41e2U1Dlf-o3y",
        "ItemFocusAnim-darkerGrey": "_1ox1csOfGebwxpQWCIrD8r",
        "ItemFocusAnim-darkGreySettings": "_1edLKf573MLIIzXMGDJQuk",
        "ItemFocusAnim-darkGrey": "_20nB-hQDWUFRiLXleHcpCi",
        "ItemFocusAnim-grey": "_3izRpxwl4_d_Vxa1pgYMiA",
        "ItemFocusAnim-translucent-white-10": "_1hhQO02787EOl9eQ9AMGYY",
        "ItemFocusAnim-translucent-white-20": "Y9TgOYN31D-9Nr_0YqCTN",
        "ItemFocusAnimBorder-darkGrey": "_3vZR4wYNZIWMbiwF5nGGUP",
        "ItemFocusAnim-green": "_1rcnmgd1hOIPA9f3ngfevA",
        focusAnimation: "_3uCspaHoybU11f9AiHQU-o",
        hoverAnimation: "fLko22pxQFa4_xcoc8AEf",
      };
    },
    22329: (e) => {
      e.exports = {
        "duration-app-launch": "800ms",
        All: "_1Ihp0wKLvNYkePQe0VrHnC",
        MessageFooter: "_3t5WMbFCdjhi0jY_LsEmHU",
        ButtonAndPriceCtn: "_1ZbewdWkMfyxa_PdVDMoDU",
        Legal: "_2_eDs58X0eEznR-ukDC_qt",
        NoButton: "_3IuUvJa9v9WI7_4WCI8-5t",
        Btn: "_3J3L60oPKSSmfBrQoNzGka",
        MessagePriceCtn: "_2g5STEIbDalv8hDLrPF3f-",
        Price: "G3m3RKwAqO-NG8_dvnifU",
        NoPrice: "_3fIuqvgfN6oD_bvpdHWEl8",
        PriceReal: "_14BApLFAAyKDCTSv-FU3cM",
        PriceRealShort: "_1E4sfQ48HHNa1aVRVPso3t",
        PriceRegular: "lhwy7sGWsQ0H-qiGzBfVO",
        Strike: "_3zM6T8KcphlmjOBnyqYGi0",
        MessageContent: "_2OxhXhvrpr4BL-kq6s7Eov",
        IsBlur: "_1M-oJo6Qwi_taE9-LcUQXw",
        MessageBody: "yNkHDKSU1gO-i7O8pRwGb",
        MessageImage: "_3R5wv6ya3QU6VH7qq2819z",
        VideoImageContainer: "_2yR5YNB4D48KDMaHJC7ELs",
        GameImage: "_1YhdlZrF81fOxpD5J2tD7o",
        BaseCtn: "_6Tfttc004NhLDjbTDS9PC",
        CapsuleCtn: "bSrl3b6Qz1HKSithRsqvM",
        DescCtn: "_20PKe9KJTJC5SAbLi4Dy-Y",
        GameNameCtn: "kgQI2KVBs-Y3neqiTpbvQ",
        GameName: "QFFkjL-A27JtOIgIqO5dH",
        LibraryDetails: "_1ddC4grHqe7uggEE2cihEv",
        Button: "_2T5KnJO7YB00MG9YIyfImx",
        ViewInLibrary: "_3he9kmZ7_zW9VD6jkZAb1N",
        PlayDetailCtn: "J3BmO963EVK__A5VRDAQM",
        TextTop: "_3eCgYrG63hATLNUS2prQdu",
        TextBottom: "_3-KPIEC01O5ncvCQdfuTum",
        ButtonContainer: "_10sC9pDV-gUZWLDMtbWFpZ",
        BackgroundAnimation: "iazxtC_6xsy3ZAlQoiLly",
        "ItemFocusAnim-darkerGrey-nocolor": "_2cB4L3LYRmW8ZPhM1KmIiK",
        "ItemFocusAnim-darkerGrey": "ydW4ABxqu6OYOVnY28tB8",
        "ItemFocusAnim-darkGreySettings": "_4i8hDH1Gg-f_tUZZ4Zp5",
        "ItemFocusAnim-darkGrey": "_2IeQuqKv4kC7vVDREUnVXJ",
        "ItemFocusAnim-grey": "X3U1lQ94969mdo-OyDB4r",
        "ItemFocusAnim-translucent-white-10": "_3I9H7g6vFex4f-XocVTj-X",
        "ItemFocusAnim-translucent-white-20": "_2_i92FJitTDXOf5j2qHIAl",
        "ItemFocusAnimBorder-darkGrey": "_3ptMvwESzMu4HLigXQtQOm",
        "ItemFocusAnim-green": "jkn_IZFEGtZHxdFRc6KON",
        focusAnimation: "_38aC9CNFSq3FVlwkvehRI-",
        hoverAnimation: "_2a2hzvLCbR64ODBqWR_ARQ",
      };
    },
    61738: (e, t, s) => {
      var n = {
        "./af": 30911,
        "./af.js": 30911,
        "./ar": 63595,
        "./ar-dz": 99358,
        "./ar-dz.js": 99358,
        "./ar-kw": 46830,
        "./ar-kw.js": 46830,
        "./ar-ly": 26067,
        "./ar-ly.js": 26067,
        "./ar-ma": 64154,
        "./ar-ma.js": 64154,
        "./ar-ps": 90753,
        "./ar-ps.js": 90753,
        "./ar-sa": 53616,
        "./ar-sa.js": 53616,
        "./ar-tn": 19026,
        "./ar-tn.js": 19026,
        "./ar.js": 63595,
        "./az": 87043,
        "./az.js": 87043,
        "./be": 28437,
        "./be.js": 28437,
        "./bg": 29843,
        "./bg.js": 29843,
        "./bm": 39421,
        "./bm.js": 39421,
        "./bn": 41300,
        "./bn-bd": 54487,
        "./bn-bd.js": 54487,
        "./bn.js": 41300,
        "./bo": 40827,
        "./bo.js": 40827,
        "./br": 35120,
        "./br.js": 35120,
        "./bs": 41991,
        "./bs.js": 41991,
        "./ca": 47504,
        "./ca.js": 47504,
        "./cs": 98346,
        "./cs.js": 98346,
        "./cv": 17525,
        "./cv.js": 17525,
        "./cy": 80872,
        "./cy.js": 80872,
        "./da": 48787,
        "./da.js": 48787,
        "./de": 30199,
        "./de-at": 33461,
        "./de-at.js": 33461,
        "./de-ch": 97995,
        "./de-ch.js": 97995,
        "./de.js": 30199,
        "./dv": 14682,
        "./dv.js": 14682,
        "./el": 52549,
        "./el.js": 52549,
        "./en-au": 5706,
        "./en-au.js": 5706,
        "./en-ca": 50584,
        "./en-ca.js": 50584,
        "./en-gb": 41685,
        "./en-gb.js": 41685,
        "./en-ie": 32050,
        "./en-ie.js": 32050,
        "./en-il": 35545,
        "./en-il.js": 35545,
        "./en-in": 42551,
        "./en-in.js": 42551,
        "./en-nz": 10620,
        "./en-nz.js": 10620,
        "./en-sg": 16222,
        "./en-sg.js": 16222,
        "./eo": 88124,
        "./eo.js": 88124,
        "./es": 59784,
        "./es-do": 30300,
        "./es-do.js": 30300,
        "./es-mx": 47292,
        "./es-mx.js": 47292,
        "./es-us": 36469,
        "./es-us.js": 36469,
        "./es.js": 59784,
        "./et": 56349,
        "./et.js": 56349,
        "./eu": 6782,
        "./eu.js": 6782,
        "./fa": 86749,
        "./fa.js": 86749,
        "./fi": 52469,
        "./fi.js": 52469,
        "./fil": 2989,
        "./fil.js": 2989,
        "./fo": 50743,
        "./fo.js": 50743,
        "./fr": 34916,
        "./fr-ca": 96853,
        "./fr-ca.js": 96853,
        "./fr-ch": 81566,
        "./fr-ch.js": 81566,
        "./fr.js": 34916,
        "./fy": 82949,
        "./fy.js": 82949,
        "./ga": 80932,
        "./ga.js": 80932,
        "./gd": 82671,
        "./gd.js": 82671,
        "./gl": 95687,
        "./gl.js": 95687,
        "./gom-deva": 67330,
        "./gom-deva.js": 67330,
        "./gom-latn": 7021,
        "./gom-latn.js": 7021,
        "./gu": 78728,
        "./gu.js": 78728,
        "./he": 28211,
        "./he.js": 28211,
        "./hi": 15487,
        "./hi.js": 15487,
        "./hr": 94106,
        "./hr.js": 94106,
        "./hu": 14147,
        "./hu.js": 14147,
        "./hy-am": 23862,
        "./hy-am.js": 23862,
        "./id": 78825,
        "./id.js": 78825,
        "./is": 57612,
        "./is.js": 57612,
        "./it": 9497,
        "./it-ch": 75653,
        "./it-ch.js": 75653,
        "./it.js": 9497,
        "./ja": 2209,
        "./ja.js": 2209,
        "./jv": 85668,
        "./jv.js": 85668,
        "./ka": 6904,
        "./ka.js": 6904,
        "./kk": 2138,
        "./kk.js": 2138,
        "./km": 81660,
        "./km.js": 81660,
        "./kn": 88613,
        "./kn.js": 88613,
        "./ko": 57894,
        "./ko.js": 57894,
        "./ku": 28468,
        "./ku-kmr": 57123,
        "./ku-kmr.js": 57123,
        "./ku.js": 28468,
        "./ky": 91808,
        "./ky.js": 91808,
        "./lb": 47070,
        "./lb.js": 47070,
        "./lo": 56505,
        "./lo.js": 56505,
        "./lt": 53656,
        "./lt.js": 53656,
        "./lv": 83746,
        "./lv.js": 83746,
        "./me": 42486,
        "./me.js": 42486,
        "./mi": 82,
        "./mi.js": 82,
        "./mk": 14792,
        "./mk.js": 14792,
        "./ml": 10845,
        "./ml.js": 10845,
        "./mn": 46939,
        "./mn.js": 46939,
        "./mr": 5575,
        "./mr.js": 5575,
        "./ms": 81424,
        "./ms-my": 43179,
        "./ms-my.js": 43179,
        "./ms.js": 81424,
        "./mt": 30341,
        "./mt.js": 30341,
        "./my": 72834,
        "./my.js": 72834,
        "./nb": 75292,
        "./nb.js": 75292,
        "./ne": 23753,
        "./ne.js": 23753,
        "./nl": 53922,
        "./nl-be": 77542,
        "./nl-be.js": 77542,
        "./nl.js": 53922,
        "./nn": 81304,
        "./nn.js": 81304,
        "./oc-lnc": 41156,
        "./oc-lnc.js": 41156,
        "./pa-in": 17851,
        "./pa-in.js": 17851,
        "./pl": 66636,
        "./pl.js": 66636,
        "./pt": 13252,
        "./pt-br": 95189,
        "./pt-br.js": 95189,
        "./pt.js": 13252,
        "./ro": 5451,
        "./ro.js": 5451,
        "./ru": 981,
        "./ru.js": 981,
        "./sd": 49139,
        "./sd.js": 49139,
        "./se": 24684,
        "./se.js": 24684,
        "./si": 85448,
        "./si.js": 85448,
        "./sk": 61682,
        "./sk.js": 61682,
        "./sl": 17595,
        "./sl.js": 17595,
        "./sq": 61360,
        "./sq.js": 61360,
        "./sr": 45897,
        "./sr-cyrl": 80616,
        "./sr-cyrl.js": 80616,
        "./sr.js": 45897,
        "./ss": 15034,
        "./ss.js": 15034,
        "./sv": 78213,
        "./sv.js": 78213,
        "./sw": 47494,
        "./sw.js": 47494,
        "./ta": 48387,
        "./ta.js": 48387,
        "./te": 90951,
        "./te.js": 90951,
        "./tet": 83675,
        "./tet.js": 83675,
        "./tg": 99753,
        "./tg.js": 99753,
        "./th": 59844,
        "./th.js": 59844,
        "./tk": 84429,
        "./tk.js": 84429,
        "./tl-ph": 54645,
        "./tl-ph.js": 54645,
        "./tlh": 56946,
        "./tlh.js": 56946,
        "./tr": 8630,
        "./tr.js": 8630,
        "./tzl": 79480,
        "./tzl.js": 79480,
        "./tzm": 13839,
        "./tzm-latn": 36313,
        "./tzm-latn.js": 36313,
        "./tzm.js": 13839,
        "./ug-cn": 26648,
        "./ug-cn.js": 26648,
        "./uk": 24192,
        "./uk.js": 24192,
        "./ur": 8335,
        "./ur.js": 8335,
        "./uz": 21351,
        "./uz-latn": 60785,
        "./uz-latn.js": 60785,
        "./uz.js": 21351,
        "./vi": 9541,
        "./vi.js": 9541,
        "./x-pseudo": 309,
        "./x-pseudo.js": 309,
        "./yo": 21512,
        "./yo.js": 21512,
        "./zh-cn": 98562,
        "./zh-cn.js": 98562,
        "./zh-hk": 7374,
        "./zh-hk.js": 7374,
        "./zh-mo": 87107,
        "./zh-mo.js": 87107,
        "./zh-tw": 34518,
        "./zh-tw.js": 34518,
      };
      function a(e) {
        var t = r(e);
        return s(t);
      }
      function r(e) {
        if (!s.o(n, e)) {
          var t = new Error("Cannot find module '" + e + "'");
          throw ((t.code = "MODULE_NOT_FOUND"), t);
        }
        return n[e];
      }
      (a.keys = function () {
        return Object.keys(n);
      }),
        (a.resolve = r),
        (e.exports = a),
        (a.id = 61738);
    },
    44165: (e, t, s) => {
      "use strict";
      s.d(t, { HD: () => u, f1: () => _, s4: () => p, sB: () => g });
      var n = s(34629),
        a = s(14947),
        r = s(19367),
        i = s.n(r),
        o = s(90626),
        l = (s(4434), s(78327));
      class c {
        constructor() {
          (0, a.Gn)(this);
        }
        bIncludeFeaturedAsGameSource = !0;
        nOverrideDateNow = void 0;
        get bRequireAllEventsLoadedInTimeBlock() {
          return !1;
        }
        get bIncludeCurators() {
          return !0;
        }
        GetTimeNowWithOverride() {
          return this.nOverrideDateNow
            ? this.nOverrideDateNow
            : Math.floor(Date.now() / 1e3);
        }
        GetTimeNowWithOverrideAsDate() {
          return this.nOverrideDateNow
            ? new Date(1e3 * this.nOverrideDateNow)
            : new Date();
        }
        BHasTimeOverride() {
          return Boolean(this.nOverrideDateNow);
        }
        ParseDevOverrides(e) {
          if (!e || 0 == e.length) return;
          const t = new URLSearchParams("?" == e[0] ? e.substring(1) : e);
          if (
            t.has("t") &&
            ("dev" == l.TS.WEB_UNIVERSE || "beta" == l.TS.WEB_UNIVERSE)
          ) {
            const e = t.get("t");
            let s = /^\d+$/.test(e ?? "")
              ? i().unix(Number.parseInt(e ?? "0"))
              : i()(e);
            (this.nOverrideDateNow = Math.floor(s.unix())),
              console.log(
                "CEventCalendarDevFeatures overriding partner event time: " +
                  this.nOverrideDateNow +
                  " " +
                  s.format(),
              );
          }
        }
      }
      (0, n.Cg)([a.sH], c.prototype, "nOverrideDateNow", void 0);
      const u = new c();
      window.g_EventCalendarDevFeatures = u;
      const d = new Date(),
        m = Math.floor(d.getTime() / 1e3);
      function g() {
        return u.nOverrideDateNow ?? m;
      }
      function _() {
        return o.useMemo(() => g(), []);
      }
      function p() {
        return o.useMemo(() => u.GetTimeNowWithOverrideAsDate(), []);
      }
    },
    56186: (e, t, s) => {
      "use strict";
      s.d(t, {
        $I: () => x,
        AP: () => T,
        Nt: () => h,
        XW: () => y,
        dr: () => A,
        fL: () => v,
        rT: () => f,
      });
      var n = s(34629),
        a = s(80902),
        r = s(90626),
        i = s(22837),
        o = s(37085),
        l = s(56545),
        c = s(45740),
        u = s(78327),
        d = s(72839),
        m = s(81393),
        g = s(14947),
        _ = s(16021),
        p = s(61859);
      class h {
        m_SteamInterface;
        m_SteamInterfacePromotions;
        m_setMessagesSeen = new Set();
        static sm_DefaultDataRequest = {
          include_release: !0,
          include_assets: !0,
        };
        constructor(e) {
          this.m_SteamInterface = e;
        }
        static sm_Instance;
        static Init(e) {
          h.sm_Instance = new h(e);
        }
        SetSteamInterfacePromotions(e) {
          this.m_SteamInterfacePromotions = e;
        }
        static Get() {
          return (
            (0, m.wT)(h.sm_Instance, "MarketingMessages store not initialized"),
            h.sm_Instance
          );
        }
        async GetMessageList(e, t = !1) {
          if (!u.iA.logged_in) return [];
          const s = l.w.Init(c.LK);
          s.Body().set_country_code(u.TS.COUNTRY),
            s.Body().set_elanguage((0, i.sfN)(u.TS.LANGUAGE)),
            s.Body().set_client_package_version(e.nClientPackageVersion),
            s.Body().set_operating_system(e.eOSType),
            t && s.Body().set_include_seen_messages(!0),
            (0, d.rV)(s),
            (0, d.Bn)(s, h.sm_DefaultDataRequest);
          const n = await c.EO.GetMarketingMessagesForUser(
            this.m_SteamInterface.GetServiceTransport(),
            s,
          );
          if (n.GetEResult() != o.R)
            throw `Error loading marketing messages: ${n.GetEResult()}`;
          if (t)
            for (const e of n.Body().messages())
              e.already_seen() && this.m_setMessagesSeen.add(e.message().gid());
          return n.Body().messages();
        }
        async GetSingleMessage(e, t) {
          const s = l.w.Init(c.e6);
          let n;
          if (
            (s.Body().set_gid(e),
            (0, d.rV)(s),
            (0, d.Bn)(s, h.sm_DefaultDataRequest),
            (n =
              t || u.iA.logged_in
                ? await c.EO.GetDisplayMarketingMessageForUser(
                    this.m_SteamInterface.GetServiceTransport(),
                    s,
                  )
                : await c.EO.GetDisplayMarketingMessage(
                    this.m_SteamInterface.GetAnonymousServiceTransport(),
                    s,
                  )),
            n.GetEResult() != o.R)
          )
            throw `Error loading marketing messages: ${n.GetEResult()}`;
          return n.Body().message();
        }
        MarkMessageSeen(e, t, s) {
          if (this.m_setMessagesSeen.has(e)) return;
          const n = l.w.Init(c.S4);
          n.Body().set_gid(e),
            n.Body().set_display_index(t),
            n.Body().set_template_type(s),
            c.EO.MarkMessageSeen(
              this.m_SteamInterface.GetServiceTransport(),
              n,
            ),
            this.m_setMessagesSeen.add(e);
        }
        BIsMessageSeen(e) {
          return this.m_setMessagesSeen.has(e);
        }
      }
      class f {
        m_message;
        m_templateVars = void 0;
        m_item;
        constructor(e) {
          (0, g.Gn)(this),
            (this.m_message = e),
            e.associated_item(!1) &&
              (this.m_item = _.A.Get().ReadItem(
                e.associated_item(),
                h.sm_DefaultDataRequest,
              )),
            (this.m_templateVars = (function (e) {
              if (!e) return null;
              try {
                const t = JSON.parse(e);
                return (
                  t.use_additional_fields &&
                    (t.use_additional_fields =
                      "true" === t.use_additional_fields ||
                      1 === t.use_additional_fields),
                  t.use_custom_legal_text &&
                    (t.use_custom_legal_text =
                      "true" === t.use_custom_legal_text ||
                      1 === t.use_custom_legal_text),
                  t.last_asset_mtime &&
                    (t.last_asset_mtime = parseInt(t.last_asset_mtime)),
                  (t.ll_image = t.ll_image || {}),
                  t
                );
              } catch (e) {
                "dev" == u.TS.WEB_UNIVERSE &&
                  console.error(
                    "MarketingMessageReadAndParseTemplateVars: Failed to prase",
                    e,
                  );
              }
              return null;
            })(e.template_vars_json()));
        }
        get id() {
          return this.m_message.gid();
        }
        GetType() {
          return this.m_message.type();
        }
        static GetTypeAsLocalizedString(e) {
          switch (e) {
            case c.D4.QJ:
              return (0, p.we)("#spotlight_weekend_deal");
            case c.D4.xl:
              return (0, p.we)("#spotlight_midweek_madness");
            case c.D4.Sk:
              return (0, p.we)("#spotlight_daily_deal");
            case c.D4.OD:
              return (0, p.we)("#msg_available_everywhere");
            case c.D4.IT:
              return (0, p.we)("#msg_prepurchase_now");
            case c.D4.T9:
              return (0, p.we)("#msg_play_now");
            case c.D4.QY:
              return (0, p.we)("#label_pre_load_now");
            case c.D4.W8:
              return (0, p.we)("#label_just_updated");
            case c.D4.eV:
              return (0, p.we)("#label_new_dlc_available");
            case c.D4.SK:
              return (0, p.we)("#label_free_weekend");
            case c.D4.eH:
              return (0, p.we)("#msg_on_sale_now");
            case c.D4.k6:
              return (0, p.we)("#msg_play_beta_now");
          }
          return null;
        }
        GetTemplateType() {
          return this.m_message.template_type();
        }
        GetTemplateTypeForReporting() {
          if (
            this.GetTemplateVars().custom_display &&
            this.GetTemplateVars().custom_display.startsWith("replay")
          )
            return c.rj.TO;
          switch (this.GetTemplateVars().custom_display) {
            case "dlc_override":
              return c.rj.k2;
            case "mm_auto_render":
              return c.rj.H;
            case "partner_event":
              return c.rj.GS;
            case "featured_video":
              return c.rj.CT;
          }
          return c.rj.BA;
        }
        GetTemplateVars() {
          return this.m_templateVars;
        }
        GetLocalizedAltText(e) {
          return (
            this.m_templateVars?.localized_alt_text?.[e] ||
            this.m_templateVars?.localized_alt_text?.[
              p.A0.GetELanguageFallback(e)
            ] ||
            void 0
          );
        }
        GetTemplateImage() {
          let e = this.m_templateVars.ll_image[u.TS.LANGUAGE],
            t = (0, i.sfN)(u.TS.LANGUAGE);
          return (
            e ||
              u.TS.LANGUAGE != (0, i.LgB)(i.FHN) ||
              ((e = this.m_templateVars.ll_image.LATAM), (t = i.FHN)),
            e || ((e = this.m_templateVars.ll_image.english), (t = i.Bhc)),
            [e?.path, t]
          );
        }
        GetTemplateBackgroundImage() {
          let e = this.m_templateVars.background[u.TS.LANGUAGE],
            t = (0, i.sfN)(u.TS.LANGUAGE);
          return (
            e || ((e = this.m_templateVars.background.english), (t = i.Bhc)),
            [e?.path, t]
          );
        }
        GetFeaturedVideoMP4URL() {
          return this.m_templateVars.featured_video_mp4;
        }
        GetFeaturedVideoWebMURL() {
          return this.m_templateVars.featured_video_webm;
        }
        GetFeaturedVideoAutoPlay() {
          return this.m_templateVars.featured_video_autoplay;
        }
        GetFeaturedVideoLoop() {
          return this.m_templateVars.featured_video_loop;
        }
        GetPosterImage() {
          let e = this.m_templateVars.poster[u.TS.LANGUAGE],
            t = (0, i.sfN)(u.TS.LANGUAGE);
          return (
            e || ((e = this.m_templateVars.poster.english), (t = i.Bhc)),
            [e?.path, t]
          );
        }
        GetSubtitleObj() {
          return this.m_templateVars.subtitles;
        }
        GetDLCAppIDs() {
          return this.m_templateVars.additional_featuring
            .filter((e) => Boolean(e.appid))
            .map((e) => e.appid);
        }
        GetAutoRenderWithoutAssetOverrides() {
          return Boolean(
            this.m_templateVars.autorender_assets_without_overrides,
          );
        }
        GetGID() {
          return this.m_message.gid();
        }
        SetDLCAppIDOverride(e) {
          (this.m_templateVars.additional_featuring = e.map((e) => ({
            appid: e,
          }))),
            (this.m_templateVars.custom_display = "dlc_override");
        }
        OverrideCustomText(e) {
          this.m_templateVars.button_text_custom = e;
        }
        OverrideURL(e) {
          this.m_templateVars.linkurl = e;
        }
        BHasTemplateAnimatedAssets() {
          return this.m_templateVars.has_animated_assets;
        }
        BHasTemplateAnimatedAssetForLanguage(e) {
          const t = (0, i.LgB)(e);
          return Boolean(this.m_templateVars.mp4[t]);
        }
        GetTemplateWebM(e) {
          const t = (0, i.LgB)(e);
          return this.m_templateVars.webm[t]?.path;
        }
        GetTemplateWebMWithFallback(e) {
          const t = (0, i.LgB)(e);
          if (this.m_templateVars.webm[t]?.path)
            return [this.m_templateVars.webm[t].path, e];
          const s = p.A0.GetELanguageFallback(e),
            n = (0, i.LgB)(s);
          return [this.m_templateVars.webm[n]?.path, s];
        }
        GetTemplateMP4(e) {
          const t = (0, i.LgB)(e);
          return this.m_templateVars.mp4[t]?.path;
        }
        GetTemplateMP4WithFallback(e) {
          const t = (0, i.LgB)(e);
          if (this.m_templateVars.mp4[t]?.path)
            return [this.m_templateVars.mp4[t].path, e];
          const s = p.A0.GetELanguageFallback(e),
            n = (0, i.LgB)(s);
          return [this.m_templateVars.mp4[n]?.path, s];
        }
        GetLegalHTML() {
          return this.GetTemplateVars().use_custom_legal_text
            ? this.GetTemplateVars().custom_legal_text
            : ((e = this.GetTemplateVars().partner),
              `© ${(new Date()).getFullYear()} Valve Corporation${e ? " and " + e : ""}. <br/>All trademarks are property of their respective owners in the US and other countries.`);
          var e;
        }
        get associated_item() {
          return this.m_item;
        }
      }
      (0, n.Cg)([g.sH], f.prototype, "m_templateVars", void 0);
      const j = "^(replay)([0-9]{4})";
      function v(e) {
        const t = e.match(j);
        return t?.[2];
      }
      function x(e) {
        if (e) {
          const t = e.match(j);
          return t?.[2] && !isNaN(Number(t?.[2]));
        }
        return !1;
      }
      function y(e, t, s) {
        const { bIncludeSeenMessages: n, ...i } = t,
          {
            data: o,
            isLoading: l,
            isError: c,
          } = (0, a.I)({
            queryKey: [
              "MarketingMessages",
              "List",
              i,
              { bIncludeSeenMessages: !!n },
            ],
            queryFn: () => e.GetMessageList(i, n),
            ...s,
          }),
          u = r.useMemo(() => o?.map((e) => new f(e.message())), [o]);
        return { rgMessages: l ? null : u, isError: c };
      }
      function A(e, t, s) {
        const { data: n, isError: i } = (0, a.I)({
          queryKey: ["MarketingMessages", s ? "SinglePreivew" : "Single", t],
          queryFn: () => e.GetSingleMessage(t, s),
          enabled: !!t,
        });
        return {
          message: r.useMemo(() => {
            if (n) {
              const e = new f(n);
              if (s) {
                const t = (0, u.Tc)(
                  "marketingmessage_preview_config",
                  "application_config",
                );
                t?.dlc_appid_overrides?.length > 0 &&
                  e.SetDLCAppIDOverride(t.dlc_appid_overrides);
              }
              return e;
            }
          }, [n, s]),
          isError: i,
        };
      }
      function T(e, t, s, n, a) {
        const r = l.w.Init(c.cX);
        r.Body().set_gid(t),
          r.Body().set_display_index(s),
          r.Body().set_template_type(n),
          r.Body().set_click_location(a),
          c.EO.MarkMessageClicked(e, r);
      }
    },
    55263: (e, t, s) => {
      "use strict";
      s.d(t, {
        G6: () => g,
        Gg: () => h,
        Ow: () => p,
        Sq: () => u,
        YM: () => x,
        eR: () => d,
        ik: () => m,
        mZ: () => f,
        t7: () => _,
        zX: () => v,
      });
      var n = s(41735),
        a = s.n(n),
        r = s(90626),
        i = s(37085),
        o = s(96171),
        l = s(84933),
        c = s(16021);
      const u = 1,
        d = 2,
        m = 3;
      function g(e, t, s, n) {
        const o = (0, r.useRef)(void 0),
          g = (0, r.useRef)(void 0),
          _ = (0, l.CH)();
        o.current = e;
        const [p, h] = (0, r.useState)(void 0),
          {
            include_assets: f,
            include_release: j,
            include_platforms: v,
            include_all_purchase_options: x,
            include_screenshots: y,
            include_trailers: A,
            include_ratings: T,
            include_tag_count: I,
            include_reviews: b,
            include_basic_info: M,
            include_supported_languages: k,
            include_full_description: G,
            include_included_items: w,
            include_assets_without_overrides: S,
            apply_user_filters: C,
            include_links: N,
          } = s;
        if (
          ((0, r.useEffect)(() => {
            const s = {
              include_assets: f,
              include_release: j,
              include_platforms: v,
              include_all_purchase_options: x,
              include_screenshots: y,
              include_trailers: A,
              include_ratings: T,
              include_tag_count: I,
              include_reviews: b,
              include_basic_info: M,
              include_supported_languages: k,
              include_full_description: G,
              include_included_items: w,
              include_assets_without_overrides: S,
              apply_user_filters: C,
              include_links: N,
            };
            let r = null;
            return (
              !e ||
                e < 0 ||
                c.A.Get().BHasStoreItem(e, t, s) ||
                (void 0 !== p && n && n == g.current) ||
                (n !== g.current && (h(void 0), (g.current = n)),
                (r = a().CancelToken.source()),
                c.A.Get()
                  .QueueStoreItemRequest(e, t, s)
                  .then((t) => {
                    r?.token.reason || o.current !== e || h(t == i.R), _();
                  })),
              () => r?.cancel("useStoreItemCache: unmounting")
            );
          }, [e, t, n, p, f, j, v, x, y, A, T, I, b, M, k, G, w, S, C, N, _]),
          !e)
        )
          return [null, d];
        if (!1 === p) return [void 0, d];
        if (c.A.Get().BIsStoreItemMissing(e, t)) return [void 0, d];
        if (!c.A.Get().BHasStoreItem(e, t, s)) return [void 0, u];
        const B = c.A.Get().GetStoreItemWithLegacyVisibilityCheck(e, t);
        return B ? [B, m] : [null, d];
      }
      function _(e, t, s) {
        return g(e, o.c6.qI, t, s);
      }
      function p(e, t, s) {
        return g(e, o.c6.xO, t, s);
      }
      function h(e, t, s) {
        return g(e, o.c6.RD, t, s);
      }
      function f(e, t, s) {
        const [n, a] = g(e, t, s);
        let r;
        n?.GetStoreItemType() != o.c6.RD ||
          n.GetAssets()?.GetHeaderURL() ||
          1 != n?.GetIncludedAppIDs().length ||
          (r = n.GetIncludedAppIDs()[0]);
        const [i, l] = _(r, s);
        return r && i?.BIsVisible() ? [i, l] : [n, a];
      }
      function j(e, t, s, n) {
        const i = (0, l.CH)(),
          {
            include_assets: o,
            include_release: g,
            include_platforms: _,
            include_all_purchase_options: p,
            include_screenshots: h,
            include_trailers: f,
            include_ratings: j,
            include_tag_count: v,
            include_reviews: x,
            include_basic_info: y,
            include_supported_languages: A,
            include_full_description: T,
            include_included_items: I,
            include_assets_without_overrides: b,
            apply_user_filters: M,
            include_links: k,
          } = s;
        if (
          ((0, r.useEffect)(() => {
            if (!e || 0 == e.length) return;
            const s = {
                include_assets: o,
                include_release: g,
                include_platforms: _,
                include_all_purchase_options: p,
                include_screenshots: h,
                include_trailers: f,
                include_ratings: j,
                include_tag_count: v,
                include_reviews: x,
                include_basic_info: y,
                include_supported_languages: A,
                include_full_description: T,
                include_included_items: I,
                include_assets_without_overrides: b,
                apply_user_filters: M,
                include_links: k,
              },
              n = e.filter(
                (e) =>
                  !(
                    c.A.Get().BHasStoreItem(e, t, s) ||
                    c.A.Get().BIsStoreItemMissing(e, t)
                  ),
              );
            if (0 == n.length) return;
            const r = a().CancelToken.source(),
              l = n.map((e) => c.A.Get().QueueStoreItemRequest(e, t, s));
            return (
              Promise.all(l).then(() => {
                r.token.reason || i();
              }),
              () => r.cancel("useStoreItemCacheMultiplePackages: unmounting")
            );
          }, [e, t, n, i, o, g, _, p, h, f, j, v, x, y, A, T, I, b, M, k]),
          !e)
        )
          return d;
        if (
          !e.every(
            (e) =>
              c.A.Get().BHasStoreItem(e, t, s) ||
              c.A.Get().BIsStoreItemMissing(e, t),
          )
        )
          return u;
        return e.every((e) =>
          c.A.Get().GetStoreItemWithLegacyVisibilityCheck(e, t),
        )
          ? m
          : d;
      }
      function v(e, t, s) {
        return j(e, o.c6.qI, t, s);
      }
      function x() {
        r.useEffect(
          () => (
            c.A.Get().SetReturnUnavailableItems(!0),
            () => c.A.Get().SetReturnUnavailableItems(!1)
          ),
          [],
        );
      }
    },
    2921: (e, t, s) => {
      "use strict";
      s.d(t, { PE: () => E, qP: () => L, Yg: () => B });
      var n = s(7850),
        a = s(76217),
        r = s(86927),
        i = s(42834),
        o = s(52471),
        l = s(39777),
        c = s(52541),
        u = s(38535),
        d = s(90626),
        m = s(92834),
        g = s(1078),
        _ = s(12155),
        p = s(52038),
        h = s(78327),
        f = s(51706),
        j = s(46253),
        v = s(22797),
        x = s(44433),
        y = s(91970),
        A = s.n(y),
        T = s(18654),
        I = s.n(T),
        b = s(32754),
        M = s(61859),
        k = s(30470),
        G = s(24484);
      function w(e) {
        const { id: t } = e,
          { data: s } = (0, l.J$)(t),
          a = (0, d.useMemo)(() => {
            if (!s) return [];
            const e =
              s.categories?.supported_player_categoryids?.slice(0, 1) || [];
            return (
              s.categories?.feature_categoryids?.forEach((t) => e.push(t)),
              s.categories?.controller_categoryids?.forEach((t) => e.push(t)),
              s.categories?.supported_player_categoryids
                ?.slice(1)
                .forEach((t) => e.push(t)),
              e
            );
          }, [s]);
        return a && 0 != a.length
          ? (0, n.jsx)("div", {
              className: (0, p.A)(I().SaleTagBlockCtn, "SaleTagBlockCtn"),
              children: Boolean(a?.length > 0)
                ? (0, n.jsx)("div", {
                    className: (0, p.A)(I().TagBox, I().Categories),
                    children: a.map((e) =>
                      (0, n.jsx)(C, { categoryID: e }, "cat_" + e),
                    ),
                  })
                : (0, n.jsx)("div", { children: (0, M.we)("#Broadcast_None") }),
            })
          : null;
      }
      class S {
        m_rgCategories;
        constructor() {
          this.m_rgCategories = (0, G.Tc)(
            "feature_categories",
            "application_config",
          );
        }
        static g_Self = null;
        static Get() {
          return S.g_Self || (S.g_Self = new S()), S.g_Self;
        }
      }
      function C(e) {
        const { categoryID: t } = e,
          s = S.Get().m_rgCategories.find((e) => e.categoryid == t);
        return s
          ? (0, n.jsx)("div", {
              className: I().Category,
              children: (0, n.jsx)(b.he, {
                toolTipContent: s.name,
                children: (0, n.jsx)("div", {
                  className: I().CategoryIcon,
                  style: {
                    background: `url(${k.TS.STORE_CDN_URL}/public/images/${s.image_path}) no-repeat center center/cover`,
                  },
                }),
              }),
            })
          : null;
      }
      var N = s(90421);
      function B(e) {
        const { id: t, bPopOutTrailerPlayback: s } = e,
          { data: a } = (0, l.Yo)(t),
          { data: r } = (0, l.j4)(t),
          { data: i } = (0, l.J$)(t),
          [o, c] = (0, d.useState)(!1),
          [m, g] = (0, d.useState)(!1),
          _ = (0, u.dy)(),
          h = a?.highlights?.filter((e) => !_ || e.all_ages),
          f = h && h?.length > 0 ? h[0] : void 0,
          j = d.useCallback(() => {
            f && (s ? g(!0) : c((e) => !e));
          }, [f, s]);
        if (!i)
          return (0, n.jsx)("div", {
            className: (0, p.A)(A().HilightGrid, A().MediaContainer),
            children: (0, n.jsx)(v.t, { size: "medium" }),
          });
        const x = f
          ? (0, n.jsx)(F, { trailer: f, bPlayVideo: o, fnTogglePlayTrailer: j })
          : null;
        return f ||
          (r && r.all_ages_screenshots && r.all_ages_screenshots.length > 0)
          ? (0, n.jsxs)("div", {
              className: (0, p.A)(A().HilightGrid, A().MediaContainer),
              children: [
                (0, n.jsx)(D, {
                  elFeaturedInCenter: x,
                  storeItemScreenshots: r,
                  trailer: f,
                  id: t,
                  name: i.name || "",
                }),
                Boolean(s)
                  ? (0, n.jsx)(E, {
                      id: t,
                      bShowModal: m,
                      hideModal: () => g(!1),
                    })
                  : (0, n.jsx)(P, {
                      name: i.name || "",
                      trailer: f,
                      bPlayVideo: o,
                      fnTogglePlayTrailer: j,
                      bControls: !0,
                    }),
              ],
            })
          : null;
      }
      function L(e) {
        const {
            id: t,
            fnOnClickButton: s,
            bLowBandwidthMode: a,
            bUseAssetWithoutOverride: r,
          } = e,
          { data: i } = (0, l.j4)(t),
          { data: c } = (0, l.J$)(t),
          { data: u } = (0, l.Yo)(t),
          d = (0, o.kB)(t);
        return void 0 !== !u && i && c
          ? (0, n.jsx)("div", {
              className: (0, p.A)(A().HilightGrid, A().MediaContainerMM),
              children: (0, n.jsx)(D, {
                id: t,
                elFeaturedInCenter: (0, n.jsx)(V, {
                  id: t,
                  bUseAssetWithoutOverride: Boolean(r),
                  fnOnClickButton: s,
                }),
                trailer: d && d.length > 0 ? d[0] : void 0,
                storeItemScreenshots: i,
                featureElementclassName: A().MainImage,
                bUseTrailerAsFirstThumb: !a,
                bNoScreenShotModals: !0,
                name: c.name || "",
              }),
            })
          : (0, n.jsx)("div", {
              className: (0, p.A)(A().HilightGrid, A().MediaContainerMM),
              children: (0, n.jsx)(v.t, { size: "medium" }),
            });
      }
      function D(e) {
        const {
            elFeaturedInCenter: t,
            id: s,
            name: a,
            trailer: r,
            storeItemScreenshots: o,
            featureElementclassName: l,
            bUseTrailerAsFirstThumb: m,
            bNoScreenShotModals: _,
          } = e,
          [h, f] = d.useState(void 0),
          [j, v] = (0, g.XC)(),
          x = (0, u.dy)(),
          y = (0, d.useRef)(null),
          [T, I] = (0, d.useState)(0);
        if (!s) return null;
        const b = t || (void 0 !== h && -1 !== h) ? h : 0,
          M = new Array(),
          k = new Array();
        m &&
          r &&
          (M.push(
            (0, n.jsx)(
              F,
              {
                trailer: r,
                bPlayVideo: !1,
                fnTogglePlayTrailer: () => {},
                onMouseEnter: () => f(0),
                onMouseLeave: () => {
                  const e = y.current;
                  e && I(e.currentTime);
                },
              },
              "trail_thumb_",
            ),
          ),
          k.push(
            (0, n.jsx)(
              P,
              {
                ref: y,
                name: a,
                trailer: r,
                bControls: !1,
                bPlayVideo: !0,
                startTime: T,
                fnTogglePlayTrailer: () => {},
              },
              "trail_inline",
            ),
          ));
        const G = (
          x ? o?.all_ages_screenshots : o?.mature_content_screenshots
        )?.filter(Boolean);
        if (
          (G?.forEach((e, s) => {
            if ((t || s > 0) && M.length < 3) {
              const t = (0, i.bu)(e, "thumb"),
                a = (0, i.bu)(e, "600x338"),
                r = M.length;
              M.push(
                (0, n.jsx)(
                  "div",
                  {
                    className: (0, p.A)({
                      [A().ThumbnailCtn]: !0,
                      [A().ThumbnialClickable]: !_,
                    }),
                    children: (0, n.jsx)("img", {
                      src: t,
                      onClick: _
                        ? void 0
                        : () => {
                            const e = [...(G || [])];
                            if (e.length > 0) {
                              for (let t = 0; t < s; ++t) {
                                const t = e.shift();
                                t && e.push(t);
                              }
                              j(e.map((e) => (0, i.bu)(e, "full")));
                            }
                          },
                      onMouseEnter: () => f(r),
                    }),
                  },
                  s + "_small_" + t,
                ),
              ),
                k.push(
                  (0, n.jsx)(
                    "div",
                    {
                      className: A().ScreenshotDisplayCtn,
                      children: (0, n.jsx)("img", { src: a }),
                    },
                    s + "_big_" + t,
                  ),
                );
            }
          }),
          !(t || (k && 0 != k.length)))
        )
          return null;
        const w = M.slice(0, 3),
          S = Array.from({ length: Math.max(0, 3 - w.length) });
        return (0, n.jsxs)(n.Fragment, {
          children: [
            v,
            (0, n.jsx)("div", {
              className: l || A().MainMediaCtn,
              children: Boolean(t && (-1 === b || void 0 === b))
                ? (0, n.jsx)(n.Fragment, { children: t })
                : (0, n.jsx)(n.Fragment, { children: void 0 !== b && k[b] }),
            }),
            Boolean(w.length > 0) &&
              (0, n.jsxs)("div", {
                className: A().ScreenshotThumbnailRow,
                onMouseLeave: () => f(-1),
                children: [
                  w,
                  S.map((e, t) =>
                    (0, n.jsx)(
                      "div",
                      { className: A().ThumbnailCtn },
                      `app_${(0, c.ER)(s)}_${t}`,
                    ),
                  ),
                ],
              }),
          ],
        });
      }
      function P(e) {
        const {
          ref: t,
          name: s,
          trailer: a,
          bControls: r,
          bPlayVideo: i,
          fnTogglePlayTrailer: l,
          startTime: c,
        } = e;
        if (
          ((0, d.useEffect)(() => {
            const e = t?.current;
            if (null != c && c > 0 && e) {
              const t = () => {
                e.currentTime = c || 0;
              };
              return (
                e.addEventListener("loadedmetadata", t),
                () => {
                  e.removeEventListener("loadedmetadata", t);
                }
              );
            }
          }, [t, c]),
          !a)
        )
          return null;
        let u = (0, p.A)(A().VideoLargeContainer, i && A().videoPlaying);
        return (0, n.jsxs)("div", {
          className: u,
          onClick: l,
          children: [
            (0, n.jsx)(j.hj, {
              name: s,
              trailerCategory: a.trailer_category,
              trailerDisplay: j.g,
              mouseOver: !1,
            }),
            Boolean(i && a.microtrailer) &&
              (0, n.jsx)("video", {
                className: A().VideoLarge,
                ref: t,
                controls: r,
                autoPlay: !0,
                loop: !0,
                muted: !0,
                poster: null != c && c > 0 ? void 0 : a.screenshot_full,
                children: a.microtrailer?.map((e) =>
                  h.TS.IN_CLIENT && "video/mp4" == e.type
                    ? null
                    : (0, n.jsx)(
                        "source",
                        { src: (0, o.M4)(a, e.filename || ""), type: e.type },
                        e.filename,
                      ),
                ),
              }),
            r &&
              (0, n.jsx)("div", {
                onClick: l,
                children: (0, n.jsx)(_.sED, {}),
              }),
          ],
        });
      }
      function E(e) {
        const { id: t, bShowModal: s, trailerBaseID: a, hideModal: r } = e,
          { data: i } = (0, l.J$)(t),
          c = (0, o.kB)(t),
          u = (0, d.useMemo)(() => {
            if (c && 0 != c.length) {
              if (a) {
                const e = c.find((e) => e.trailer_base_id == a);
                if (e) return e;
              }
              return c[0];
            }
          }, [c, a]),
          g = d.useId(),
          _ = d.useId();
        if (!u || !u.adaptive_trailers) return null;
        const { rgDashTrailers: p, rgHlsTrailers: h } = (0, o.hg)(u);
        if (0 == p?.length) return null;
        let j = (0, o.Wv)(u);
        return (0, n.jsx)(f.EN, {
          active: s,
          children: (0, n.jsxs)(f.eV, {
            "aria-labelledby": (0, m.q)(g, _),
            bAllowFullSize: !0,
            bOKDisabled: !0,
            closeModal: r,
            children: [
              (0, n.jsx)("div", {
                className: A().VideoPopupContainers,
                children: (0, n.jsx)(x.P, {
                  dashManifests: p || [],
                  hlsManifest: (h.length > 0 && h?.[0]) || "",
                  screenshot: (0, o.hl)(u),
                  altText: u.trailer_name,
                  muteWhenAutoplayBlocked: !0,
                  captionManifest: j,
                }),
              }),
              (0, n.jsx)("div", {
                id: g,
                style: { display: "none" },
                children: i?.name || "",
              }),
              (0, n.jsx)("div", {
                id: _,
                style: { display: "none" },
                children: u.trailer_name,
              }),
            ],
          }),
        });
      }
      function F(e) {
        const {
          trailer: t,
          fnTogglePlayTrailer: s,
          bPlayVideo: a,
          onMouseEnter: r,
          onMouseLeave: i,
        } = e;
        return (0, n.jsxs)("div", {
          className: (0, p.A)({
            [A().VideoThumbnail]: !a,
            [A().videoPlaying]: a,
            [A().ThumbnailCtn]: !0,
          }),
          onClick: s,
          onMouseEnter: r,
          onMouseLeave: i,
          children: [
            (0, n.jsx)("img", { src: (0, o.hl)(t) }),
            (0, n.jsx)("div", {
              className: A().VideoPlayButton,
              children: (0, n.jsx)(_.jGG, {}),
            }),
          ],
        });
      }
      function V(e) {
        const { id: t, fnOnClickButton: s, bUseAssetWithoutOverride: o } = e,
          [c, u] = (0, r.OP)(),
          { data: d } = (0, l.lv)(t, o),
          { data: m } = (0, l.J$)(t),
          { data: g } = (0, l.wl)(t),
          { data: _ } = (0, l.xz)(t);
        if (!d || !g || !m) return null;
        const h = (0, i.b0)(d, "main_capsule");
        return (0, n.jsxs)(a.Z, {
          focusable: !0,
          noFocusRing: !0,
          className: A().MainCapsuleWithHover,
          ...u,
          onActivate: s,
          children: [
            (0, n.jsx)("img", { className: A().MainCapsule, src: h }),
            (0, n.jsxs)("div", {
              className: A().AppDetails,
              children: [
                (0, n.jsx)("div", {
                  className: (0, p.A)(A().GameName),
                  children: m.name || "",
                }),
                (0, n.jsxs)("div", {
                  className: A().ShortDesc,
                  children: [g.short_description, " "],
                }),
                (0, n.jsx)(N.n, {
                  rgTagIDs: _ ? _.slice(0, 10).map((e) => e.tagid || 0) : [],
                  instanceNum: 0,
                  bLargeText: !0,
                  bHideTitle: !0,
                  bNoStoreLinks: !0,
                }),
                (0, n.jsx)(w, { id: t }),
              ],
            }),
          ],
        });
      }
    },
    78997: (e, t, s) => {
      "use strict";
      s.d(t, { NZ: () => c, g1: () => u, ho: () => l });
      var n = s(7850),
        a = s(90626),
        r = s(66703),
        i = s(78327);
      const o = a.createContext({ setLegalText: void 0 });
      function l() {
        return !!a.useContext(o).setLegalText;
      }
      function c() {
        return a.useContext(o).setLegalText || function (e) {};
      }
      function u(e) {
        const [t, s] = a.useState(),
          l = (0, r.Dp)("BrowserView.RegisterForMessageFromParent"),
          c = (0, i.Qn)(),
          u = a.useMemo(
            () => ({ setLegalText: l && !c ? s : void 0 }),
            [l, s, c],
          );
        return (0, n.jsxs)(o.Provider, {
          value: u,
          children: [l && (0, n.jsx)(d, { strLegalText: t }), e.children],
        });
      }
      function d(e) {
        const { strLegalText: t } = e,
          s = a.useRef(void 0);
        return (
          a.useEffect(() => {
            (s.current = t),
              SteamClient.BrowserView.PostMessageToParent(
                "MarketingMessageLegal",
                t || "",
              );
          }, [t]),
          a.useEffect(
            () =>
              SteamClient.BrowserView.RegisterForMessageFromParent((e) => {
                "MarketingMessageDialogReady" == e &&
                  SteamClient.BrowserView.PostMessageToParent(
                    "MarketingMessageLegal",
                    s.current,
                  );
              }).unregister,
            [],
          ),
          null
        );
      }
    },
    46773: (e, t, s) => {
      "use strict";
      s.d(t, { Y: () => r, m: () => a });
      var n = s(90626);
      function a(e) {
        const { title: t, bodyClassName: s, children: a } = e;
        return (
          n.useEffect(() => {
            const e = document.title;
            return (
              (document.title = t),
              () => {
                document.title = e;
              }
            );
          }, [t]),
          r(s),
          a
        );
      }
      function r(e) {
        n.useEffect(() => {
          if (!e) return;
          const t = [];
          for (const s of e.split(/ /))
            document.body.classList.contains(s) || t.push(s);
          return (
            document.body.classList.add(...t),
            () => document.body.classList.remove(...t)
          );
        }, [e]);
      }
    },
    25809: (e, t, s) => {
      "use strict";
      s.d(t, { l: () => a });
      var n = s(61859);
      function a(e, t = "#Played_", s = !1) {
        if (e >= 120) {
          let s = e / 60;
          s = Math.round(10 * s) / 10;
          let a = n.pf.GetPreferredLocales(),
            r = s.toLocaleString(a, {
              minimumFractionDigits: 0,
              maximumFractionDigits: 1,
            });
          return (0, n.we)(t + "Hours", r);
        }
        return s && 1 == e
          ? (0, n.we)(t + "Minute", e)
          : (0, n.we)(t + "Minutes", e);
      }
    },
    30700: (e, t, s) => {
      "use strict";
      s.d(t, { q: () => o });
      var n = s(90626),
        a = s(78327),
        r = s(84933);
      const i = 2e4;
      function o(e) {
        const t = (0, n.useRef)(!1),
          s = (0, n.useRef)(null),
          o = (0, n.useCallback)(() => {
            s.current = setTimeout(() => {
              e.current &&
                !e.current.paused &&
                (e.current.pause(),
                (t.current = !0),
                "dev" == a.TS.WEB_UNIVERSE &&
                  console.log(
                    "useVideoAutoPauseOnBlur: Video paused due to inactivity.",
                  ));
            }, i);
          }, [e]),
          l = (0, n.useCallback)(() => {
            s.current && (clearTimeout(s.current), (s.current = null)),
              e.current &&
                t.current &&
                (e.current.play(),
                (t.current = !1),
                "dev" == a.TS.WEB_UNIVERSE &&
                  console.log(
                    "useVideoAutoPauseOnBlur: Video resumed on focus.",
                  ));
          }, [e]);
        (0, r.l6)(window, "blur", o), (0, r.l6)(window, "focus", l);
      }
    },
    80620: (e, t, s) => {
      "use strict";
      s.d(t, { WN: () => v, lS: () => f, xf: () => j });
      var n = s(80902),
        a = s(56545),
        r = s(23809),
        i = s(17690),
        o = s(42457),
        l = s(30470),
        c = s(78327),
        u = s(56011),
        d = s(90626),
        m = s(60014),
        g = s(56186),
        _ = s(54926),
        p = s(45740),
        h = s(64726);
      function f(e) {
        const t = (0, r.KV)();
        let s = (0, n.I)({
          queryKey: ["useGamePlaytimeInfo", e],
          queryFn: async () =>
            (async function (e, t) {
              const s = a.w.Init(o.G9h);
              s.Body().set_steamid(l.iA.steamid),
                s.Body().set_appids_filter([e]),
                s.Body().set_include_played_free_games(!0),
                s.Body().set_language(l.TS.LANGUAGE);
              const n = await o.xtC.GetOwnedGames(t, s);
              if (n.Body().games().length > 0)
                return n.Body().games()[0].toObject();
              return {};
            })(e, t),
          enabled: Boolean(e && e != i.sc),
        });
        return s.isSuccess ? s.data : null;
      }
      function j(e) {
        return (0, c.Y2)() && e?.startsWith("https://store.steampowered.com/")
          ? e.replace("https://store.steampowered.com/", l.TS.STORE_BASE_URL)
          : e;
      }
      function v(e, t, s = !1) {
        let n,
          a = (0, r.KV)(),
          i = (0, _.J)(),
          o = (0, h.Ng)();
        (o = null !== o ? o + 1 : 0),
          t == p.cU.wY
            ? (n = "image")
            : t == p.cU.xe
              ? (n = "button")
              : t == p.cU.FQ
                ? (n = "dlc_capsule")
                : t == p.cU.vx
                  ? (n = "header_area")
                  : t == p.cU.C1
                    ? (n = "game_capsule")
                    : t == p.cU.vm && (n = "partner_event");
        let l = (0, m.aL)(j(e), n);
        return (
          s && (l = e),
          l.startsWith("steam://") || (l = `steam://openurl/${l}`),
          (0, d.useCallback)(
            (e) => {
              (0, g.AP)(a, i.id, o, i.GetTemplateTypeForReporting(), t),
                ((0, u.uX)(e).location.href = l);
            },
            [l, a, i, o, t],
          )
        );
      }
    },
    54926: (e, t, s) => {
      "use strict";
      s.d(t, { Q: () => De, J: () => Le });
      var n = s(7850),
        a = s(22837),
        r = s(90626),
        i = s(56186),
        o = s(78997),
        l = s(78327),
        c = s(69666),
        u = s(60014),
        d = s(80620),
        m = s(76217),
        g = s(38390),
        _ = s(58918),
        p = s(30470);
      function h(e, t, s, n) {
        if (!n || !n.path) return null;
        const a = s ? "?t=" + s : "";
        return n.path.startsWith("images")
          ? `${p.TS.MEDIA_CDN_URL}steam/marketing/${e}/${n.path}${a}`
          : `${p.TS.BASE_URL_SHARED_CDN}store_item_assets/mm/${e}/${t}/${n.path}${a}`;
      }
      var f = s(84933),
        j = s(30700);
      function v(e) {
        const { path: t, message: s, eLanguage: a, ...r } = e,
          i = s.GetTemplateVars()?.last_asset_mtime,
          o = h(s.id, a, i, { type: "file", path: t }),
          l = s.GetLocalizedAltText(a);
        return (0, n.jsx)("img", { alt: l, ...r, src: o });
      }
      function x(e) {
        const { message: t, mp4Path: s, webmPath: a, language: i, ...o } = e,
          l = t.GetTemplateVars()?.last_asset_mtime,
          c = h(t.id, i, l, { type: "file", path: a }),
          u = h(t.id, i, l, { type: "file", path: s }),
          d = (0, r.useRef)(null);
        (0, j.q)(d);
        const m = (0, r.useRef)(!1),
          g = (0, r.useCallback)(() => {
            d.current &&
              ("visible" === document.visibilityState
                ? m.current && (d.current.play(), (m.current = !1))
                : d.current.paused || (d.current.pause(), (m.current = !0)));
          }, []);
        return (
          (0, f.l6)(document, "visibilitychange", g),
          (0, r.useEffect)(() => g(), [g]),
          (0, n.jsxs)("video", {
            ...o,
            ref: d,
            children: [
              (0, n.jsx)("source", { src: c, type: "video/webm" }),
              (0, n.jsx)("source", { src: u, type: "video/mp4" }),
            ],
          })
        );
      }
      var y = s(32630),
        A = s(52038),
        T = s(61859),
        I = s(720),
        b = s.n(I),
        M = s(45699),
        k = s(81393),
        G = s(45740),
        w = s(68009),
        S = s(17376),
        C = s(65946),
        N = s(75279),
        B = s(84811),
        L = s(3740),
        D = s(8893),
        P = s(25809),
        E = s(22329);
      function F(e) {
        return (0, n.jsx)("div", {
          className: E.All,
          children: (0, n.jsx)("div", {
            className: E.MessageContent,
            children: e.children,
          }),
        });
      }
      function V(e) {
        return (0, n.jsx)("div", {
          className: E.MessageBody,
          children: e.children,
        });
      }
      function R(e) {
        const { isBackgroundBlur: t, bOverrideUseBackgroundImage: s } = e,
          a = Le(),
          r = (0, d.WN)(a.GetTemplateVars().linkurl, G.cU.wY),
          [i, o] = s ? a.GetTemplateBackgroundImage() : a.GetTemplateImage();
        return (0, n.jsx)(m.Z, {
          focusable: !0,
          noFocusRing: !0,
          className: (0, A.A)(E.MessageImage, t && E.IsBlur),
          onActivate: r,
          children: i && (0, n.jsx)(v, { message: a, path: i, eLanguage: o }),
        });
      }
      function O(e) {
        const { fnOnClick: t } = e,
          s = Le(),
          r = (0, a.sfN)(l.TS.LANGUAGE),
          [i, o] = s.GetTemplateMP4WithFallback(r),
          [c, u] = s.GetTemplateWebMWithFallback(r);
        return (
          (0, k.wT)(
            o == u,
            `GameAnimatedImageViaVideo mismatch fallback languages eLang ${r} mp4 ${o} webm ${u}`,
          ),
          (0, n.jsx)("div", {
            className: E.VideoImageContainer,
            children: (0, n.jsx)(m.Z, {
              focusable: !0,
              noFocusRing: !0,
              className: (0, A.A)(E.MessageImage),
              onActivate: t,
              children: (0, n.jsx)(x, {
                muted: !0,
                autoPlay: !0,
                controls: !1,
                loop: !0,
                mp4Path: i,
                message: s,
                webmPath: c,
                language: l.TS.IN_CLIENT ? u : o,
              }),
            }),
          })
        );
      }
      function U(e) {
        const { storeItem: t } = e,
          s = t.GetAssets().GetMainCapsuleURL();
        return s
          ? (0, n.jsx)("div", {
              className: (0, A.A)(E.MessageImage, E.GameImage, E.IsBlur),
              children: (0, n.jsx)("img", { src: s }),
            })
          : null;
      }
      function W(e) {
        const { storeItem: t, bPreview: s, bPreferAssetWithoutOverride: a } = e,
          i =
            ((0, r.useMemo)(() => ({ id: t.GetID(), type: "game" }), [t]),
            (0, l.Qn)()),
          o = (0, N.DJ)(t),
          { bIsOwned: c } = (0, _.ZJ)(o),
          u = (0, S.bB)(t.GetAppID()),
          m = (0, w.Fh)(t.GetAppID()),
          g = (0, d.lS)(t.GetAppID()),
          p = (0, r.useMemo)(() => {
            if (s && (!g?.playtime_forever || !g?.rtime_last_played)) {
              return {
                playtime_forever: 300,
                rtime_last_played: Math.floor(Date.now() / 1e3) - 432e3,
              };
            }
            return g;
          }, [g, s]);
        let h = "steam://openurl/" + t.GetStorePageURL();
        c &&
          (h = i
            ? `steam://open/games/details/${t.GetID()}`
            : `steam://nav/games/details/${t.GetID()}`);
        const f = (0, d.WN)(h, G.cU.vx, c);
        return (0, n.jsxs)("div", {
          className: E.BaseCtn,
          children: [
            (0, n.jsx)(M.Ii, {
              className: E.CapsuleCtn,
              onClick: f,
              children: (0, n.jsx)(D.G, {
                id: o,
                bPreferLibrary: !0,
                bPreferAssetWithoutOverride: a,
              }),
            }),
            (0, n.jsxs)("div", {
              className: E.DescCtn,
              children: [
                (0, n.jsx)("div", {
                  className: E.GameNameCtn,
                  children: (0, n.jsx)(M.Ii, {
                    className: E.GameName,
                    onClick: f,
                    children: t.GetName(),
                  }),
                }),
                (0, n.jsxs)("div", {
                  className: E.LibraryDetails,
                  children: [
                    (0, n.jsx)(M.Ii, {
                      onClick: f,
                      className: (0, A.A)(E.Button, E.ViewInLibrary),
                      children: (0, T.we)(
                        c
                          ? "#EventDisplay_ViewInLibrary"
                          : "#EventDisplay_ViewStorePage",
                      ),
                    }),
                    !c &&
                      (0, n.jsx)(n.Fragment, {
                        children: (0, n.jsxs)("div", {
                          className: E.PlayDetailCtn,
                          children: [
                            u &&
                              (0, n.jsx)("span", {
                                children: (0, T.we)("#EventDisplay_OnWishlist"),
                              }),
                            !u &&
                              m &&
                              (0, n.jsx)("span", {
                                children: (0, T.we)("#EventDisplay_Follow"),
                              }),
                          ],
                        }),
                      }),
                    c &&
                      (0, n.jsxs)(n.Fragment, {
                        children: [
                          Boolean(p?.rtime_last_played) &&
                            (0, n.jsxs)("div", {
                              className: E.PlayDetailCtn,
                              children: [
                                (0, n.jsx)("span", {
                                  children: (0, T.we)(
                                    "#MarketingMessages_DLC_lastplayed",
                                  ),
                                }),
                                (0, T._l)(p.rtime_last_played),
                              ],
                            }),
                          Boolean(p?.playtime_forever) &&
                            (0, n.jsxs)("div", {
                              className: E.PlayDetailCtn,
                              children: [
                                (0, n.jsx)("span", {
                                  children: (0, T.we)(
                                    "#MarketingMessages_DLC_hours",
                                  ),
                                }),
                                (0, P.l)(p.playtime_forever),
                              ],
                            }),
                        ],
                      }),
                  ],
                }),
              ],
            }),
          ],
        });
      }
      function H(e) {
        const t = Le(),
          s = (0, d.WN)(t.GetTemplateVars().linkurl, G.cU.xe);
        return (0, n.jsx)(z, { bHidePrice: e.bHidePrice, fnOnClickButton: s });
      }
      function z(e) {
        const { bHidePrice: t, fnOnClickButton: s } = e,
          a = Le(),
          [r, i] = (0, C.q3)(() => [
            a.GetTemplateVars().button_text_custom ||
              a.GetTemplateVars().button_text,
            Boolean(a.GetTemplateVars().hide_price),
          ]),
          o = Boolean(t || i);
        return (0, n.jsxs)("div", {
          className: (0, A.A)(E.MessageFooter, !1),
          children: [
            (0, n.jsxs)("div", {
              className: E.ButtonAndPriceCtn,
              children: [
                (0, n.jsx)(m.Z, {
                  focusable: !0,
                  noFocusRing: !0,
                  className: E.Btn,
                  onActivate: s,
                  children: r,
                }),
                Boolean(!o) &&
                  (0, n.jsx)(B.tH, { children: (0, n.jsx)(Q, {}) }),
              ],
            }),
            (0, n.jsx)(K, {}),
          ],
        });
      }
      function Q() {
        const e = Le().associated_item,
          t = (0, N.DJ)(e);
        return e &&
          e.GetBestPurchaseOption() &&
          e.GetBestPurchaseOption().formatted_final_price
          ? (0, n.jsx)("div", {
              className: E.MessagePriceCtn,
              children: (0, n.jsx)(L.N, { id: t, bHidePrePurchase: !0 }),
            })
          : (0, n.jsx)("div", { className: E.NoPrice });
      }
      function K(e) {
        const t = Le();
        return (0, o.ho)()
          ? null
          : (0, n.jsx)("div", {
              className: E.Legal,
              dangerouslySetInnerHTML: { __html: t.GetLegalHTML() },
            });
      }
      var q = s(79613);
      function Y(e) {
        const { bPreview: t, bUseAnimated: s } = e,
          { message: a, eventModel: r } = (function (e) {
            const t = Le();
            let s = t.GetTemplateVars().update_event_clan_accountid,
              n = t.GetTemplateVars().update_event_gid;
            const {
              eventModel: a,
              bLoading: r,
              sErrorMessage: i,
            } = (0, g.B9)(s, n, e);
            return { message: t, eventModel: a };
          })(t),
          i = a.associated_item.GetStorePageURL(),
          o = X(r, i, G.cU.wY),
          l = X(r, i, G.cU.xe);
        return (0, n.jsx)(y.Ay, {
          submethod: "partner_event",
          children: (0, n.jsxs)(F, {
            children: [
              (0, n.jsx)(U, { storeItem: a.associated_item }),
              (0, n.jsxs)(V, {
                children: [
                  (0, n.jsx)(W, {
                    storeItem: a.associated_item,
                    bPreview: t,
                    bPreferAssetWithoutOverride: !1,
                  }),
                  (0, n.jsx)(Z, {
                    message: a,
                    eventModel: r,
                    fnOnClickButton: o,
                    bUseAnimated: s,
                  }),
                ],
              }),
              (0, n.jsx)(z, { bHidePrice: !0, fnOnClickButton: l }),
            ],
          }),
        });
      }
      function Z(e) {
        const {
            message: t,
            fnOnClickButton: s,
            eventModel: r,
            bUseAnimated: i,
          } = e,
          [o, l] = t.GetTemplateImage(),
          c = (0, a.sfN)(p.TS.LANGUAGE);
        return (0, n.jsxs)("div", {
          className: b().UpdateEventCtn,
          children: [
            (0, n.jsx)("div", {
              className: b().EventType,
              children: (0, T.we)("#MarketingMessages_MajorUpdate"),
            }),
            (0, n.jsx)("div", {
              className: b().EventTitle,
              children: r?.GetNameWithFallback(c),
            }),
            (0, n.jsxs)(m.Z, {
              focusable: !0,
              noFocusRing: !0,
              className: (0, A.A)(b().EventImage),
              onActivate: s,
              children: [
                o && !i && (0, n.jsx)(v, { message: t, path: o, eLanguage: l }),
                i && (0, n.jsx)(O, { fnOnClick: s }),
              ],
            }),
          ],
        });
      }
      function X(e, t, s) {
        let n = (0, _.S6)(e?.appid),
          a = (0, u.aL)((0, d.xf)(t), "partner_event");
        if (e?.BIsVisibleEvent() && n && e.BIsValidForRealm(p.TS.EREALM)) {
          const s = (0, q.MP)();
          p.TS.IN_CLIENT && (s > 1726604483 || 0 == s)
            ? (t = `steam://open/library/event/${e.appid}|${e.GID}`)
            : ((t = `${p.TS.STORE_BASE_URL}news/app/${e.appid}?emclan=${e.clanSteamID.ConvertTo64BitString()}&emgid=${e.GID}`),
              p.TS.IN_CLIENT && (t = `steam://openurl/${t}`));
        } else
          (t = a),
            e && (t += `${a.includes("?") ? "&" : "?"}emgid=${e.GID}`),
            p.TS.IN_CLIENT && (t = `steam://openurl/${t}`);
        return (0, d.WN)(t, s, !0);
      }
      var $ = s(41735),
        J = s.n($),
        ee = s(80902),
        te = s(37085),
        se = s(17690);
      function ne(e) {
        const {
          isLoading: t,
          isError: s,
          data: n,
        } = (0, ee.I)({
          queryKey: ["useDLCHubCount", e],
          queryFn: async () =>
            (async function (e) {
              const t = `${p.TS.STORE_BASE_URL}dlc/${e}/ajaxgetdlccount`,
                s = { origin: self.origin },
                n = await J().get(t, { params: s, withCredentials: !1 });
              if (200 !== n.status || n.data.success !== te.R)
                throw new Error(
                  `FetchDLCCount failed: status == ${n.status}, eresult == ${n.data?.success}, err_msg == ${n.data?.err_msg}`,
                );
              return n.data.count;
            })(e),
          enabled: Boolean(e && e != se.sc),
        });
        return t || s ? null : n;
      }
      var ae = s(62792),
        re = s(55263),
        ie = s(41399),
        oe = s(22687),
        le = s(16205);
      const ce = { include_assets: !0 };
      function ue(e) {
        const { bPreview: t } = e,
          s = Le(),
          [a, i] = (0, r.useMemo)(
            () => [
              s.GetDLCAppIDs(),
              s.GetDLCAppIDs().map((e) => ({ id: e, type: "game" })),
            ],
            [s],
          );
        (0, re.zX)(a, ce);
        const o = ne(s?.associated_item?.GetAppID());
        return (
          (0, r.useEffect)(() => {
            if (s) {
              const e = Boolean(o)
                ? (0, T.Yp)("#MarketingMessages_See_Count_Items", o)
                : (0, T.we)("#MarketingMessages_See_All_Items");
              s.OverrideCustomText(e),
                s.OverrideURL(
                  `${p.TS.STORE_BASE_URL}dlc/${s.associated_item.GetAppID()}`,
                );
            }
          }, [s, o]),
          (0, n.jsx)(y.Ay, {
            submethod: "dlc_override",
            children: (0, n.jsxs)(F, {
              children: [
                (0, n.jsx)(U, { storeItem: s.associated_item }),
                (0, n.jsxs)(V, {
                  children: [
                    (0, n.jsx)(W, {
                      storeItem: s.associated_item,
                      bPreview: t,
                      bPreferAssetWithoutOverride: !1,
                    }),
                    (0, n.jsx)(me, {
                      rgDLCSaleCapsules: i,
                      messageType: s.GetType(),
                    }),
                  ],
                }),
                (0, n.jsx)(H, { bHidePrice: !0 }),
              ],
            }),
          })
        );
      }
      function de(e) {
        const { messageType: t, itemCount: s } = e,
          a = i.rT.GetTypeAsLocalizedString(t);
        return (0, n.jsxs)(n.Fragment, {
          children: [
            Boolean(a) &&
              (0, n.jsx)("div", { className: le.Type, children: a }),
            1 == s
              ? (0, n.jsx)("div", {
                  className: le.DealDesc,
                  children: (0, T.we)("#MarketingMessages_DLC_desc_singular"),
                })
              : (0, n.jsx)("div", {
                  className: le.DealDesc,
                  children: (0, T.we)("#MarketingMessages_DLC_desc"),
                }),
          ],
        });
      }
      function me(e) {
        const { rgDLCSaleCapsules: t, messageType: s } = e;
        return t.length >= 4
          ? (0, n.jsxs)("div", {
              className: le.DlcCtn,
              children: [
                (0, n.jsx)(de, { messageType: s }),
                (0, n.jsx)(ge, { first: t[0], second: t[1] }),
                (0, n.jsx)(ge, { first: t[2], second: t[3] }),
              ],
            })
          : t.length >= 3
            ? (0, n.jsxs)("div", {
                className: le.DlcCtn,
                children: [
                  (0, n.jsx)(de, { messageType: s }),
                  (0, n.jsx)("div", {
                    className: le.OneItemRow,
                    children: (0, n.jsx)(he, { id: t[0].id, type: t[0].type }),
                  }),
                  (0, n.jsx)(ge, { first: t[1], second: t[2] }),
                ],
              })
            : t.length >= 2
              ? (0, n.jsxs)("div", {
                  className: le.DlcCtn,
                  children: [
                    (0, n.jsx)(de, { messageType: s }),
                    (0, n.jsx)("div", {
                      className: le.OneItemRow,
                      children: (0, n.jsx)(he, {
                        id: t[0].id,
                        type: t[0].type,
                      }),
                    }),
                    (0, n.jsx)("div", {
                      className: le.OneItemRow,
                      children: (0, n.jsx)(he, {
                        id: t[1].id,
                        type: t[1].type,
                      }),
                    }),
                  ],
                })
              : t.length >= 1
                ? (0, n.jsxs)("div", {
                    className: le.DlcCtn,
                    children: [
                      (0, n.jsx)(de, { messageType: s, itemCount: t.length }),
                      (0, n.jsx)("div", {
                        className: le.OneBigItem,
                        children: (0, n.jsx)(pe, {
                          capsule: t[0],
                          imageType: "main",
                        }),
                      }),
                    ],
                  })
                : null;
      }
      function ge(e) {
        const { first: t, second: s } = e;
        return (0, n.jsxs)("div", {
          className: le.TwoCapsuleRow,
          children: [
            (0, n.jsx)("div", {
              className: le.DlcCtn,
              children: (0, n.jsx)(pe, { capsule: t, imageType: "header" }),
            }),
            (0, n.jsx)("div", {
              className: le.DlcCtn,
              children: (0, n.jsx)(pe, { capsule: s, imageType: "header" }),
            }),
          ],
        });
      }
      function _e(e, t) {
        const [s] = (0, re.G6)(e, (0, ae.SW)(t), { include_platforms: !0 });
        return (0, d.WN)(
          s ? s.GetStorePageURL() : p.TS.STORE_BASE_URL,
          G.cU.FQ,
        );
      }
      function pe(e) {
        const { capsule: t, imageType: s } = e;
        return (
          (t.overrideNavigation = _e(t.id, t.type)),
          (0, n.jsx)(oe.W, {
            capsule: t,
            imageType: s,
            bShowParentApp: !1,
            bHideStoreHover: !0,
            bPreferAssetWithoutOverride: !1,
          })
        );
      }
      function he(e) {
        const { id: t, type: s } = e,
          a = _e(t, s);
        return (0, n.jsx)(ie.p, {
          id: t,
          type: s,
          fnOnClickOverride: a,
          bIsMarketingMessage: !0,
          bPreferAssetWithoutOverride: !1,
        });
      }
      var fe = s(12155),
        je = s(61336),
        ve = s(31343);
      function xe(e) {
        const t = Le(),
          [s, a] = (0, r.useState)(() => t.GetFeaturedVideoAutoPlay()),
          i = (0, r.useRef)(null);
        (0, j.q)(i);
        const o = (0, d.WN)(t.GetTemplateVars().linkurl, G.cU.wY),
          c = (0, l.Qn)();
        return (0, n.jsx)(m.Z, {
          focusable: !0,
          noFocusRing: !0,
          onActivate: (e) =>
            !c && t.GetFeaturedVideoAutoPlay() ? o(e) : a(!0),
          className: ve.PosterCtn,
          children: s
            ? (0, n.jsxs)("video", {
                controls: !t.GetFeaturedVideoLoop(),
                ref: i,
                muted: !0,
                autoPlay: !0,
                className: ve.Video,
                loop: t.GetFeaturedVideoLoop(),
                crossOrigin: "dev" == p.TS.WEB_UNIVERSE ? void 0 : "anonymous",
                children: [
                  (0, n.jsx)("source", {
                    src: (0, je.L$)(t.GetFeaturedVideoWebMURL()),
                    type: "video/webm",
                  }),
                  Boolean(!p.TS.IN_CLIENT) &&
                    (0, n.jsx)("source", {
                      src: (0, je.L$)(t.GetFeaturedVideoMP4URL()),
                      type: "video/mp4",
                    }),
                  (0, n.jsx)(Ae, { message: t }),
                ],
              })
            : (0, n.jsx)(ye, { message: t }),
        });
      }
      function ye(e) {
        const { message: t } = e,
          s = t.GetTemplateVars()?.last_asset_mtime,
          [a, r] = t.GetPosterImage(),
          i = h(t.id, r, s, { type: "file", path: a });
        return (0, n.jsxs)(n.Fragment, {
          children: [
            (0, n.jsx)("img", { src: i, className: ve.Poster }),
            (0, n.jsx)(fe.IOc, {}),
          ],
        });
      }
      function Ae(e) {
        const { message: t } = e,
          s = (0, r.useMemo)(() => {
            const e = t.GetSubtitleObj(),
              s = t.GetTemplateVars()?.last_asset_mtime,
              r = new Array();
            for (let i = a.Bhc; i < a.bP9; ++i) {
              if (!T.A0.IsELanguageValidInRealm(i, p.TS.EREALM)) continue;
              const o = (0, a.LgB)(i);
              if (e && e[o]) {
                const l = e[o].path,
                  c = h(t.id, i, s, { type: "file", path: l });
                r.push(
                  (0, n.jsx)(
                    "track",
                    {
                      src: c,
                      kind: "subtitles",
                      srcLang: (0, a.wwZ)(i),
                      default: p.TS.LANGUAGE == o,
                      label: (0, T.we)("#language_selection_" + (0, a.LgB)(i)),
                    },
                    t.id + " " + i,
                  ),
                );
              }
            }
            return r;
          }, [t]);
        return (0, n.jsx)(n.Fragment, { children: s });
      }
      function Te(e) {
        const { bLowBandwidthMode: t } = e,
          s = Le(),
          a = !t && "featured_video" === s.GetTemplateVars().custom_display;
        return (0, n.jsx)(y.Ay, {
          children: (0, n.jsxs)(F, {
            children: [
              (0, n.jsx)(R, {
                isBackgroundBlur: !0,
                bOverrideUseBackgroundImage: a,
              }),
              (0, n.jsxs)(V, {
                children: [
                  (0, n.jsx)(R, { bOverrideUseBackgroundImage: a }),
                  Boolean(a) && (0, n.jsx)(xe, {}),
                ],
              }),
              (0, n.jsx)(H, {}),
            ],
          }),
        });
      }
      function Ie(e) {
        const t = Le(),
          s = (0, d.WN)(t.GetTemplateVars().linkurl, G.cU.vm);
        return (0, n.jsx)(y.Ay, {
          children: (0, n.jsxs)(F, {
            children: [
              (0, n.jsx)(R, { isBackgroundBlur: !0 }),
              (0, n.jsx)(V, { children: (0, n.jsx)(O, { fnOnClick: s }) }),
              (0, n.jsx)(H, {}),
            ],
          }),
        });
      }
      var be = s(87937),
        Me = s(2921),
        ke = s(70300),
        Ge = s(91675),
        we = s(92609),
        Se = s(95034);
      function Ce(e) {
        const { bPreview: t, bLowBandwidthMode: s } = e,
          a = Le(),
          r = (0, d.WN)(a.GetTemplateVars().linkurl, G.cU.C1),
          o = a.associated_item,
          l = (0, N.DJ)(o);
        return (0, n.jsx)(y.Ay, {
          submethod: "mm-auto-render",
          children: (0, n.jsxs)(F, {
            children: [
              (0, n.jsx)(U, { storeItem: o }),
              (0, n.jsx)(V, {
                children: (0, n.jsxs)("div", {
                  className: we.AutoRenderContents,
                  children: [
                    (0, n.jsxs)("div", {
                      className: we.TitleContainer,
                      children: [
                        (0, n.jsx)("div", {
                          className: we.TypeTitle,
                          children: i.rT.GetTypeAsLocalizedString(a.GetType()),
                        }),
                        (0, n.jsx)(Ne, { message: a, storeItem: o }),
                      ],
                    }),
                    (0, n.jsx)(ke.oj, {
                      appid: o.GetAppID(),
                      children: (0, n.jsx)(Me.qP, {
                        id: l,
                        fnOnClickButton: r,
                        bLowBandwidthMode: s,
                        bUseAssetWithoutOverride:
                          a.GetAutoRenderWithoutAssetOverrides(),
                      }),
                    }),
                  ],
                }),
              }),
              (0, n.jsx)(z, {
                bHidePrice: a.GetTemplateVars().hide_price,
                fnOnClickButton: r,
              }),
            ],
          }),
        });
      }
      function Ne(e) {
        const { message: t, storeItem: s } = e,
          [a] = (0, Se.QD)("timezone"),
          [r] = (0, Se.QD)("locale");
        if (t.GetType() == G.D4.SK) {
          if (s?.GetFreeWeekendPlayTextOverride())
            return (0, n.jsx)("div", {
              className: we.TypeSubTitle,
              children: s?.GetFreeWeekendPlayTextOverride(),
            });
          if (s?.GetFreeWeekendEnd()) {
            r && T.pf.SetPreferredLocales([r]);
            const e = a || Intl.DateTimeFormat().resolvedOptions().timeZone,
              t = s.GetFreeWeekendEnd(),
              i = be.unix(t).tz(e),
              o = i.format("z"),
              l = o.match(/^-?\d/)
                ? `UTC${be.unix(t).tz(e).format("Z").replace(":00", "")}`
                : o,
              c = (0, Ge.P0)(i.unix(), !1, l, a);
            return (0, n.jsx)("div", {
              className: we.TypeSubTitle,
              children: (0, T.we)("#msg_free_play_until", c),
            });
          }
          return (0, n.jsx)("div", {
            className: we.TypeSubTitle,
            children: (0, T.we)("#msg_free_play_weekend"),
          });
        }
        const i = t.GetTemplateVars()?.autorender_subtitle_token;
        return i
          ? (0, n.jsx)("div", {
              className: we.TypeSubTitle,
              children: (0, T.we)(i),
            })
          : null;
      }
      const Be = r.createContext(null);
      function Le() {
        return r.useContext(Be);
      }
      function De(e) {
        const { message: t, preview: s } = e,
          a = !1 !== e.active,
          i = (0, o.NZ)();
        return (
          r.useEffect(() => {
            a && i(t.GetLegalHTML());
          }, [a, t, i]),
          (0, n.jsx)(Be.Provider, {
            value: t,
            children: (0, n.jsx)(r.Suspense, {
              fallback: null,
              children: (0, n.jsx)(Ee, { message: t, active: a, preview: s }),
            }),
          })
        );
      }
      const Pe = r.lazy(() => s.e(8287).then(s.bind(s, 79194)));
      function Ee(e) {
        const { message: t, active: s, preview: r } = e,
          o = (0, a.sfN)(l.TS.LANGUAGE),
          { bLowBandwidthMode: u } = (0, c.ri)();
        if (
          (0, i.$I)(t.GetTemplateVars().custom_display || "") &&
          l.iA.logged_in
        ) {
          const e = Number((0, i.fL)(t.GetTemplateVars().custom_display || ""));
          return isNaN(e) ? null : (0, n.jsx)(Pe, { active: s, year: e });
        }
        switch (t.GetTemplateVars().custom_display) {
          case "dlc_override":
            return (0, n.jsx)(ue, { bPreview: r });
          case "partner_event":
            return (0, n.jsx)(Y, {
              bPreview: r,
              bUseAnimated: (0, c.vn)(t, o, u),
            });
          case "mm_auto_render":
            return (0, n.jsx)(Ce, { bPreview: r, bLowBandwidthMode: u });
        }
        return "image" === t.GetTemplateType()
          ? (0, c.vn)(t, o, u)
            ? (0, n.jsx)(Ie, {})
            : (0, n.jsx)(Te, { bLowBandwidthMode: u })
          : null;
      }
    },
    64726: (e, t, s) => {
      "use strict";
      s.d(t, { Mf: () => x, Ng: () => v, eI: () => I });
      var n = s(7850),
        a = s(56186),
        r = s(90626),
        i = s(92757),
        o = s(56283),
        l = s(61859),
        c = s(69666),
        u = s(84121),
        d = s(12155),
        m = s(52038),
        g = s(90740),
        _ = s(46773),
        p = s(22797),
        h = s(54926);
      const f = 8,
        j = r.createContext(null);
      function v() {
        return r.useContext(j);
      }
      function x(e) {
        const { MarketingMessagesStore: t } = e,
          s = (0, c.ri)(),
          { rgMessages: o, isError: d } = (0, a.XW)(t, s),
          [m, g] = r.useState(!1),
          [p, h] = r.useState(0);
        (0, _.Y)(u.MarketingMessagePage);
        const j = (0, i.W6)();
        if (
          (r.useEffect(() => {
            !o ||
              o.length ||
              d ||
              (s.bIncludeSeenMessages
                ? g(!0)
                : j.replace({
                    ...j.location,
                    search: (0, c.GY)({ ...s, bIncludeSeenMessages: !0 }),
                  }));
          }, [o, s, j, d]),
          d)
        )
          return (0, n.jsx)(b, {
            children: (0, l.we)("#Error_ErrorCommunicatingWithNetwork"),
          });
        if (m)
          return (0, n.jsx)(b, {
            children: (0, l.we)("#MarketingMessages_NoneAvailable"),
          });
        const v = s.bIncludeSeenMessages ? o : o?.slice(0, f);
        return (0, n.jsxs)("div", {
          className: u.MessageListPage,
          children: [
            (0, n.jsx)("div", {
              className: u.MessageListScroll,
              children: (0, n.jsx)(y, {
                MarketingMessagesStore: t,
                rgMessages: v,
                iActiveMessage: p,
              }),
            }),
            (0, n.jsx)(A, { cMessages: v?.length, iMessage: p, setMessage: h }),
          ],
        });
      }
      function y(e) {
        const {
          MarketingMessagesStore: t,
          rgMessages: s,
          iActiveMessage: a,
        } = e;
        return s
          ? (0, n.jsx)("div", {
              className: u.MessageListContainer,
              children: s?.map((e, s) =>
                (0, n.jsx)(
                  M,
                  {
                    displayIndex: s,
                    message: e,
                    MarketingMessagesStore: t,
                    active: s == a,
                    next: s == a + 1 || s == a - 1,
                  },
                  e.id,
                ),
              ),
            })
          : (0, n.jsx)("div", {
              className: (0, m.A)(u.MessageListContainer, u.Loading),
              children: (0, n.jsx)(p.t, {
                size: "xxlarge",
                msDelayAppear: 500,
              }),
            });
      }
      function A(e) {
        const { cMessages: t, iMessage: s, setMessage: a } = e,
          i = r.useCallback(() => a(s - 1), [a, s]),
          o = r.useCallback(() => a(s + 1), [a, s]),
          l = [];
        for (let e = 0; e < t; e++)
          l.push(
            (0, n.jsx)(T, { active: e == s, iMessage: e, setMessage: a }, e),
          );
        const c = s > 0 ? i : void 0,
          g = s < t - 1 ? o : void 0;
        return (0, n.jsxs)(n.Fragment, {
          children: [
            (0, n.jsx)("div", { className: u.CarouselSpacer }),
            (0, n.jsx)("div", {
              className: u.CarouselBar,
              children: (0, n.jsxs)("div", {
                className: u.Content,
                children: [
                  (0, n.jsx)("div", {
                    className: (0, m.A)(u.LeftArrow, c && u.Active),
                    onClick: c,
                    children: (0, n.jsx)(d.l8x, { angle: 180 }),
                  }),
                  (0, n.jsx)("div", { className: u.Spacer }),
                  (0, n.jsx)("div", {
                    className: u.PipContainer,
                    children: (0, n.jsx)("div", {
                      className: u.Pips,
                      children: l,
                    }),
                  }),
                  (0, n.jsx)("div", { className: u.Spacer }),
                  (0, n.jsx)("div", {
                    className: (0, m.A)(u.LeftArrow, g && u.Active),
                    onClick: g,
                    children: (0, n.jsx)(d.l8x, { angle: 0 }),
                  }),
                ],
              }),
            }),
          ],
        });
      }
      function T(e) {
        const { active: t, iMessage: s, setMessage: a } = e,
          i = r.useCallback(() => a(s), [a, s]);
        return (0, n.jsx)("div", {
          className: (0, m.A)(u.Pip, t && u.Active),
          onClick: i,
        });
      }
      function I(e) {
        const { MarketingMessagesStore: t, preview: s } = e,
          r = (0, i.W5)(),
          { message: o, isError: c } = (0, a.dr)(t, r.params.messageid, s);
        return (
          (0, _.Y)(u.MarketingMessagePage),
          c
            ? (0, n.jsx)(b, {
                children: (0, l.we)("#Error_ErrorCommunicatingWithNetwork"),
              })
            : r.params.messageid
              ? o
                ? (0, n.jsx)(h.Q, { message: o, preview: s })
                : null
              : (0, n.jsx)(b, {
                  children: (0, l.we)("#MarketingMessages_NoneAvailable"),
                })
        );
      }
      function b(e) {
        return (0, n.jsxs)(o.UC, {
          style: { maxWidth: "400px", margin: "0 auto" },
          children: [
            (0, n.jsxs)(o.Y9, { children: [(0, l.we)("#Error_Generic"), " "] }),
            (0, n.jsx)(o.nB, { children: e.children }),
          ],
        });
      }
      function M(e) {
        const {
            message: t,
            MarketingMessagesStore: s,
            active: a,
            next: i,
            displayIndex: o,
          } = e,
          l = r.useRef(void 0),
          c = r.useRef(a || i);
        if (
          ((function (e, t, s) {
            r.useEffect(() => {
              t &&
                e.MarkMessageSeen(t.id, s + 1, t.GetTemplateTypeForReporting());
            }, [t, e, s]);
          })(s, a ? t : null, o),
          (a || i) && (c.current = !0),
          !c.current)
        )
          return null;
        let d = {
          enter: u.Enter,
          enterActive: u.EnterActive,
          enterDone: u.EnterDone,
          exit: u.Exit,
          exitActive: u.ExitActive,
          exitDone: u.ExitDone,
        };
        return (0, n.jsx)(g.A, {
          in: a,
          nodeRef: l,
          classNames: d,
          timeout: 300,
          mountOnEnter: !i,
          unmountOnExit: !i,
          children: (0, n.jsx)(j.Provider, {
            value: o,
            children: (0, n.jsx)("div", {
              className: (0, m.A)(u.MessageWrapper, a && u.Active),
              ref: l,
              children: (0, n.jsx)(h.Q, { message: t, active: a }),
            }),
          }),
        });
      }
    },
    69666: (e, t, s) => {
      "use strict";
      s.d(t, { GY: () => o, ri: () => i, vn: () => l });
      var n = s(90626),
        a = s(92757),
        r = s(61859);
      function i() {
        const e = (0, a.zy)();
        return n.useMemo(() => {
          const t = new URLSearchParams(e.search);
          return {
            bIncludeSeenMessages: !!t.get("include_seen"),
            nClientPackageVersion: parseInt(
              t.get("client_package_version") || "0",
            ),
            eOSType: parseInt(t.get("os_type") || "0"),
            bLowBandwidthMode: !!t.get("low_bandwidth"),
          };
        }, [e.search]);
      }
      function o(e) {
        const t = new URLSearchParams();
        return (
          e.bIncludeSeenMessages && t.append("include_seen", "1"),
          e.nClientPackageVersion &&
            t.append(
              "client_package_version",
              e.nClientPackageVersion.toString(),
            ),
          e.eOSType && t.append("os_type", e.eOSType.toString()),
          e.bLowBandwidthMode && t.append("low_bandwidth", "1"),
          t.toString()
        );
      }
      function l(e, t, s) {
        if (!s && e.BHasTemplateAnimatedAssets()) {
          const s = r.A0.GetELanguageFallback(t);
          return (
            e.BHasTemplateAnimatedAssetForLanguage(t) ||
            e.BHasTemplateAnimatedAssetForLanguage(s)
          );
        }
        return !1;
      }
    },
    11417: (e, t, s) => {
      "use strict";
      s.r(t), s.d(t, { MarketingMessageRoutes: () => f, default: () => j });
      var n = s(7850),
        a = s(43527),
        r = s(90626),
        i = s(92757),
        o = s(24484);
      const l = r.createContext({ prioritized_list: !1 });
      function c(e) {
        const [t, s] = r.useState(),
          a = (0, i.zy)(),
          c = r.useMemo(() => {
            new URLSearchParams(a.search);
            return {};
          }, [a.search]);
        return (
          r.useEffect(() => {
            (0, o.Tc)("marketingmessage_config", "application_config");
            s({});
          }, [c]),
          t
            ? (0, n.jsxs)(l.Provider, { value: t, children: [e.children, " "] })
            : null
        );
      }
      var u = s(64726),
        d = s(23809),
        m = s(72034),
        g = s(56186),
        _ = s(78997),
        p = s(32630),
        h = s(78327);
      const f = {
        List: () => `${a.B.MarketingMessages()}list/`,
        Message: (e) => `${a.B.MarketingMessages()}${e}`,
        MessagePreview: (e) => `${a.B.MarketingMessages()}preview/${e}`,
      };
      function j(e) {
        const t = (function () {
          const [e, t] = r.useState(null),
            s = (0, d.TR)();
          return (
            r.useEffect(() => {
              e ||
                t(
                  (function (e) {
                    if (!v) {
                      const t = (0, h.Tc)(
                        "marketingmessage_config",
                        "application_config",
                      );
                      if (((v = new g.Nt(e)), t?.promotion_operation_token)) {
                        const e = new m.D(
                          h.TS.WEBAPI_BASE_URL,
                          t.promotion_operation_token,
                        );
                        v.SetSteamInterfacePromotions(e);
                      }
                    }
                    return v;
                  })(s),
                );
            }, [e, s]),
            e
          );
        })();
        return t
          ? (0, n.jsx)(p.Ay, {
              domain: "store.steampowered.com",
              controller: "message",
              method: "default",
              children: (0, n.jsx)(c, {
                children: (0, n.jsx)(_.g1, {
                  children: (0, n.jsxs)(i.dO, {
                    children: [
                      (0, n.jsx)(i.qh, {
                        path: `${f.List()}`,
                        children: (0, n.jsx)(u.Mf, {
                          MarketingMessagesStore: t,
                        }),
                      }),
                      (0, n.jsx)(i.qh, {
                        path: `${f.MessagePreview(":messageid")}`,
                        children: (0, n.jsx)(u.eI, {
                          MarketingMessagesStore: t,
                          preview: !0,
                        }),
                      }),
                      (0, n.jsx)(i.qh, {
                        path: `${f.Message(":messageid")}`,
                        children: (0, n.jsx)(u.eI, {
                          MarketingMessagesStore: t,
                        }),
                      }),
                      (0, n.jsx)(i.qh, {
                        children: (0, n.jsx)(i.rd, { to: `${f.List()}` }),
                      }),
                    ],
                  }),
                }),
              }),
            })
          : null;
      }
      let v;
    },
  },
]);
