/**** (c) Valve Corporation. Use is governed by the terms of the Steam Subscriber Agreement http://store.steampowered.com/subscriber_agreement/.
 ****/
(self.webpackChunkcommunity = self.webpackChunkcommunity || []).push([
  [657],
  {
    76048: (e) => {
      e.exports = {
        QAFullPageView: "_3tZpFqpGKrksArepdxSE5j",
        QAName: "_2mOBpH-Q1B2CGYjpyVcZvL",
        QAColumns: "pBgAbPu5S77EkhwULNJMQ",
        QAMainCtn: "hHqRc8tmJX5DmOC_J9OSu",
        QAModeratorColumn: "nfftCsjuFUxqo7oht5ZvS",
        QASplitColumns: "_1CRIgUwMI8-DV8bby3TWQb",
        ModeratorInstructions: "RqtnW1y7dqFHzNYQgEXMn",
        AnsweredSectionHeader: "_3cWpkis2Mqzl8A8xF-a9em",
        InputButton: "_1z1rtA-m0g-2LV2Vt1oofr",
        Reload: "_3LIo4Gupf-bEyoN38m0xLy",
        Reloading: "_1hooLg2kfnpqXDhLgPG6gH",
        SignInButton: "GRHnXc4HVJuz6yC7Gkr5k",
        QAHeader: "_3skHmzAqxcsZBq_tkNXLzN",
        QuestionACK: "_16zhk22TxzqPMUoz7I3WEB",
        Visible: "_2jHMeCl9ijq2--JlPwQ6Fz",
        QATitleRow: "_2qdcmBnuOzyLEABfqaT8qv",
        QATitle: "_1c7nCnSMqU4Ukz3wdWSqXF",
        Description: "_3eGNNYyxMkZO39myQDE1XE",
        InputCtn: "oF1kTvqJ3SFh3lHuR5ZHs",
        LogInPrompt: "_3Gbueor3CAgrLqcm5BgjPx",
        ButtonCtn: "_3kFHrlDJDcAvIcJq4lX8lD",
        SaveSpinner: "_20svvC9NhGF5A1hTD-y9kJ",
        QuestionsCtn: "_1W6uJMpV-79gv8A662-iKM",
        InsetShadowTopCtn: "ak9JV2IB-GQLWSc_4GpKB",
        InsetShadowTop: "_1C2e4GaJWY9UCfxJeu3Ifp",
        QuestionsHeader: "jUQG923rSCGTIp6iR40rb",
        QuestionsTitle: "_2CYLB3xFHbAEJS6Z-bSlKz",
        QuestionList: "_28dwOiBG4CQlQcXClFrN9K",
        NoQuestions: "_1R9HBMmOpYYYhZwQh1m5U1",
        Question: "_3BIOouufQgR-V7ivTXLZAm",
        Hidden: "_3XPCkzLM8mFswGMcSCywUA",
        QuestionCtn: "_31sLydaeDe38gXM5axlD9O",
        UserAvatar: "_2ojKekaqQWNpGw8G3x8l_r",
        PersonaName: "_3CaA53te3MV1l4rP0TmdB9",
        QuestionText: "_2NuCE315-vCtCSzxcT1qEm",
        QuestionAndVotes: "ChywOlxnWN_3W_lql2DeN",
        QuestionTitle: "_2AXcHkyp6G5iqdDfTZiwAg",
        QuestionStats: "_1DzgV6zsHS23fEBSvckX88",
        PostTime: "_2hfBugBzm8yU7a82qW2cHx",
        Votes: "_2IyuQJGwbrnTb3a7m8KsZ9",
        Vote_Ready: "_3_8_aVxn-PwduHvKlx36tI",
        Vote_Positive: "eWDCyQ-vvgH6BcvF4_o_e",
        RateIcon: "_2j0GwqqS7RK_-mNPfvuX0f",
        Vote_CannotVote: "_2Pq4zAswQX0s4q5BmDOcoK",
        DateAndTime: "_4VjnZ9DL9exM7YcbLbSh",
        Answer: "wNABP6_Qx7yRFrLkXK41M",
        AnswerTitle: "_1c4moP5DV4s2TT8XzHbSDw",
        AnswerCheck: "u8uV8ZaFofO6c9D42VGY2",
        AnswerText: "_3ar8cNQn3f1-OIjiWPPitA",
        ModerationHeader: "_37OKcYRfuIkylm4nu1umCb",
        ModeratorSection: "_3tP54OFg61u8qq9vrI1GA2",
        ToolRow: "_2c2voljR8QU6ly18xBe9Zx",
        ToolLabel: "_3prTNENecqxmB7UBSWgUyl",
        QADashboardCtn: "_32zJGAGkfjaRrwwUDmbx_Y",
        DashboardHeader: "_1R1EmLdoSBJK82hy_FSG9G",
        DashboardHeaderTitle: "_1cRHaKXhGTjxUnsyLMS0IW",
        DashboardHeaderButtonCtn: "uoLDQx7BVP24M7CtMdUVy",
        DashboardRow: "gpraRkr_GCHPzeFW5wUm7",
        GIDSession: "_3d_6fxEUu5QKFk1X6BmmdU",
        QuestionCount: "_3kZ9Sdx0fxIPNbawZ-iPRU",
        QAndADeleteColumn: "_363MyYnsIHR3suqCkDJb1a",
        QAndAName: "_2-5V8wZPsLM9riXorBMJ1F",
        Prompt: "_2ZFvNUfdh3b32ljUOOm1NB",
        Editing: "_1wUKv_abvHzqvyJg-4E_04",
        NameInput: "_3avr0gEWefqtJlQehAE4CH",
        DeleteButton: "o8aBaeNMu_AdleoGeHA_Z",
        DashboardColumnHeaders: "_2WOM0O_ir8k-lcFjHONR4_",
        CreateQAndADialog: "_1wBzuya0z43LtB3hBPU90I",
        DeleteQAndADialog: "_3valPN89TuPANLrExHyKuc",
        DeleteInfo: "_3EhGHfB3z9uPkO3ZTnwUVm",
      };
    },
    64734: (e) => {
      e.exports = {
        SectionTitleHeader: "_2g5oNomwd2lv8wL2qlsLVA",
        SectionTitleButtons: "RGHKm1_KeaBjdzuvisfYN",
        required_title: "_3yDPZjnsoLc2FkrAH2UOEd",
      };
    },
    28579: (e, s, t) => {
      "use strict";
      t.d(s, { xh: () => z, AC: () => ne, u6: () => ie });
      var n = t(7850),
        o = t(90626),
        i = t(37085),
        a = t(17720),
        r = t(44165),
        l = t(34629),
        d = t(6419),
        u = t(6144),
        c = t(73745),
        m = t(78327);
      const A = [
        "mousemove",
        "keypress",
        "scroll",
        "wheel",
        "focusin",
        "touchstart",
        "touchmove",
      ];
      class h {
        static Get() {
          return h.s_Singleton || (h.s_Singleton = new h()), h.s_Singleton;
        }
        constructor() {
          (this.m_callbackList = new u.lu()),
            (this.m_timer = null),
            (this.m_bAreEventListenersRegistered = !1),
            "dev" == m.TS.WEB_UNIVERSE && (window.g_FidgetStore = this);
        }
        ResetFidgetTimer(e) {
          this.m_timer && window.clearTimeout(this.m_timer);
          this.m_timer = window.setTimeout(() => {
            (this.m_timer = null), this.m_callbackList.Dispatch();
          }, 2e3);
        }
        BIsUserFidgeting() {
          return !!this.m_timer;
        }
        GetCallbackList() {
          return this.m_callbackList;
        }
        BRegisterEventListeners() {
          if (this.m_bAreEventListenersRegistered) return !1;
          const e = window;
          for (const s of A) e.addEventListener(s, this.ResetFidgetTimer);
          return (this.m_bAreEventListenersRegistered = !0), !0;
        }
        BUnregisterEventListeners() {
          if (!this.m_bAreEventListenersRegistered) return !1;
          const e = window;
          for (const s of A) e.removeEventListener(s, this.ResetFidgetTimer);
          return (this.m_bAreEventListenersRegistered = !1), !1;
        }
      }
      function v() {
        o.useEffect(() => {
          const e = h.Get().BRegisterEventListeners();
          return () => {
            e && h.Get().BUnregisterEventListeners();
          };
        }, []);
      }
      (0, l.Cg)([d.o], h.prototype, "ResetFidgetTimer", null);
      var Q = t(41735),
        _ = t.n(Q),
        p = t(4434),
        S = t(68797);
      const g = 0,
        C = 1,
        w = 2,
        N = 3;
      class x {
        static Get() {
          return x.s_Singleton || (x.s_Singleton = new x()), x.s_Singleton;
        }
        constructor() {
          (this.m_mapQAndASessions = new Map()),
            (this.m_mapQAndAChangeCallbackList = new Map()),
            (this.m_dashboardCallbackList = new u.lu()),
            (this.m_mapQuestions = new Map()),
            (this.m_mapQuestionCallbackList = new Map()),
            (this.m_rgPendingQuestionGIDs = []),
            "dev" == m.TS.WEB_UNIVERSE && (window.g_QAndAStore = this);
        }
        GetQAndACallbackList(e) {
          return (
            this.m_mapQAndAChangeCallbackList.has(e) ||
              this.m_mapQAndAChangeCallbackList.set(e, new u.lu()),
            this.m_mapQAndAChangeCallbackList.get(e)
          );
        }
        GetQuestionCallbackList(e) {
          return (
            this.m_mapQuestionCallbackList.has(e) ||
              this.m_mapQuestionCallbackList.set(e, new u.lu()),
            this.m_mapQuestionCallbackList.get(e)
          );
        }
        GetDashboardCallbackList() {
          return this.m_dashboardCallbackList;
        }
        GetSession(e) {
          return this.m_mapQAndASessions.get(e);
        }
        GetAllSessions() {
          return Array.from(this.m_mapQAndASessions.values());
        }
        GetQuestion(e) {
          return this.m_mapQuestions.get(e);
        }
        async LoadAllSessions(e) {
          var s, t, n, o;
          if (!m.iA.logged_in || !m.TS.SESSIONID) return i.sW;
          const a =
              m.TS.COMMUNITY_BASE_URL +
              "questions/" +
              m.UF.VANITY_ID +
              "/ajaxgetqandasforgroup",
            r = {};
          let l = null;
          try {
            const t = await _().get(a, {
              params: r,
              cancelToken: null == e ? void 0 : e.token,
            });
            if (
              (console.log(t),
              200 == (null == t ? void 0 : t.status) &&
                (null === (s = t.data) || void 0 === s ? void 0 : s.success) ==
                  i.R &&
                t.data.qandas)
            ) {
              for (const e of t.data.qandas)
                this.m_mapQAndASessions.set(e.gidSession, e);
              return (
                this.m_dashboardCallbackList.Dispatch(this.GetAllSessions()),
                i.R
              );
            }
            l = { response: t };
          } catch (e) {
            l = e;
          }
          const d = (0, S.H)(l);
          return (
            console.error(
              "Could not load question and answer sessions for group",
              d.strErrorMsg,
              d,
            ),
            null !==
              (o =
                null ===
                  (n =
                    null === (t = null == l ? void 0 : l.response) ||
                    void 0 === t
                      ? void 0
                      : t.data) || void 0 === n
                  ? void 0
                  : n.success) && void 0 !== o
              ? o
              : i.zi
          );
        }
        async CreateNewSession(e, s) {
          var t, n, o, a;
          if (!m.iA.logged_in || !m.TS.SESSIONID) return i.sW;
          const r =
              m.TS.COMMUNITY_BASE_URL +
              "questions/" +
              m.UF.VANITY_ID +
              "/ajaxcreateqanda",
            l = new FormData();
          l.append("sessionid", m.TS.SESSIONID), l.append("name", e);
          let d = null;
          try {
            const e = await _().post(r, l, {
              withCredentials: !0,
              cancelToken: null == s ? void 0 : s.token,
            });
            if (
              200 == (null == e ? void 0 : e.status) &&
              (null === (t = e.data) || void 0 === t ? void 0 : t.success) ==
                i.R &&
              e.data.qanda
            )
              return (
                this.m_mapQAndASessions.set(
                  e.data.qanda.gidSession,
                  e.data.qanda,
                ),
                this.m_dashboardCallbackList.Dispatch(this.GetAllSessions()),
                i.R
              );
            d = { response: e };
          } catch (e) {
            d = e;
          }
          const u = (0, S.H)(d);
          return (
            console.error("Could not create Q&A", e, u.strErrorMsg, u),
            null !==
              (a =
                null ===
                  (o =
                    null === (n = null == d ? void 0 : d.response) ||
                    void 0 === n
                      ? void 0
                      : n.data) || void 0 === o
                  ? void 0
                  : o.success) && void 0 !== a
              ? a
              : i.zi
          );
        }
        async RenameSession(e, s, t) {
          var n, o, a, r;
          if (!m.iA.logged_in || !m.TS.SESSIONID) return i.sW;
          const l =
              m.TS.COMMUNITY_BASE_URL +
              "questions/" +
              m.UF.VANITY_ID +
              "/ajaxrenameqanda/" +
              e,
            d = new FormData();
          d.append("sessionid", m.TS.SESSIONID), d.append("name", s);
          let u = null;
          try {
            const o = await _().post(l, d, {
              withCredentials: !0,
              cancelToken: null == t ? void 0 : t.token,
            });
            if (
              200 == (null == o ? void 0 : o.status) &&
              (null === (n = o.data) || void 0 === n ? void 0 : n.success) ==
                i.R
            )
              return (
                (this.m_mapQAndASessions.get(e).strName = s),
                this.GetQAndACallbackList(e).Dispatch(
                  this.m_mapQAndASessions.get(e),
                ),
                this.m_dashboardCallbackList.Dispatch(this.GetAllSessions()),
                i.R
              );
            u = { response: o };
          } catch (e) {
            u = e;
          }
          const c = (0, S.H)(u);
          return (
            console.error("Could not rename Q&A", e, s, c.strErrorMsg, c),
            null !==
              (r =
                null ===
                  (a =
                    null === (o = null == u ? void 0 : u.response) ||
                    void 0 === o
                      ? void 0
                      : o.data) || void 0 === a
                  ? void 0
                  : a.success) && void 0 !== r
              ? r
              : i.zi
          );
        }
        async DeleteSession(e, s) {
          var t, n, o, a;
          if (!m.iA.logged_in || !m.TS.SESSIONID) return i.sW;
          const r =
              m.TS.COMMUNITY_BASE_URL +
              "questions/" +
              m.UF.VANITY_ID +
              "/ajaxdeleteqanda/" +
              e,
            l = new FormData();
          l.append("sessionid", m.TS.SESSIONID);
          let d = null;
          try {
            const n = await _().post(r, l, {
              withCredentials: !0,
              cancelToken: null == s ? void 0 : s.token,
            });
            if (
              200 == (null == n ? void 0 : n.status) &&
              (null === (t = n.data) || void 0 === t ? void 0 : t.success) ==
                i.R
            )
              return (
                this.m_mapQAndASessions.delete(e),
                this.m_dashboardCallbackList.Dispatch(this.GetAllSessions()),
                i.R
              );
            d = { response: n };
          } catch (e) {
            d = e;
          }
          const u = (0, S.H)(d);
          return (
            console.error("Could not delete Q&A", e, u.strErrorMsg, u),
            null !==
              (a =
                null ===
                  (o =
                    null === (n = null == d ? void 0 : d.response) ||
                    void 0 === n
                      ? void 0
                      : n.data) || void 0 === o
                  ? void 0
                  : o.success) && void 0 !== a
              ? a
              : i.zi
          );
        }
        async LoadSession(e, s, t) {
          var n, o, a, r, l, d, u, c, A, h;
          const v =
              m.iA.logged_in &&
              (!this.m_mapQAndASessions.has(e) ||
                this.m_mapQAndASessions.get(e).bUserCanModerate ||
                this.m_rgPendingQuestionGIDs.length > 0 ||
                s)
                ? "/ajaxgetqandasessionforuser/"
                : "/ajaxgetqandasession/",
            Q = m.TS.COMMUNITY_BASE_URL + "questions/" + m.UF.VANITY_ID + v + e,
            p = {};
          let g = null;
          try {
            const s = await _().get(Q, {
              params: p,
              cancelToken: null == t ? void 0 : t.token,
            });
            if (
              200 == (null == s ? void 0 : s.status) &&
              (null === (n = s.data) || void 0 === n ? void 0 : n.success) ==
                i.R &&
              s.data.qanda &&
              s.data.questions
            ) {
              for (const e of s.data.questions) {
                const s = e.gidQuestion,
                  t =
                    null !==
                      (a =
                        null === (o = this.m_mapQuestions.get(s)) ||
                        void 0 === o
                          ? void 0
                          : o.bUserUpVoted) && void 0 !== a
                      ? a
                      : e.bUserUpVoted,
                  n = { ...e, bUserUpVoted: t };
                this.m_mapQuestions.set(s, n),
                  this.GetQuestionCallbackList(s).Dispatch(n);
              }
              const t =
                  null !==
                    (l =
                      null === (r = this.m_mapQAndASessions.get(e)) ||
                      void 0 === r
                        ? void 0
                        : r.bUserCanModerate) && void 0 !== l
                    ? l
                    : s.data.qanda.bUserCanModerate,
                n =
                  null !==
                    (u =
                      null === (d = this.m_mapQAndASessions.get(e)) ||
                      void 0 === d
                        ? void 0
                        : d.bUserCanAskQuestions) && void 0 !== u
                    ? u
                    : s.data.qanda.bUserCanAskQuestions,
                c = {
                  ...s.data.qanda,
                  bUserCanModerate: t,
                  bUserCanAskQuestions: n,
                };
              return (
                this.m_mapQAndASessions.set(e, c),
                this.GetQAndACallbackList(e).Dispatch(c),
                this.UpdatePendingQuestionList(s.data.qanda.rgQuestionGIDs),
                i.R
              );
            }
            g = { response: s };
          } catch (e) {
            g = e;
          }
          const C = (0, S.H)(g);
          return (
            console.error("Could not load Q&A", e, C.strErrorMsg, C),
            null !==
              (h =
                null ===
                  (A =
                    null === (c = null == g ? void 0 : g.response) ||
                    void 0 === c
                      ? void 0
                      : c.data) || void 0 === A
                  ? void 0
                  : A.success) && void 0 !== h
              ? h
              : i.zi
          );
        }
        UpdatePendingQuestionList(e) {
          var s;
          const t = new Set();
          for (const s of e) {
            const e = this.m_mapQuestions.get(s);
            e.posterAccountID == m.iA.accountid &&
              e.eState == C &&
              t.add(e.gidQuestion);
          }
          if (this.m_rgPendingQuestionGIDs.length > 0) {
            const n = new Set(e);
            for (const e of this.m_rgPendingQuestionGIDs)
              n.has(e) &&
                (null === (s = this.m_mapQuestions.get(e)) || void 0 === s
                  ? void 0
                  : s.eState) == C &&
                t.add(e);
          }
          this.m_rgPendingQuestionGIDs = Array.from(t);
        }
        async AskNewQuestion(e, s, t) {
          var n, o, a, r, l;
          if (
            !m.iA.logged_in ||
            !m.TS.SESSIONID ||
            !(null === (n = this.m_mapQAndASessions.get(e)) || void 0 === n
              ? void 0
              : n.bUserCanAskQuestions)
          )
            return i.sW;
          const d =
              m.TS.COMMUNITY_BASE_URL +
              "questions/" +
              m.UF.VANITY_ID +
              "/ajaxaskquestion/" +
              e,
            u = new FormData();
          u.append("sessionid", m.TS.SESSIONID), u.append("question", s);
          let c = null;
          try {
            const s = await _().post(d, u, {
              withCredentials: !0,
              cancelToken: null == t ? void 0 : t.token,
            });
            if (
              200 == (null == s ? void 0 : s.status) &&
              (null === (o = s.data) || void 0 === o ? void 0 : o.success) ==
                i.R
            ) {
              this.m_rgPendingQuestionGIDs.push(s.data.gidQuestion);
              const n = !0;
              return this.LoadSession(e, n, t), i.R;
            }
            c = { response: s };
          } catch (e) {
            c = e;
          }
          const A = (0, S.H)(c);
          return (
            console.error("Could not upload question", e, s, A.strErrorMsg, A),
            null !==
              (l =
                null ===
                  (r =
                    null === (a = null == c ? void 0 : c.response) ||
                    void 0 === a
                      ? void 0
                      : a.data) || void 0 === r
                  ? void 0
                  : r.success) && void 0 !== l
              ? l
              : i.zi
          );
        }
        async ModerateQuestion(e, s, t, n) {
          var o, a, r, l, d;
          if (
            !m.iA.logged_in ||
            !m.TS.SESSIONID ||
            !(null === (o = this.m_mapQAndASessions.get(e)) || void 0 === o
              ? void 0
              : o.bUserCanModerate)
          )
            return i.sW;
          if (t != C && t != N) return i.nO;
          if (this.m_mapQuestions.get(s).eState == t) return i.R;
          const u =
              m.TS.COMMUNITY_BASE_URL +
              "questions/" +
              m.UF.VANITY_ID +
              "/ajaxmoderatequestion/" +
              e,
            c = new FormData();
          c.append("sessionid", m.TS.SESSIONID),
            c.append("gidquestion", s),
            c.append("action", t == C ? "show" : "hide");
          let A = null;
          try {
            const s = await _().post(u, c, {
              withCredentials: !0,
              cancelToken: null == n ? void 0 : n.token,
            });
            if (
              200 == (null == s ? void 0 : s.status) &&
              (null === (a = s.data) || void 0 === a ? void 0 : a.success) ==
                i.R
            ) {
              const s = !0;
              return this.LoadSession(e, s, n), i.R;
            }
            A = { response: s };
          } catch (e) {
            A = e;
          }
          const h = (0, S.H)(A);
          return (
            console.error(
              "Could not moderate question",
              e,
              s,
              t,
              h.strErrorMsg,
              h,
            ),
            null !==
              (d =
                null ===
                  (l =
                    null === (r = null == A ? void 0 : A.response) ||
                    void 0 === r
                      ? void 0
                      : r.data) || void 0 === l
                  ? void 0
                  : l.success) && void 0 !== d
              ? d
              : i.zi
          );
        }
        async AnswerQuestion(e, s, t, n) {
          var o, a, r, l, d;
          if (
            !m.iA.logged_in ||
            !m.TS.SESSIONID ||
            !(null === (o = this.m_mapQAndASessions.get(e)) || void 0 === o
              ? void 0
              : o.bUserCanAskQuestions)
          )
            return i.sW;
          const u =
              m.TS.COMMUNITY_BASE_URL +
              "questions/" +
              m.UF.VANITY_ID +
              "/ajaxanswerquestion/" +
              e,
            c = new FormData();
          c.append("sessionid", m.TS.SESSIONID),
            c.append("gidquestion", s),
            c.append("answer", t);
          let A = null;
          try {
            const s = await _().post(u, c, {
              withCredentials: !0,
              cancelToken: null == n ? void 0 : n.token,
            });
            if (
              200 == (null == s ? void 0 : s.status) &&
              (null === (a = s.data) || void 0 === a ? void 0 : a.success) ==
                i.R
            ) {
              const s = !0;
              return this.LoadSession(e, s, n), i.R;
            }
            A = { response: s };
          } catch (e) {
            A = e;
          }
          const h = (0, S.H)(A);
          return (
            console.error("Could not upload answer", e, s, t, h.strErrorMsg, h),
            null !==
              (d =
                null ===
                  (l =
                    null === (r = null == A ? void 0 : A.response) ||
                    void 0 === r
                      ? void 0
                      : r.data) || void 0 === l
                  ? void 0
                  : l.success) && void 0 !== d
              ? d
              : i.zi
          );
        }
        async VoteOnQuestion(e, s, t, n) {
          var o, a, r, l, d;
          if (
            !m.iA.logged_in ||
            !m.TS.SESSIONID ||
            !(null === (o = this.m_mapQAndASessions.get(e)) || void 0 === o
              ? void 0
              : o.bUserCanAskQuestions)
          )
            return i.sW;
          if (t === this.m_mapQuestions.get(s).bUserUpVoted) return i.R;
          let u = this.m_mapQuestions.get(s);
          const c = u.nVoteCount,
            A = c + (t ? 1 : -1);
          (u = { ...u, nVoteCount: A, bUserUpVoted: t }),
            this.m_mapQuestions.set(s, u),
            this.GetQuestionCallbackList(s).Dispatch(u);
          const h =
              m.TS.COMMUNITY_BASE_URL +
              "questions/" +
              m.UF.VANITY_ID +
              "/ajaxvoteforquestion/" +
              e,
            v = new FormData();
          v.append("sessionid", m.TS.SESSIONID),
            v.append("gidquestion", s),
            v.append("up", t ? "1" : "0");
          let Q = null;
          try {
            const e = await _().post(h, v, {
              withCredentials: !0,
              cancelToken: null == n ? void 0 : n.token,
            });
            if (
              200 == (null == e ? void 0 : e.status) &&
              (null === (a = e.data) || void 0 === a ? void 0 : a.success) ==
                i.R
            )
              return i.R;
            Q = { response: e };
          } catch (e) {
            Q = e;
          }
          (u = { ...u, nVoteCount: c, bUserUpVoted: !t }),
            this.m_mapQuestions.set(s, u),
            this.GetQuestionCallbackList(s).Dispatch(u);
          const p = (0, S.H)(Q);
          return (
            console.error("Could not upload vote", e, s, t, p.strErrorMsg, p),
            null !==
              (d =
                null ===
                  (l =
                    null === (r = null == Q ? void 0 : Q.response) ||
                    void 0 === r
                      ? void 0
                      : r.data) || void 0 === l
                  ? void 0
                  : l.success) && void 0 !== d
              ? d
              : i.zi
          );
        }
      }
      function b(e) {
        const s = (0, p.m)("useQAndASession"),
          t = o.useCallback(
            async (t) => {
              var n;
              return (
                (null === (n = x.Get().GetSession(e)) || void 0 === n
                  ? void 0
                  : n.bUserCanAskQuestions) &&
                (await x.Get().AskNewQuestion(e, t, s))
              );
            },
            [e, s],
          ),
          n = o.useCallback(
            (t, n) => {
              var o;
              return (
                (null === (o = x.Get().GetSession(e)) || void 0 === o
                  ? void 0
                  : o.bUserCanAskQuestions) &&
                x.Get().VoteOnQuestion(e, t, n, s)
              );
            },
            [e, s],
          ),
          i = o.useCallback(
            (e) => {
              var s;
              const o = !!e,
                i = null == e ? void 0 : e.strName,
                a = null == e ? void 0 : e.bUserCanModerate,
                r = null == e ? void 0 : e.bUserCanAskQuestions,
                l = [],
                d = [],
                u = [],
                c = [];
              for (const t of null !==
                (s = null == e ? void 0 : e.rgQuestionGIDs) && void 0 !== s
                ? s
                : []) {
                const e = x.Get().GetQuestion(t);
                switch (null == e ? void 0 : e.eState) {
                  case C:
                    l.push(t);
                    break;
                  case w:
                    d.push(t);
                    break;
                  case g:
                    u.push(t);
                    break;
                  case N:
                    c.push(t);
                }
              }
              return (
                l.sort(
                  (e, s) =>
                    x.Get().GetQuestion(s).nVoteCount -
                    x.Get().GetQuestion(e).nVoteCount,
                ),
                d.sort(
                  (e, s) =>
                    x.Get().GetQuestion(s).rtAnswerTime -
                    x.Get().GetQuestion(e).rtAnswerTime,
                ),
                u.sort(
                  (e, s) =>
                    x.Get().GetQuestion(s).rtPostTime -
                    x.Get().GetQuestion(e).rtPostTime,
                ),
                c.sort(
                  (e, s) =>
                    x.Get().GetQuestion(s).rtPostTime -
                    x.Get().GetQuestion(e).rtPostTime,
                ),
                {
                  bIsLoaded: o,
                  strName: i,
                  bUserCanModerate: a,
                  bUserCanAskQuestions: r,
                  fnAskQuestion: t,
                  fnVoteOnQuestion: n,
                  rgModeratedQuestionGIDs: l,
                  rgAnsweredQuestionGIDs: d,
                  rgNewQuestionGIDs: u,
                  rgHiddenQuestionGIDs: c,
                }
              );
            },
            [t, n],
          ),
          [a, r] = o.useState(() => i(x.Get().GetSession(e))),
          l = o.useCallback((e) => r(i(e)), [i]);
        return (
          (0, c.hL)(x.Get().GetQAndACallbackList(e), l),
          o.useEffect(() => {
            a.bIsLoaded || x.Get().LoadSession(e);
          }, [a.bIsLoaded, e]),
          a
        );
      }
      let I = null;
      function T(e) {
        const s = (0, p.m)("useReloadFunction"),
          [t, n] = o.useState(!1),
          i = o.useCallback(() => {
            n(!0);
            const t = x.Get().LoadSession(e, !1, s);
            return t.then(() => n(!1)), t;
          }, [e, s]),
          a = (function (e) {
            const s = o.useRef(null),
              t = o.useCallback(() => {
                s.current && (s.current(), (s.current = null));
              }, []);
            return (
              (0, c.hL)(h.Get().GetCallbackList(), t),
              o.useEffect(() => () => {
                s.current = null;
              }),
              o.useCallback(
                (...t) => {
                  h.Get().BIsUserFidgeting()
                    ? (s.current = () => e(...t))
                    : ((s.current = null), e(...t));
                },
                [e],
              )
            );
          })(i);
        return (
          o.useEffect(() => {
            if (I) return () => {};
            return (
              (I = window.setInterval(a, 1e4)), () => window.clearInterval(I)
            );
          }, [a]),
          o.useMemo(() => ({ fnReload: i, bIsReloading: t }), [i, t])
        );
      }
      function j(e) {
        const s = (0, p.m)("useQuestionModeratorActions"),
          t = o.useCallback(
            (t, n) => {
              var o;
              return (
                (null === (o = x.Get().GetSession(e)) || void 0 === o
                  ? void 0
                  : o.bUserCanModerate) && x.Get().AnswerQuestion(e, t, n, s)
              );
            },
            [e, s],
          ),
          n = o.useCallback(
            (t, n) => {
              var o;
              return (
                (null === (o = x.Get().GetSession(e)) || void 0 === o
                  ? void 0
                  : o.bUserCanModerate) && x.Get().ModerateQuestion(e, t, n, s)
              );
            },
            [e, s],
          );
        return o.useMemo(
          () => ({ fnAnswerQuestion: t, fnModerateQuestion: n }),
          [t, n],
        );
      }
      var f = t(68255),
        D = t(82477),
        k = t(76684),
        G = t(9154),
        M = t(738),
        U = t(12155),
        L = t(22797),
        B = t(32754),
        R = t(48479),
        q = t(52038),
        E = t(61859),
        V = t(41338),
        H = t(14771),
        P = t(76048),
        y = t.n(P),
        F = t(41471),
        O = t(6083);
      function z(e) {
        const s = (function () {
          const [e, s] = o.useState(x.Get().GetAllSessions());
          return (
            (0, c.hL)(x.Get().GetDashboardCallbackList(), s),
            o.useEffect(() => {
              0 == e.length && x.Get().LoadAllSessions();
            }, [e.length]),
            e
          );
        })();
        return (0, n.jsxs)("div", {
          className: y().QADashboardCtn,
          children: [
            (0, n.jsx)(Y, {}),
            (0, n.jsxs)("div", {
              className: (0, q.A)(y().DashboardRow, y().DashboardColumnHeaders),
              children: [
                (0, n.jsx)("div", {
                  className: y().QAndAName,
                  children: (0, E.we)("#QAndA_Column_Name"),
                }),
                (0, n.jsx)("div", {
                  className: y().GIDSession,
                  children: (0, E.we)("#QAndA_Column_GID"),
                }),
                (0, n.jsx)("div", {
                  className: y().QuestionCount,
                  children: (0, E.we)("#QAndA_Column_QuestionCount"),
                }),
                (0, n.jsx)("div", {
                  className: y().QAndADeleteColumn,
                  children: (0, E.we)("#QAndA_Column_DeleteButtons"),
                }),
              ],
            }),
            s.map((e) => (0, n.jsx)(X, { qanda: e }, e.gidSession)),
          ],
        });
      }
      function Y(e) {
        const s = o.useCallback(
          () =>
            (0, M.mK)((0, n.jsx)(K, {}), window, {
              strTitle: (0, E.we)("#QAndA_CreateQAndA_Title"),
            }),
          [],
        );
        return (0, n.jsxs)("div", {
          className: y().DashboardHeader,
          children: [
            (0, n.jsx)("div", {
              className: y().DashboardHeaderTitle,
              children: (0, E.we)("#QAndA_Dashboard"),
            }),
            (0, n.jsx)("div", {
              className: y().DashboardHeaderButtonCtn,
              children: (0, n.jsx)(f.$n, {
                onClick: s,
                children: (0, E.we)("#QAndA_CreateQAndA_Button"),
              }),
            }),
          ],
        });
      }
      const W = 120;
      function K(e) {
        const s = (function () {
            const e = (0, p.m)("useCreateQAndAFn");
            return o.useCallback((s) => x.Get().CreateNewSession(s, e), [e]);
          })(),
          [t, i] = o.useState("");
        return (0, n.jsx)(G.o0, {
          strTitle: (0, E.we)("#QAndA_CreateQAndA_Title"),
          strDescription: (0, E.we)("#QAndA_CreateQAndA_Instructions"),
          strOKButtonText: (0, E.we)("#Button_Create"),
          onOK: () => s(t),
          bOKDisabled: 0 == t.length,
          closeModal: e.closeModal,
          className: y().CreateQAndADialog,
          children: (0, n.jsx)(f.pd, {
            type: "text",
            className: y().NameInput,
            value: t,
            placeholder: (0, E.we)("#QAndA_EnterNamePrompt"),
            onFocus: (e) => e.target.select(),
            onChange: (e) => i(e.currentTarget.value),
            maxLength: W,
          }),
        });
      }
      function X(e) {
        const { qanda: s } = e,
          t =
            m.TS.COMMUNITY_BASE_URL +
            "questions/" +
            m.UF.VANITY_ID +
            "/view/" +
            s.gidSession,
          i = o.useCallback(
            (e) => {
              e.stopPropagation(),
                e.preventDefault(),
                (0, M.mK)((0, n.jsx)(Z, { qanda: s }), window, {
                  strTitle: (0, E.we)("#QAndA_CreateQAndA_Title"),
                });
            },
            [s],
          );
        return (0, n.jsxs)("a", {
          className: y().DashboardRow,
          href: t,
          children: [
            (0, n.jsx)(J, { qanda: s }),
            (0, n.jsx)("div", {
              className: y().GIDSession,
              children: s.gidSession,
            }),
            (0, n.jsx)("div", {
              className: y().QuestionCount,
              children: s.rgQuestionGIDs.length,
            }),
            (0, n.jsx)("div", {
              className: y().QAndADeleteColumn,
              children: (0, n.jsx)("div", {
                className: y().DeleteButton,
                onClick: i,
                children: (0, n.jsx)(U.lMJ, {}),
              }),
            }),
          ],
        });
      }
      function J(e) {
        const { qanda: s } = e,
          [t, i] = o.useState(!1),
          [a, r] = o.useState(s.strName),
          l = (function () {
            const e = (0, p.m)("useRenameQAndAFn");
            return o.useCallback((s, t) => x.Get().RenameSession(s, t, e), [e]);
          })(),
          d =
            s.strName.length > 0
              ? s.strName
              : (0, n.jsx)("span", {
                  className: y().Prompt,
                  children: (0, E.we)("#QAndA_EnterNamePrompt"),
                }),
          u = (0, n.jsxs)("div", {
            className: y().NameInput,
            children: [
              (0, n.jsx)(f.pd, {
                type: "text",
                className: y().NameInput,
                value: a,
                placeholder: (0, E.we)("#QAndA_EnterNamePrompt"),
                onFocus: (e) => e.target.select(),
                onChange: (e) => r(e.currentTarget.value),
                maxLength: W,
              }),
              (0, n.jsx)(f.jn, {
                onClick: (e) => {
                  e.stopPropagation(),
                    e.preventDefault(),
                    0 != (0, V.kd)(a, s.strName)
                      ? l(s.gidSession, a).then(() => i(!1))
                      : i(!1);
                },
                className: y().InputButton,
                children: (0, E.we)("#Button_Save"),
              }),
            ],
          });
        return (0, n.jsx)("div", {
          className: (0, q.A)(y().QAndAName, t && y().Editing),
          onClick: (e) => {
            e.stopPropagation(), e.preventDefault(), t || i(!0);
          },
          children: t ? u : d,
        });
      }
      function Z(e) {
        const { qanda: s } = e,
          t = (function () {
            const e = (0, p.m)("useDeleteQAndAFn");
            return o.useCallback((s) => x.Get().DeleteSession(s, e), [e]);
          })();
        return (0, n.jsxs)(G.o0, {
          strTitle: (0, E.we)("#QAndA_Column_DeleteButtons"),
          strDescription: (0, E.we)("#QAndA_DeleteQAndA_Instructions"),
          onOK: () => t(s.gidSession),
          closeModal: e.closeModal,
          className: y().DeleteQAndADialog,
          children: [
            (0, n.jsx)("div", {
              className: y().DeleteInfo,
              children: (0, E.we)("#QAndA_Column_Name") + ": " + s.strName,
            }),
            (0, n.jsx)("div", {
              className: y().DeleteInfo,
              children:
                (0, E.we)("#QAndA_Column_QuestionCount") +
                ": " +
                s.rgQuestionGIDs.length,
            }),
          ],
        });
      }
      function $(e) {
        const {
            gidSession: s,
            gidQuestion: t,
            bUserCanModerate: i,
            bUserCanVote: a,
            fnVoteOnQuestion: r,
            rtNow: l,
          } = e,
          [d, u] = (function (e) {
            const [s, t] = o.useState(x.Get().GetQuestion(e));
            (0, c.hL)(x.Get().GetQuestionCallbackList(e), t);
            const [n, i] = o.useState(!!s);
            return [s, !!s || n];
          })(t);
        if (!d) return null;
        const m = l - d.rtPostTime,
          A = m < H.Kp.PerDay ? (0, E.Hq)(m) : (0, E._l)(d.rtPostTime),
          h = a ? () => r(t, !d.bUserUpVoted) : null,
          v = a
            ? d.bUserUpVoted
              ? y().Vote_Positive
              : y().Vote_Ready
            : y().Vote_CannotVote,
          Q = (0, q.A)(
            y().Question,
            d.eState == N && y().Hidden,
            d.eState == C && y().Moderated,
            d.eState == g && y().Unmoderated,
            d.eState == w && y().Answered,
          );
        return (0, n.jsxs)("div", {
          className: Q,
          children: [
            (0, n.jsxs)("div", {
              className: (0, q.A)(y().QuestionCtn, "questionCtn"),
              children: [
                (0, n.jsx)(se, { accountID: d.posterAccountID }),
                (0, n.jsxs)("div", {
                  className: y().QuestionText,
                  children: [
                    (0, n.jsxs)("div", {
                      className: y().QuestionAndVotes,
                      children: [
                        (0, n.jsx)("div", {
                          className: y().QuestionTitle,
                          children: d.strQuestionText,
                        }),
                        (0, n.jsx)(B.he, {
                          toolTipContent:
                            a &&
                            (d.bUserUpVoted
                              ? (0, E.we)("#QAndA_VoteRemoveTooltip")
                              : (0, E.we)("#QAndA_VoteTooltip")),
                          children: (0, n.jsxs)("div", {
                            className: (0, q.A)(v, y().Votes),
                            onClick: h,
                            children: [
                              (0, n.jsx)(U.bfp, { className: y().RateIcon }),
                              (0, n.jsx)("div", {
                                className: y().VoteCount,
                                children: (0, O.D)(Number(d.nVoteCount)),
                              }),
                            ],
                          }),
                        }),
                      ],
                    }),
                    (0, n.jsx)("div", {
                      className: y().QuestionStats,
                      children: (0, n.jsx)("div", {
                        className: y().PostTime,
                        children: (0, n.jsx)(k.gS, {
                          rtFullDate: d.rtPostTime,
                          className: y().PostTime,
                          stylesmodule: y(),
                          children: A,
                        }),
                      }),
                    }),
                  ],
                }),
              ],
            }),
            d.rtAnswerTime && (0, n.jsx)(ee, { question: d, rtNow: l }),
            i &&
              (0, n.jsx)(te, {
                gidSession: s,
                gidQuestion: t,
                eState: null == d ? void 0 : d.eState,
              }),
          ],
        });
      }
      function ee(e) {
        const { question: s, rtNow: t } = e,
          o = s.rtAnswerTime && t - s.rtAnswerTime,
          i =
            s.rtAnswerTime &&
            (o < H.Kp.PerDay ? (0, E.Hq)(o) : (0, E._l)(s.rtAnswerTime));
        return s.strAnswerText
          ? (0, n.jsxs)("div", {
              className: y().Answer,
              children: [
                (0, n.jsxs)("div", {
                  className: y().AnswerTitle,
                  children: [
                    (0, n.jsx)("div", {
                      className: y().AnswerCheck,
                      children: (0, n.jsx)(U.X4B, { color: "#67c1f5" }),
                    }),
                    (0, E.we)("#QAndA_ModeratorAnswerTitle"),
                  ],
                }),
                (0, n.jsxs)("div", {
                  className: y().AnswerText,
                  children: [" ", s.strAnswerText, " "],
                }),
                (0, n.jsx)("div", {
                  className: y().QuestionStats,
                  children: (0, n.jsx)(k.gS, {
                    rtFullDate: s.rtAnswerTime,
                    className: y().PostTime,
                    stylesmodule: y(),
                    children: i,
                  }),
                }),
              ],
            })
          : (0, n.jsxs)("div", {
              className: y().AnswerTitle,
              children: [
                (0, n.jsx)("div", {
                  className: y().AnswerCheck,
                  children: (0, n.jsx)(U.X4B, { color: "#67c1f5" }),
                }),
                (0, E.we)("#QAndA_AlreadyAnsweredTitle"),
              ],
            });
      }
      function se(e) {
        var s;
        const t = a.b.InitFromAccountID(e.accountID).ConvertTo64BitString(),
          { isLoading: o, data: i } = (0, F.js)(e.accountID);
        return o
          ? null
          : (0, n.jsx)("a", {
              href: i.GetCommunityProfileURL(),
              className: y().PersonaName,
              "data-miniprofile": "s" + t,
              children:
                null !== (s = null == i ? void 0 : i.m_strPlayerName) &&
                void 0 !== s
                  ? s
                  : t,
            });
      }
      function te(e) {
        const { gidSession: s, gidQuestion: t, eState: o } = e,
          { fnAnswerQuestion: i, fnModerateQuestion: a } = j(s),
          r = o == g || o == N,
          l = o != N,
          d = o == C;
        return (0, n.jsxs)("div", {
          className: y().ModeratorSection,
          children: [
            (0, E.we)("#QAndA_ModeratorLabel"),
            (0, n.jsx)("div", {
              className: y().ToolRow,
              children: (0, n.jsx)(re, {
                strPrompt: (0, E.we)("#QAndA_AnswerPrompt"),
                fnSubmit: (e) => i(t, e),
              }),
            }),
            (0, n.jsxs)("div", {
              className: y().ToolRow,
              children: [
                (0, E.we)("#QAndA_ModeratorOtherOptionsLabel"),
                (0, n.jsx)(B.he, {
                  toolTipContent: (0, E.we)(
                    "#QAndA_ModerateApproveButton_ttip",
                  ),
                  children: (0, n.jsx)(f.$n, {
                    onClick: () => a(t, C),
                    className: y().InputButton,
                    disabled: !r,
                    children: (0, E.we)("#QAndA_ModerateApproveButton"),
                  }),
                }),
                (0, n.jsx)(B.he, {
                  toolTipContent: (0, E.we)("#QAndA_ModerateHideButton_ttip"),
                  children: (0, n.jsx)(f.$n, {
                    onClick: () => a(t, N),
                    className: y().InputButton,
                    disabled: !l,
                    children: (0, E.we)("#QAndA_ModerateHideButton"),
                  }),
                }),
                (0, n.jsx)(B.he, {
                  toolTipContent: (0, E.we)("#QAndA_QuickAnswerButton_ttip"),
                  children: (0, n.jsx)(f.$n, {
                    onClick: () => i(t, ""),
                    className: y().InputButton,
                    disabled: !d,
                    children: (0, E.we)("#QAndA_QuickAnswerButton"),
                  }),
                }),
              ],
            }),
          ],
        });
      }
      function ne(e) {
        const { gidSession: s } = e;
        v();
        const { strName: t, bUserCanModerate: o } = b(s);
        return (0, n.jsxs)("div", {
          className: y().QAFullPageView,
          children: [
            (0, n.jsx)("div", {
              className: y().QAName,
              children:
                (0, E.we)("#QAndA_Header") +
                ((null == t ? void 0 : t.length) > 0 ? ": " + t : ""),
            }),
            (0, n.jsxs)("div", {
              className: y().QAColumns,
              children: [
                (0, n.jsx)(ie, { gidSession: s, bHalfOfFullPage: o }),
                o && (0, n.jsx)(oe, { gidSession: s }),
              ],
            }),
          ],
        });
      }
      function oe(e) {
        const { gidSession: s } = e,
          {
            bUserCanModerate: t,
            rgNewQuestionGIDs: o,
            rgHiddenQuestionGIDs: i,
          } = b(s),
          a = (0, r.P_)(10);
        return t
          ? (0, n.jsxs)("div", {
              className: (0, q.A)(y().QAModeratorColumn, y().QASplitColumns),
              children: [
                (0, n.jsx)(R.qx, {
                  title: (0, E.we)("#QAndA_ModeratorViewHeader"),
                  className: y().ModeratorInstructions,
                  children: (0, n.jsx)("div", {
                    className: y().QuestionList,
                    children: o.map((e) =>
                      (0, n.jsx)(
                        $,
                        {
                          gidSession: s,
                          gidQuestion: e,
                          bUserCanModerate: t,
                          bUserCanVote: !1,
                          rtNow: a,
                        },
                        e,
                      ),
                    ),
                  }),
                }),
                (0, n.jsx)(R.qx, {
                  title: (0, E.we)("#QAndA_HiddenQuestionsHeader"),
                  className: y().AnsweredSectionHeader,
                  bStartMinimized: !0,
                  children: (0, n.jsx)("div", {
                    className: y().QuestionList,
                    children: i.map((e) =>
                      (0, n.jsx)(
                        $,
                        {
                          gidSession: s,
                          gidQuestion: e,
                          bUserCanModerate: t,
                          bUserCanVote: !1,
                          rtNow: a,
                        },
                        e,
                      ),
                    ),
                  }),
                }),
              ],
            })
          : null;
      }
      function ie(e) {
        const { gidSession: s, bHalfOfFullPage: t } = e;
        v();
        const {
            bIsLoaded: o,
            bUserCanAskQuestions: i,
            bUserCanModerate: a,
            fnAskQuestion: l,
            fnVoteOnQuestion: d,
            rgModeratedQuestionGIDs: u,
            rgAnsweredQuestionGIDs: c,
            rgNewQuestionGIDs: m,
          } = b(s),
          { fnReload: A, bIsReloading: h } = T(s),
          Q = (0, r.P_)(10),
          _ = a && t,
          { fnModerateQuestion: p } = j(s);
        return (0, n.jsxs)("div", {
          className: (0, q.A)(y().QAMainCtn, t && y().QASplitColumns),
          children: [
            (0, n.jsxs)("div", {
              className: (0, q.A)(y().QAHeader, _ && y().ModerationHeader),
              children: [
                (0, n.jsx)(f.$n, {
                  onClick: A,
                  disabled: !o,
                  className: (0, q.A)(
                    y().InputButton,
                    y().Reload,
                    h && y().Reloading,
                  ),
                  children: (0, n.jsx)("div", {
                    className: y().ReloadIcon,
                    children: (0, n.jsx)(U.MvQ, {}),
                  }),
                }),
                _
                  ? (0, n.jsxs)(n.Fragment, {
                      children: [
                        (0, E.we)("#QAndA_ModeratorLabelHeader"),
                        (0, n.jsx)(B.he, {
                          toolTipContent: (0, E.we)(
                            "#QAndA_ModerateHideAllButton_ttip",
                          ),
                          children: (0, n.jsx)(f.$n, {
                            onClick: async () => {
                              for (const e of [m, u, c])
                                for (const s of e) await p(s, N);
                            },
                            className: y().InputButton,
                            children: (0, E.we)("#QAndA_ModerateHideAllButton"),
                          }),
                        }),
                      ],
                    })
                  : i
                    ? (0, n.jsx)(ae, { fnAskQuestion: l })
                    : o && (0, n.jsx)(le, {}),
              ],
            }),
            u.length + c.length == 0
              ? (0, n.jsx)("div", {
                  className: y().NoQuestions,
                  children: (0, E.we)("#QAndA_NoQuestionsYet"),
                })
              : (0, n.jsxs)("div", {
                  className: y().QuestionsCtn,
                  children: [
                    (0, n.jsx)("div", {
                      className: y().InsetShadowTopCtn,
                      children: (0, n.jsx)("div", {
                        className: y().InsetShadowTop,
                      }),
                    }),
                    (0, n.jsx)("div", {
                      className: y().QuestionList,
                      children: o
                        ? u.map((e) =>
                            (0, n.jsx)(
                              $,
                              {
                                gidSession: s,
                                gidQuestion: e,
                                bUserCanModerate: _,
                                bUserCanVote: i,
                                fnVoteOnQuestion: d,
                                rtNow: Q,
                              },
                              e,
                            ),
                          )
                        : (0, n.jsx)(L.t, { position: "center" }),
                    }),
                    c.length > 0 &&
                      (0, n.jsxs)(n.Fragment, {
                        children: [
                          (0, n.jsx)("div", {
                            className: y().AnsweredSectionHeader,
                            children: (0, E.we)(
                              "#QAndA_AnsweredQuestionsHeader",
                            ),
                          }),
                          (0, n.jsx)("div", {
                            className: y().QuestionList,
                            children: c.map((e) =>
                              (0, n.jsx)(
                                $,
                                {
                                  gidSession: s,
                                  gidQuestion: e,
                                  bUserCanModerate: _,
                                  bUserCanVote: i,
                                  fnVoteOnQuestion: d,
                                  rtNow: Q,
                                },
                                e,
                              ),
                            ),
                          }),
                        ],
                      }),
                  ],
                }),
          ],
        });
      }
      function ae(e) {
        const [s, t] = o.useState(!1),
          a = (0, p.m)("QuestionBox");
        return (0, n.jsxs)(n.Fragment, {
          children: [
            (0, n.jsx)("div", {
              className: y().Description,
              children: (0, E.we)("#QAndA_HeaderDescription"),
            }),
            (0, n.jsx)(re, {
              strPrompt: (0, E.we)("#QAndA_QuestionPrompt"),
              fnSubmit: async (s) => {
                const n = await e.fnAskQuestion(s);
                return (
                  n != i.R ||
                    a.token.reason ||
                    (t(!0),
                    window.setTimeout(() => {
                      a.token.reason || t(!1);
                    }, 1e3)),
                  n
                );
              },
            }),
            (0, n.jsx)("div", {
              className: (0, q.A)(y().QuestionACK, s && y().Visible),
              children: (0, E.we)("#QAndA_QuestionWasPosted"),
            }),
          ],
        });
      }
      function re(e) {
        const { strPrompt: s, fnSubmit: t } = e,
          [a, r] = o.useState(""),
          [l, d] = o.useState(!1);
        return (0, n.jsx)(n.Fragment, {
          children: (0, n.jsxs)("div", {
            className: y().InputCtn,
            children: [
              (0, n.jsx)(f.pd, {
                type: "text",
                placeholder: s,
                maxLength: 512,
                value: a,
                onChange: (e) => r(e.target.value),
              }),
              (0, n.jsxs)("div", {
                className: y().ButtonCtn,
                children: [
                  l &&
                    (0, n.jsx)("div", {
                      className: y().SaveSpinner,
                      children: (0, n.jsx)(L.t, {
                        size: "small",
                        position: "center",
                      }),
                    }),
                  (0, n.jsx)(f.$n, {
                    onClick: async () => {
                      d(!0);
                      (await t(a)) == i.R && r(""), d(!1);
                    },
                    disabled: l || !a,
                    className: y().InputButton,
                    children: (0, E.we)("#QAndA_SubmitCommentButton"),
                  }),
                ],
              }),
            ],
          }),
        });
      }
      function le(e) {
        return (0, n.jsxs)("div", {
          className: y().Description,
          children: [
            (0, n.jsx)("div", {
              className: y().LogInPrompt,
              children: (0, E.we)("#QAndA_LogInPrompt"),
            }),
            !m.iA.logged_in &&
              (0, n.jsx)(f.$n, {
                onClick: D.vg,
                className: (0, q.A)(y().SignInButton, y().InputButton),
                children: (0, E.we)("#Login_SignIn"),
              }),
          ],
        });
      }
    },
    48479: (e, s, t) => {
      "use strict";
      t.d(s, { AQ: () => h, pn: () => Q, qx: () => v });
      var n = t(7850),
        o = t(68255),
        i = t(61859),
        a = t(12155),
        r = t(90626),
        l = t(52038),
        d = t(95695),
        u = t(84811),
        c = t(64734),
        m = t(65946),
        A = t(26408);
      function h(e) {
        const {
            title: s,
            tooltip: t,
            getMinimized: o,
            toggleMinimized: i,
            className: a,
            children: r,
            elAdditionalButtons: h,
          } = e,
          v = (0, m.q3)(() => o());
        return (0, n.jsxs)(n.Fragment, {
          children: [
            (0, n.jsxs)("div", {
              className: (0, l.A)(
                a,
                c.SectionTitleHeader,
                c.required_title,
                "SectionTitleHeader",
              ),
              children: [
                (0, n.jsxs)("div", {
                  className: (0, l.A)(
                    d.CollapsableSectionTitle,
                    "EventEditorTextTitle",
                  ),
                  children: [s, Boolean(t) && (0, n.jsx)(A.o, { tooltip: t })],
                }),
                (0, n.jsxs)("div", {
                  className: c.SectionTitleButtons,
                  children: [
                    h,
                    (0, n.jsx)(Q, { bIsMinimized: v, fnToggleMinimize: i }),
                  ],
                }),
              ],
            }),
            !v && (0, n.jsx)(u.tH, { children: r }),
          ],
        });
      }
      function v(e) {
        const [s, t] = r.useState(Boolean(e.bStartMinimized));
        return (0, n.jsx)(h, {
          ...e,
          getMinimized: () => s,
          toggleMinimized: () => t(!s),
          children: e.children,
        });
      }
      function Q(e) {
        const { bIsMinimized: s, fnToggleMinimize: t } = e,
          r = s ? "#Section_Maximize_Tooltip" : "#Section_Minimize_Tooltip";
        return (0, n.jsx)(o.$n, {
          "data-tooltip-text": (0, i.we)(r),
          onClick: t,
          children: e.bIsMinimized
            ? (0, n.jsx)(a.hz4, {})
            : (0, n.jsx)(a.Xjb, {}),
        });
      }
    },
  },
]);
