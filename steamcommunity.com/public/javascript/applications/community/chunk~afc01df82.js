/**** (c) Valve Corporation. Use is governed by the terms of the Steam Subscriber Agreement http://store.steampowered.com/subscriber_agreement/.
 ****/
(self.webpackChunkcommunity = self.webpackChunkcommunity || []).push([
  [9281],
  {
    55486: (e) => {
      e.exports = {
        PickerContainer: "_1qhUOyXySrP5wxstyHcNfi",
        SearchInput: "lwt4uhPJr5zMlKtOXk7KH",
        Hilight: "_5oO3lqjoO6KmQtAJqUDAz",
        ImagesContainer: "_2MNEtVBTbM588lRBH4iSWU",
        UploaderContainer: "dR0NeWIeEFgvr-YGjRojm",
        UploaderTitle: "DL9t5BhLQBnOak9WodTTy",
        UploaderRunning: "_1lRF9BfrKTlDFFAmqE7f6H",
        UploaderDesc: "_2fhbnZhBGr6n624Mkviv_M",
        UploadError: "_3B5rV9V3B5jZyTkaOOpLZC",
        UploadMessageAndButtonsContainer: "_3OQ66CWxfAqQrPyk3ENMN9",
        UploadSuccess: "_2RXjwOiBQmS6QJlGOp0zjR",
        UploadDismissButton: "_3j1ztsyZKmnW7dgsO7MeBX",
        EnableClanImagesV2Ctn: "_25bW2wRPaIRCWnvjX0pmYP",
        EnableClanImagesV2: "_3khYXUyapfKawwe6aLY1YD",
      };
    },
    40888: (e) => {
      e.exports = {
        LightboxDialog: "jdJJwFEyHc1BcauL8BGi8",
        LightboxImageContainer: "_2AfDMWU1r2BkWWOuPJBEG4",
        LightboxMainImage: "_3lke7Ru5iG92hZgusH_6mW",
        LightboxPrevImage: "_12Q6eXlxa7az2UzSkyoVZi",
        LightboxNextImage: "_1-u-acR7PtZy8y0uwVsHpe",
        LightboxToolbar: "_1nvyoL-4JdDLMQAb0G0qbB",
        LightboxImageTitle: "_12w2ThvzUOKOyHZoctVBYy",
        LightboxToolbarButtons: "_2EbKH8e98l6jfPsQZbpF5d",
        LightboxCloseButton: "_3V6J1j2mWeEq42tDXjdlyT",
        LightboxLeftButton: "_1Pkq-IzPmH3SL1Ch4rxT7E",
        LightboxRightButton: "_3hJAw5_2ezz8JlA8lniMOV",
      };
    },
    33924: (e) => {
      e.exports = {
        OtherEventsCtn: "_9H6b5yfaxlmcnHvkqtwDK",
        OtherEvents_MainImageCtn: "_2qyLPxO8_nkczRvFiaju8N",
        OtherEvents: "_16DzRvjcqFcYr0NYcWmTrg",
        EventSizer: "_2JC5DEuXUeE50kjpb7Eeau",
        OtherEvents_EventCtn: "_1MwNf8slOG9lOvAeOshmuu",
        EventSummaryText: "ENbI1gFgvIca6HSKAbfiJ",
        ShowInWideMode: "RLbLb742gN095uDUITtIB",
        EventSummaryContainer: "_2GYp44BuZLfKRQdeILTDC3",
        HideInWideMode: "_3itHivPkrgI7TWENi1yxjI",
        OtherEvents_ContentCtn: "_22jEpNTfml-w_aRJV-fKDm",
        HoversEnabled: "_3o6M87A6T172WsUE6MNvdW",
        OtherEvents_TextTitle: "_2jc1DpJ_WzFtigRh5qDWce",
        OtherEvents_MainImage: "_3_wKbXvT7_y5YkrtadL0I6",
        PartnerEventRowCapsule_MainImage: "bC2Zkx7FlANno4SW8FwB-",
        EventSummaryType: "_11JXznGoylLSEmZXZbgcsq",
        OtherEvents_BGImage: "_2pPj9UWoWM6h318uBN0-8X",
        MaskImages: "_1kFdtNfhXozP4yI_qOv2H-",
        OtherEvents_TextCtn: "_3-EtNa1Nr_737K0kglkT9C",
        UpcomingCtn: "_2CXrGPtlQh-j3aSa6XsQDI",
        OtherEvents_SubTitle: "_1Swox5XYdeesack-J7fNLH",
        EventType: "_2BWwVF5N-3fDuJRblB6gHb",
        AppCapsuleImage: "_3OzV3h4jW1bkLmB6TqbYmo",
        CapsuleShadow: "_2rjkJQtvus70aLmbfGoneD",
        AppCapsuleCtn: "_16au-uWHggl6G731aw_eHt",
        AppCapsuleImageHover: "IeC3X0McKdGC79BsC3VvM",
        AppCapsulePrice: "_2-l2M5GPuxKFwV8h1tc_fH",
      };
    },
    53732: (e) => {
      e.exports = {
        ImageWrapperContainer: "_2or51Nzh1oEwvdNjKQ1XsS",
        ImageWrapper: "_34WcpEIVKr8Z72GaesGoR4",
        VideoBackground: "_3IizOeZqT1lZaoPEmdVxG",
        ImageWrapperFilename: "_3_vYFjDjTuDvhsL10XO9BU",
        ResultNotification: "_1X95b1CVvEsEa5dfoR5Pfv",
        ErrorCode: "_-7Alg3skQ6oFTYIpKTHsI",
        Hilight: "_3lBJMYeg4_hihNl0QTX1Qi",
        ImageButton: "_2MUWDtjaZWaMDdJaQr4o5a",
        Thumb: "_3M02zvAfoMwX5XlzlvFkc3",
        Full: "_1RN-YKVciU9zYHOYX6OV0",
        Delete: "_1X87fLS_CT0g2Vu5-fClUZ",
        FloatingThrobber: "_2EHZ15YQSAK_T5SCxVobtG",
        Localized: "_3FFrtt5Of4jP9unTFjYiHs",
        ClanImageGrid: "_3J5Yc20Wkz7gjSxxWcHst",
        ClanImageGridItem: "_1vXdD6QZTKcjYoRTOAuOeX",
        Selected: "_3JVN2Ta1MlQnuMnqPo0XR8",
        ImgCtn: "_248ADrw9QzPyhcxjqlaykT",
        Name: "TzsVI0_4scOG258SCeyqz",
      };
    },
    9709: (e) => {
      e.exports = {
        TitleImg: "_3E4IFPQP4lnTaJ8fo462Br",
        PreviewImg: "_2COOlV_DzUDN3N0P3ToybN",
        ArtworkBar: "_3OWH-tupjKqql_tcQsLYIp",
      };
    },
    71647: (e) => {
      e.exports = {
        DragAndDropContainer: "_2RL1a79W53-tCW7090DcUp",
        DragAndDropContainerDragging: "wn604fTvW5SH1o852jAnI",
        ImageUploadBar: "_2Zk7b2c_FLMvZPqYvzTzt5",
        SelectImageButton: "_3Cd9cpywFS-01PilCrgOQo",
      };
    },
    49460: (e) => {
      e.exports = {
        SearchInput: "z7qI4Gjuleb-g6osRQpw2",
        PickerTitle: "_1yPqhNpX8e1HgnrarYmsZg",
      };
    },
    27344: (e) => {
      e.exports = {
        ImageDimensionTooSmall: "_1A6oRywbsuzGxawqTexX6G",
        UploadPreviewCtn: "_1x7wvgGW08t0c2auyfWyAs",
        UploadPreviewButtonsCtn: "_2Vsz0Teq375iSLvbdoaCw0",
        UploadPreviewDelete: "_1898rmbQKDsZukkFbEda-H",
        UploadPreviewButton: "wUyDKp6qikfxWISsHWYI5",
        UploadPreviewError: "_2sh7mSiQmyBdLyJPYPva2L",
        UploadPreviewWarning: "-khhIHR9pWYus_nTScWdO",
        UploadPreviewMessage: "_3kt_NxdtRh4OR_iFeApvM9",
        UploadPreview: "_3dSNtZdgIHIa6P9ZODRBJs",
        PreviewImgCtn: "a4db1xuziijkLJ6HQXeEs",
        PreviewImgInfo: "ddYEDOKiU6ZFhNI4sb_eQ",
      };
    },
    25359: (e) => {
      e.exports = {
        EventEditorArtworkCtn: "_3etoSeNgIJIJoQjVvKBkdK",
        ArtworkPreview: "_1fBG8S7L5v1-Ll8UMASqW5",
        EventEditorArtworkBarContainer: "TLT1tvLtG6-1EdFGwToo1",
        EventEditorButton: "_2EbfH5kGhG6VdMYM0aSFsw",
        EventEditorInputPaneTopRow: "_3loSsH7QVVzJW4dbA_k8pH",
        EventCoverImageCtn: "vcULy1uwr1V-xetzQ3t5_",
        DragTarget: "_2qaqHaHt0FsJ5g6E50Rpbn",
        DragOnTopOfMe: "_1-0mEm0at-4Czr10kmQ82K",
        EventEditorArtworkTextCtn: "wbzVx6PSPvY3jxjmybwT7",
        EventEditorDragTargetArea: "_352Z7ynHHExwu7pbLG0mi3",
        EventEditorArtworkTitle: "_1BtkzIs3COLhdqubhPqTJa",
        EventEditorArtworkSubTitle: "_3NsjbDpfSxc8ZHhYE5TuTv",
        EventEditorArtworkResolution: "pScoegXLiCfPTrVdDHgRc",
        ReassignCtn: "_2kzxUHYwRnfLZc2qUJp54m",
        ImagePreviewContainer: "_4M__i4jyU9-VJE6K30Rat",
        NoneSet: "csDC3rD7ooQ8gGXZhh594",
        TitleSafePreview: "_2Gel5eBC4smzhCMPJN4poX",
        TitleSafeCaption: "_2oU3ulhvWy8BrTtr-wLTHL",
        LanguageSelector: "_33sdnBObDSgcIemY_8d188",
        LanguageSelectorSelected: "_35iac6gVYl3NbfLM5oGhAp",
        LanguageSelectorNoData: "_2MrExNFgrVVmzV4_XxWk7m",
        LanguageContainer: "_1GqYxNpFolOmvCXZZ5SqS9",
        LanguageOptions: "_1OF4inXEccSHpEi-94BNyB",
        LanguageListContainer: "_2NKwVWWJzUopyzUpm5K8PU",
        SelectImageContainerTopRow: "_33RDQ6gt9hW0N3baDbAfnl",
        SelectImageContainerBottomRow: "_3Mstp8zLfqhPc0yqJGve2N",
        TextTitle: "_1b_OxtjP85MZc-IlQfnnHR",
        TextSubTitle: "EqzVNygGbzsiBalSQOtWy",
        SelectImageEqualColumns: "Qz0mmjcnBMcs99N6fgVCv",
        SelectImageBlock: "X_wtWeV0nNEF-9Rz0wZRL",
        MainPreviewBlock: "_3kAV8hXf4G70C4tDE8HDjI",
        Tips: "_2jAkKq9D5KKOH2cgMu59yN",
        ExamplesCtn: "WiG3FOkzY58mDmTzVy40z",
        SelectImageExampleImg: "_3Lcquzc_EacniSS2QxdUHx",
        SelectImageLanguagesCtn: "_27huHYrHSwivfUIglfRube",
        SelectImageTitle: "lJEQ6yKHtjwXClD4NVqUY",
        ArtworkSelectorContainer: "_2dxWXru9IFUHuJgzC9_WwQ",
        Title: "_2HiqsrLG8k4zf4raXVygUP",
        SaleHeaderExampleCtn: "_2Nwi2WWTWdc4JkMEiHDFFK",
        SaleHeaderExampleCol: "_2s4zAjRHJabF47kK9uxCY6",
        BroadcastPreview: "_3NxzN3dNq98rjVdkyQ9QIH",
        AssetExampleSpotlightCtn: "_29B1UOzVRMVZSd22IyP43x",
        BackgroundConfigCtn: "_3SVRvFP-sXikNXmksKkDQ7",
        OptionCtn: "_2XnObldRTEs5T4Sswyv5Fo",
        ButtonRow: "_2W9rAanKV4V6A7Exx4sWGF",
        BackgroundColorBtn: "_2YD-avez2pqO4MJHAO5_v0",
        BackgroundColorResetBtn: "baRhk4ouyxcNfo_um5C76",
        UploadSuccess: "inXVzuN-asDe-A5jnsvvV",
        HighlightBox: "_3qTodEPOW76BNBFtgX0AUa",
      };
    },
    79949: (e) => {
      e.exports = {
        MultipleExampleContainer: "_3HrpHSdcqC7wp8s07bOS2l",
        ExampleSectionTitle: "MxxIR01BbdH_tAWmTbjoz",
        DetailPageExample: "_3Mi3a8sT7hZn6-L_TPm3gr",
        DetailExample: "TYQJH_hhcEuSRvl75g6GA",
        DetailExample2: "HQAziOChjZK2M_cKTNA8",
        MainImageCtn: "_1mRJSs13tWFRJ55fG6WrK8",
        ExampleBodyPosition: "_2wNW_eWECTcvaYU7AYXXY2",
        ExampleContentCtn: "_2bAs9Bkh1K8PYVhcLLerfA",
        TextTitle: "_3fulSVNkgCeQyqxT0FjHOp",
        TextSubTitle: "_3ThX6fPp7MJY_TrTP_RCRY",
        TextBody: "_2nG13rbAd05OnozWt7nQWL",
        SpotlightExample: "_3KsBV1q-e0ZnxgK9GdUiON",
        ExampleSpacer: "oAEZygc5smKi6PjD-981",
        BroadcastPreviewContainer: "_3aLcrZxS4I4KVtUF0BdHds",
        SaleHeaderPreviewContainer: "GORXZE3lrdjE-QiVxXceW",
      };
    },
    15496: (e) => {
      e.exports = {
        narrowWidth: "500px",
        "duration-app-launch": "800ms",
        ReadMoreLink: "_2mvgc6dpEDHRJlTWhGDz7h",
        MajorEventContainer: "dVJB2r43CGIAgr-Xtt4P3",
        MajorEventImageContainer: "_1PkTBeZJVs3WI8US0zffEx",
        MajorEventImage: "_25fL1JQcG1kh_9L5danMxc",
        BottomShadow: "_1ueE9cjv0hzERo311Gr6qL",
        MajoreEventImageContentContainer: "_3mREW5LJ_7jyeol7BtXcym",
        MajorEventImageTemplate: "lQR9_4nAXfydIY7zwOzSF",
        MajorEventBackground: "_388IuJImOHcpIL9kvqJdet",
        MajorEventImageBackgroundBlur: "_3sVs6YBElnuTON_cY_6ne5",
        MajorEventHeader: "_1HL2nt3zhHJo3RkMzmD-Gb",
        PartnerEventLargeImage_Title: "bYwbk-ycz_n2JnQgyrgDx",
        EventType: "_3zVyXPaFJl95Q5qnxtDpuB",
        GameIconAndName: "IltgR1LrH0neRnKq0TLxy",
        GameIcon: "_3Dkj3XaiQV2I1d2m-RRA_L",
        MajorEventSpotlightBackground: "_1ahePoGx6gPXhapzZw2L21",
        MajorEventContent: "_2nr7NuawYs9NhC8OUkY0fK",
        MajorEventTextCtn: "Ojdg2vBD3O1oroxYVU2zB",
        MajorEventTitle: "nEBZT02OOnxIbyIl9Dk44",
        MajorEventSummary: "HPngOFPPykmeXFSxcC1Zv",
        MajorEvent_Ctn: "_2_kU7nUB6wwDu-LsbQZmNc",
        AppDetailsSpotlightContainer: "_1zDJ1bfFg-UkuAluUAoGKj",
        BackgroundAnimation: "_2zmvTGYcnxB2bhgSNFXnSi",
        "ItemFocusAnim-darkerGrey-nocolor": "_2DCLV3hUeBViGvq3yTsiQE",
        "ItemFocusAnim-darkerGrey": "_1iMoXsAEHqrsXXcoaw1SIy",
        "ItemFocusAnim-darkGreySettings": "_23bSFoV4nDLAGl_G32zEdY",
        "ItemFocusAnim-darkGrey": "_1_Uo-zxJJlBTZyvRjgeG4_",
        "ItemFocusAnim-grey": "_3AjpDoqzZuBj6F7fMiO2Q-",
        "ItemFocusAnim-translucent-white-10": "_3PpKBwmAjZpmyTB-ooDvNd",
        "ItemFocusAnim-translucent-white-20": "_2k5z_bdbdZRy3o_pIFzFBF",
        "ItemFocusAnimBorder-darkGrey": "DuzyT2w758OaPfDpfQkO6",
        "ItemFocusAnim-green": "kF7es13166bQnCHSRaw6l",
        focusAnimation: "_3lfKCkcI6nWWMWFgLOGbyh",
        hoverAnimation: "_24fZDwdgB8kUq2hGCnbx88",
      };
    },
    15736: (e) => {
      e.exports = { SmallAvatar: "_2cuu0nLVc4medg6FpU6PQl" };
    },
    33561: (e, a, n) => {
      "use strict";
      n.d(a, { A4: () => d, LU: () => g, sK: () => m });
      var t = n(7850),
        l = n(90626),
        r = n(60860),
        s = n(22837),
        i = n(78327);
      const o = { editModel: null, bClanImagesV2: !1, setClanImagesV2: void 0 },
        c = l.createContext(o);
      function d(e) {
        const { children: a, editModel: n } = e,
          { bClanImagesV2: s, setClanImagesV2: i } = (function () {
            const e = "storeUseClanImagesV2",
              [a, n] = l.useState(() => !!localStorage.getItem(e)),
              t = l.useCallback((a) => {
                a ? localStorage.setItem(e, "1") : localStorage.removeItem(e),
                  n(a);
              }, []);
            return { bClanImagesV2: a, setClanImagesV2: t };
          })();
        return (0, t.jsx)(c.Provider, {
          value: { ...o, editModel: n, bClanImagesV2: s, setClanImagesV2: i },
          children: (0, t.jsx)(r.Cs, { location: r.uF, children: a }),
        });
      }
      function g() {
        return l.useContext(c).editModel;
      }
      function m() {
        const e = l.useContext(c),
          a =
            ("dev" == i.TS.WEB_UNIVERSE || "beta" == i.TS.WEB_UNIVERSE) &&
            i.TS.EUNIVERSE == s.Rv &&
            i.iA.is_support;
        return {
          bClanImagesV2: e.bClanImagesV2 && a,
          bClanImagesV2Allowed: a,
          setClanImagesV2: e.setClanImagesV2,
        };
      }
    },
    97780: (e, a, n) => {
      "use strict";
      n.d(a, { G: () => U });
      var t = n(7850),
        l = n(22837),
        r = n(41735),
        s = n.n(r),
        i = n(75844),
        o = n(65946),
        c = n(90626),
        d = n(68255),
        g = n(81047),
        m = n(28954),
        u = n(2805),
        p = n(55486),
        h = n.n(p),
        _ = n(61924),
        v = n(36509),
        x = n(95695),
        I = n.n(x),
        j = n(9154),
        w = n(738),
        f = n(22797),
        C = n(68797),
        A = n(52038),
        E = n(61859),
        S = n(73745),
        b = n(36782),
        L = n(40888),
        k = n.n(L);
      function D(e) {
        const a = (0, c.useRef)(null);
        (0, c.useEffect)(() => {
          var e;
          null === (e = a.current) || void 0 === e || e.showModal();
          const n = document.body.style.overflow;
          return (
            (document.body.style.overflow = "hidden"),
            () => {
              document.body.style.overflow = n;
            }
          );
        }, []);
        const n =
            "string" == typeof e.prevSrc
              ? (0, t.jsx)("img", { src: e.prevSrc })
              : e.prevSrc,
          l =
            "string" == typeof e.mainSrc
              ? (0, t.jsx)("img", { src: e.mainSrc })
              : e.mainSrc,
          r =
            "string" == typeof e.nextSrc
              ? (0, t.jsx)("img", { src: e.nextSrc })
              : e.nextSrc,
          s = [
            k().LightboxDialog,
            ...(e.backgroundClassName ? [e.backgroundClassName] : []),
          ];
        return (0, t.jsxs)("dialog", {
          ref: a,
          className: (0, A.A)(...s),
          onClose: (a) => e.onCloseRequest(a),
          onKeyDown: (a) => {
            "ArrowRight" === a.key && e.nextSrc
              ? e.onMoveNextRequest(a)
              : "ArrowLeft" === a.key && e.prevSrc && e.onMovePrevRequest(a);
          },
          children: [
            e.prevSrc &&
              (0, t.jsx)("div", {
                className: (0, A.A)(
                  k().LightboxImageContainer,
                  k().LightboxPrevImage,
                ),
                children: n,
              }),
            (0, t.jsx)("div", {
              className: (0, A.A)(
                k().LightboxImageContainer,
                k().LightboxMainImage,
              ),
              tabIndex: 0,
              onClick: (a) => {
                "IMG" !== a.target.nodeName && e.onCloseRequest(a);
              },
              children: l,
            }),
            e.nextSrc &&
              (0, t.jsx)("div", {
                className: (0, A.A)(
                  k().LightboxImageContainer,
                  k().LightboxNextImage,
                ),
                children: r,
              }),
            (0, t.jsxs)("div", {
              className: k().LightboxToolbar,
              children: [
                (0, t.jsx)("span", {
                  className: k().LightboxImageTitle,
                  children: e.imageTitle,
                }),
                (0, t.jsxs)("div", {
                  className: k().LightboxToolbarButtons,
                  children: [
                    e.toolbarButtons,
                    (0, t.jsx)("button", {
                      className: k().LightboxCloseButton,
                      onClick: e.onCloseRequest,
                    }),
                  ],
                }),
              ],
            }),
            e.prevSrc &&
              (0, t.jsx)("button", {
                className: k().LightboxLeftButton,
                onClick: e.onMovePrevRequest,
              }),
            e.nextSrc &&
              (0, t.jsx)("button", {
                className: k().LightboxRightButton,
                onClick: e.onMoveNextRequest,
              }),
          ],
        });
      }
      var N = n(33561),
        y = n(86355);
      let R = 1;
      const U = (0, i.PA)((e) => {
        const {
            clanSteamID: a,
            appid: n,
            imageInsertCallBack: r,
            partnerEventStore: i,
            bHideDragAndDrop: o,
            bShowLightBox: d,
            fnSetImageURL: p,
            rgRealmList: x,
            fnLangHasData: L,
            fnGetImageHash: k,
          } = e,
          [U, B] = c.useState(""),
          [M, O] = c.useState(0),
          [P, F] = c.useState(),
          [H, z] = c.useState(void 0),
          [V, W] = c.useState(!y.pU.BHasLoadedClanImages(a)),
          [Y, q] = c.useState(void 0),
          Z = (0, S.YR)(() => s().CancelToken.source());
        c.useEffect(
          () => () => Z.cancel("ClanImagePicker component unmounted"),
          [Z],
        );
        const Q = (0, S.YR)(() => new Map()),
          [J, X] = (0, c.useState)(null),
          { bClanImagesV2: K } = (0, N.sK)(),
          $ = a.GetAccountID(),
          ee = c.useCallback(() => y.pU.LoadClanImages(a, !1, Z.token), [Z, $]),
          ae = c.useCallback(async () => {
            if (!Q.has(a.GetAccountID()))
              try {
                await ee(), Z.token.reason || W(!1);
              } catch (e) {
                let n = (0, C.H)(e);
                console.error(
                  "ClanImagePicker Failed: " +
                    n.strErrorMsg +
                    " errCode: " +
                    n.strErrorMsg,
                ),
                  Q.set(a.GetAccountID(), n.strErrorMsg),
                  Z.token.reason || (W(!1), q(n.strErrorMsg));
              }
          }, [Z, a, Q, ee]);
        c.useEffect(() => {
          V && ae();
        }, [ae, V]);
        const ne = c.useCallback(() => {
          W(!0), ae();
        }, [ae]);
        c.useEffect(() => {
          ne();
        }, [$, ne]);
        const te = y.pU.BHasLoadedClanImages(a);
        c.useEffect(() => {
          te || ne();
        }, [te, ne]);
        const le = () => {
            F(void 0), z(void 0);
          },
          re = (e) => {
            if (null != P) {
              const a = H[P];
              le(),
                requestAnimationFrame(() => {
                  r(a, e);
                });
            }
          };
        if (Y)
          return (0, t.jsxs)("div", {
            children: [
              (0, t.jsx)("div", { children: (0, E.we)("#Error_Message") }),
              (0, t.jsx)("div", { children: Y }),
              (0, t.jsx)("div", {
                className: (0, A.A)(I().Button),
                onClick: ae,
                children: (0, E.we)("#Button_Retry"),
              }),
            ],
          });
        if (V)
          return (0, t.jsx)(f.t, {
            position: "center",
            string: (0, E.we)("#Loading"),
          });
        let se = H;
        return (0, t.jsxs)("div", {
          className: (0, A.A)(h().PickerContainer),
          children: [
            (0, t.jsx)(v.g, { fnSetImageSearch: (e) => B(e) }),
            (0, t.jsx)("div", {
              className: h().ImagesContainer,
              children: (0, t.jsx)(u.GF, {
                clanAccountID: a.GetAccountID(),
                fileNameSearch: U,
                imageInsertCallBack: r,
                fnOnExpandImage: (e) => {
                  if (e && d) {
                    let n = y.pU.GetFilteredClanImages(
                        a,
                        U.trim().toLowerCase(),
                      ),
                      t = n.findIndex((a, n, t) => e.imageid == a.imageid);
                    t >= 0 && (F(t), z(n));
                  }
                },
                InternalOpenLocalizeImageGroup: (e) => {
                  (0, w.pg)(
                    (0, t.jsx)(b.$, {
                      primaryLocalizedImage: e,
                      appid: n,
                      clanSteamID: a,
                      fnSetImageURL: p,
                      rgRealmList: x,
                      fnLangHasData: L,
                      fnGetImageHash: k,
                      partnerEventStore: i,
                    }),
                    window,
                  );
                },
              }),
            }),
            M > 0 &&
              (0, t.jsx)(
                G,
                {
                  uploader: J,
                  onDismiss: () => {
                    B(U), O(null);
                  },
                },
                "clanimageuploaderview" + M,
              ),
            !o &&
              (0, t.jsx)(_.D, {
                onDropFiles: async (e) => {
                  if (e && e.length > 0) {
                    const n = K ? new m.vN(a, null) : new m.VE(a, null);
                    X(n);
                    let r = !0,
                      s = Array.from(e);
                    for (let e = 0; r && e < s.length; e++) {
                      let a = s[e];
                      (r = await n.AddImage(a, l.Bhc)),
                        r ||
                          (console.error(
                            "ClanImagePicker.OnDropFiles: failed on i=" +
                              e +
                              " file=" +
                              a.name,
                          ),
                          (0, w.pg)(
                            (0, t.jsx)(j.KG, {
                              strDescription: (0, E.we)(
                                "#ImagePicker_Error",
                                a.name,
                              ),
                            }),
                            window,
                          ));
                    }
                    return r && (B(""), O(++R), n.UploadAllImages()), r;
                  }
                  return !1;
                },
              }),
            null != P &&
              (0, t.jsx)(D, {
                mainSrc: se[P].url,
                imageTitle: se[P].file_name,
                nextSrc: P < se.length - 1 ? se[P + 1].url : void 0,
                prevSrc: P > 0 ? se[P - 1].url : void 0,
                onMoveNextRequest: () => F(P + 1),
                onMovePrevRequest: () => F(P - 1),
                onCloseRequest: le,
                toolbarButtons: [
                  (0, t.jsx)(
                    "button",
                    {
                      className: h().Full,
                      onClick: () => {
                        re(g._o.k_eInsertFullImage);
                      },
                      children: (0, E.we)("#ImagePicker_FullSize"),
                    },
                    "fullsize",
                  ),
                  (0, t.jsx)(
                    "button",
                    {
                      className: h().Full,
                      onClick: () => {
                        re(g._o.k_eInsertThumbnail);
                      },
                      children: (0, E.we)("#ImagePicker_Thumbnail"),
                    },
                    "thumbnail",
                  ),
                ],
              }),
            (0, t.jsx)(T, {}),
          ],
        });
      });
      function T(e) {
        const {
          bClanImagesV2: a,
          bClanImagesV2Allowed: n,
          setClanImagesV2: l,
        } = (0, N.sK)();
        return (0, t.jsx)(t.Fragment, {
          children:
            n &&
            l &&
            (0, t.jsx)(d.Yh, {
              className: h().EnableClanImagesV2,
              label: "New media conversion",
              checked: a,
              onChange: (e) => l(e),
            }),
        });
      }
      function G(e) {
        const { uploader: a, onDismiss: n } = e,
          [l, r] = (0, o.q3)(() => [
            a.BAllDone(),
            !a.BAllDone() && !a.BHasError(),
          ]),
          s = a.GetLastErrorFile();
        return (0, t.jsxs)("div", {
          className: h().UploaderContainer,
          children: [
            r &&
              (0, t.jsxs)("div", {
                className: h().UploaderRunning,
                children: [
                  (0, t.jsx)("div", {
                    className: h().UploaderDesc,
                    children: (0, E.we)(
                      "#ImageUpload_Desc",
                      a.GetCompletedFiles(),
                      a.GetTotalFiles(),
                    ),
                  }),
                  (0, t.jsx)("button", {
                    className: "DialogButton",
                    onClick: () => {
                      a.CancelAllUploads(), n();
                    },
                    children: (0, E.we)("#Button_Cancel"),
                  }),
                ],
              }),
            a.BHasError() &&
              s &&
              (0, t.jsxs)("div", {
                className: h().UploadMessageAndButtonsContainer,
                children: [
                  (0, t.jsx)("div", {
                    className: h().UploadError,
                    children: (0, E.we)(
                      "#ImageUpload_Error",
                      s.file.name,
                      s.status,
                      s.message,
                    ),
                  }),
                  (0, t.jsx)("button", {
                    className: h().UploadButtonCancel,
                    onClick: n,
                    children: (0, E.we)("#Button_Cancel"),
                  }),
                  (0, t.jsx)("button", {
                    className: h().UploadButtonRetry,
                    onClick: () => a.RetryAllFailedUploads(),
                    children: (0, E.we)("#Button_Retry"),
                  }),
                ],
              }),
            Boolean(l) &&
              (0, t.jsx)("div", {
                className: h().UploadMessageAndButtonsContainer,
                children: (0, t.jsx)("div", {
                  className: h().UploadSuccess,
                  children: (0, E.we)("#ImageUpload_Success"),
                }),
              }),
          ],
        });
      }
    },
    36782: (e, a, n) => {
      "use strict";
      n.d(a, { $: () => E });
      var t = n(7850),
        l = n(37085),
        r = n(41735),
        s = n.n(r),
        i = n(75844),
        o = n(90626),
        c = n(17720),
        d = n(68359),
        g = n(68255),
        m = n(9709),
        u = n(9449),
        p = n(95695),
        h = n.n(p),
        _ = n(9154),
        v = n(22797),
        x = n(68797),
        I = n(61859),
        j = n(73745),
        w = n(56692),
        f = n(26408),
        C = n(82817),
        A = n(40414);
      const E = (0, i.PA)((e) => {
        const {
            closeModal: a,
            appid: n,
            partnerEventStore: r,
            primaryLocalizedImage: i,
            clanSteamID: p,
            fnSetImageURL: E,
            rgRealmList: S,
            fnLangHasData: b,
            fnGetImageHash: L,
          } = e,
          [k, D] = o.useState(),
          [N, y] = o.useState(!0),
          [R, U] = o.useState(),
          T = (0, j.YR)(() => s().CancelToken.source());
        o.useEffect(
          () => () => T.cancel("LocalizedImageDialog component unmounted"),
          [T],
        );
        const G = o.useCallback(async () => {
            var e;
            try {
              const a = c.b.InitFromClanID(i.clanAccountID),
                n = await A.zU.AsyncGetImageResolution(
                  a,
                  i.image_hash,
                  i.file_type,
                  T,
                  !1,
                );
              if (T.token.reason) return;
              if (n.success != l.R || !n.width || !n.height)
                return (
                  console.error(
                    "LocalizedImageDialog : failed to determine the primary image resolution. " +
                      (null !== (e = n.err_msg) && void 0 !== e ? e : ""),
                  ),
                  void U({ strMsg: n.err_msg })
                );
              U(void 0), D(n);
            } catch (e) {
              let a = (0, x.H)(e);
              console.error("LocalizedImageDialog : " + a.strErrorMsg, a),
                U({ strMsg: a.strErrorMsg });
            }
          }, [T, i.clanAccountID, i.file_type, i.image_hash]),
          B = o.useCallback(async () => {
            try {
              await d.R.DetermineAvailableLocalizationForGroup(T),
                T.token.reason || y(!1);
            } catch (e) {
              let a = (0, x.H)(e);
              console.error("LocalizedImageDialog : " + a.strErrorMsg, a);
            }
          }, [T]),
          M = o.useCallback(() => G().then(B), [B, G]),
          O = o.useCallback(() => {
            U(void 0), M();
          }, [M]);
        o.useEffect(() => {
          a &&
            (d.R.SetPrimaryImageForImageGroup(i, "localized_image_group"), M());
        }, [a, M, i]);
        const P = !N && k,
          F = k ? ` - ${k.width}x${k.height}` : "",
          H = (0, C.EG)(i.file_type).slice(1),
          z =
            (null == k ? void 0 : k.width) && (null == k ? void 0 : k.height)
              ? { width: k.width, height: k.height }
              : void 0,
          V = A.zU.GenerateURLFromHashAndExt(p, A.zU.GetHashAndExt(i));
        return (0, t.jsx)(_.eV, {
          bAllowFullSize: !0,
          title: (0, I.we)("#ImagePickerLoc_Title"),
          onCancel: a,
          closeModal: a,
          children: (0, t.jsx)(g.nB, {
            children: (0, t.jsxs)(g.a3, {
              children: [
                (0, t.jsx)("div", {
                  className: h().FlexRowContainer,
                  children: (0, t.jsxs)("span", {
                    className: h().FlexColumnContainer,
                    children: [
                      (0, t.jsxs)("div", {
                        children: [
                          (0, I.we)("#ImagePickerLoc_Default"),
                          (0, t.jsx)(f.o, {
                            tooltip: (0, I.we)("#ImagePickerLoc_Default_Hint"),
                          }),
                        ],
                      }),
                      (0, t.jsx)("img", { className: m.TitleImg, src: V }),
                      (0, t.jsx)("div", {
                        children: i.file_name + F + " - " + H,
                      }),
                    ],
                  }),
                }),
                Boolean(R) &&
                  (0, t.jsxs)("div", {
                    children: [
                      (0, t.jsx)("div", {
                        children: (0, I.we)("#Error_Message"),
                      }),
                      Boolean(null == R ? void 0 : R.strMsg) &&
                        (0, t.jsx)("div", { children: R.strMsg }),
                      (0, t.jsx)("div", {
                        className: h().Button,
                        onClick: O,
                        children: (0, I.we)("#Button_Retry"),
                      }),
                    ],
                  }),
                !P &&
                  !R &&
                  (0, t.jsx)(v.t, {
                    position: "center",
                    string: (0, I.we)("#Loading"),
                  }),
                P &&
                  (0, t.jsxs)(o.Fragment, {
                    children: [
                      (0, t.jsx)(u.t, {
                        clanSteamID: p,
                        rgSupportArtwork: [],
                        localizedPrimaryImage: i,
                        forceResolution: z,
                        bAllowPreviousClanImageSelection: !0,
                        fnSetImageURL: E,
                        rgRealmList: S,
                      }),
                      (0, t.jsx)("div", {
                        className: m.ArtworkBar,
                        children: (0, t.jsx)(w.it, {
                          clanSteamID: p,
                          eventModel: void 0,
                          artworkType: "localized_image_group",
                          title: (0, I.we)("#ImagePickerLoc_Title"),
                          realms: S,
                          fnLangHasData: b,
                          appid: n,
                          fnGetImageHashAndExt: L,
                          fnSetImageURL: E,
                          partnerEventStore: r,
                        }),
                      }),
                    ],
                  }),
              ],
            }),
          }),
        });
      });
    },
    50433: (e, a, n) => {
      "use strict";
      n.d(a, { u: () => y, z: () => R });
      var t = n(7850),
        l = n(7221),
        r = n(22837),
        s = n(76217),
        i = n(46107),
        o = n(32803),
        c = n(66418),
        d = n(94734),
        g = n(42834),
        m = n(84518),
        u = n(39777),
        p = n(60014),
        h = n(38535),
        _ = n(65946),
        v = n(83392),
        x = n(90626),
        I = n(70078),
        j = n(35380),
        w = n(33924),
        f = n.n(w),
        C = n(18654),
        A = n.n(C),
        E = n(72860),
        S = n(76684),
        b = n(3088),
        L = n(12155),
        k = n(52038),
        D = n(78327);
      const N = 30;
      function y(e) {
        var a, n;
        const {
            event: s,
            imageURLOverride: d,
            bShowAssociatedApp: m,
            langOverride: p,
            onClick: w,
            eEventRount: C,
            bHidePrices: A,
            nSummaryMaxLength: E,
          } = e,
          L = (0, h.Zj)(s.appid),
          D = p || (0, r.sfN)(c.TS.LANGUAGE),
          y =
            null !==
              (a = (0, i.m0)(
                void 0 !== d ? void 0 : s,
                "capsule",
                D,
                l.wI.capsule_main,
              )) && void 0 !== a
              ? a
              : d,
          U =
            null !==
              (n = (0, i.m0)(
                void 0 !== d ? void 0 : s,
                "capsule",
                D,
                l.wI.full,
              )) && void 0 !== n
              ? n
              : d,
          [T, G, B, M] = (0, _.q3)(() => [
            s.GetNameWithFallback(D) || "",
            s.GetCategoryAsString(),
            s.GetSummaryWithFallback(D, E),
            s.GetSubTitleWithLanguageFallback(D) || "",
          ]),
          O = (0, j.$5)(s.appid),
          { data: P } = (0, u.lv)(O),
          F = [];
        if ((y && F.push(y), U && U !== y && F.push(U), P)) {
          const e = (0, g.b0)(P, "main_capsule");
          e && F.push(e);
        }
        const [H, z] = (0, x.useState)(y);
        if (!s)
          return (0, t.jsx)("div", { className: f().OtherEvents_EventCtn });
        const V = c.TS.NOW,
          W = s ? s.GetStartTimeAndDateUnixSeconds() : 0;
        let Y = M;
        return (
          M && (M.length > N || T.length > N) && (Y = void 0),
          (0, t.jsxs)("div", {
            className: f().EventSizer,
            children: [
              (0, t.jsxs)(o.tj, {
                className: (0, k.A)(
                  f().OtherEvents_EventCtn,
                  "OtherEvents_EventCtn",
                  f().HoversEnabled,
                ),
                eventModel: s,
                route: C || o.PH.k_eView,
                onClick: w,
                preferredFocus: !0,
                children: [
                  (0, t.jsxs)("div", {
                    className: (0, k.A)(
                      f().EventSummaryContainer,
                      f().HideInWideMode,
                    ),
                    children: [
                      (0, t.jsx)("div", {
                        className: f().EventSummaryType,
                        children: G,
                      }),
                      (0, t.jsx)("div", {
                        className: f().EventSummaryText,
                        children: B,
                      }),
                    ],
                  }),
                  (0, t.jsx)("div", {
                    className: f().OtherEvents_BGImage,
                    style: {
                      backgroundColor: "#ffffff",
                      backgroundImage: H ? `url(${(0, I.j3)(H)})` : "none",
                    },
                  }),
                  (0, t.jsxs)("div", {
                    className: f().OtherEvents_ContentCtn,
                    children: [
                      (0, t.jsx)("div", {
                        className: (0, k.A)(
                          f().OtherEvents_MainImageCtn,
                          L && f().MaskImages,
                        ),
                        children: (0, t.jsx)(b.c, {
                          rgSources: F,
                          onIncrementalError: (e, a, n) => {
                            n >= F.length && z(void 0), z(F[n + 1]);
                          },
                          className: f().OtherEvents_MainImage,
                          alt: "",
                        }),
                      }),
                      (0, t.jsxs)("div", {
                        className: f().OtherEvents_TextCtn,
                        children: [
                          (0, t.jsx)("div", {
                            className: f().OtherEvents_TextTitle,
                            children: T,
                          }),
                          Boolean(Y) &&
                            (0, t.jsx)("div", {
                              className: f().OtherEvents_SubTitle,
                              children: Y,
                            }),
                          (0, t.jsxs)(v.s, {
                            direction: "row",
                            gap: "3",
                            align: "center",
                            children: [
                              (0, t.jsx)("div", {
                                className: (0, k.A)(
                                  f().EventType,
                                  f().ShowInWideMode,
                                ),
                                children: G,
                              }),
                              Boolean(W > V)
                                ? (0, t.jsx)("div", {
                                    className: (0, k.A)(
                                      f().UpcomingCtn,
                                      "UpcomingCtn",
                                    ),
                                    children: (0, t.jsx)(S.K4, {
                                      bSingleLine: !0,
                                      dateAndTime:
                                        s.GetStartTimeAndDateUnixSeconds(),
                                    }),
                                  })
                                : (0, t.jsx)(S.K4, {
                                    bSingleLine: !0,
                                    bOnlyDate: !0,
                                    dateAndTime:
                                      s.GetStartTimeAndDateUnixSeconds(),
                                  }),
                            ],
                          }),
                          (0, t.jsx)("div", {
                            className: (0, k.A)(
                              f().EventSummaryText,
                              f().ShowInWideMode,
                            ),
                            children: B,
                          }),
                        ],
                      }),
                    ],
                  }),
                ],
              }),
              Boolean(m && s.appid) &&
                (0, t.jsx)(R, { appid: s.appid, bHidePrice: A }),
            ],
          })
        );
      }
      function R(e) {
        const { appid: a, bHidePrice: n } = e,
          l = (0, j.$5)(a),
          { data: r } = (0, u.J$)(l),
          { data: i } = (0, u.lv)(l),
          { data: o } = (0, u.Q_)(l),
          c = (0, p.n9)(),
          h = (0, D.Qn)();
        if (!i || !r) return null;
        const _ = o && o.hide_discount_pct_for_compliance;
        return (0, t.jsx)(m.A, {
          appID: a,
          children: (0, t.jsxs)(s.Z, {
            className: (0, k.A)(f().AppCapsuleCtn, "AppCapsuleCtn"),
            ...(0, E.S)(r, c, h, !1),
            children: [
              (0, t.jsx)(d.Q, {
                id: l,
                hoverProps: {
                  direction: "overlay",
                  style: { minWidth: "320px" },
                },
                children: (0, t.jsx)("img", {
                  className: (0, k.A)(f().AppCapsuleImage, f().CapsuleShadow),
                  src: (0, g.b0)(i, "small_capsule"),
                  alt: r.name,
                }),
              }),
              Boolean(!n && !r.is_free) &&
                (0, t.jsxs)("span", {
                  className: (0, k.A)(
                    f().AppCapsulePrice,
                    Boolean(null == o ? void 0 : o.discount_pct)
                      ? A().Discounted
                      : "",
                  ),
                  children: [
                    Boolean((null == o ? void 0 : o.discount_pct) && _) &&
                      (0, t.jsx)("div", {
                        className: A().DiscountIconCtn,
                        children: (0, t.jsx)(L.XH_, {}),
                      }),
                    Boolean((null == o ? void 0 : o.discount_pct) && !_) &&
                      (0, t.jsx)("span", {
                        className: A().StoreSaleDiscountBox,
                        children: `-${null == o ? void 0 : o.discount_pct}%`,
                      }),
                    o &&
                      o.final_price_in_cents &&
                      (0, t.jsx)("span", {
                        className: A().StoreSalePriceBox,
                        children: o.formatted_final_price,
                      }),
                  ],
                }),
            ],
          }),
        });
      }
    },
    81047: (e, a, n) => {
      "use strict";
      n.d(a, { _o: () => t, fW: () => o, fw: () => i });
      var t,
        l = n(43261),
        r = n(12611),
        s = n(40414);
      function i(e, a = !1) {
        return a
          ? `${r.lw}/${e.clanAccountID}/${s.zU.GetThumbHashAndExt(e)}`
          : `${r.lw}/${e.clanAccountID}/${s.zU.GetHashAndExt(e)}`;
      }
      function o(e, a, n) {
        let r = "";
        const s = i(a);
        if (n == t.k_eInsertVideo)
          (r = "[video webm="),
            a.file_type == l.bg.pJ && (r += s),
            (r += " mp4="),
            a.file_type == l.bg.nn && (r += s),
            (r += " autoplay=true controls=false][/video]");
        else if (n == t.k_eInsertFullImage) r = "[img]" + s + "[/img]";
        else {
          r = "[url=" + s + "][img]" + i(a, !0) + "[/img][/url]";
        }
        e.InsertText(r);
      }
      !(function (e) {
        (e[(e.k_eInsertThumbnail = 1)] = "k_eInsertThumbnail"),
          (e[(e.k_eInsertFullImage = 2)] = "k_eInsertFullImage"),
          (e[(e.k_eShowImageGroup = 3)] = "k_eShowImageGroup"),
          (e[(e.k_eInsertVideo = 4)] = "k_eInsertVideo");
      })(t || (t = {}));
    },
    68359: (e, a, n) => {
      "use strict";
      n.d(a, { R: () => h });
      var t = n(34629),
        l = n(37085),
        r = n(22837),
        s = n(41735),
        i = n.n(s),
        o = n(14947),
        c = n(17720),
        d = n(62490),
        g = n(78327),
        m = n(7221),
        u = n(40414);
      class p {
        constructor() {
          (this.m_curLocImageGroup = null),
            (this.m_curLocImageGroupType = null),
            (0, o.Gn)(this),
            "dev" == g.TS.WEB_UNIVERSE && (window.g_ClanImageStore = this);
        }
        static async BDoesClanImageFileExistsOnCDNOrOrigin(e, a, n, t) {
          let r =
              g.TS.COMMUNITY_BASE_URL +
              "gid/" +
              a.ConvertTo64BitString() +
              "/hasclanimagefile",
            s = { image_hash_and_ext: n, lang: "" + t };
          return (
            (await i().get(r, { params: s, cancelToken: e && e.token })).data
              .success == l.R
          );
        }
        SetPrimaryImageForImageGroup(e, a) {
          (this.m_curLocImageGroup &&
            this.m_curLocImageGroup.primaryImage.imageid == e.imageid &&
            a == this.m_curLocImageGroupType) ||
            ((this.m_curLocImageGroup = {
              primaryImage: e,
              localized_images: [],
            }),
            (this.m_curLocImageGroupType = a),
            (this.m_curLocImageGroup.localized_images = (0, d.$Y)(
              this.m_curLocImageGroup.localized_images,
              r.bP9,
              null,
            )));
        }
        GetPrimaryImageForImageGroup() {
          var e;
          return null === (e = this.m_curLocImageGroup) || void 0 === e
            ? void 0
            : e.primaryImage;
        }
        ClearImageGroup() {
          (this.m_curLocImageGroup = null),
            (this.m_curLocImageGroupType = null);
        }
        GetLocalizedImageGroupForEdit() {
          return this.m_curLocImageGroup;
        }
        GetLocalizedImageGroupForEditAsURL(e, a) {
          var n;
          if (this.m_curLocImageGroup) {
            let t = this.m_curLocImageGroup.primaryImage;
            return this.m_curLocImageGroup.localized_images[a]
              ? this.m_curLocImageGroup.localized_images[a]
              : u.zU.GenerateURLFromHashAndExt(
                  e,
                  null !== (n = u.zU.GetHashAndExt(t)) && void 0 !== n ? n : "",
                );
          }
          return null;
        }
        async DetermineAvailableLocalizationForGroup(e) {
          var a;
          if (!this.m_curLocImageGroup) return;
          const n = this.m_curLocImageGroup.primaryImage,
            t = c.b.InitFromClanID(n.clanAccountID),
            l = null !== (a = u.zU.GetHashAndExt(n)) && void 0 !== a ? a : "",
            s = [];
          for (let a = r.Bhc; a < r.bP9; ++a)
            s.push(p.BDoesClanImageFileExistsOnCDNOrOrigin(e, t, l, a));
          const i = await Promise.all(s);
          (0, o.h5)(() => {
            var e;
            for (let a = r.Bhc; a < r.bP9; ++a)
              i[a] &&
                (this.m_curLocImageGroup.localized_images[a] =
                  u.zU.GenerateURLFromHashAndExtAndLang(
                    t,
                    l,
                    m.wI.full,
                    a,
                    null !== (e = this.m_curLocImageGroupType) && void 0 !== e
                      ? e
                      : void 0,
                  ));
          });
        }
        SetLocalizedImageGroupAtLang(e, a, n) {
          var t;
          this.m_curLocImageGroup &&
            (this.m_curLocImageGroup.localized_images[e] = n
              ? u.zU.GenerateURLFromHashAndExtAndLang(
                  a,
                  n,
                  m.wI.full,
                  e,
                  null !== (t = this.m_curLocImageGroupType) && void 0 !== t
                    ? t
                    : void 0,
                )
              : null);
        }
        AddLocalizeImageUploaded(e, a) {
          var n;
          if (!this.m_curLocImageGroup) return;
          let t = this.m_curLocImageGroup.primaryImage;
          if ((null == t ? void 0 : t.image_hash) == e) {
            const e = c.b.InitFromClanID(t.clanAccountID),
              l = u.zU.GetHashAndExt(t);
            l &&
              (this.m_curLocImageGroup.localized_images[a] =
                u.zU.GenerateURLFromHashAndExtAndLang(
                  e,
                  l,
                  m.wI.full,
                  a,
                  null !== (n = this.m_curLocImageGroupType) && void 0 !== n
                    ? n
                    : void 0,
                ));
          }
        }
        GetAllLocalizedGroupImages() {
          return (
            (this.m_curLocImageGroup &&
              this.m_curLocImageGroup.localized_images) ||
            []
          );
        }
        GetAllLocalizedGroupImageHashAndExts() {
          return this.GetAllLocalizedGroupImages()
            .filter(Boolean)
            .map((e) => u.zU.GetHashAndExtFromURL(e));
        }
      }
      (0, t.Cg)([o.sH], p.prototype, "m_curLocImageGroup", void 0);
      const h = new p();
    },
    66033: (e, a, n) => {
      "use strict";
      function t(e) {
        if (!e) return e;
        const a = e.lastIndexOf(".");
        return -1 === a ? e : e.substring(0, a);
      }
      n.d(a, { u: () => t });
    },
    9449: (e, a, n) => {
      "use strict";
      n.d(a, { t: () => x });
      var t = n(7850),
        l = n(65946),
        r = n(90626),
        s = n(28954),
        i = n(68359),
        o = n(69343),
        c = n(86355),
        d = n(63556),
        g = n(9154),
        m = n(738),
        u = n(22797),
        p = n(68797),
        h = n(61859),
        _ = n(4033),
        v = n(2805);
      function x(e) {
        const {
            clanSteamID: a,
            rgSupportArtwork: n,
            localizedPrimaryImage: x,
            bAllowPreviousClanImageSelection: I,
            fnSetImageURL: j,
            rgRealmList: w,
          } = e,
          [f] = (0, l.q3)(() => [d.O.Get().GetCurEditLanguage()]),
          C = (0, s.zO)(a, n, x),
          A = e.uploaderOverride || C,
          [E, S] = r.useState(!1),
          b = r.useCallback(
            async (e, a) => {
              var n, l;
              if (!E) {
                S(!0);
                try {
                  const { language: a } = (0, o.jj)(
                      null !== (n = e.file_name) && void 0 !== n ? n : "",
                      f,
                    ),
                    t = (0, o.PD)(a, f, w);
                  await A.AddExistingClanImage(e, t);
                } catch (e) {
                  let a = (0, p.H)(e);
                  console.error("AddExistingClanImage: " + a.strErrorMsg, a),
                    (0, m.pg)(
                      (0, t.jsx)(g.KG, {
                        strDescription: (0, h.we)(
                          "#EventError_Code",
                          null !== (l = a.strErrorMsg) && void 0 !== l ? l : "",
                        ),
                      }),
                      window,
                    );
                }
                S(!1);
              }
            },
            [E, A, f, w],
          ),
          L = r.useMemo(
            () =>
              I
                ? [
                    [
                      (0, t.jsx)(
                        v.Hd,
                        { clanSteamID: a, OnClanImageSelected: b },
                        "clanartworkpicker",
                      ),
                    ],
                  ]
                : null,
            [b, I, a],
          );
        return (0, t.jsx)(_.O9, {
          ...e,
          imageUploader: A,
          rgRealmList: w,
          elAdditonalButtons: E
            ? [
                (0, t.jsx)(
                  u.t,
                  {
                    position: "center",
                    size: "medium",
                    string: (0, h.we)("#Loading"),
                  },
                  "throbbing",
                ),
              ]
            : L,
          fnUploadComplete: (e) => {
            var n;
            for (const t of e) {
              const e = t.uploadResult;
              if (null == e ? void 0 : e.origimagehash) {
                const a = (0, o.PD)(e.language, f, w);
                i.R.AddLocalizeImageUploaded(e.origimagehash, a);
              } else {
                const l = c.pU.GetClanImageByImageHash(
                    a,
                    null !== (n = null == e ? void 0 : e.image_hash) &&
                      void 0 !== n
                      ? n
                      : "",
                  ),
                  r = t.image.GetCurrentImageOption();
                if (l && r) {
                  const e = (0, o.PD)(t.image.language, f, w);
                  j(r.artworkType, l, e);
                }
              }
            }
          },
        });
      }
    },
    2805: (e, a, n) => {
      "use strict";
      n.d(a, { GF: () => L, Hd: () => G, ge: () => D });
      var t = n(7850),
        l = n(37085),
        r = n(43261),
        s = n(90626),
        i = n(81047),
        o = n(17720),
        c = n(86355),
        d = n(82429),
        g = n(29347),
        m = n(95695),
        u = n.n(m),
        p = n(96236),
        h = n(9154),
        _ = n(738),
        v = n(22797),
        x = n(68797),
        I = n(52038),
        j = n(56011),
        w = n(61859),
        f = n(53732),
        C = n.n(f),
        A = n(71647),
        E = n.n(A),
        S = n(36509),
        b = n(40414);
      const L = s.memo(function (e) {
        const {
          fileNameSearch: a,
          clanAccountID: n,
          imageInsertCallBack: l,
          fnOnExpandImage: r,
          showImageActions: s = !0,
          InternalOpenLocalizeImageGroup: i,
        } = e;
        return (0, t.jsx)(k, {
          clanAccountID: n,
          fileNameSearch: a,
          children: (e, a) =>
            e.map((e) =>
              (0, t.jsx)(
                N,
                {
                  clanImage: e,
                  searchStringHilight: a,
                  imageInsertCallBack: l,
                  showImageActions: s,
                  fnOnOpenLocalizedImageGroup: i,
                  OnImageClick: r,
                },
                e.imageid,
              ),
            ),
        });
      });
      function k(e) {
        const { clanAccountID: a, fileNameSearch: n, children: l } = e,
          r = (0, c.n9)(a),
          s = n.trim().toLowerCase() || "",
          i = c.pU.GetFilteredClanImagesList(r, s);
        if (0 == i.length) {
          const e = o.b.InitFromClanID(a);
          let n = c.pU.GetLoadState(e);
          return n && n.loaded
            ? (0, t.jsx)(
                "div",
                {
                  className: C().ResultNotification,
                  children:
                    s.length > 0
                      ? (0, w.we)("#ImagePicker_EmptySearch")
                      : (0, w.we)("#ImagePicker_Empty"),
                },
                "ImagePicker_Result",
              )
            : n && n.errMsg
              ? (0, t.jsx)(
                  "div",
                  {
                    className: C().ErrorCode,
                    children: (0, w.we)("#ImagePicker_Error", n.errMsg),
                  },
                  "ImagePicker_Result",
                )
              : (0, t.jsx)(
                  "div",
                  {
                    className: C().ResultNotification,
                    children: (0, w.we)("#Loading"),
                  },
                  "ImagePicker_Result",
                );
        }
        return l(i, s);
      }
      function D(e) {
        const {
          clanAccountID: a,
          fileNameSearch: n,
          onImageSelected: l,
          selectedItem: r,
        } = e;
        return (0, t.jsx)(k, {
          clanAccountID: a,
          fileNameSearch: n,
          children: (e) =>
            (0, t.jsx)("div", {
              className: C().ClanImageGrid,
              children: e.map((e) =>
                (0, t.jsx)(
                  U,
                  { clanImage: e, selected: e == r, onImageSelected: l },
                  e.imageid,
                ),
              ),
            }),
        });
      }
      function N(e) {
        const {
            clanImage: a,
            searchStringHilight: n,
            imageInsertCallBack: r,
            OnImageClick: d,
            showImageActions: m,
            fnOnOpenLocalizedImageGroup: u,
          } = e,
          [I, f] = s.useState(!1),
          A = () => r(a, i._o.k_eInsertFullImage),
          E = (e) => {
            console.log("ClanImageWrapper on delete error: " + e),
              (0, _.pg)(
                (0, t.jsx)(h.KG, {
                  strTitle: (0, w.we)("#Error_FailureNotice"),
                  strDescription: (0, w.we)("#EventDisplay_DeleteEvent_Error"),
                  children: (0, t.jsx)("p", { children: e }),
                }),
                window,
              );
          },
          S = () => {
            f(!0);
            let e = o.b.InitFromClanID(a.clanAccountID);
            c.pU
              .DeleteClanImage(e, a)
              .then((e) => {
                e.success != l.R && E((0, x.H)(e).strErrorMsg), f(!1);
              })
              .catch((e) => {
                E((0, x.H)(e).strErrorMsg), f(!1);
              }),
              L();
          },
          L = () => {},
          k = a.file_name ? a.file_name : "",
          D = (0, g.r)(n, k, String(a.imageid), C().Hilight),
          N = b.zU.BIsClanImageVideo(a),
          U = m && !I && !N,
          T = m && !I && !N,
          G = m && !I && N,
          B = m && !I && !N;
        return (0, t.jsx)(p.K, {
          placeholderHeight: "100vh",
          className: C().ImageWrapperContainer,
          rootMargin: "0px 0px 100% 0px",
          children: (0, t.jsxs)("div", {
            className: C().ImageButton,
            children: [
              (0, t.jsx)("div", {
                className: C().ImageWrapper,
                style: { backgroundImage: N ? "" : `url( '${a.thumb_url}' )` },
                draggable: !0,
                onDragStart: (e) => {
                  a.url &&
                    (e.dataTransfer.setData("text", a.url),
                    c.pU.GetClanImageDragListener().forEach((e) => {
                      e(o.b.InitFromClanID(a.clanAccountID), !0);
                    }));
                },
                onDragEnd: (e) => {
                  a.url &&
                    c.pU.GetClanImageDragListener().forEach((e) => {
                      e(o.b.InitFromClanID(a.clanAccountID), !1);
                    });
                },
                onDoubleClick: A,
                onClick: () => {
                  d && d(a);
                },
                children: (0, t.jsx)(R, {
                  clanImage: a,
                  className: C().VideoBackground,
                }),
              }),
              U &&
                (0, t.jsx)("span", {
                  className: C().Full,
                  onClick: A,
                  children: (0, w.we)("#ImagePicker_FullSize"),
                }),
              I &&
                (0, t.jsx)(v.t, {
                  size: "medium",
                  className: C().FloatingThrobber,
                }),
              T &&
                (0, t.jsx)("span", {
                  className: C().Thumb,
                  onClick: () => r(a, i._o.k_eInsertThumbnail),
                  children: (0, w.we)("#ImagePicker_Thumbnail"),
                }),
              B &&
                u &&
                (0, t.jsx)(y, {
                  bDeleting: I,
                  clanImage: a,
                  fnOnOpenLocalizedImageGroup: u,
                }),
              G &&
                (0, t.jsx)("span", {
                  className: C().Full,
                  onClick: () => r(a, i._o.k_eInsertVideo),
                  children: (0, w.we)("#ImagePicker_Video"),
                }),
              !I &&
                (0, t.jsx)("span", {
                  className: C().Delete,
                  onClick: (e) => {
                    var n, l;
                    (0, _.pg)(
                      (0, t.jsx)(h.o0, {
                        strTitle: (0, w.we)("#ImagePicker_DeleteImageTitle"),
                        strDescription: "",
                        onOK: S,
                        onCancel: L,
                        closeModal: L,
                        children: (0, t.jsxs)(s.Fragment, {
                          children: [
                            (0, t.jsx)("div", {
                              children: (0, w.we)(
                                "#ImagePicker_DeleteAreYouSure",
                                null !== (n = a.file_name) && void 0 !== n
                                  ? n
                                  : "",
                              ),
                            }),
                            (0, t.jsx)("br", {}),
                            (0, t.jsx)("br", {}),
                            (0, t.jsx)("div", {
                              children: (0, w.we)("#ImagePicker_DeleteWarning"),
                            }),
                          ],
                        }),
                      }),
                      null !== (l = (0, j.uX)(e)) && void 0 !== l ? l : window,
                    );
                  },
                  children: (0, t.jsx)("img", {}),
                }),
              (0, t.jsx)("div", {
                className: C().ImageWrapperFilename,
                title: k,
                children: D,
              }),
            ],
          }),
        });
      }
      function y(e) {
        const {
            clanImage: a,
            fnOnOpenLocalizedImageGroup: n,
            bDeleting: l,
          } = e,
          r = (0, d.Ec)(a.clanAccountID);
        return l || !r.valve_admin
          ? null
          : (0, t.jsx)("span", {
              className: (0, I.A)(C().Localized, u().ValveOnlyBackground),
              onClick: () => (null == n ? void 0 : n(a)),
              children: "(VO) " + (0, w.we)("#ImagePicker_Localized"),
            });
      }
      function R(e) {
        const { clanImage: a, className: n } = e;
        return b.zU.BIsClanImageVideo(a)
          ? (0, t.jsx)("video", {
              autoPlay: !0,
              loop: !0,
              muted: !0,
              className: n,
              children: (0, t.jsx)("source", {
                src: a.url,
                type: "video/" + (a.file_type == r.bg.nn ? "mp4" : "webm"),
              }),
            })
          : null;
      }
      function U(e) {
        const { clanImage: a, onImageSelected: n, selected: l } = e;
        return (0, t.jsxs)("div", {
          className: (0, I.A)(C().ClanImageGridItem, l && C().Selected),
          onClick: () => n(a, !1),
          onDoubleClick: () => n(a, !0),
          title: a.file_name,
          children: [
            (0, t.jsx)("div", {
              className: C().ImgCtn,
              children: b.zU.BIsClanImageVideo(a)
                ? (0, t.jsx)(R, { clanImage: a })
                : (0, t.jsx)("img", { src: a.url, loading: "lazy" }),
            }),
            (0, t.jsx)("div", { className: C().Name, children: a.file_name }),
          ],
        });
      }
      function T(e) {
        const { clanSteamID: a, closeModal: n, OnClanImageSelected: l } = e,
          r = s.useCallback(
            (e, a) => {
              null == l || l(e, a), null == n || n();
            },
            [l, n],
          ),
          [i, o] = s.useState("");
        return (0, t.jsxs)(h.o0, {
          strTitle: (0, w.we)("#ImagePicker_Images"),
          strDescription: (0, w.we)("#ImagePicker_DoubleClickToSelect"),
          bAlertDialog: !0,
          onOK: n,
          onCancel: n,
          children: [
            (0, t.jsx)(S.g, { fnSetImageSearch: o }),
            (0, t.jsx)(L, {
              clanAccountID: a.GetAccountID(),
              fileNameSearch: i,
              imageInsertCallBack: r,
              showImageActions: !1,
            }),
          ],
        });
      }
      function G(e) {
        const { clanSteamID: a, OnClanImageSelected: n } = e;
        return (0, t.jsxs)("div", {
          className: E().ImageUploadBar,
          children: [
            (0, t.jsxs)("label", {
              htmlFor: "clanimagedialog",
              children: [
                (0, t.jsxs)("span", {
                  children: [(0, w.we)("#ImagePicker_PreviousImages"), " "],
                }),
                (0, t.jsx)("span", {
                  className: E().SelectImageButton,
                  children: (0, w.we)("#ImagePicker_PreviousImages2"),
                }),
              ],
            }),
            (0, t.jsx)("input", {
              style: { display: "none" },
              id: "clanimagedialog",
              type: "button",
              onClick: (e) => {
                var l;
                (0, _.pg)(
                  (0, t.jsx)(T, { clanSteamID: a, OnClanImageSelected: n }),
                  null !== (l = (0, j.uX)(e)) && void 0 !== l ? l : window,
                );
              },
            }),
          ],
        });
      }
    },
    61924: (e, a, n) => {
      "use strict";
      n.d(a, { D: () => c });
      var t = n(7850),
        l = n(52038),
        r = n(61859),
        s = n(27650),
        i = n(71647),
        o = n.n(i);
      function c(e) {
        const {
            onDropFiles: a,
            renderDesciption: n,
            elAdditonalButtons: i,
            elOverrideDragAndDropText: c,
          } = e,
          [d, g] = (0, s.hk)(a),
          [m, u] = (0, s.Ss)(a, {
            accept: "image/png, image/jpeg, image/gif, image/webp",
            multiple: !0,
          });
        return (0, t.jsxs)("div", {
          ...d,
          className: (0, l.A)(
            g ? o().DragAndDropContainerDragging : o().DragAndDropContainer,
            "DragAndDropContainer",
          ),
          children: [
            Boolean(n) && n(),
            (0, t.jsx)("div", {
              children: c || (0, r.we)("#ImagePicker_DragAndDrop"),
            }),
            (0, t.jsxs)("div", {
              className: o().ImageUploadBar,
              children: [
                m,
                (0, t.jsxs)("label", {
                  onClick: u,
                  children: [
                    (0, t.jsxs)("span", {
                      children: [(0, r.we)("#ImagePicker_OrBrowse"), " "],
                    }),
                    (0, t.jsx)("span", {
                      className: o().SelectImageButton,
                      children: (0, r.we)("#selectimage_select_file"),
                    }),
                  ],
                }),
              ],
            }),
            i,
            e.children,
          ],
        });
      }
    },
    36509: (e, a, n) => {
      "use strict";
      n.d(a, { g: () => i });
      var t = n(7850),
        l = n(90626),
        r = n(61859),
        s = n(49460);
      function i(e) {
        const { fnSetImageSearch: a } = e,
          n = (0, l.useRef)(null);
        return (0, t.jsx)("div", {
          className: s.PickerTitle,
          children: (0, t.jsx)("input", {
            ref: n,
            className: s.SearchInput,
            type: "text",
            placeholder: (0, r.we)("#ImagePicker_Search"),
            onChange: (e) => a(e.currentTarget.value),
            onKeyDown: (e) => {
              "Escape" == e.key && (a(""), n.current && (n.current.value = ""));
            },
          }),
        });
      }
    },
    4033: (e, a, n) => {
      "use strict";
      n.d(a, { O9: () => N, PY: () => y, fY: () => R });
      var t = n(7850),
        l = n(65946),
        r = n(75844),
        s = n(90626),
        i = n(22837),
        o = n(2160),
        c = n(43261),
        d = n(64953),
        g = n(69343),
        m = n(63556),
        u = n(68255),
        p = n(61924),
        h = n(95695),
        _ = n.n(h),
        v = n(9154),
        x = n(738),
        I = n(64641),
        j = n.n(I),
        w = n(12155),
        f = n(22797),
        C = n(68797),
        A = n(52038),
        E = n(56011),
        S = n(66331),
        b = n(61859),
        L = n(27344),
        k = n.n(L),
        D = n(78511);
      function N(e) {
        const {
            imageUploader: a,
            fnUploadComplete: n,
            elOverrideDragAndDropText: r,
            forceResolution: i,
            elAdditonalButtons: o,
            rgRealmList: c,
          } = e,
          [d, u] = (0, l.q3)(() => [
            a.GetUploadImages(),
            m.O.Get().GetCurEditLanguage(),
          ]),
          h = s.useCallback(
            async (e) => {
              var n;
              let l = Array.from(e),
                r = !0;
              for (let e = 0; e < l.length; e++) {
                const s = l[e],
                  { language: i } = (0, g.jj)(null == s ? void 0 : s.name, u);
                try {
                  const n = (0, g.PD)(i, u, c);
                  (r = await a.AddImageForLanguage(s, n)),
                    r ||
                      (console.error(
                        "ImageUploaderPanel.OnDropFiles: failed on i=" +
                          e +
                          " file=" +
                          s.name,
                      ),
                      (0, x.pg)(
                        (0, t.jsx)(v.KG, {
                          strDescription: (0, b.we)(
                            "#ImagePicker_Error",
                            s.name,
                          ),
                        }),
                        window,
                      ));
                } catch (e) {
                  let a = (0, C.H)(e);
                  console.error(
                    "ImageUploaderPanel.OnDropFiles: " + a.strErrorMsg,
                    a,
                  ),
                    (0, x.pg)(
                      (0, t.jsx)(v.KG, {
                        strDescription: (0, b.we)(
                          "#EventError_Code",
                          null !== (n = a.strErrorMsg) && void 0 !== n ? n : "",
                        ),
                      }),
                      window,
                    );
                }
              }
              return r;
            },
            [u, a, c],
          ),
          _ = s.useMemo(
            () =>
              o instanceof Array
                ? o
                : [
                    (0, t.jsx)(
                      s.Fragment,
                      { children: o },
                      "elAdditonalButtons",
                    ),
                  ],
            [o],
          );
        (0, l.q3)(() =>
          d.map((e) => ({ a: e.GetCurrentImageOption(), b: e.language })),
        );
        return (0, t.jsxs)(p.D, {
          onDropFiles: h,
          elAdditonalButtons: _,
          elOverrideDragAndDropText: r,
          children: [
            (0, t.jsx)(s.Fragment, {
              children: (0, t.jsx)("div", {
                className: k().UploadPreviewCtn,
                children: d.map((e) =>
                  (0, t.jsx)(
                    U,
                    {
                      asset: e,
                      forceResolution: i,
                      fnOnRemove: () => a.DeleteUploadImage(e),
                      languageRealms: c,
                    },
                    "arttabupload_" + e.filename + "_" + e.uploadTime,
                  ),
                ),
              }),
            }),
            (0, t.jsx)(y, {
              imageUploader: a,
              fnOnUploadImageRequested: async () => {
                const e = await a.UploadAllImages(i);
                null == n || n(e);
              },
            }),
          ],
        });
      }
      function y(e) {
        const { imageUploader: a, fnOnUploadImageRequested: n } = e,
          [r] = (0, l.q3)(() => [a.GetUploadImages()]),
          s = r.some((e) => "pending" == e.status),
          i = r.some(
            (e) =>
              "waiting" == e.status ||
              "uploading" == e.status ||
              "processing" == e.status,
          );
        return (0, t.jsxs)("div", {
          style: { display: "flex" },
          className: k().UploadPreviewButtonsCtn,
          children: [
            Boolean(r.length) &&
              (0, t.jsx)(u.$n, {
                style: { margin: "8px" },
                onClick: n,
                disabled: !s,
                children: (0, b.we)("#ImageUpload_Upload"),
              }),
            Boolean(r.length) &&
              (0, t.jsx)(u.$n, {
                style: { margin: "8px" },
                onClick: a.ClearImages,
                disabled: i,
                children: (0, b.we)("#ImageUpload_Clear"),
              }),
          ],
        });
      }
      function R(e, a, n, l, r) {
        let s = new Array();
        return (
          e.GetUploadImages().forEach((i) => {
            s.push(
              (0, t.jsx)(
                U,
                {
                  asset: i,
                  forceResolution: n,
                  forceFileType: l,
                  fnOnRemove: () => e.DeleteUploadImage(i),
                  languageRealms: r,
                },
                a + i.file + "_" + i.uploadTime,
              ),
            );
          }),
          s
        );
      }
      const U = (0, r.PA)(function (e) {
        var a, n, l, r, s;
        const { asset: g, fnOnRemove: m, languageRealms: p } = e,
          h =
            null === (a = g.ImageOptions) || void 0 === a
              ? void 0
              : a
                  .map((e) => {
                    let a,
                      n,
                      t = null == e ? void 0 : e.fnGetLabelText();
                    e.bEnforceDimensions && (t += ` - ${e.width}x${e.height}`),
                      e.bDeprecated &&
                        ((t += ` ${(0, b.we)("#ImageUpload_Deprecated")}`),
                        (a = (0, b.we)("#ImageUpload_Deprecated_ttip"))),
                      (g.BIsOriginalMinimumDimensions(e) &&
                        g.FileTypeMatchesImageTypes(e)) ||
                        (n = k().ImageDimensionTooSmall);
                    return { label: t, data: e, strOptionClass: n, tooltip: a };
                  })
                  .filter((e) => !e.data.bHiddenFromDropdown),
          v = {
            pending: (0, b.we)("#ImageUpload_Pending"),
            waiting: (0, b.we)("#ImageUpload_Waiting"),
            uploading: (0, b.we)("#ImageUpload_Uploading"),
            processing: (0, b.we)("#ImageUpload_Processing"),
            success: (0, b.we)("#ImageUpload_SuccessCard"),
            failed: (0, b.we)("#ImageUpload_Failed"),
          },
          I = g.BSupportsLanguages()
            ? (function (e) {
                const a = [],
                  n = new Array();
                for (const a of e) {
                  if (a == i.X51) continue;
                  const e = (0, b.we)("#Language_" + (0, i.LgB)(a));
                  n.push({ label: e, data: a });
                }
                return (
                  n.sort((e, a) => e.label.localeCompare(a.label)),
                  n.forEach((e) => a.push({ label: e.label, data: e.data })),
                  n
                );
              })(
                b.A0.GetLanguageListForRealms(
                  null != p ? p : [o.TU.k_ESteamRealmGlobal],
                ),
              )
            : null,
          C = g.IsValidAssetType(e.forceResolution, e.forceFileType),
          E = "pending" == g.status;
        let L = v[g.status];
        "pending" == g.status &&
          (C.needsCrop
            ? (L = (0, b.we)("#ImageUpload_NeedsCrop"))
            : C.error && (L = (0, b.we)("#ImageUpload_Invalid")));
        let N;
        const y = g.GetCurrentImageOption();
        y &&
          (N =
            null ===
              (n = null == h ? void 0 : h.find((e) => e.data.sKey == y.sKey)) ||
            void 0 === n
              ? void 0
              : n.data);
        N ||
          (N =
            null === (l = null == h ? void 0 : h[0]) || void 0 === l
              ? void 0
              : l.data);
        return (0, t.jsxs)("div", {
          className: k().UploadPreview,
          children: [
            (0, t.jsx)("div", {
              className: k().UploadPreviewDelete,
              onClick: () => m(g),
              children: (0, t.jsx)(w.sED, {}),
            }),
            (0, t.jsx)(T, { asset: g }),
            I &&
              (0, t.jsx)(u.m, {
                strDropDownClassName: _().DropDownScroll,
                rgOptions: I,
                selectedOption: g.language,
                onChange: (e) => (g.language = e.data),
                disabled: !E,
              }),
            h &&
              (null == h ? void 0 : h.length) > 1 &&
              (0, t.jsx)(u.m, {
                label: g.GetImageOptionLabel(),
                rgOptions: h,
                selectedOption: N,
                onChange: (e) => g.SetCurrentImageOption(e.data),
                disabled: !E,
              }),
            E &&
              (null === (r = C.warnings) || void 0 === r
                ? void 0
                : r.map((e, a) =>
                    (0, t.jsx)(
                      "div",
                      { className: k().UploadPreviewWarning, children: e },
                      `warning${a}`,
                    ),
                  )),
            E &&
              (null === (s = C.messages) || void 0 === s
                ? void 0
                : s.map((e, a) =>
                    (0, t.jsx)(
                      "div",
                      { className: k().UploadPreviewMessage, children: e },
                      `message${a}`,
                    ),
                  )),
            (0, t.jsxs)("div", {
              className: (0, A.A)({
                [_().FlexColumnContainer]: !0,
                [k().UploadPreviewError]: "failed" == g.status,
              }),
              children: [
                L,
                (0, D.o)(g.status) &&
                  (0, t.jsx)("div", {
                    className: j().FlexCenter,
                    children: (0, t.jsx)(f.t, { size: "small" }),
                  }),
              ],
            }),
            (0, t.jsx)("div", {
              className: k().UploadPreviewError,
              children: g.message,
            }),
            E &&
              C.error &&
              (0, t.jsx)("div", {
                className: k().UploadPreviewError,
                children: C.error,
              }),
            E &&
              C.needsCrop &&
              (0, t.jsx)(u.jn, {
                onClick: () =>
                  ((a) => {
                    if (a instanceof d.M7) {
                      a.ResetImage();
                      const n = window,
                        l = (0, t.jsx)(S.q, {
                          ownerWin: n,
                          uploadFile: a,
                          forceResolution: e.forceResolution,
                          fileType: e.forceFileType || c.bg.dU,
                        });
                      (0, x.HT)(l, n, "CropModal", {
                        strTitle: (0, b.we)("#ImageUpload_CropModalTitle"),
                      });
                    } else
                      console.log(
                        "ImageUploadEmbeddedDialog trying to crop non image",
                        a.fileType,
                        JSON.stringify(a.GetCurrentImageOption()),
                      );
                  })(g),
                children: (0, b.we)("#ImageUpload_OpenEditor"),
              }),
          ],
        });
      });
      function T(e) {
        const { asset: a } = e;
        return a.BIsVideo()
          ? (0, t.jsxs)("div", {
              className: k().PreviewImgCtn,
              onClick: (e) =>
                (0, x.pg)((0, t.jsx)(G, { asset: a }), (0, E.uX)(e)),
              children: [
                (0, t.jsxs)("span", {
                  className: k().PreviewImgInfo,
                  children: [a.width, " x ", a.height],
                }),
                (0, t.jsx)("video", {
                  height: 120,
                  controls: !1,
                  autoPlay: !0,
                  loop: !0,
                  muted: !0,
                  children: (0, t.jsx)("source", { src: a.dataUrl }),
                }),
              ],
            })
          : (0, t.jsx)("div", {
              className: k().PreviewImgCtn,
              style: { backgroundImage: `url(${a.dataUrl})` },
              children: (0, t.jsxs)("span", {
                className: k().PreviewImgInfo,
                children: [a.width, " x ", a.height],
              }),
            });
      }
      function G(e) {
        const { asset: a, closeModal: n } = e;
        return (0, t.jsx)(v.o0, {
          bAlertDialog: !0,
          closeModal: n,
          bAllowFullSize: !0,
          children: (0, t.jsx)("video", {
            controls: !0,
            autoPlay: !0,
            loop: !0,
            muted: !0,
            children: (0, t.jsx)("source", { src: a.dataUrl }),
          }),
        });
      }
    },
    56692: (e, a, n) => {
      "use strict";
      n.d(a, { uE: () => re, it: () => se });
      var t = n(34629),
        l = n(7850),
        r = n(22837),
        s = n(14947),
        i = n(65946),
        o = n(75844),
        c = n(90626),
        d = n(73744),
        g = n(63556),
        m = n(55263),
        u = n(68255),
        p = n(25359),
        h = n.n(p),
        _ = n(93016),
        v = n(56330),
        x = n(95695),
        I = n.n(x),
        j = n(9154),
        w = n(738),
        f = n(12155),
        C = n(32754),
        A = n(52038),
        E = n(56011),
        S = n(61859),
        b = n(73745),
        L = n(78327),
        k = n(66033),
        D = n(40414),
        N = n(68359),
        y = n(7221),
        R = n(65606),
        U = n(55294),
        T = n(11260),
        G = n(50433),
        B = n(9709),
        M = n(45832),
        O = n(76217),
        P = n(3088),
        F = n(10224),
        H = n(15496),
        z = n.n(H),
        V = n(4703);
      function W(e) {
        var a;
        const {
            event: n,
            spotlightURLOverride: t,
            fnHandleOpenEvent: s,
            fnImageFailureCallback: o,
            fnFilterImageURLsForKnownFailures: d,
            langOverride: g,
          } = e,
          m = (0, F.c5)(),
          u = c.useCallback(
            (e) => {
              e.preventDefault(), s && s(n);
            },
            [n, s],
          ),
          p = g || (0, r.sfN)(L.TS.LANGUAGE),
          [h, _, v] = (0, i.q3)(() => [
            n.GetSummaryWithFallback(p),
            n.GetNameWithFallback(p),
            n.BShowLibrarySpotlightText(),
          ]);
        let x = "spotlight",
          I = y.wI.spotlight_main;
        (2434320 != n.appid && L.TS.EUNIVERSE != r.Rv) ||
          ((x = m
            ? "localized_store_app_spotlight_mobile"
            : "localized_store_app_spotlight"),
          (I = y.wI.full));
        let j =
          null !== (a = (0, V.WC)(void 0 !== t ? void 0 : n, x, p, I)) &&
          void 0 !== a
            ? a
            : void 0 !== t
              ? [t]
              : [];
        d && j && (j = d(j));
        const w = h.replace(/https:\/\/[^ ]*/gi, "").trimLeft();
        return (0, l.jsx)(c.Fragment, {
          children: (0, l.jsx)("div", {
            className: z().MajorEvent_Ctn,
            ref: e.containerRef,
            children: (0, l.jsxs)(O.Z, {
              className: (0, A.A)(
                z().AppDetailsSpotlightContainer,
                z().MajorEventContainer,
              ),
              onActivate: u,
              focusable: !0,
              children: [
                (0, l.jsx)("div", {
                  className: z().MajorEventBackground,
                  children: (0, l.jsx)(P.c, {
                    className: z().MajorEventImageBackgroundBlur,
                    rgSources: j,
                    onIncrementalError: (e, a, n) => o && o(a),
                  }),
                }),
                (0, l.jsxs)("div", {
                  className: z().MajorEventImageContainer,
                  children: [
                    (0, l.jsx)(P.c, {
                      className: z().MajorEventImage,
                      rgSources: j,
                      onIncrementalError: (e, a, n) => o && o(a),
                    }),
                    (0, l.jsx)("div", {
                      className: z().MajorEventImageTemplate,
                    }),
                    (0, l.jsx)("div", {
                      className: z().MajoreEventImageContentContainer,
                      children:
                        v &&
                        (0, l.jsxs)("div", {
                          className: z().MajorEventContent,
                          children: [
                            (0, l.jsx)(P.c, {
                              className: z().MajorEventSpotlightBackground,
                              rgSources: j,
                              onIncrementalError: (e, a, n) => o && o(a),
                            }),
                            (0, l.jsxs)("div", {
                              className: z().MajorEventTextCtn,
                              children: [
                                (0, l.jsx)("div", {
                                  className: z().MajorEventTitle,
                                  children: _,
                                }),
                                (0, l.jsx)("div", {
                                  className: z().MajorEventSummary,
                                  children: w,
                                }),
                              ],
                            }),
                          ],
                        }),
                    }),
                  ],
                }),
                (0, l.jsx)("div", { className: z().BottomShadow }),
              ],
            }),
          }),
        });
      }
      var Y = n(79949),
        q = n.n(Y);
      function Z(e) {
        var a;
        const {
            langOverride: n,
            artworkType: t,
            fnOnLanguagePreviewChange: r,
            clanSteamID: s,
            eventModel: o,
            partnerEventStore: c,
            fnOnRemoveImage: d,
            fnOnArtworkLangChange: g,
            realms: m,
            fnLangHasData: u,
            fnGetImageHashAndExt: p,
          } = e,
          _ = p(t, n),
          v = _
            ? D.zU.GenerateURLFromHashAndExtAndLang(s, _, y.wI.full, n)
            : "",
          [x] = (0, i.q3)(() => [te(t, p)]);
        return 0 == x
          ? (0, l.jsxs)("div", {
              className: h().ImagePreviewContainer,
              children: [
                "capsule" === t &&
                  (0, l.jsx)(J, {
                    imgURL:
                      L.TS.IMG_URL + "events/defaults/default_img_cover.jpg",
                    eventModel: o,
                  }),
                "background" === t &&
                  (0, l.jsx)(X, {
                    imgURL:
                      L.TS.IMG_URL + "events/defaults/default_img_header.jpg",
                    lang: n,
                    eventModel: o,
                    partnerEventStore: c,
                  }),
                Boolean(
                  [
                    "spotlight",
                    "localized_store_app_spotlight",
                    "localized_store_app_spotlight_mobile",
                  ].includes(t),
                ) &&
                  (0, l.jsx)(Q, {
                    langOverride: n,
                    artworkType: t,
                    eventModel: o,
                  }),
                (0, l.jsx)("div", {
                  children: (0, S.we)("#EventEditor_ArtworkMissing"),
                }),
              ],
            })
          : (0, l.jsxs)("div", {
              className: h().ImagePreviewContainer,
              children: [
                "capsule" === t &&
                  (0, l.jsx)(J, { imgURL: v, eventModel: o, langOverride: n }),
                "background" === t &&
                  (0, l.jsx)(X, {
                    imgURL: v,
                    lang: n,
                    eventModel: o,
                    partnerEventStore: c,
                  }),
                "spotlight" === t &&
                  (0, l.jsx)(K, { imgURL: v, event: o, lang: n }),
                "localized_store_app_spotlight" === t &&
                  (0, l.jsx)(K, { imgURL: v, event: o, lang: n }),
                "localized_store_app_spotlight_mobile" === t &&
                  (0, l.jsx)(K, { imgURL: v, event: o, lang: n }),
                ("broadcast_left" === t || "broadcast_right" === t) &&
                  (0, l.jsx)($, {
                    imgURL: v,
                    side: "broadcast_right" === t ? "right" : "left",
                  }),
                "sale_header" === t && (0, l.jsx)(ee, { imgURL: v }),
                "sale_overlay" === t && (0, l.jsx)(ae, { imgURL: v }),
                y.pb.includes(t) &&
                  (0, l.jsx)("img", {
                    className: B.PreviewImg,
                    src:
                      null !==
                        (a = N.R.GetLocalizedImageGroupForEditAsURL(s, n)) &&
                      void 0 !== a
                        ? a
                        : void 0,
                  }),
                "product_banner" === t && (0, l.jsx)(ne, { imgURL: v }),
                "product_mobile_banner" === t && (0, l.jsx)(ne, { imgURL: v }),
                "sale_logo" === t && (0, l.jsx)(ne, { imgURL: v }),
                "bestofyear_banner" === t && (0, l.jsx)(ne, { imgURL: v }),
                "bestofyear_banner_mobile" === t &&
                  (0, l.jsx)(ne, { imgURL: v }),
                (0, l.jsx)(M.h, {
                  langOverride: n,
                  clanSteamID: s,
                  fnOnLanguagePreviewChange: r,
                  fnOnRemoveImage: d,
                  fnOnArtworkLangChange: g,
                  realms: m,
                  fnLangHasData: u,
                  fnGetImageHash: (e) => {
                    var a;
                    return (0, k.u)(
                      null !== (a = p(t, e)) && void 0 !== a ? a : "",
                    );
                  },
                }),
              ],
            });
      }
      function Q(e) {
        const { artworkType: a, langOverride: n, eventModel: t } = e,
          r = d.Fj[a],
          s = c.useMemo(
            () =>
              (function (e, a) {
                const n = 780,
                  t = 200,
                  l = document.createElement("canvas");
                (l.width = n), (l.height = t);
                const r = l.getContext("2d"),
                  s = 20;
                for (let e = 0; e < t; e += s)
                  for (let a = 0; a < n; a += s)
                    (r.fillStyle =
                      (a / s + e / s) % 2 == 0 ? "#a405e3ff" : "#000000"),
                      r.fillRect(a, e, s, s);
                const i = r.createLinearGradient(0, 0, n, 0);
                i.addColorStop(0, "rgba(32,32,32,0.8)"),
                  i.addColorStop(1, "rgba(60,60,60,0.8)"),
                  (r.fillStyle = i),
                  r.fillRect(0, 0, n, t);
                const o = r.createRadialGradient(
                  n / 2,
                  t / 2,
                  0,
                  n / 2,
                  t / 2,
                  Math.max(n, t) / 1.2,
                );
                o.addColorStop(0, "rgba(0,0,0,0)"),
                  o.addColorStop(1, "rgba(0,0,0,0.6)"),
                  (r.fillStyle = o),
                  r.fillRect(0, 0, n, t),
                  (r.fillStyle = "#fff"),
                  (r.font = "32px Arial"),
                  (r.textAlign = "center"),
                  (r.textBaseline = "middle"),
                  r.fillText(e, n / 2, t / 2 - 20),
                  a &&
                    ((r.font = "18px Arial"), r.fillText(a, n / 2, t / 2 + 25));
                return l.toDataURL("image/png");
              })(
                (0, S.we)("#EventEditor_ArtworkType_" + a),
                `${r.width} X ${r.height}`,
              ),
            [r.height, r.width, a],
          );
        return (0, l.jsx)(K, { lang: n, imgURL: s, event: t });
      }
      function J(e) {
        const { imgURL: a, eventModel: n, langOverride: t } = e,
          r = (0, g.E)();
        return (0, l.jsx)("div", {
          style: { display: "flex", width: "304px" },
          children: (0, l.jsx)(G.u, {
            event: n,
            imageURLOverride: a,
            langOverride: null != t ? t : r,
          }),
        });
      }
      function X(e) {
        const { lang: a, eventModel: n, partnerEventStore: t } = e,
          s = (0, R.LJ)(),
          [o, d, m, u, p] = (0, i.q3)(() => [
            n.GetNameWithFallback(a),
            n.GetDescriptionWithFallback(a),
            n.GetSubTitleWithLanguageFallback(a),
            n.type,
            n.AnnouncementGID,
          ]);
        let h = d
          ? (0, l.jsx)(T.fh, {
              text: d || "",
              partnerEventStore: t,
              showErrorInfo: !1,
              event: n,
              languageOverride: g.O.Get().GetCurEditLanguage(),
            })
          : (0, S.we)("#selectimage_display_event_body");
        return (0, l.jsxs)("div", {
          className: q().MultipleExampleContainer,
          children: [
            (0, l.jsx)("div", {
              className: q().ExampleSectionTitle,
              children: (0, S.we)("#selectimage_preview_title_1"),
            }),
            (0, l.jsx)("div", {
              className: (0, A.A)(q().DetailPageExample, "DetailPageExample"),
              children: (0, l.jsxs)("div", {
                className: q().DetailExample,
                children: [
                  (0, l.jsx)("div", {
                    className: q().MainImageCtn,
                    children: (0, l.jsx)("img", { src: e.imgURL }),
                  }),
                  (0, l.jsx)("div", {
                    className: q().ExampleBodyPosition,
                    children: (0, l.jsxs)("div", {
                      className: q().ExampleContentCtn,
                      children: [
                        (0, l.jsx)("div", {
                          className: q().TextTitle,
                          children:
                            o || (0, S.we)("#selectimage_display_event_title"),
                        }),
                        (0, l.jsx)("div", {
                          className: q().TextSubTitle,
                          children:
                            m ||
                            (0, S.we)("#selectimage_display_event_subtitle"),
                        }),
                        (0, l.jsx)("div", {
                          className: q().TextBody,
                          children: h,
                        }),
                      ],
                    }),
                  }),
                ],
              }),
            }),
            u != r.Fwr &&
              (0, l.jsxs)(c.Fragment, {
                children: [
                  (0, l.jsx)("div", { className: q().ExampleSpacer }),
                  (0, l.jsx)("div", {
                    className: q().ExampleSectionTitle,
                    children: (0, S.we)("#selectimage_preview_title_2"),
                  }),
                  (0, l.jsx)("div", {
                    className: (0, A.A)(
                      q().DetailPageExample,
                      "DetailPageExample",
                    ),
                    children: (0, l.jsx)("div", {
                      className: q().DetailExample2,
                      children: (0, l.jsx)(
                        U.He,
                        {
                          event: n,
                          emoticonStore: s,
                          partnerEventStore: t,
                          headerClassnames: "editor",
                          langOverride: a,
                          bDisableBroadcastPlayer: !0,
                        },
                        p,
                      ),
                    }),
                  }),
                ],
              }),
          ],
        });
      }
      const K = (e) => {
          var a;
          const [n] = (0, m.t7)(e.event.appid, { include_assets: !0 });
          if (!n) return null;
          const t = n.GetName(),
            r =
              null === (a = n.GetAssets()) || void 0 === a
                ? void 0
                : a.GetCommunityIconURL();
          return (0, l.jsx)("div", {
            className: q().SpotlightExample,
            children: (0, l.jsx)(W, {
              event: e.event,
              strDisplayName: null != t ? t : "",
              gameIconUrl: r,
              spotlightURLOverride: e.imgURL,
              langOverride: e.lang,
            }),
          });
        },
        $ = (e) => {
          const a = [
            (0, l.jsx)("img", { src: e.imgURL }, "img"),
            (0, l.jsx)("div", { className: h().BroadcastPreview }, "video"),
          ];
          return (
            "right" === e.side && a.reverse(),
            (0, l.jsx)("div", {
              className: q().BroadcastPreviewContainer,
              children: a,
            })
          );
        },
        ee = (e) =>
          (0, l.jsx)("div", {
            className: q().SaleHeaderPreviewContainer,
            children: (0, l.jsx)("img", {
              style: { width: "100%" },
              src: e.imgURL,
            }),
          }),
        ae = (e) =>
          (0, l.jsx)("div", {
            className: q().SaleHeaderPreviewContainer,
            children: (0, l.jsx)("img", {
              style: { width: "100%" },
              src: e.imgURL,
            }),
          }),
        ne = (e) =>
          (0, l.jsx)("div", {
            className: q().SaleHeaderPreviewContainer,
            children: (0, l.jsx)("img", {
              style: { width: "100%" },
              src: e.imgURL,
            }),
          });
      function te(e, a) {
        var n, t;
        let l = 0;
        for (let s = r.Bhc; s < r.bP9; ++s)
          (null !==
            (t = null === (n = a(e, s)) || void 0 === n ? void 0 : n.length) &&
          void 0 !== t
            ? t
            : 0) > 0 && (l += 1);
        return l;
      }
      const le =
        "https://partner.steamgames.com/doc/store/localization#supported_languages";
      var re;
      function se(e) {
        var a, n;
        const {
            artworkType: t,
            headerHint: s,
            appid: o,
            fnToggleMinimize: c,
            realms: g,
            eventModel: p,
            fnLangHasData: _,
            fnGetImageHashAndExt: x,
            fnSetImageURL: j,
            partnerEventStore: b,
          } = e,
          [k] = (0, m.t7)(o, { include_assets: !0 }),
          [D, N] = (0, i.q3)(() => [
            null == p ? void 0 : p.GetEventType(),
            null == p ? void 0 : p.BHasTag("vo_marketing_message"),
          ]),
          y = D == r.ajI;
        let R = null;
        s === re.k_Required
          ? (R = (0, l.jsx)("span", {
              style: { color: "#C6512B" },
              children: (0, S.we)("#EventEditor_Required"),
            }))
          : s === re.k_Suggested
            ? (R = (0, l.jsx)("span", {
                style: { color: "#D7BC86" },
                children: (0, S.we)("#EventEditor_Suggested"),
              }))
            : s === re.k_Requested &&
              (R = (0, l.jsx)("span", {
                style: { color: "#D7BC86" },
                children: (0, S.we)("#EventEditor_Requested"),
              }));
        let U = null;
        "capsule" === t
          ? (U = y
              ? (0, l.jsxs)(l.Fragment, {
                  children: [
                    (0, l.jsxs)("p", {
                      children: [
                        (0, l.jsx)("strong", {
                          children: (0, S.we)("#selectimage_tip_design_title"),
                        }),
                        ": ",
                        (0, S.we)("#selectimage_tip_capsule_creatorhome_1"),
                      ],
                    }),
                    (0, l.jsxs)("p", {
                      children: [
                        (0, l.jsx)("strong", {
                          children: (0, S.we)("#selectimage_tip_usage_title"),
                        }),
                        ": ",
                        (0, S.we)("#selectimage_tip_capsule_creatorhome_2"),
                      ],
                    }),
                  ],
                })
              : (0, l.jsxs)(l.Fragment, {
                  children: [
                    Boolean(N) &&
                      (0, l.jsxs)("div", {
                        className: h().HighlightBox,
                        children: [
                          (0, l.jsx)("p", {
                            children: (0, S.we)("#PartnerEvent_MM_ArtworkTip"),
                          }),
                          (0, l.jsx)("p", {
                            children: (0, l.jsx)("a", {
                              href: `${L.TS.PARTNER_BASE_URL}doc/store/assets/promos#popup_update`,
                              children: (0, S.we)("#PartnerEvent_MM_LearnMore"),
                            }),
                          }),
                        ],
                      }),
                    (0, l.jsxs)("p", {
                      children: [
                        (0, l.jsx)("strong", {
                          children: (0, S.we)("#selectimage_tip_design_title"),
                        }),
                        ": ",
                        (0, S.we)("#selectimage_tip_capsule_1"),
                      ],
                    }),
                    (0, l.jsxs)("p", {
                      children: [
                        (0, l.jsx)("strong", {
                          children: (0, S.we)("#selectimage_tip_usage_title"),
                        }),
                        ": ",
                        (0, S.we)("#selectimage_tip_capsule_2"),
                      ],
                    }),
                  ],
                }))
          : "background" === t
            ? (U = (0, l.jsx)(l.Fragment, {
                children: (0, l.jsxs)("p", {
                  children: [
                    (0, l.jsx)("strong", {
                      children: (0, S.we)("#selectimage_tip_design_title"),
                    }),
                    ": ",
                    (0, S.we)("#selectimage_tip_background_1"),
                  ],
                }),
              }))
            : "spotlight" === t || "localized_store_app_spotlight" === t
              ? (U = (0, l.jsx)(l.Fragment, {
                  children: (0, l.jsxs)("p", {
                    children: [
                      (0, l.jsx)("strong", {
                        children: (0, S.we)("#selectimage_tip_usage_title"),
                      }),
                      ": ",
                      (0, S.we)("#selectimage_tip_store_spotlight_1"),
                    ],
                  }),
                }))
              : "localized_store_app_spotlight_mobile" === t
                ? (U = (0, l.jsx)(l.Fragment, {
                    children: (0, l.jsxs)("p", {
                      children: [
                        (0, l.jsx)("strong", {
                          children: (0, S.we)("#selectimage_tip_usage_title"),
                        }),
                        ": ",
                        (0, S.we)("#selectimage_tip_store_mobile_spotlight"),
                      ],
                    }),
                  }))
                : "broadcast_left" === t || "broadcast_right" === t
                  ? (U = (0, l.jsx)(l.Fragment, {
                      children: (0, l.jsx)("p", {
                        children: (0, S.we)("#selectimage_tip_broadcast_1"),
                      }),
                    }))
                  : "sale_header" === t
                    ? (U = (0, l.jsxs)(l.Fragment, {
                        children: [
                          (0, l.jsx)("div", {
                            className: I().EventElementRequired,
                            children: (0, S.we)(
                              "#selectimage_tip_required_title",
                            ),
                          }),
                          (0, l.jsxs)("p", {
                            children: [
                              (0, l.jsx)("b", {
                                children: (0, S.we)(
                                  "#selectimage_tip_usage_title",
                                ),
                              }),
                              ": ",
                              (0, S.we)("#selectimage_tip_sale_header_1"),
                            ],
                          }),
                          (0, l.jsxs)("p", {
                            children: [
                              (0, l.jsx)("b", {
                                children: (0, S.we)(
                                  "#selectimage_tip_design_title",
                                ),
                              }),
                              ": ",
                              (0, S.we)("#selectimage_tip_sale_header_2"),
                            ],
                          }),
                          (0, l.jsx)("p", {
                            children: (0, S.we)(
                              "#selectimage_tip_sale_header_4",
                            ),
                          }),
                          (0, l.jsxs)("p", {
                            children: [
                              (0, l.jsx)("b", {
                                children: (0, S.we)(
                                  "#selectimage_tip_template_title",
                                ),
                              }),
                              ": ",
                              (0, l.jsx)("a", {
                                href: "https://www.dropbox.com/scl/fo/mhf604o6bdbcfr1scq7bx/h?rlkey=9bk0ggiwuvs4o1jdnej4xsy0c&dl=0",
                                children: (0, S.we)(
                                  "#selectimage_tip_sale_header_3",
                                ),
                              }),
                            ],
                          }),
                          (0, l.jsx)("br", {}),
                        ],
                      }))
                    : "hero" === t
                      ? k &&
                        (U = (0, l.jsxs)(l.Fragment, {
                          children: [
                            (0, l.jsx)("p", {
                              children: (0, S.we)("#selectimage_tip_hero_1"),
                            }),
                            !Boolean(
                              null === (a = k.GetAssets()) || void 0 === a
                                ? void 0
                                : a.GetLibraryHeroURL(),
                            ) &&
                              (0, l.jsx)("p", {
                                className: v.ErrorStylesBackground,
                                children: (0, S.we)(
                                  "#EventEdtior_ArtworkType_hero_warning",
                                ),
                              }),
                          ],
                        }))
                      : (U =
                          "localized_image_group" === t ||
                          "link_capsule" === t ||
                          "sale_section_title" === t ||
                          "schedule_track_art" === t ||
                          "localized_background_art" === t
                            ? (0, l.jsxs)(l.Fragment, {
                                children: [
                                  (0, l.jsx)("p", {
                                    children: (0, S.we)("#ImagePickerLoc_Desc"),
                                  }),
                                  (0, l.jsx)("p", {
                                    children: (0, S.PP)(
                                      "#ImagePickerLoc_Files",
                                      (0, l.jsx)("a", {
                                        href: le,
                                        target: L.TS.IN_CLIENT
                                          ? void 0
                                          : "_blank",
                                        children: (0, S.we)(
                                          "#ImagePickerLoc_URL",
                                        ),
                                      }),
                                    ),
                                  }),
                                ],
                              })
                            : "product_banner" === t
                              ? (0, l.jsxs)(l.Fragment, {
                                  children: [
                                    (0, l.jsx)("div", {
                                      className: I().EventElementOptional,
                                      children: (0, S.we)(
                                        "#selectimage_tip_optional_title",
                                      ),
                                    }),
                                    (0, l.jsxs)("p", {
                                      children: [
                                        (0, l.jsx)("b", {
                                          children: (0, S.we)(
                                            "#selectimage_tip_usage_title",
                                          ),
                                        }),
                                        ": ",
                                        (0, S.we)(
                                          "#selectimage_tip_sale_product_banner",
                                        ),
                                      ],
                                    }),
                                  ],
                                })
                              : "product_mobile_banner" === t ||
                                  "product_banner_override" === t ||
                                  "product_mobile_banner_override" === t
                                ? (0, l.jsxs)(l.Fragment, {
                                    children: [
                                      (0, l.jsx)("div", {
                                        className: I().EventElementOptional,
                                        children: (0, S.we)(
                                          "#selectimage_tip_optional_title",
                                        ),
                                      }),
                                      (0, l.jsxs)("p", {
                                        children: [
                                          (0, l.jsx)("b", {
                                            children: (0, S.we)(
                                              "#selectimage_tip_usage_title",
                                            ),
                                          }),
                                          ": ",
                                          (0, S.we)(
                                            "#selectimage_tip_sale_product_banner",
                                          ),
                                          "product_mobile_banner" === t &&
                                            (0, l.jsxs)("span", {
                                              children: [
                                                "  ",
                                                (0, S.we)(
                                                  "#selectimage_tip_sale_product_banner_mobile",
                                                ),
                                              ],
                                            }),
                                        ],
                                      }),
                                    ],
                                  })
                                : "tab_bar_background" === t
                                  ? (0, l.jsxs)(l.Fragment, {
                                      children: [
                                        (0, l.jsxs)("p", {
                                          children: [
                                            (0, l.jsx)("strong", {
                                              children: (0, S.we)(
                                                "#selectimage_tip_design_title",
                                              ),
                                            }),
                                            ":",
                                            (0, S.we)(
                                              "#Sale_Tabs_Background_Design",
                                            ),
                                          ],
                                        }),
                                        (0, l.jsxs)("p", {
                                          children: [
                                            (0, l.jsx)("strong", {
                                              children: (0, S.we)(
                                                "#selectimage_tip_usage_title",
                                              ),
                                            }),
                                            ":",
                                            (0, S.we)(
                                              "#Sale_Tabs_Background_Usage",
                                            ),
                                          ],
                                        }),
                                      ],
                                    })
                                  : "sale_logo" === t
                                    ? (0, l.jsxs)(l.Fragment, {
                                        children: [
                                          (0, l.jsx)("div", {
                                            className: I().EventElementOptional,
                                            children: (0, S.we)(
                                              "#selectimage_tip_optional_title",
                                            ),
                                          }),
                                          (0, l.jsxs)("p", {
                                            children: [
                                              (0, l.jsx)("b", {
                                                children: (0, S.we)(
                                                  "#selectimage_tip_usage_title",
                                                ),
                                              }),
                                              ": ",
                                              (0, S.we)(
                                                "#selectimage_tip_pageLogo",
                                              ),
                                            ],
                                          }),
                                        ],
                                      })
                                    : (0, l.jsxs)(l.Fragment, {
                                        children: [
                                          (0, l.jsx)("div", {
                                            className: I().EventElementRequired,
                                            children: (0, S.we)(
                                              "#selectimage_tip_required_title",
                                            ),
                                          }),
                                          (0, l.jsxs)("p", {
                                            children: [
                                              (0, l.jsx)("b", {
                                                children: (0, S.we)(
                                                  "#selectimage_tip_usage_title",
                                                ),
                                              }),
                                              ": ",
                                              (0, S.we)(
                                                "#selectimage_tip_bestofyear",
                                              ),
                                            ],
                                          }),
                                        ],
                                      }));
        const T = d.Fj[e.artworkType].width,
          G = d.Fj[e.artworkType].height;
        return (0, l.jsxs)("div", {
          id: e.id,
          className: h().ArtworkSelectorContainer,
          children: [
            Boolean(e.title) &&
              (0, l.jsxs)("div", {
                className: h().Title,
                onDoubleClick: c,
                children: [
                  e.title,
                  (0, l.jsx)("span", { children: " " }),
                  R,
                  c &&
                    (0, l.jsx)(u.$n, {
                      onClick: c,
                      children: (0, l.jsx)(C.he, {
                        toolTipContent: (0, S.we)(
                          e.bIsMinimized
                            ? "#Sale_Section_Maximize_Tooltip"
                            : "#Sale_Section_Minimize_Tooltip",
                        ),
                        children: e.bIsMinimized
                          ? (0, l.jsx)(f.hz4, {})
                          : (0, l.jsx)(f.Xjb, {}),
                      }),
                    }),
                ],
              }),
            !e.bIsMinimized &&
              (0, l.jsxs)("div", {
                className: (0, A.A)(h().SelectImageBlock, h().Tips),
                children: [
                  U,
                  Boolean(T && G) &&
                    (0, l.jsxs)("p", {
                      children: [
                        (0, l.jsx)("b", {
                          children: (0, S.we)(
                            "#selectimage_tip_dimensions_title",
                          ),
                        }),
                        ": ",
                        (0, S.PP)(
                          "#selectimage_tip1",
                          (0, d.qj)(T),
                          (0, d.qj)(G),
                        ),
                      ],
                    }),
                  Boolean(e.strWarning) &&
                    (0, l.jsx)("div", {
                      children: (0, l.jsx)("p", {
                        className: v.WarningStylesWithIcon,
                        children: e.strWarning,
                      }),
                    }),
                  e.elEventArtworkExample,
                  " ",
                  (0, l.jsx)("br", {}),
                  e.elAdditionalControls,
                  Boolean(e.fnRemoveAllArtwork) &&
                    (0, l.jsx)(u.$n, {
                      onClick: (a) => {
                        var n;
                        (0, w.pg)(
                          (0, l.jsx)(ie, {
                            fnRemoveAllArtwork: e.fnRemoveAllArtwork,
                          }),
                          null !== (n = (0, E.uX)(a)) && void 0 !== n
                            ? n
                            : window,
                        );
                      },
                      children: (0, S.we)("#Sale_RemoveAll"),
                    }),
                ],
              }),
            !e.bIsMinimized &&
              (0, l.jsx)(oe, {
                clanSteamID: e.clanSteamID,
                title: null !== (n = e.title) && void 0 !== n ? n : "",
                eventModel: p,
                artworkType: e.artworkType,
                realms: g,
                appid: o,
                fnGetImageHashAndExt: x,
                fnSetImageURL: j,
                fnLangHasData: _,
                partnerEventStore: b,
              }),
          ],
        });
      }
      function ie(e) {
        const { fnRemoveAllArtwork: a, closeModal: n } = e;
        return (0, l.jsx)(j.o0, {
          strTitle: (0, S.we)("#Sale_RemoveAll"),
          strDescription: (0, S.we)("#ImageUpload_DeleteAll_Confirm"),
          onOK: () => {
            null == a || a(), null == n || n();
          },
          onCancel: n,
        });
      }
      function oe(e) {
        const {
            artworkType: a,
            realms: n,
            clanSteamID: t,
            fnLangHasData: r,
            fnGetImageHashAndExt: i,
            fnSetImageURL: o,
            eventModel: d,
            appid: m,
            partnerEventStore: p,
          } = e,
          _ = "localized_image_group" === a,
          [v, x] = c.useState((0, g.E)()),
          [I, j] = c.useState(new Array()),
          w = c.useCallback(
            (e, a, n) => {
              let t = [];
              I.find((a) => a.clanImage.imageid == e.imageid)
                ? (t = I.map((n) =>
                    n.clanImage.imageid == e.imageid
                      ? { clanImage: e, lang: a }
                      : n,
                  ))
                : n && (t = I.concat({ clanImage: e, lang: a })),
                j(t);
            },
            [I],
          ),
          f = c.useCallback(
            (e, n, t) => {
              (0, s.h5)(() => {
                var l;
                (0, k.u)(null !== (l = i(a, n)) && void 0 !== l ? l : "") ==
                  e.image_hash && o(a, null, n),
                  o(a, e, t),
                  w(e, t, !1);
              });
            },
            [i, a, o, w],
          );
        return "hero" === a
          ? (0, l.jsx)("div", {
              style: { padding: "16px" },
              children: (0, l.jsx)(u.$n, {
                style: { textTransform: "uppercase", width: "200px" },
                onClick: () =>
                  window.open(
                    `${L.TS.PARTNER_BASE_URL}admin/game/editbyappid/${m}?activetab=tab_graphicalassets`,
                  ),
                children: (0, S.we)("#ImageUpload_EditHeroImage"),
              }),
            })
          : (0, l.jsxs)("div", {
              children: [
                (0, l.jsx)(ce, {
                  list: I,
                  fnOnArtworkLanguageChange: f,
                  realms: n,
                  fnLangHasData: r,
                }),
                (0, l.jsx)("div", {
                  children: (0, l.jsx)("div", {
                    className: (0, A.A)(
                      h().SelectImageBlock,
                      h().MainPreviewBlock,
                    ),
                    children: (0, l.jsx)(Z, {
                      eventModel: d,
                      clanSteamID: t,
                      fnOnLanguagePreviewChange: (e) => {
                        e != v && x(e);
                      },
                      langOverride: v,
                      fnOnArtworkLangChange: _ ? null : f,
                      artworkType: a,
                      fnOnRemoveImage: _ ? null : (e) => o(a, null, e),
                      realms: n,
                      fnLangHasData: r,
                      fnGetImageHashAndExt: i,
                      partnerEventStore: p,
                    }),
                  }),
                }),
              ],
            });
      }
      !(function (e) {
        (e[(e.k_None = 0)] = "k_None"),
          (e[(e.k_Suggested = 1)] = "k_Suggested"),
          (e[(e.k_Required = 2)] = "k_Required"),
          (e[(e.k_Requested = 3)] = "k_Requested");
      })(re || (re = {}));
      let ce = class extends c.Component {
        ShowLangChangeDialog(e, a) {
          const {
            fnOnArtworkLanguageChange: n,
            realms: t,
            fnLangHasData: r,
          } = this.props;
          (0, w.pg)(
            (0, l.jsx)(_.e, {
              clanImage: e,
              lang: a,
              fnOnArtworkLangChange: n,
              fnLangHasData: r,
              realms: t,
            }),
            window,
          );
        }
        GenerateImageMappings() {
          let e = new Array();
          const { list: a } = this.props;
          return (
            a.forEach((a) => {
              var n;
              const { clanImage: t, lang: s } = a;
              let i = (0, S.we)("#Language_" + (0, r.LgB)(s));
              e.push(
                (0, l.jsxs)(
                  "div",
                  {
                    className: I().FlexRowContainer,
                    children: [
                      (0, l.jsx)("span", {
                        children: (0, S.we)(
                          "#ImageUpload_Success_Mapping",
                          null !== (n = t.file_name) && void 0 !== n ? n : "",
                          i,
                        ),
                      }),
                      (0, l.jsx)("a", {
                        onClick: () => this.ShowLangChangeDialog(t, s),
                        children: (0, S.we)(
                          "#ImageUpload_Success_Mapping_Change",
                        ),
                      }),
                    ],
                  },
                  "img_lang_" + a.clanImage.imageid + "_" + s,
                ),
              );
            }),
            e
          );
        }
        render() {
          const { list: e } = this.props;
          if (!e || 0 == e.length) return (0, l.jsx)("div", {});
          let a = this.GenerateImageMappings();
          return (0, l.jsx)("div", {
            className: h().UploadSuccess,
            children: a,
          });
        }
      };
      (0, t.Cg)([b.oI], ce.prototype, "ShowLangChangeDialog", null),
        (ce = (0, t.Cg)([o.PA], ce));
    },
    45832: (e, a, n) => {
      "use strict";
      n.d(a, { h: () => E });
      var t = n(7850),
        l = n(2160),
        r = n(22837),
        s = n(81393),
        i = n(64753),
        o = n(65946),
        c = n(90626),
        d = n(44894);
      const g =
        "data:image/png;base64,";
      var m = n(40414),
        u = n(7221),
        p = n(86355),
        h = n(25359),
        _ = n.n(h),
        v = n(93016),
        x = n(9154),
        I = n(12155),
        j = n(32754),
        w = n(61859),
        f = n(84811),
        C = n(68255),
        A = n(26408);
      function E(e) {
        const {
          clanSteamID: a,
          fnGetImageHash: n,
          fnLangHasData: s,
          fnOnRemoveImage: o,
        } = e;
        (0, p.mr)(a.GetAccountID());
        const d = c.useMemo(() => {
            let e = new Array();
            const a = w.A0.GetLanguageListForRealms([
              l.TU.k_ESteamRealmGlobal,
              l.TU.k_ESteamRealmChina,
            ]);
            for (const t of a) {
              const a = n(t);
              if (a) {
                const n = (0, r.LgB)(t),
                  l = (0, w.we)("#Language_" + n);
                e.push({ lang: t, strLang: n, locLang: l, imgHash: a });
              }
            }
            return (
              (e = e.sort((e, a) =>
                e.locLang > a.locLang ? 1 : e.locLang < a.locLang ? -1 : 0,
              )),
              e
            );
          }, [n]),
          [g, m, u] = (0, i.uD)();
        return (0, t.jsxs)("div", {
          className: _().SelectImageLanguagesCtn,
          children: [
            (0, t.jsx)("div", {
              className: _().SelectImageTitle,
              children: (0, w.we)("#selectimage_uploaded_languages"),
            }),
            (0, t.jsx)("div", {
              className: _().LanguageListContainer,
              children: d.map((n) =>
                (0, t.jsx)(
                  S,
                  { langData: n, ...e },
                  "lang_select_" + a.GetAccountID() + " " + n.strLang,
                ),
              ),
            }),
            !!o &&
              (0, t.jsxs)(C.$n, {
                onClick: m,
                children: [
                  (0, w.we)("#Sale_RemoveAll"),
                  (0, t.jsx)(A.o, {
                    tooltip: (0, w.we)("#Sale_RemoveAll_Tooltip"),
                  }),
                ],
              }),
            (0, t.jsx)(x.EN, {
              active: g,
              children: (0, t.jsx)(x.o0, {
                strTitle: (0, w.we)("#Dialog_AreYouSure"),
                strDescription: (0, w.we)("#ImageUpload_DeleteAll_Confirm"),
                closeModal: u,
                onOK: () => {
                  for (let e = 0; e < r.bP9; e++) s && o && s(e) && o(e);
                },
              }),
            }),
          ],
        });
      }
      function S(e) {
        const {
            clanSteamID: a,
            langData: n,
            langOverride: l,
            fnOnLanguagePreviewChange: s,
            fnOnArtworkLangChange: i,
            fnOnRemoveImage: c,
          } = e,
          [d, g] = (0, o.q3)(() => {
            const e = p.pU.GetClanImageByImageHash(a, n.imgHash);
            let t = "";
            e &&
              (t = m.zU.GenerateURLFromHashAndExtAndLang(
                a,
                m.zU.GetHashAndExt(e),
                u.wI.full,
                n.lang,
              ));
            let r = _().LanguageSelectorSelected;
            return (
              l != n.lang &&
                (r = n.imgHash
                  ? _().LanguageSelector
                  : _().LanguageSelectorNoData),
              [t, r]
            );
          });
        return (0, t.jsxs)("div", {
          id: n.strLang,
          className: _().LanguageContainer,
          onClick: (e) => {
            let a = (0, r.sfN)(e.currentTarget.id);
            s(a);
          },
          children: [
            (0, t.jsx)("div", { className: g, children: n.locLang }),
            (0, t.jsxs)("span", {
              className: _().LanguageOptions,
              children: [
                Boolean(d) &&
                  (0, t.jsx)("a", {
                    href: d,
                    target: "_blank",
                    children: (0, t.jsx)(j.he, {
                      toolTipContent: (0, w.we)("#selectimage_viewimage_ttip"),
                      children: I.YNO(),
                    }),
                  }),
                !!i && (0, t.jsx)(b, { ...e }),
                !!c && (0, t.jsx)(L, { fnOnRemoveImage: c, langData: n }),
              ],
            }),
          ],
        });
      }
      function b(e) {
        const {
            clanSteamID: a,
            langData: n,
            fnOnArtworkLangChange: l,
            fnGetImageHash: r,
            fnLangHasData: c,
            realms: d,
          } = e,
          [m, u, h] = (0, i.uD)(),
          _ = (0, o.q3)(() => {
            const e = r(n.lang);
            (0, s.wT)(
              !e || !e.includes("."),
              "ChangeLanguageButton: Unexpected File Extension: " + e,
            );
            return p.pU.GetClanImageByImageHash(a, e);
          });
        if (_)
          return (0, t.jsxs)(t.Fragment, {
            children: [
              (0, t.jsx)(j.he, {
                toolTipContent: (0, w.we)("#selectimage_reassign_image_ttip"),
                children: (0, t.jsx)("img", {
                  "data-lang": n.lang,
                  src: g,
                  onClick: () => u(),
                }),
              }),
              (0, t.jsx)(f.tH, {
                children: (0, t.jsx)(x.EN, {
                  active: m,
                  children: (0, t.jsx)(v.e, {
                    clanImage: _,
                    lang: n.lang,
                    fnOnArtworkLangChange: l,
                    fnLangHasData: c,
                    realms: d,
                    closeModal: h,
                  }),
                }),
              }),
            ],
          });
        console.error("image does not exists on server");
      }
      function L(e) {
        const { fnOnRemoveImage: a, langData: n } = e,
          [l, s, o] = (0, i.uD)();
        return (0, t.jsxs)(t.Fragment, {
          children: [
            (0, t.jsx)(j.he, {
              toolTipContent: (0, w.we)("#selectimage_delete_image_ttip"),
              children: (0, t.jsx)("img", {
                "data-lang": n.lang,
                src: d.A,
                onClick: s,
              }),
            }),
            (0, t.jsx)(f.tH, {
              children: (0, t.jsx)(x.EN, {
                active: l,
                children: (0, t.jsx)(x.o0, {
                  strTitle: (0, w.we)("#selectimage_remove_image"),
                  strDescription: (0, w.we)(
                    "#selectimage_remove_details",
                    (0, w.we)("#Language_" + (0, r.LgB)(n.lang)),
                  ),
                  onOK: () => {
                    a(n.lang);
                  },
                  closeModal: o,
                }),
              }),
            }),
          ],
        });
      }
    },
    93016: (e, a, n) => {
      "use strict";
      n.d(a, { e: () => _ });
      var t = n(7850),
        l = n(65946),
        r = n(90626),
        s = n(17720),
        i = n(1909),
        o = n(95695),
        c = n.n(o),
        d = n(9154),
        g = n(52038),
        m = n(61859),
        u = n(25359),
        p = n.n(u),
        h = n(40414);
      function _(e) {
        const {
            clanImage: a,
            closeModal: n,
            lang: o,
            fnOnArtworkLangChange: u,
            realms: _,
            fnLangHasData: v,
          } = e,
          [x, I] = (0, r.useState)(o),
          j = s.b.InitFromClanID(a.clanAccountID),
          w = (0, l.q3)(() => {
            var e;
            return h.zU.GenerateURLFromHashAndExt(
              j,
              null !== (e = h.zU.GetHashAndExt(a)) && void 0 !== e ? e : "",
            );
          });
        return (0, t.jsx)(d.o0, {
          strTitle: (0, m.we)("#selectimage_change_artwork_lang_title"),
          strDescription: (0, m.we)("#selectimage_change_artworl_lang_desc"),
          onOK: () => (null == u ? void 0 : u(a, o, x)),
          onCancel: n,
          closeModal: n,
          children: (0, t.jsxs)("div", {
            className: (0, g.A)(c().FlexColumnContainer, p().ReassignCtn),
            children: [
              (0, t.jsx)("div", {
                className: p().ImagePreviewContainer,
                children: (0, t.jsx)("img", {
                  className: p().ArtworkPreview,
                  src: w,
                }),
              }),
              (0, t.jsx)(i.Ng, {
                selectedLang: x,
                fnLangHasData: v,
                fnOnLanguageChanged: I,
                realms: _,
              }),
            ],
          }),
        });
      }
    },
    85890: (e, a, n) => {
      "use strict";
      n.d(a, { p: () => w });
      var t = n(7850),
        l = n(90626),
        r = n(17720),
        s = n(41735),
        i = n.n(s),
        o = n(58632),
        c = n.n(o),
        d = n(88942),
        g = n(37085),
        m = n(68797),
        u = n(78327),
        p = n(56545),
        h = n(78619),
        _ = n(23809);
      const v = "nicknames";
      const x = new (c())(
          (e) =>
            (async function (e) {
              var a, n, t, l;
              if (!e || 0 == e.length) return [];
              const s =
                "community" == (0, u.yK)()
                  ? u.TS.COMMUNITY_BASE_URL
                  : u.TS.STORE_BASE_URL;
              if (1 == e.length) {
                const t = { accountid: e[0], origin: self.origin },
                  l = await i().get(`${s}actions/ajaxgetavatarpersona`, {
                    params: t,
                  });
                if (
                  !l ||
                  200 != l.status ||
                  (null === (a = l.data) || void 0 === a
                    ? void 0
                    : a.success) != g.R ||
                  !(null === (n = l.data) || void 0 === n ? void 0 : n.userinfo)
                )
                  throw `Load single avatar/persona failed ${((0, m.H))(l).strErrorMsg}`;
                return [l.data.userinfo];
              }
              {
                const a = { accountids: e.join(","), origin: self.origin },
                  n = await i().get(`${s}actions/ajaxgetmultiavatarpersona`, {
                    params: a,
                  });
                if (
                  !n ||
                  200 != n.status ||
                  (null === (t = n.data) || void 0 === t
                    ? void 0
                    : t.success) != g.R ||
                  !(null === (l = n.data) || void 0 === l
                    ? void 0
                    : l.userinfos)
                )
                  throw `Load single avatar/persona failed ${((0, m.H))(n).strErrorMsg}`;
                const o = new Map();
                return (
                  n.data.userinfos.forEach((e) =>
                    o.set(new r.b(e.steamid).GetAccountID(), e),
                  ),
                  e.map((e) => o.get(e))
                );
              }
            })(e),
          { cache: !1 },
        ),
        I = "avatarandpersonas";
      var j = n(15736);
      function w(e) {
        const {
            accountID: a,
            bHideWhenNotAvailable: n,
            bHideName: s,
            bLink: i = !0,
          } = e,
          [o] = (function (e) {
            const { data: a, isLoading: n } = (0, d.I)({
              queryKey: [I, e],
              queryFn: () => x.load(e),
            });
            return [a, n];
          })(a),
          c = (function (e) {
            const a = (0, _.KV)(),
              { data: n, isLoading: t } = (0, d.I)({
                queryKey: [v],
                queryFn: async () => {
                  const e = new Map();
                  if (u.iA.logged_in) {
                    const n = p.w.Init(h.w_T),
                      t = (await h.xtC.GetNicknameList(a, n)).Body().toObject();
                    (null == t ? void 0 : t.nicknames) &&
                      t.nicknames.length > 0 &&
                      t.nicknames.forEach((a) => {
                        e.set(a.accountid, a.nickname);
                      });
                  }
                  return e;
                },
              });
            return n ? n.get(e) : null;
          })(a),
          g = l.useMemo(() => r.b.InitFromAccountID(a), [a]),
          m = `${u.TS.COMMUNITY_BASE_URL}profiles/${g.ConvertTo64BitString()}`,
          w = i ? "a" : "span";
        return (0, t.jsx)(t.Fragment, {
          children: Boolean(!o)
            ? (0, t.jsx)(t.Fragment, {
                children: Boolean(!n) && (0, t.jsx)("span", { children: a }),
              })
            : (0, t.jsxs)(w, {
                href: i ? m : void 0,
                children: [
                  (0, t.jsx)("img", {
                    className: j.SmallAvatar,
                    src: o.avatar_url,
                    "data-miniprofile": "s" + g.ConvertTo64BitString(),
                  }),
                  Boolean(!s) &&
                    (0, t.jsx)("span", {
                      children: c ? `${c} (${o.persona_name})` : o.persona_name,
                    }),
                ],
              }),
        });
      }
    },
    64846: (e, a, n) => {
      "use strict";
      n.d(a, { E: () => o, m: () => i });
      var t = n(7850),
        l = n(82429),
        r = n(95695),
        s = n(52038);
      function i(e, a) {
        if (!e || !e.BIsClanAccount()) return !1;
        const n = l.KN.Get().GetPartnerEventPermissions(e);
        return !!n && (a ? n.valve_admin : n.valve_admin || n.support_user);
      }
      function o(e) {
        const { clanSteamID: a, id: n } = e;
        return i(a, e.requireAdmin)
          ? (0, t.jsx)("div", {
              id: n,
              className: (0, s.A)(
                e.className,
                e.requireAdmin
                  ? r.ValveOnlyAdminBackground
                  : r.ValveOnlyBackground,
              ),
              children: e.children,
            })
          : null;
      }
    },
    27650: (e, a, n) => {
      "use strict";
      n.d(a, { DB: () => c, PW: () => g, Ss: () => i, hk: () => o });
      var t = n(7850),
        l = n(90626),
        r = n(72739),
        s = n(56011);
      function i(e, a) {
        const n = l.useRef(void 0),
          s = l.useCallback(
            (a) => {
              a.currentTarget.files.length > 0 &&
                (e(a.currentTarget.files), (a.currentTarget.value = ""));
            },
            [e],
          ),
          i = l.useCallback(() => n.current.click(), []);
        return [
          r.createPortal(
            (0, t.jsx)("form", {
              onSubmit: d,
              style: { display: "none" },
              children: (0, t.jsx)("input", {
                ...a,
                type: "file",
                ref: n,
                onChange: s,
              }),
            }),
            window.document.body,
          ),
          i,
        ];
      }
      function o(e) {
        const [a, n] = l.useState(!1);
        return [
          {
            onDragEnter: l.useCallback((e) => {
              ((e.dataTransfer.files && e.dataTransfer.files[0]) ||
                (e.dataTransfer.types && "Files" == e.dataTransfer.types[0])) &&
                n(!0);
            }, []),
            onDragLeave: l.useCallback((e) => {
              s.NO(e) && n(!1);
            }, []),
            onDragEnd: l.useCallback(() => n(!1), []),
            onDragOver: a ? d : void 0,
            onDrop: l.useCallback(
              (a) => {
                var t;
                (null === (t = a.dataTransfer.files) || void 0 === t
                  ? void 0
                  : t.length) &&
                  (e(a.dataTransfer.files, a),
                  a.preventDefault(),
                  a.stopPropagation()),
                  n(!1);
              },
              [e],
            ),
          },
          a,
        ];
      }
      async function c(e, a = 1e3) {
        return await new Promise((n, t) => {
          const l = new Image();
          (l.src = e),
            (l.onload = () => n("success")),
            (l.onerror = () => n("error")),
            a > 0 && window.setTimeout(() => n("timeout"), a);
        });
      }
      function d(e) {
        e.preventDefault();
      }
      function g(e) {
        switch (e.type) {
          case "image/jpeg":
            return "jpg";
          case "image/png":
            return "png";
          case "image/gif":
            return "gif";
          default:
            const a = e.name.match(/(?<=\.)[^.]+$/);
            return a ? a[0] : void 0;
        }
      }
    },
    44894: (e, a, n) => {
      "use strict";
      n.d(a, { A: () => t });
      const t =
        "data:image/png;base64,";
    },
  },
]);
