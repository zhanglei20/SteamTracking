/**** (c) Valve Corporation. Use is governed by the terms of the Steam Subscriber Agreement http://store.steampowered.com/subscriber_agreement/.
 ****/
(self.webpackChunkcommunity = self.webpackChunkcommunity || []).push([
  [7679],
  {
    99312: (e) => {
      e.exports = {
        DragTarget: "_2sUvh2ZpsDAw1xNqgRBELg",
        DragOnTopOfMe: "_1mvpIyLL0-Pd4QMIoRXHtu",
        DescriptionCtn: "_3DQEBNkYGY3hyLFAjhtq7V",
        EventEditorLinkInput: "_25nbuIEmk-BBWxsSvWGG1n",
        DefaultEditor: "NENu2K19GJmLf1Asga-WF",
        ImportHTMLCtn: "_3sVZHF23hli8ijIwtMs8oU",
        ImportHTMLTextArea: "DHbRFUDVAeXGluFH-smoE",
        ImportHTMLConvertButtons: "lnyZaHhcGtBzDU0SMlFd1",
        ImportHTMLCheckBoxLine: "_3R3FNRLSeiOwBgELGjSPbz",
        OptionRow: "_2Y3MLEmGvWMI8BoNZgCllJ",
        TextEditorToolBarContainer: "_2bOpQtX5QAuQxfGhEJ_iYg",
        EmoteOuter: "_1x3UOXJkizqKhkssRfFjSS",
        YouTubeInput: "_3WXTC22teDkm8BMc01ZTLA",
        YouTubePreviewInsertOption: "_6ocliVvrdQxHPu-upv6-s",
        DropDownScroll: "P0-tbY3743fHY8SAzfF6b",
        InsertSpeakerCtn: "_2f-6Yv5h7xjUcZCrepnQhg",
        TitleGroup: "_1ddLhT39tQNuR4ljq6Nfg5",
        AssociateRowCtn: "_2HeY5m9J-kxRVzGn8dAwv5",
        PhotoCtn: "_2-f4CX_EyXfhRUmPdIey4w",
        AboutCtn: "_3hF9cNUOsfV0BkzEaWn7FM",
        PreviewCtn: "_VCRyh7nyN-2xDV6yH6Sg",
        BBCodeEditorInputStyles: "F506h2OVFDcZeXFtyqthY",
      };
    },
    34510: (e) => {
      e.exports = {
        BBCode_Toggle: "_3dX8-PpYvSNsGv4k5lvP-R",
        Active: "_2vTzhbuJFb9_vHvquo2L-L",
        BBCode: "_1pH9CKzm5VpicOgzyWpsy_",
      };
    },
    64233: (e) => {
      e.exports = { Image: "_1po_jxHTSix3Li3w5ZnMBB" };
    },
    79786: (e) => {
      e.exports = {
        Emoticon_Toggle: "Y5J3nttqNZsLax6MbnH-L",
        Active: "YCbwLzK9cJ7QissjKq11n",
        Emoticon: "_2o57_fRPxv5_x6BkjL_cgc",
      };
    },
    17618: (e) => {
      e.exports = {
        ImagesOuterContainer: "_3A8RGZO2pwg1yKDAdFqp9r",
        Hilight: "_1v_zQLXgFsvon1SwxrWjE-",
        ImageContainer: "_2ti3yMwzfkGoiW68FuNjTG",
        Image: "y902_9A0Wj5bTshbt4xRb",
        ImageFilename: "_2jzLZXXxgDMMcA9X0QDSdg",
      };
    },
    41609: (e) => {
      e.exports = {
        WhitelistCtn: "_1UhmxrINvvaNnHzhCPoill",
        WhitelistRow: "_28TC1EYm0jlWPjyk89xXCL",
        WhitelistNumber: "IY3dF3eWXX1OmE8oYcQKp",
        Disabled: "_2VzE-3UQEHXyAext8t7gLW",
        Grabbing: "_1vSZ5gJndAOamRhVGni8HG",
        DragActive: "_31uDZXKZQlYMd8FK9xdaJb",
        Dropped: "_3bfDVSvzMDkk4s1j0Vw8jI",
        JumpToSection: "oABTo2lkoYYI5YMYaeq_Q",
        BeingDragged: "_3y7I4DL9Hua5OhZ4HgcBB5",
        DragGhost: "_61nYWo98IhSjR8PWtQX9O",
        Grabbable: "riuelIz655g_IBddWfLQ-",
        DisabledGrab: "_2K0C_m1AZvB6yeNaEXXjDD",
        WhitelistAvatar: "_3DGjmH9KW9BAXsEYwH1WpE",
        ButtonCtn: "_1hSqlvDTyj9P6eWTHXutUt",
        DragHighlightContainer: "_2jRMC5JVSK6dsktYus9Gjf",
        DragHighlight: "Y9ryg1Npznt3dpkr7BGp1",
      };
    },
    77700: (e) => {
      e.exports = {
        narrowWidth: "500px",
        mentionDialogPosition: "_3isL0ZmZcmPqrXDdNiNSsm",
        mentionDialog: "_1QU3cLCGXCmYTUvjYiqqz6",
        mentionSearchText: "_1xVcZo7UqD1Idiz3hcGoHg",
        suggestOption: "vquL9mspYzz2tBtxrzqw9",
        mentionSearchOption: "_3O0sMruBIaruOmKJLJre-J",
        manyMatches: "_1cweL4uxVeoeKoymO9IuaT",
        selected: "boNOGnexLhWO9Nd0e6-0A",
        nickname: "_2dKJqMZUnKQIInZReBkcRI",
        mentionSearchMatch: "_2_0t_pDYqkDefMC0gDZV8G",
      };
    },
    50666: (e) => {
      e.exports = {
        CropImage: "_3qfqTaQ35U6AO3FNeijcFV",
        CropPreviewGroup: "_1RI-QM2ZjK9MaVjeCLE_LF",
        CropPreviewLabel: "_3_zyLDUyxZNyexfX3kNOPv",
      };
    },
    73744: (e, t, s) => {
      "use strict";
      s.d(t, {
        Ek: () => u,
        Fj: () => g,
        Ho: () => p,
        Kf: () => r,
        N_: () => v,
        XY: () => C,
        dM: () => a,
        qj: () => n,
        s4: () => I,
        vz: () => _,
        x: () => c,
        yu: () => m,
      });
      var i = s(43261);
      function n(e) {
        return Array.isArray(e) ? e[0] : e;
      }
      function a(e) {
        const t = Array.isArray(e) ? e : [e];
        return Math.min(...t);
      }
      function r(e, t) {
        return void 0 === t ? n(e) : Array.isArray(e) ? e[t] : e;
      }
      const o = [i.bg.iS, i.bg.dU, i.bg.CK, i.bg.wD],
        l = [i.bg.iS, i.bg.dU, i.bg.CK],
        h = [i.bg.iS, i.bg.dU],
        p = [i.bg.pJ, i.bg.nn],
        c = [i.bg.pi, i.bg.k7],
        d = [i.bg.iS, i.bg.dU, i.bg.CK, i.bg.wD, i.bg.pJ, i.bg.nn],
        g = {
          capsule: { width: 800, height: 450, rgAcceptableTypes: h },
          marketingmessage_art_2_eventcapsule: {
            width: 800,
            height: 450,
            rgAcceptableTypes: h,
          },
          marketingmessage_art_eventcapsule: {
            width: 800,
            height: 450,
            rgAcceptableTypes: h,
          },
          spotlight: { width: 2108, height: 460, rgAcceptableTypes: h },
          localized_store_app_spotlight: {
            width: 1200,
            height: 260,
            rgAcceptableTypes: h,
          },
          localized_store_app_spotlight_mobile: {
            width: 500,
            height: 160,
            rgAcceptableTypes: h,
          },
          localized_title_image: {
            width: 1920,
            height: 622,
            rgAcceptableTypes: h,
          },
          background: { width: 1920, height: 622, rgAcceptableTypes: h },
          hero: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: h,
          },
          email_full: { width: 800, height: 300, rgAcceptableTypes: h },
          email_centered: { width: 644, height: 300, rgAcceptableTypes: h },
          broadcast_left: {
            width: [199, 155],
            height: [433, 337],
            rgAcceptableTypes: h,
          },
          broadcast_right: {
            width: [199, 155],
            height: [433, 337],
            rgAcceptableTypes: h,
          },
          sale_header: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: l,
          },
          sale_overlay: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: l,
          },
          localized_image_group: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: h,
          },
          localized_background_art: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: h,
          },
          sale_section_background: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: l,
          },
          sale_section_title: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: l,
          },
          link_capsule: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: h,
          },
          product_banner: {
            width: [1200, 1100],
            height: [175, 160],
            rgAcceptableTypes: h,
          },
          product_mobile_banner: {
            width: 500,
            height: 160,
            rgAcceptableTypes: h,
          },
          product_banner_override: {
            width: [1200, 1100],
            height: [175, 160],
            rgAcceptableTypes: h,
          },
          product_mobile_banner_override: {
            width: 500,
            height: 160,
            rgAcceptableTypes: h,
          },
          schedule_track_art: { width: 196, height: 92, rgAcceptableTypes: h },
          tab_bar_background: {
            width: 1500,
            height: 100,
            rgAcceptableTypes: h,
          },
          sale_logo: {
            width: [1200, 940],
            height: [460, 460],
            rgAcceptableTypes: h,
          },
          bestofyear_banner: { width: 1100, height: 160, rgAcceptableTypes: l },
          bestofyear_banner_mobile: {
            width: 500,
            height: 160,
            rgAcceptableTypes: l,
          },
          localized_marketing_message: {
            width: 570,
            height: 600,
            rgAcceptableTypes: o,
          },
          localized_optin_banner: {
            width: 1e3,
            height: 150,
            rgAcceptableTypes: h,
          },
          localized_marketingmessage_webm: {
            width: 570,
            height: 600,
            rgAcceptableTypes: [i.bg.pJ],
          },
          localized_marketingmessage_mp4: {
            width: 570,
            height: 600,
            rgAcceptableTypes: [i.bg.nn],
          },
          localized_partnerevent_webm: {
            width: 800,
            height: 450,
            rgAcceptableTypes: [i.bg.pJ],
          },
          localized_partnerevent_mp4: {
            width: 800,
            height: 450,
            rgAcceptableTypes: [i.bg.nn],
          },
          localized_subtitles: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: [i.bg.k7, i.bg.pi],
          },
          localized_marketingmessage_poster: {
            width: 528,
            height: 297,
            rgAcceptableTypes: [i.bg.iS, i.bg.dU],
          },
          localized_marketingmessage_background: {
            width: 570,
            height: 600,
            rgAcceptableTypes: h,
          },
          localized_email_image: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: h,
          },
          template_asset: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: d,
          },
          user_poll_background: {
            width: 0,
            height: 0,
            bDisableEnforceDimensions: !0,
            rgAcceptableTypes: h,
          },
          sale_store_capsule_header: {
            width: 920,
            height: 430,
            rgAcceptableTypes: h,
          },
          sale_store_capsule_small: {
            width: 462,
            height: 174,
            rgAcceptableTypes: h,
          },
          sale_store_capsule_main: {
            width: 1232,
            height: 706,
            rgAcceptableTypes: h,
          },
          sale_store_capsule_vertical: {
            width: 748,
            height: 896,
            rgAcceptableTypes: h,
          },
          spotlight_art: { width: 306, height: 260, rgAcceptableTypes: l },
          spotlight_art_hero: { width: 748, height: 896, rgAcceptableTypes: h },
          old_spotlight_art: { width: 306, height: 350, rgAcceptableTypes: l },
          marketingmessage_art: {
            width: 570,
            height: 600,
            rgAcceptableTypes: l,
          },
          marketingmessage_art_2: {
            width: 570,
            height: 600,
            rgAcceptableTypes: l,
          },
          takeover_art: { width: 1850, height: 450, rgAcceptableTypes: l },
          takeover_webm_art: {
            width: 1850,
            height: 450,
            rgAcceptableTypes: [i.bg.pJ],
          },
          takeover_mp4_art: {
            width: 1850,
            height: 450,
            rgAcceptableTypes: [i.bg.nn],
          },
          takeover_mobile_art: {
            width: 500,
            height: 350,
            rgAcceptableTypes: l,
          },
          takeover_webm_mobile_art: {
            width: 500,
            height: 350,
            rgAcceptableTypes: [i.bg.pJ],
          },
          takeover_mp4_mobile_art: {
            width: 500,
            height: 350,
            rgAcceptableTypes: [i.bg.nn],
          },
          takeunder_art: { width: 1200, height: 190, rgAcceptableTypes: l },
          takeunder_mobile_art: {
            width: 500,
            height: 160,
            rgAcceptableTypes: l,
          },
          app_header_capsule: { width: 920, height: 430, rgAcceptableTypes: h },
          app_main_capsule: { width: 1232, height: 706, rgAcceptableTypes: h },
        };
      function u(e, t, s, i) {
        let n = null;
        if (Array.isArray(s)) {
          if (
            ((n = s
              .map((t, s) => (e === t ? s : void 0))
              .filter((e) => void 0 !== e)),
            n.length <= 0)
          )
            return !1;
        } else if (e !== s) return !1;
        if (Array.isArray(i)) {
          const e = i
            .map((e, s) => (t === e ? s : void 0))
            .filter((e) => void 0 !== e);
          if (e.length <= 0) return !1;
          if ((null == n ? void 0 : n.length) && !e.some((e) => n.includes(e)))
            return !1;
        } else if (t !== i) return !1;
        return !0;
      }
      function m(e, t, s, i) {
        const n = g[s];
        return (
          !!n &&
          (n.bDisableEnforceDimensions ? !!i : u(e, t, n.width, n.height))
        );
      }
      function I(e, t, s) {
        const i = g[s];
        if (!i) return !1;
        if (i.bDisableEnforceDimensions) return !0;
        if (Array.isArray(i.width)) {
          if (i.width.filter((t) => e < t).length == i.width.length) return !1;
        } else if (e < i.width) return !1;
        if (Array.isArray(i.height)) {
          if (i.height.filter((e) => t < e).length == i.height.length)
            return !1;
        } else if (t < i.height) return !1;
        return !0;
      }
      function C(e) {
        const t = g[e];
        return (
          t.rgAcceptableTypes.includes(i.bg.k7) ||
          t.rgAcceptableTypes.includes(i.bg.pi)
        );
      }
      function _(e, t) {
        return t.filter((t) => v(e, t));
      }
      function v(e, t) {
        return g[t].rgAcceptableTypes.includes(e);
      }
    },
    78511: (e, t, s) => {
      "use strict";
      s.d(t, { o: () => o, q: () => l });
      var i = s(34629),
        n = s(14947),
        a = s(43261),
        r = s(82705);
      function o(e) {
        return "waiting" == e || "uploading" == e || "processing" == e;
      }
      class l {
        constructor(e, t, s, i, o) {
          var l;
          (this.m_originalSize = { width: 0, height: 0 }),
            (this.m_originalDataUrl = ""),
            (this.dataUrl = void 0),
            (this.width = 0),
            (this.height = 0),
            (this.status = "pending"),
            (this.message = ""),
            (this.language = void 0),
            (0, n.Gn)(this),
            (this.file = e),
            (this.filename = t),
            (this.fileType =
              null !== (l = (0, r.yh)(t)) && void 0 !== l ? l : a.bg.w3),
            (this.language = s),
            (this.uploadTime = Date.now()),
            (this.status = "pending"),
            (this.m_originalSize = o),
            (this.height = o.height),
            (this.width = o.width),
            (this.m_originalDataUrl = i),
            (this.dataUrl = i);
        }
        ResetImage() {
          (this.height = this.m_originalSize.height),
            (this.width = this.m_originalSize.width),
            (this.dataUrl = this.m_originalDataUrl);
        }
        GetImageOptionLabel() {}
      }
      (0, i.Cg)([n.sH], l.prototype, "dataUrl", void 0),
        (0, i.Cg)([n.sH], l.prototype, "width", void 0),
        (0, i.Cg)([n.sH], l.prototype, "height", void 0),
        (0, i.Cg)([n.sH], l.prototype, "status", void 0),
        (0, i.Cg)([n.sH.ref], l.prototype, "message", void 0),
        (0, i.Cg)([n.sH], l.prototype, "language", void 0);
    },
    28954: (e, t, s) => {
      "use strict";
      s.d(t, { M0: () => S, VE: () => T, vN: () => y, zO: () => A });
      var i = s(34629),
        n = s(22837),
        a = s(81393),
        r = s(64753),
        o = s(41735),
        l = s.n(o),
        h = s(14947),
        p = s(90626),
        c = s(73744),
        d = s(86355),
        g = s(68797),
        u = s(6419),
        m = s(56011),
        I = s(61859),
        C = s(78327),
        _ = s(82817),
        v = s(17267),
        f = s(64953),
        x = s(69343);
      function b(e, t, s) {
        const i = (0, c.Kf)(t.width, s),
          n = (0, c.Kf)(t.height, s);
        return {
          sKey: `${e}_${i}x${n}`,
          width: i,
          height: n,
          bEnforceDimensions: !t.bDisableEnforceDimensions,
          artworkType: e,
          bHiddenFromDropdown: "hero" === e,
          bDeprecated: (null != s ? s : 0) >= 1,
          fnGetLabelText() {
            return "spotlight" == this.artworkType
              ? (0, I.we)("#EventEditor_ArtworkType_store_spotlight")
              : (0, I.we)("#EventEditor_ArtworkType_" + this.artworkType);
          },
        };
      }
      function S(e) {
        return null == e
          ? void 0
          : e.flatMap((e) => {
              const t = c.Fj[e];
              if (
                ((0, a.wT)(Boolean(t), `Artwork Type not in Map ${e}`),
                "number" == typeof t.width && "number" == typeof t.height)
              )
                return [b(e, t)];
              {
                let s = Math.max(
                  Array.isArray(t.width) ? t.width.length : 1,
                  Array.isArray(t.height) ? t.height.length : 1,
                );
                return Array.from({ length: s }, (s, i) => b(e, t, i));
              }
            });
      }
      class w extends x.Vr {
        constructor(e, t, s, i) {
          super(),
            (this.m_filesToUpload = h.sH.array()),
            (this.m_filesCompleted = h.sH.array()),
            (this.m_lastError = void 0),
            (0, h.Gn)(this),
            (this.m_clanSteamID = e),
            (this.m_rgImageOptions = S(t)),
            (this.m_localizedImageGroupPrimaryImage = s),
            (this.m_clanImagesV2 = null != i && i);
        }
        GetClanSteamID() {
          return this.m_clanSteamID;
        }
        async AddImage(e, t = n.Bhc) {
          const { language: s } = (0, x.jj)(e.name, t);
          return this.AddImageForLanguage(e, s);
        }
        async AddImageForLanguage(e, t) {
          if (!((0, _.aL)(e.type) || (C.iA.is_support && (0, _.Uz)(e.type))))
            return !1;
          const s = await (0, _.zB)(e, (0, _.Uz)(e.type));
          if (!s) return !1;
          const i = new f.M7(
            e,
            e.name,
            t,
            this.m_rgImageOptions,
            s.src,
            (0, _.II)(s),
            s,
            this.m_localizedImageGroupPrimaryImage,
          );
          return (this.m_filesToUpload = [...this.m_filesToUpload, i]), !0;
        }
        async AddExistingClanImage(e, t = n.Bhc) {
          const s = v.zU.GetHashAndExt(e);
          if (!s) return !1;
          const i = v.zU.GenerateEditableURLFromHashAndExt(
              this.m_clanSteamID,
              s,
            ),
            a = await l()({ url: i, method: "GET", responseType: "blob" }),
            r = (0, m.pE)(a.data, e.file_name);
          return await this.AddImage(r, t);
        }
        DeleteUploadImageByIndex(e) {
          this.m_filesToUpload.splice(e, 1),
            (this.m_filesToUpload = [...this.m_filesToUpload]);
        }
        DeleteUploadImage(e) {
          let t = this.m_filesToUpload.findIndex(
            (t) => e.file == t.file && e.uploadTime == t.uploadTime,
          );
          t >= 0 && this.DeleteUploadImageByIndex(t);
        }
        ClearImages() {
          this.m_filesToUpload = h.sH.array();
        }
        GetFilesUploaded() {
          return this.m_filesCompleted;
        }
        GetLastErrorFile() {
          return this.m_lastError;
        }
        GetCompletedFiles() {
          return this.m_filesCompleted.length;
        }
        GetTotalFiles() {
          return this.m_filesToUpload.length;
        }
        GetFilesToUpload() {
          return this.m_filesToUpload.map((e) => e.file);
        }
        GetUploadImages() {
          return this.m_filesToUpload;
        }
        BHasError() {
          return null != this.m_lastError;
        }
        BAllDone() {
          return (
            this.m_filesCompleted.length > 0 &&
            this.m_filesCompleted.length == this.m_filesToUpload.length
          );
        }
        BIsFileCompleted(e) {
          return -1 != this.m_filesCompleted.indexOf(e);
        }
        RetryAllFailedUploads() {
          this.CancelAllUploads(), this.UploadAllImages();
        }
        async handleUploadRefresh(e) {
          await d.pU.LoadClanImages(this.m_clanSteamID, !0, e);
        }
        BGetUploadsAreInSerial() {
          return !1;
        }
        async UploadSingleImage(e, t, s, i) {
          var n;
          const a = e.file,
            r = e.GetCurrentImageOption(),
            o = e.GetResizeDimension(),
            h = new FormData();
          h.append("clanimage", a, t),
            h.append("sessionid", (0, C.KC)()),
            this.m_clanImagesV2 && h.append("clan_images_v2", "1"),
            (null == r ? void 0 : r.artworkType) &&
              h.append("arttype", r.artworkType),
            o &&
              o.length > 0 &&
              h.append(
                "resize",
                o.map((e) => e.width + "x" + e.height).join(","),
              );
          let p = "/uploadimage/";
          const c = this.m_localizedImageGroupPrimaryImage;
          c &&
            ((p = "/ajaxuploadlocalizedimage/"),
            h.append("origimagehash", c.image_hash),
            c.thumbnail_hash && h.append("thumbhash", c.thumbnail_hash),
            h.append("extension", "" + c.file_type),
            h.append("language", "" + s));
          const d =
            null === (n = t.split(".").pop()) || void 0 === n
              ? void 0
              : n.toLocaleLowerCase();
          ("webm" != d && "mp4" != d) ||
            (h.append("video_width", "" + e.width),
            h.append("video_height", "" + e.height));
          let u,
            m =
              C.TS.COMMUNITY_BASE_URL +
              "/gid/" +
              this.m_clanSteamID.ConvertTo64BitString() +
              p,
            I = {
              cancelToken: i,
              withCredentials: !0,
              headers: { "Content-Type": "multipart/form-data" },
            },
            _ = !0;
          try {
            (u = await l().post(m, h, I)), this.m_filesCompleted.push(a);
          } catch (e) {
            (_ = !1),
              (this.m_lastError = {
                file: a,
                status: e.response ? e.response.status : 500,
                message: (0, g.H)(e).strErrorMsg,
              }),
              (u = e.response);
          }
          return (
            c || (await this.handleUploadRefresh(i)),
            { bSuccess: _, result: u.data }
          );
        }
      }
      (0, i.Cg)([h.sH], w.prototype, "m_filesToUpload", void 0),
        (0, i.Cg)([h.sH], w.prototype, "m_filesCompleted", void 0),
        (0, i.Cg)([h.sH], w.prototype, "m_lastError", void 0),
        (0, i.Cg)([u.o], w.prototype, "AddImage", null),
        (0, i.Cg)([u.o], w.prototype, "AddExistingClanImage", null),
        (0, i.Cg)([u.o], w.prototype, "DeleteUploadImageByIndex", null),
        (0, i.Cg)([u.o], w.prototype, "DeleteUploadImage", null),
        (0, i.Cg)([u.o], w.prototype, "ClearImages", null);
      class T extends w {
        constructor(e, t, s) {
          super(e, t, s, !1);
        }
      }
      class y extends w {
        constructor(e, t, s) {
          super(e, t, s, !0);
        }
      }
      function A(e, t, s) {
        const i = (0, r.wm)(t instanceof Array ? t : [t]),
          n = e.ConvertTo64BitString();
        return p.useMemo(() => new T(e, i, s), [n, i]);
      }
    },
    64953: (e, t, s) => {
      "use strict";
      s.d(t, { IS: () => c, M7: () => g, T2: () => d });
      var i = s(34629),
        n = s(14947),
        a = s(73744),
        r = s(61859),
        o = s(78511),
        l = s(66331),
        h = s(82817);
      class p extends o.q {
        constructor(e, t, s, i, a, r) {
          super(e, t, s, a, r),
            (this.m_currentImageOption = void 0),
            (this.m_currentImageOptionKey = void 0),
            (0, n.Gn)(this),
            (this.m_rgImageOptions = i);
        }
        IsValidAssetType(e, t) {
          var s, i;
          let n = 0,
            o = 0,
            p = !1,
            c =
              !this.m_rgImageOptions ||
              0 === this.m_rgImageOptions.length ||
              this.m_rgImageOptions.some((e) => {
                var t;
                return (
                  e.sKey ==
                  (null === (t = this.GetCurrentImageOption()) || void 0 === t
                    ? void 0
                    : t.sKey)
                );
              });
          if (e) (n = e.width), (o = e.height), (p = !0);
          else if (this.GetCurrentImageOption()) {
            const e = a.Fj[this.GetCurrentImageOption().artworkType];
            e &&
              ((n = e.width),
              (o = e.height),
              (p = !e.bDisableEnforceDimensions));
          }
          const d = this.width >= (0, a.dM)(n) && this.height >= (0, a.dM)(o),
            g = p ? (0, a.Ek)(this.width, this.height, n, o) : d,
            u = t && t != this.fileType,
            m =
              !!(this.m_rgImageOptions && this.m_rgImageOptions.length > 0) &&
              0 ==
                (0, a.vz)(
                  this.fileType,
                  (null === (s = this.m_rgImageOptions) || void 0 === s
                    ? void 0
                    : s.map((e) => e.artworkType)) || [],
                ).length,
            I = Boolean((0, l.t)(this.fileType));
          let C,
            _ = "",
            v = !1;
          return (
            c
              ? m
                ? (_ = (0, r.we)("#ImageUpload_InvalidFileType"))
                : u
                  ? (_ = (0, r.we)(
                      "#ImageUpload_InvalidFormat",
                      null !== (i = (0, h.EG)(t)) && void 0 !== i ? i : "",
                    ))
                  : g || I
                    ? d
                      ? !g && I
                        ? ((_ = (0, r.we)(
                            "#ImageUpload_InvalidDimensions",
                            (0, a.qj)(n),
                            (0, a.qj)(o),
                          )),
                          (v = !0))
                        : ((Array.isArray(n) && this.width != (0, a.qj)(n)) ||
                            (Array.isArray(o) &&
                              this.height != (0, a.qj)(o))) &&
                          ((C = null != C ? C : []),
                          C.push(
                            (0, r.we)(
                              "#ImageUpload_PreferredDimension",
                              (0, a.qj)(n),
                              (0, a.qj)(o),
                            ),
                          ))
                      : (_ = (0, r.we)(
                          "#ImageUpload_TooSmall",
                          (0, a.qj)(n),
                          (0, a.qj)(o),
                        ))
                    : (_ = (0, r.we)(
                        "#ImageUpload_InvalidResolution",
                        (0, a.qj)(n),
                        (0, a.qj)(o),
                      ))
              : (_ = (0, r.we)("#ImageUpload_InvalidFormatSelected")),
            {
              error: _,
              warnings: C,
              needsCrop: v,
              match: this.GetCurrentImageOption(),
            }
          );
        }
        BSupportsLanguages() {
          return !0;
        }
        ComputeDefaultImageOption() {
          var e;
          if (!this.m_rgImageOptions || 0 == this.m_rgImageOptions.length)
            return;
          const t = (0, a.vz)(
            this.fileType,
            null === (e = this.m_rgImageOptions) || void 0 === e
              ? void 0
              : e.map((e) => e.artworkType),
          );
          let s = u(this.width, this.height, t, !1);
          if ((void 0 === s && (s = u(this.width, this.height, t, !0)), s)) {
            const e = this.m_rgImageOptions.find(
              (e) =>
                e.artworkType == s &&
                (!e.bEnforceDimensions ||
                  (e.width == this.width && e.height == this.height)),
            );
            if (e) return e;
          }
          return this.m_rgImageOptions[0];
        }
        get ImageOptions() {
          return this.m_rgImageOptions;
        }
        GetCurrentImageOptionKey() {
          return this.m_currentImageOptionKey;
        }
        GetCurrentImageOption() {
          var e;
          return null !== (e = this.m_currentImageOption) && void 0 !== e
            ? e
            : this.ComputeDefaultImageOption();
        }
        SetCurrentImageOption(e) {
          (this.m_currentImageOption = e),
            (this.m_currentImageOptionKey = null == e ? void 0 : e.sKey);
        }
      }
      (0, i.Cg)([n.sH], p.prototype, "m_currentImageOption", void 0),
        (0, i.Cg)([n.sH], p.prototype, "m_currentImageOptionKey", void 0);
      class c extends p {
        constructor(e, t, s, i, n, a, r) {
          super(e, t, s, i, n, a), (this.video = r);
        }
        BIsOriginalMinimumDimensions(e) {
          return (0, a.s4)(
            this.video.videoWidth,
            this.video.videoHeight,
            e.artworkType,
          );
        }
        FileTypeMatchesImageTypes(e) {
          return (0, a.N_)(this.fileType, e.artworkType);
        }
        BIsVideo() {
          return a.Ho.includes(this.fileType);
        }
        GetResizeDimension() {}
      }
      class d extends p {
        constructor(e, t, s, i) {
          super(e, t, s, i, URL.createObjectURL(e), { width: 0, height: 0 });
        }
        BIsOriginalMinimumDimensions(e) {
          return (0, a.XY)(e.artworkType);
        }
        FileTypeMatchesImageTypes(e) {
          return (0, a.N_)(this.fileType, e.artworkType);
        }
        BIsVideo() {
          return a.Ho.includes(this.fileType);
        }
        GetResizeDimension() {}
      }
      class g extends p {
        constructor(e, t, s, i, a, r, o, l) {
          super(e, t, s, i, a, r),
            (this.bCropped = !1),
            (0, n.Gn)(this),
            (this.media = o),
            (this.localizedImageGroupPrimaryImage = l);
        }
        IsValidAssetType(e, t) {
          var s;
          return (
            (t =
              null != t
                ? t
                : null === (s = this.localizedImageGroupPrimaryImage) ||
                    void 0 === s
                  ? void 0
                  : s.file_type),
            super.IsValidAssetType(e, t)
          );
        }
        GetCanvasImageSource() {
          return this.media;
        }
        BIsOriginalMinimumDimensions(e) {
          var t, s, i, n;
          return (0, a.s4)(
            null !==
              (s =
                null === (t = this.media) || void 0 === t ? void 0 : t.width) &&
              void 0 !== s
              ? s
              : 0,
            null !==
              (n =
                null === (i = this.media) || void 0 === i
                  ? void 0
                  : i.height) && void 0 !== n
              ? n
              : 0,
            e.artworkType,
          );
        }
        FileTypeMatchesImageTypes(e) {
          return (0, a.N_)(this.fileType, e.artworkType);
        }
        BIsVideo() {
          return a.Ho.includes(this.fileType);
        }
        GetResizeDimension() {
          var e;
          return (function (e) {
            if ("background" === e)
              return [
                { width: 960, height: 311 },
                { width: 480, height: 156 },
              ];
            if ("capsule" === e)
              return [
                {
                  width: (0, a.qj)(a.Fj[e].width) / 2,
                  height: (0, a.qj)(a.Fj[e].height) / 2,
                },
              ];
            if ("spotlight" === e)
              return [
                {
                  width: (0, a.qj)(a.Fj[e].width) / 2,
                  height: (0, a.qj)(a.Fj[e].height) / 2,
                },
              ];
            return;
          })(
            null === (e = this.GetCurrentImageOption()) || void 0 === e
              ? void 0
              : e.artworkType,
          );
        }
      }
      function u(e, t, s, i = !1) {
        if (s)
          for (let n of s) {
            if (i ? (0, a.s4)(e, t, n) : (0, a.yu)(e, t, n)) return n;
          }
      }
      (0, i.Cg)([n.sH], g.prototype, "bCropped", void 0);
    },
    69343: (e, t, s) => {
      "use strict";
      s.d(t, { PD: () => d, Vr: () => c, jj: () => g });
      var i = s(2160),
        n = s(22837),
        a = s(61859),
        r = s(41735),
        o = s.n(r);
      class l {}
      function h(e, t, s) {
        const i = e.filter((e) => {
          const i = e.IsValidAssetType(t, s);
          return "pending" === e.status && !i.error && !i.needsCrop;
        });
        return (
          i.forEach((e) => {
            (e.status = "waiting"), (e.message = "");
          }),
          i
        );
      }
      async function p(e, t, s, i, a, r) {
        const o = h(e, i, a),
          l = [];
        let p = 0;
        const c = Array.from({ length: Math.floor(t) }, () =>
          (async () => {
            for (var e; p < o.length; ) {
              const t = p++,
                i = o[t];
              i.status = "uploading";
              const a = await s(
                i,
                i.filename,
                null !== (e = i.language) && void 0 !== e ? e : n.xPp,
                r,
              );
              (i.status = a.bSuccess ? "success" : "failed"),
                (i.message =
                  !a.bSuccess && a.elErrorMessage ? a.elErrorMessage : ""),
                (l[t] = { image: i, uploadResult: a });
            }
          })(),
        );
        return (
          await Promise.all(c),
          l.map((e) => ({
            bSuccess: e.uploadResult.bSuccess,
            image: e.image,
            uploadResult: e.uploadResult.result,
          }))
        );
      }
      class c extends l {
        constructor() {
          super(...arguments), (this.m_cancel = void 0);
        }
        async UploadAllImages(e, t) {
          this.m_cancel = o().CancelToken.source();
          const s = this.BGetUploadsAreInSerial() ? 1 : 4;
          let i;
          const a = this.UploadSingleImage.bind(this);
          return (
            (i =
              s > 1
                ? await p(
                    this.GetUploadImages(),
                    s,
                    a,
                    e,
                    t,
                    this.m_cancel.token,
                  )
                : await (async function (e, t, s, i, a) {
                    var r;
                    const o = h(e, s, i),
                      l = [];
                    for (const e of o) {
                      e.status = "uploading";
                      const s = await t(
                        e,
                        e.filename,
                        null !== (r = e.language) && void 0 !== r ? r : n.xPp,
                        a,
                      );
                      (e.status = s.bSuccess ? "success" : "failed"),
                        (e.message =
                          !s.bSuccess && s.elErrorMessage
                            ? s.elErrorMessage
                            : ""),
                        l.push({
                          bSuccess: s.bSuccess,
                          image: e,
                          uploadResult: s.result,
                        });
                    }
                    return l;
                  })(this.GetUploadImages(), a, e, t, this.m_cancel.token)),
            i
          );
        }
        CancelAllUploads() {
          var e;
          null === (e = this.m_cancel) ||
            void 0 === e ||
            e.cancel((0, a.we)("#ImageUpload_CancelRequest"));
        }
      }
      function d(e, t, s) {
        if (((null != e && null != e) || (e = t), !s || 0 === s.length))
          return e;
        for (const t of s) if (a.A0.IsELanguageValidInRealm(e, t)) return e;
        for (const e of s) if (a.A0.IsELanguageValidInRealm(t, e)) return t;
        return s.includes(i.TU.k_ESteamRealmGlobal) ? n.Bhc : n.ZLm;
      }
      function g(e, t = n.Bhc) {
        let s = e.lastIndexOf(".");
        -1 != s && (e = e.slice(0, s).toLowerCase());
        let i = null,
          a = 0;
        e.endsWith("korean") && ((i = n.Pn1), (a = 6));
        for (let t = n.Bhc; t < n.bP9; ++t) {
          const s = (0, n.wwZ)(t);
          if (s.length <= a) continue;
          if (e.endsWith(s) && e.length > s.length + 2) {
            const n = e[e.length - s.length - 1];
            /\p{Alphabetic}|\p{Number}/u.test(n) || ((i = t), (a = s.length));
          }
          const r = (0, n.LgB)(t);
          r.length <= a || (e.endsWith(r) && ((i = t), (a = r.length)));
        }
        return {
          language: null != i ? i : t,
          baseFilename:
            a > 0
              ? ((r = e.substring(0, e.length - a)), r.replace(/[\s_-]+$/g, ""))
              : e,
        };
        var r;
      }
    },
    86355: (e, t, s) => {
      "use strict";
      s.d(t, { mr: () => _, n9: () => C, pU: () => I });
      var i = s(34629),
        n = s(37085),
        a = s(41735),
        r = s.n(a),
        o = s(14947),
        l = s(90626),
        h = s(17720),
        p = s(4434),
        c = s(81393),
        d = s(6144),
        g = s(73745),
        u = s(78327);
      class m {
        constructor() {
          (this.m_mapClanToImages = new Map()),
            (this.m_mapClanImageLoadPromises = new Map()),
            (this.m_imageListChangeCallback = new Map()),
            (this.m_mapClanImageLoadState = new Map()),
            (this.m_mapImageIDToResolution = new Map()),
            (this.m_vecClanImageDragListener = new Array()),
            (0, o.Gn)(this),
            "dev" == u.TS.WEB_UNIVERSE && (window.g_ClanImageStore = this);
        }
        BHasImageResolution(e) {
          return this.m_mapImageIDToResolution.has(e.imageid);
        }
        GetImageResolution(e) {
          return this.m_mapImageIDToResolution.get(e.imageid);
        }
        SetImageResolution(e, t) {
          this.m_mapImageIDToResolution.set(e.imageid, t);
        }
        GetImageListCallbackForClanAccountIDInternal(e) {
          return (
            this.m_imageListChangeCallback.has(e) ||
              this.m_imageListChangeCallback.set(e, new d.lu()),
            this.m_imageListChangeCallback.get(e)
          );
        }
        GetImageListCallbackForClanAccountID(e) {
          return this.GetImageListCallbackForClanAccountIDInternal(e);
        }
        AddClanImageDragListener(e) {
          -1 == this.m_vecClanImageDragListener.indexOf(e) &&
            this.m_vecClanImageDragListener.push(e);
        }
        RemoveClanImageDragListener(e) {
          let t = this.m_vecClanImageDragListener.indexOf(e);
          -1 != t && this.m_vecClanImageDragListener.splice(t, 1);
        }
        GetClanImageDragListener() {
          return this.m_vecClanImageDragListener;
        }
        BHasLoadedClanImages(e) {
          return this.m_mapClanToImages.has(e.GetAccountID());
        }
        async LoadClanImages(e, t, s) {
          const i = e.GetAccountID();
          if (t || !this.m_mapClanImageLoadPromises.has(i)) {
            const n = this.InternalLoadClanImages(e, t, s);
            this.m_mapClanImageLoadPromises.set(i, n);
          }
          return this.m_mapClanImageLoadPromises.get(i);
        }
        async InternalLoadClanImages(e, t, s) {
          let i = e.GetAccountID();
          if (
            ((0, c.wT)(e && 0 != i, "ClanSteamID missing:" + e),
            e && (!this.m_mapClanToImages.has(i) || t))
          ) {
            let t,
              n = {};
            const a =
              u.TS.COMMUNITY_BASE_URL +
              "/gid/" +
              e.ConvertTo64BitString() +
              "/getimages/";
            if (
              ((t = await r().get(a, {
                params: n,
                withCredentials: !0,
                cancelToken: s,
              })),
              t)
            ) {
              for (let e of t.data.images) e.clanAccountID = i;
              (0, o.h5)(() => {
                this.m_mapClanImageLoadState.set(i, { loaded: !0 }),
                  this.m_mapClanToImages.set(i, t.data.images),
                  this.GetImageListCallbackForClanAccountIDInternal(i).Dispatch(
                    t.data.images,
                  );
              });
            }
          }
          return this.m_mapClanToImages.get(i);
        }
        GetLoadState(e) {
          return this.m_mapClanImageLoadState.get(e.GetAccountID());
        }
        GetClanImages(e) {
          return this.GetClanImagesByAccount(e.GetAccountID());
        }
        GetClanImagesByAccount(e) {
          let t = this.m_mapClanToImages.get(e);
          return t || new Array();
        }
        GetFilteredClanImages(e, t) {
          let s = I.GetClanImages(e);
          return this.GetFilteredClanImagesList(s, t);
        }
        GetFilteredClanImagesList(e, t) {
          if (t && t.trim().length > 0) {
            t = t.trim().toLowerCase();
            let s = new Array();
            for (let i of e)
              i.file_name &&
                i.file_name.toLowerCase().indexOf(t) >= 0 &&
                s.push(i);
            return s;
          }
          return e;
        }
        GetClanImageByID(e, t) {
          let s = e.GetAccountID(),
            i = this.m_mapClanToImages.get(s);
          return i ? i.find((e) => e.imageid == t) : void 0;
        }
        GetClanImageByURL(e, t) {
          let s = e.GetAccountID(),
            i = this.m_mapClanToImages.get(s);
          return i ? i.find((e) => e.thumb_url == t || e.url == t) : void 0;
        }
        GetClanImageByFile(e, t) {
          let s = e.GetAccountID(),
            i = this.m_mapClanToImages.get(s);
          return i ? i.find((e) => e.file_name == t.name) : void 0;
        }
        GetClanImageByImageHash(e, t) {
          let s = e.GetAccountID(),
            i = this.m_mapClanToImages.get(s);
          return i ? i.find((e) => e.image_hash == t) : void 0;
        }
        async DeleteClanImageByID(e, t) {
          let s = { sessionid: (0, u.KC)(), imageid: t },
            i = e.GetAccountID(),
            a = await r().get(
              u.TS.COMMUNITY_BASE_URL +
                "/gid/" +
                e.ConvertTo64BitString() +
                "/deleteimage/",
              { params: s },
            );
          if (!a || 200 != a.status || a.data.success != n.R) return a.data;
          let o = this.m_mapClanToImages.get(i);
          if (o) {
            let e = o.findIndex((e, s, i) => e.imageid == t);
            e >= 0 &&
              (o.splice(e, 1),
              this.GetImageListCallbackForClanAccountIDInternal(i).Dispatch([
                ...o,
              ]));
          }
          return a.data;
        }
        async DeleteClanImage(e, t) {
          return this.DeleteClanImageByID(e, t.imageid);
        }
      }
      (0, i.Cg)([o.sH], m.prototype, "m_mapClanToImages", void 0),
        (0, i.Cg)([o.sH], m.prototype, "m_mapClanImageLoadState", void 0);
      const I = new m();
      function C(e) {
        const [t, s] = (0, l.useState)(I.GetClanImagesByAccount(e));
        return (0, g.hL)(I.GetImageListCallbackForClanAccountID(e), s), t;
      }
      function _(e) {
        const t = h.b.InitFromClanID(e),
          s = (0, p.m)("useLoadClanImages"),
          [i, n] = (0, l.useState)(() => I.BHasLoadedClanImages(t));
        return (
          (0, l.useEffect)(() => {
            const t = h.b.InitFromClanID(e);
            return (
              I.BHasLoadedClanImages(t) ||
                I.LoadClanImages(t, !1, s.token).then(() => n(!0)),
              () => s.cancel()
            );
          }, [e, s]),
          i
        );
      }
    },
    82705: (e, t, s) => {
      "use strict";
      s.d(t, { yh: () => n });
      s(90626);
      var i = s(43261);
      function n(e) {
        return (
          e.indexOf("?") > 0 && (e = e.split("?")[0]),
          e.endsWith(".jpg") || e.endsWith(".jpeg")
            ? i.bg.iS
            : e.endsWith(".png")
              ? i.bg.dU
              : e.endsWith(".gif")
                ? i.bg.CK
                : e.endsWith(".mp4")
                  ? i.bg.nn
                  : e.endsWith(".webm")
                    ? i.bg.pJ
                    : e.endsWith(".vtt")
                      ? i.bg.k7
                      : e.endsWith(".srt")
                        ? i.bg.pi
                        : e.endsWith(".webp")
                          ? i.bg.wD
                          : void 0
        );
      }
    },
    63556: (e, t, s) => {
      "use strict";
      s.d(t, { E: () => d, O: () => c });
      var i = s(34629),
        n = s(14947),
        a = s(65946),
        r = s(22837),
        o = s(62490),
        l = s(6419),
        h = s(78327),
        p = s(91986);
      class c {
        GetCallback() {
          return this.m_callback;
        }
        GetCurEditLanguage() {
          return this.m_eCurLang;
        }
        SetCurEditLanguage(e) {
          return (
            this.m_eCurLang != e &&
            ((this.m_eCurLang = e), this.GetCallback().Dispatch(e), !0)
          );
        }
        SetHasLanguage(e) {
          e.forEach((e, t) => {
            this.m_rgHasData[t] != e && (this.m_rgHasData[t] = e);
          });
        }
        BHasLanguageData(e) {
          return this.m_rgHasData[e];
        }
        GetHasLocalizationContext() {
          return this.m_bHasLocalizationContext;
        }
        SetHasLocalizationContext(e) {
          e != this.m_bHasLocalizationContext &&
            (this.m_bHasLocalizationContext = e);
        }
        static Get() {
          return (
            c.s_globalSingletonStore ||
              ((c.s_globalSingletonStore = new c()),
              "dev" == h.TS.WEB_UNIVERSE &&
                (window.DUS = c.s_globalSingletonStore)),
            c.s_globalSingletonStore
          );
        }
        constructor() {
          (this.m_eCurLang = (0, r.sfN)(h.TS.LANGUAGE)),
            (this.m_rgHasData = (0, o.$Y)([], r.bP9, !1)),
            (this.m_bHasLocalizationContext = !1),
            (this.m_callback = new p.l()),
            (0, n.Gn)(this);
        }
      }
      function d() {
        return (0, a.q3)(() => c.Get().GetCurEditLanguage());
      }
      (0, i.Cg)([n.sH], c.prototype, "m_eCurLang", void 0),
        (0, i.Cg)([n.sH], c.prototype, "m_rgHasData", void 0),
        (0, i.Cg)([n.sH], c.prototype, "m_bHasLocalizationContext", void 0),
        (0, i.Cg)([l.o], c.prototype, "GetCurEditLanguage", null),
        (0, i.Cg)([l.o], c.prototype, "SetCurEditLanguage", null),
        (0, i.Cg)([n.XI.bound], c.prototype, "SetHasLanguage", null),
        (0, i.Cg)([l.o], c.prototype, "BHasLanguageData", null);
    },
    9239: (e, t, s) => {
      "use strict";
      s.d(t, { I: () => ue });
      var i = s(34629),
        n = s(7850),
        a = s(41735),
        r = s.n(a),
        o = s(75844),
        l = s(90626),
        h = s(79821),
        p = s(88997),
        c = s(70995),
        d = s(17720),
        g = s(17267),
        u = s(86355),
        m = s(73745),
        I = s(86328),
        C = s(81962),
        _ = s(10820),
        v = s(14947),
        f = s(77700),
        x = s.n(f),
        b = s(52038);
      class S extends l.Component {
        constructor(e) {
          super(e),
            (this.m_rgCurrentMatches = []),
            (this.m_mapMatchByKey = new Map()),
            (this.containerRef = l.createRef()),
            (this.m_hMobxSearchDisposer = (0, v.fm)(async () => {
              await this.UpdateSearchResults(this.props.strSearch),
                this.forceUpdate();
            })),
            (this.state = { selectedIndex: void 0 });
        }
        OnKeyDown(e) {
          this.BHandleKeyPress(e.keyCode) && e.preventDefault();
        }
        BHandleKeyPress(e) {
          switch (e) {
            case I.Oy:
              this.SetSelectedIndexDelta(-1);
              break;
            case I.BH:
              this.SetSelectedIndexDelta(1);
              break;
            case I.po:
              this.SetSelectedIndex(0);
              break;
            case I.o7:
              this.SetSelectedIndexDelta(-this.GetPageSize());
              break;
            case I.xF:
              this.SetSelectedIndex(-1);
              break;
            case I.v3:
              this.SetSelectedIndexDelta(this.GetPageSize());
              break;
            case I.wd:
            case I.$R:
              if (void 0 === this.state.selectedIndex)
                return (
                  this.props.onSuggestionSelected(this.props.strSearch), !1
                );
              this.ChooseSuggestion(
                this.m_rgCurrentMatches[this.state.selectedIndex],
              );
              break;
            case I.zV:
              this.props.onSuggestionSelected(this.props.strSearch);
              break;
            default:
              return !1;
          }
          return !0;
        }
        GetPageSize() {
          let e = this.containerRef.current,
            t = e && e.firstElementChild,
            s = this.m_rgCurrentMatches.length;
          if (
            (s > this.getMaxMatches() && (s = this.getMaxMatches()), t && s)
          ) {
            let e = t.scrollHeight / s,
              i = t.clientHeight / e;
            return Math.max(1, Math.floor(i));
          }
          return 5;
        }
        ChooseSuggestion(e) {
          this.props.onSuggestionSelected(
            this.props.strSearch,
            e ? this.getSelection(e) : void 0,
          );
        }
        SetSelectedIndexDelta(e) {
          void 0 !== this.state.selectedIndex
            ? this.SetSelectedIndex(this.state.selectedIndex + e)
            : this.SetSelectedIndex(1 === e ? 0 : e);
        }
        SetSelectedIndex(e) {
          if (!this.m_rgCurrentMatches.length) return;
          let t = this.m_rgCurrentMatches.length;
          t > this.getMaxMatches() && (t = this.getMaxMatches()),
            (e %= t) < 0 && (e += t),
            this.setState({ selectedIndex: e });
        }
        FindKeyIndex(e) {
          if (!this.m_mapMatchByKey.size && this.m_rgCurrentMatches.length)
            for (let e = 0; e < this.m_rgCurrentMatches.length; e++)
              this.m_mapMatchByKey.set(
                this.getKey(this.m_rgCurrentMatches[e]),
                e,
              );
          return this.m_mapMatchByKey.get(e);
        }
        OnClickSuggestion(e) {
          let t = this.FindKeyIndex(e);
          void 0 !== t && this.ChooseSuggestion(this.m_rgCurrentMatches[t]);
        }
        OnMouseOverSuggestion(e) {
          let t = this.FindKeyIndex(e);
          void 0 !== t && this.SetSelectedIndex(t);
        }
        BindSelectedElement(e) {
          if (!e) return;
          let t = this.containerRef.current,
            s = e.containerRef.current,
            i = t && t.firstElementChild;
          s &&
            i &&
            (i.scrollTop + i.clientHeight < s.offsetTop ||
              s.offsetTop < i.scrollTop) &&
            s.scrollIntoView();
        }
        async UpdateSearchResults(e) {
          (this.m_rgCurrentMatches = await this.performSearch(e)),
            (this.m_strLastSearch = e),
            this.m_mapMatchByKey.clear(),
            this.m_rgCurrentMatches.length
              ? this.state && void 0 !== this.state.selectedIndex
                ? this.state.selectedIndex >= this.m_rgCurrentMatches.length &&
                  this.setState({ selectedIndex: 0 })
                : (void 0 ===
                    this.props.nMinimumSearchLengthBeforeAutoSelection ||
                    e.length >=
                      this.props.nMinimumSearchLengthBeforeAutoSelection) &&
                  this.setState({ selectedIndex: 0 })
              : this.state &&
                void 0 !== this.state.selectedIndex &&
                this.setState({ selectedIndex: void 0 });
        }
        componentWillUnmount() {
          this.m_hMobxSearchDisposer &&
            (this.m_hMobxSearchDisposer(),
            (this.m_hMobxSearchDisposer = void 0));
        }
        async componentDidUpdate(e) {
          this.m_strLastSearch != this.props.strSearch &&
            (await this.UpdateSearchResults(this.props.strSearch),
            this.forceUpdate());
        }
        render() {
          let e = [];
          if (this.m_rgCurrentMatches.length) {
            let t = this.getMaxMatches();
            for (
              let s = 0;
              s < Math.min(t, this.m_rgCurrentMatches.length);
              s++
            ) {
              let t = this.m_rgCurrentMatches[s],
                i = this.getKey(t),
                a = s === this.state.selectedIndex;
              e.push(
                (0, n.jsx)(
                  w,
                  {
                    matchKey: i,
                    fnOnClick: this.OnClickSuggestion,
                    fnOnMouseOver: this.OnMouseOverSuggestion,
                    bIsSelected: a,
                    ref: a ? this.BindSelectedElement : void 0,
                    children: this.renderMatch(t),
                  },
                  i,
                ),
              );
            }
            this.m_rgCurrentMatches.length > t &&
              e.push(
                this.renderTooManyMatchesMessage(
                  this.m_rgCurrentMatches.length - t,
                ),
              );
          } else {
            let t = this.renderNoMatchMessage();
            if (!t) return null;
            e.push(t);
          }
          return (0, n.jsx)("div", {
            className: x().mentionDialogPosition,
            ref: this.containerRef,
            children: (0, n.jsxs)("div", {
              className: x().mentionDialog,
              tabIndex: 0,
              onKeyDown: this.OnKeyDown,
              children: [this.renderHeader(), e],
            }),
          });
        }
      }
      (0, i.Cg)([m.oI], S.prototype, "OnKeyDown", null),
        (0, i.Cg)([m.oI], S.prototype, "OnClickSuggestion", null),
        (0, i.Cg)([m.oI], S.prototype, "OnMouseOverSuggestion", null),
        (0, i.Cg)([m.oI], S.prototype, "BindSelectedElement", null);
      class w extends l.PureComponent {
        constructor() {
          super(...arguments), (this.containerRef = l.createRef());
        }
        OnMouseOver(e) {
          this.props.fnOnMouseOver(this.props.matchKey);
        }
        OnClick(e) {
          this.props.fnOnClick(this.props.matchKey);
        }
        render() {
          return (0, n.jsx)("div", {
            className: (0, b.A)(
              x().suggestOption,
              x().mentionSearchOption,
              this.props.bIsSelected ? x().selected : "",
            ),
            onMouseEnter: this.OnMouseOver,
            onClick: this.OnClick,
            ref: this.containerRef,
            children: this.props.children,
          });
        }
      }
      (0, i.Cg)([m.oI], w.prototype, "OnMouseOver", null),
        (0, i.Cg)([m.oI], w.prototype, "OnClick", null);
      const T = S;
      var y = s(79786),
        A = s.n(y);
      const D = class extends T {
        performSearch(e) {
          return this.props.emoticonStore.SearchEmoticons(e, 10, !1);
        }
        getSelection(e) {
          return e.name;
        }
        getKey(e) {
          return e.name;
        }
        renderMatch(e) {
          return (0, n.jsxs)("div", {
            className: (0, b.A)(
              A().EmoticonSuggestion,
              e.recent ? "Recent" : "",
            ),
            children: [
              (0, n.jsxs)("div", {
                className: A().Emoticon,
                children: [
                  (0, n.jsx)(C.n, { emoticon: e.name }),
                  e.new && (0, n.jsx)(_.iD, {}),
                ],
              }),
              ":",
              e.name,
              ":",
            ],
          });
        }
        renderNoMatchMessage() {
          return null;
        }
        renderTooManyMatchesMessage(e) {
          return null;
        }
        renderHeader() {
          return null;
        }
        getMaxMatches() {
          return Number.MAX_VALUE;
        }
      };
      var L = s(34510),
        E = s.n(L),
        j = s(61859);
      class M extends T {
        performSearch(e) {
          let t = Array();
          return (
            this.props.supportBBCodes.forEach((s) => {
              s.indexOf(e) >= 0 && t.push({ name: s });
            }),
            t
          );
        }
        getSelection(e) {
          return "[" + e.name + "][/" + e.name + "]";
        }
        getKey(e) {
          return e.name;
        }
        renderMatch(e) {
          return (0, n.jsxs)(
            "div",
            {
              className: (0, b.A)(E().BBCodeSuggestion),
              children: [
                (0, n.jsx)("div", { className: E().BBCode, children: e.name }),
                "[",
                e.name,
                "]...[/",
                e.name,
                "]",
              ],
            },
            e.name,
          );
        }
        renderNoMatchMessage() {
          return (0, n.jsx)(
            "div",
            {
              className: (0, b.A)(x().mentionSearchOption, x().noMatches),
              children: (0, j.we)("#Bbcode_No_Match"),
            },
            "nomatches",
          );
        }
        renderTooManyMatchesMessage(e) {
          return null;
        }
        renderHeader() {
          return null;
        }
        getMaxMatches() {
          return Number.MAX_VALUE;
        }
      }
      class O extends l.Component {
        constructor(e) {
          super(e),
            (this.descTextAreaRef = l.createRef()),
            (this.m_bDisabled = !0),
            (this.state = {
              mentionSearch: void 0,
              activeSuggestSearchType: void 0,
            });
        }
        BindMentionDialog(e) {
          this.m_MentionDialog = null != e ? e : void 0;
        }
        OnKeyDown(e) {
          this.state.activeSuggestSearchType &&
          this.m_MentionDialog &&
          !e.shiftKey &&
          !e.ctrlKey &&
          this.m_MentionDialog.BHandleKeyPress(e.keyCode)
            ? e.preventDefault()
            : (e.keyCode != I.Dh && e.keyCode != I.jt) ||
              (this.m_iMentionSearchCancelledOffset = void 0);
        }
        FindMatchOpener(e, t, s) {
          for (let i = s - 1; i >= 0; i--) {
            if (t[i] == e) return i;
            if (" " == t[i] || "\n" == t[i]) break;
          }
        }
        ReplaceSuggestedText(e, t) {
          const s = this.descTextAreaRef.current;
          if (!s) return;
          let i = s.selectionStart,
            n = s.value,
            a = this.FindMatchOpener(e, n, i);
          if (
            (void 0 === a && "@" == e && (a = this.FindMatchOpener("＠", n, i)),
            void 0 !== a)
          ) {
            let e = n.substr(0, a);
            (e += t), (i >= n.length || " " != n[i]) && (e += " ");
            let r = e.length;
            (e += n.substr(i)),
              (s.value = e),
              (s.selectionStart = s.selectionEnd = r),
              this.props.fnSetText(e),
              this.FocusTextInput();
          }
        }
        OnFocus(e) {
          this.UpdateAutoSearchState();
        }
        OnKeyPress(e) {
          this.UpdateAutoSearchState();
        }
        OnClick(e) {
          this.UpdateAutoSearchState();
        }
        ClearMentionSearchState() {
          (this.m_iMentionSearchStartOffset = void 0),
            this.state.activeSuggestSearchType &&
              this.setState({
                activeSuggestSearchType: void 0,
                mentionSearch: void 0,
              });
        }
        OnEmoticonSuggestionSelected(e, t) {
          if (!t)
            return (
              (this.m_iMentionSearchCancelledOffset =
                this.m_iMentionSearchStartOffset),
              void this.ClearMentionSearchState()
            );
          this.ReplaceSuggestedText(":", ":" + t + ":"),
            this.ClearMentionSearchState();
        }
        OnBBCodeSuggestionSelected(e, t) {
          if (!t)
            return (
              (this.m_iMentionSearchCancelledOffset =
                this.m_iMentionSearchStartOffset),
              void this.ClearMentionSearchState()
            );
          this.ReplaceSuggestedText("[", t), this.ClearMentionSearchState();
        }
        FocusTextInput() {
          this.descTextAreaRef.current && this.descTextAreaRef.current.focus();
        }
        UpdateAutoSearchState() {
          let e = this.descTextAreaRef.current;
          if (!e || this.m_bDisabled) return;
          if (e.selectionStart != e.selectionEnd || !e.selectionStart)
            return void this.ClearMentionSearchState();
          let t,
            s,
            i = e.selectionStart,
            n = e.value;
          for (let e = i - 1; e >= 0; e--) {
            let i = e > 0 ? n[e - 1] : void 0;
            if (this.props.emoticonStore && ":" == n[e] && n.length > 2) {
              (i && " " != i && "\n" != i && ":" != i) ||
                ((t = e), (s = "Emoticon"));
              break;
            }
            if ("[" == n[e] && (e + 1 > n.length || "/" != n[e + 1])) {
              (i && " " != i && "\n" != i && "]" != i) ||
                ((t = e), (s = "BBCode"));
              break;
            }
            if (" " == n[e] || "\n" == n[e]) break;
          }
          if (void 0 === t || t === this.m_iMentionSearchCancelledOffset)
            return void this.ClearMentionSearchState();
          let a = n.substr(t + 1, i - t - 1);
          (this.m_iMentionSearchStartOffset = t),
            this.setState({ activeSuggestSearchType: s, mentionSearch: a });
        }
        GetTextAreaRef() {
          return this.descTextAreaRef;
        }
        GetTextAreaCurrent() {
          return this.descTextAreaRef.current;
        }
        render() {
          let e,
            {
              emoticonStore: t,
              supportBBCodes: s,
              fnSetText: i,
              ...a
            } = this.props;
          switch (this.state.activeSuggestSearchType) {
            case "Emoticon":
              t &&
                (e = (0, n.jsx)(D, {
                  emoticonStore: t,
                  strSearch: this.state.mentionSearch,
                  nMinimumSearchLengthBeforeAutoSelection: 2,
                  onSuggestionSelected: this.OnEmoticonSuggestionSelected,
                  ref: this.BindMentionDialog,
                }));
              break;
            case "BBCode":
              e = (0, n.jsx)(M, {
                supportBBCodes: s,
                strSearch: this.state.mentionSearch,
                nMinimumSearchLengthBeforeAutoSelection: 2,
                onSuggestionSelected: this.OnBBCodeSuggestionSelected,
                ref: this.BindMentionDialog,
              });
          }
          return (0, n.jsxs)(l.Fragment, {
            children: [
              e,
              (0, n.jsx)("textarea", {
                ...a,
                ref: this.descTextAreaRef,
                onKeyDown: this.OnKeyDown,
                onKeyUp: this.OnKeyPress,
                onFocus: this.OnFocus,
                onClick: this.OnClick,
              }),
            ],
          });
        }
      }
      (0, i.Cg)([m.oI], O.prototype, "BindMentionDialog", null),
        (0, i.Cg)([m.oI], O.prototype, "OnKeyDown", null),
        (0, i.Cg)([m.oI], O.prototype, "OnFocus", null),
        (0, i.Cg)([m.oI], O.prototype, "OnKeyPress", null),
        (0, i.Cg)([m.oI], O.prototype, "OnClick", null),
        (0, i.Cg)([m.oI], O.prototype, "OnEmoticonSuggestionSelected", null),
        (0, i.Cg)([m.oI], O.prototype, "OnBBCodeSuggestionSelected", null),
        (0, i.Cg)([m.oI], O.prototype, "FocusTextInput", null),
        (0, i.Cg)([m.oI], O.prototype, "GetTextAreaRef", null),
        (0, i.Cg)([m.oI], O.prototype, "GetTextAreaCurrent", null);
      var k = s(68255),
        B = s(283),
        U = s(95695),
        R = s.n(U),
        H = s(9154),
        G = s(738),
        N = s(41609),
        F = s.n(N),
        P = s(56011),
        z = s(78327),
        Y = s(44483),
        K = s(38038),
        V = s(41471),
        W = s(60155),
        X = s(48079),
        Z = s(22797),
        J = s(32754),
        Q = s(99312),
        q = s.n(Q),
        $ = s(22837),
        ee = s(43261),
        te = s(81393),
        se = s(66331),
        ie = s(29347),
        ne = s(64233),
        ae = s(28954);
      const re = (0, o.PA)((e) => {
        const {
            clanSteamID: t,
            inputClanImage: s,
            nWidth: i,
            nHeight: a,
            setImage: r,
          } = e,
          o = l.useMemo(() => ({ width: i, height: a }), [i, a]),
          [h, p] = l.useState(void 0),
          [c, u] = l.useState(Boolean(s)),
          [m, I] = l.useState(!1),
          C = (0, ae.zO)(t, "dummy"),
          _ = l.useCallback(
            async (e) => {
              if (
                (C.ClearImages(),
                e && (u(!0), await C.AddExistingClanImage(e, $.Bhc)))
              ) {
                p(e);
                const t = C.GetUploadImages()[0].IsValidAssetType(o);
                0 != t.error.length ||
                  t.needsCrop ||
                  (s && s.image_hash == e.image_hash) ||
                  r(e);
              }
              u(!1);
            },
            [C, s, r, o],
          );
        l.useEffect(() => {
          _(s);
        }, [_, s]);
        let v,
          f = "",
          x = !1;
        if (C && C.GetFilesToUpload().length > 0) {
          v = C.GetUploadImages()[0];
          const e = v.IsValidAssetType(o);
          (f = e.error), (x = e.needsCrop);
        }
        return (0, n.jsxs)(n.Fragment, {
          children: [
            c
              ? (0, n.jsx)(Z.t, {
                  size: "medium",
                  string: (0, j.we)("#Loading"),
                })
              : h &&
                (0, n.jsx)("div", {
                  className: ne.Image,
                  style: {
                    backgroundImage: `url( '${v ? v.dataUrl : h.url}' )`,
                    height: `${a}px`,
                    width: `${i}px`,
                  },
                }),
            Boolean(f) && (0, n.jsx)("p", { children: f }),
            x &&
              (0, n.jsx)(k.$n, {
                onClick: (e) => {
                  var t;
                  const s =
                    null !== (t = (0, P.uX)(e)) && void 0 !== t ? t : window;
                  let r = C.GetUploadImages()[0];
                  (0, G.pg)(
                    (0, n.jsx)(se.q, {
                      ownerWin: s,
                      uploadFile: r,
                      forceResolution: { width: i, height: a },
                      fileType: ee.bg.dU,
                    }),
                    s,
                  );
                },
                children: (0, j.we)("#BBCode_ResizeImage"),
              }),
            v &&
              v.bCropped &&
              (0, n.jsxs)(n.Fragment, {
                children: [
                  (0, n.jsx)("div", {
                    children: (0, j.we)(
                      "#ClanImagePickAndResize_UploadStatus",
                      v.status,
                    ),
                  }),
                  m
                    ? (0, n.jsx)(Z.t, {
                        string: (0, j.we)("#Uploading"),
                        size: "small",
                      })
                    : (0, n.jsx)(k.$n, {
                        onClick: async () => {
                          I(!0);
                          try {
                            const e = await C.UploadAllImages(o),
                              s = Object.values(e);
                            if (s && s.length > 0) {
                              (0, te.wT)(
                                1 == s.length,
                                "ClanImagePickForCertainSize expected size 1, got " +
                                  s.length,
                              );
                              const e = s[0].bSuccess
                                  ? s[0].uploadResult
                                  : void 0,
                                i = g.zU.GetHashAndExt(null != e ? e : null),
                                n = g.zU.GetThumbHashAndExt(
                                  null != e ? e : null,
                                );
                              if (
                                (null == e ? void 0 : e.image_hash) &&
                                void 0 !== e.file_type &&
                                i &&
                                n
                              ) {
                                const s = g.zU.GenerateURLFromHashAndExt(t, i),
                                  a = g.zU.GenerateURLFromHashAndExt(t, n),
                                  o = {
                                    imageid: -11231412,
                                    image_hash: e.image_hash,
                                    thumbnail_hash: e.thumbnail_hash,
                                    file_type: e.file_type,
                                    file_name: e.file_name,
                                    clanAccountID: t.GetAccountID(),
                                    url: s,
                                    thumb_url: a,
                                    uploaded_time: Date.now() / 1e3,
                                  };
                                p(o), r(o);
                              }
                            }
                          } finally {
                            I(!1);
                          }
                        },
                        children: (0, j.we)(
                          "#ClanImagePickAndResize_UploadImage",
                        ),
                      }),
                ],
              }),
            (0, n.jsx)(k.$n, {
              onClick: (e) => {
                var t;
                const s = new d.b(z.UF.CLANSTEAMID);
                (0, G.pg)(
                  (0, n.jsx)(ie.z, {
                    clanSteamID: s,
                    fnImageSelectCallBack: (e) => _(e),
                  }),
                  null !== (t = (0, P.uX)(e)) && void 0 !== t ? t : window,
                );
              },
              children: (0, j.we)("#BBCode_ChooseImage", i, a),
            }),
          ],
        });
      });
      var oe,
        le = s(68797);
      let he = (oe = class extends l.Component {
        constructor(e) {
          var t;
          super(e),
            (this.m_cancelSignal = r().CancelToken.source()),
            (this.state = {
              formattingHelp: {
                __html:
                  null !== (t = oe.s_formattingHelp.get(e.formatType)) &&
                  void 0 !== t
                    ? t
                    : "",
              },
            });
        }
        componentDidMount() {
          this.AjaxGetFormattingHelp().catch((e) => {
            this.setState((0, le.H)(e));
          });
        }
        componentWillUnmount() {
          this.m_cancelSignal.cancel(
            "FormattingHelpWidget component unmounted",
          );
        }
        static GetHelpURL(e, t) {
          return (
            z.TS.COMMUNITY_BASE_URL +
            "comment/" +
            e +
            "/formattinghelp" +
            (t ? "?ajax=1" : "")
          );
        }
        async AjaxGetFormattingHelp() {
          if ("" == this.state.formattingHelp.__html) {
            let e,
              t = { sessionid: (0, z.KC)() };
            (e = await r().get(oe.GetHelpURL(this.props.formatType, !0), {
              params: t,
              cancelToken: this.m_cancelSignal.token,
            })),
              oe.s_formattingHelp.set(this.props.formatType, e.data),
              this.setState({ formattingHelp: { __html: e.data } });
          }
        }
        render() {
          return this.state.strErrorMsg
            ? (0, n.jsxs)("div", {
                children: [
                  this.state.strErrorMsg,
                  (0, n.jsx)("br", {}),
                  this.state.errorCode,
                ],
              })
            : "" == this.state.formattingHelp.__html
              ? (0, n.jsx)(Z.t, {})
              : (0, n.jsx)(H.o0, {
                  strTitle: (0, j.we)(
                    "#EventEditor_FormattingHelp_GetHelpLink",
                  ),
                  strDescription: "",
                  closeModal: this.props.closeModal,
                  onOK: this.props.closeModal,
                  onCancel: this.props.closeModal,
                  bAlertDialog: !0,
                  className: "ModernBBStyles",
                  children: (0, n.jsx)("div", {
                    dangerouslySetInnerHTML: this.state.formattingHelp,
                  }),
                });
        }
      });
      (he.s_formattingHelp = new Map()), (he = oe = (0, i.Cg)([o.PA], he));
      var pe,
        ce = s(12611),
        de = s(74410),
        ge = s(26408);
      let ue = class extends l.Component {
        constructor(e) {
          super(e),
            (this.descAutoTextAreaRef = l.createRef()),
            (this.state = { bShowDragTarget: !1 });
        }
        componentDidMount() {
          u.pU.AddClanImageDragListener(this.ClanImageDragListener);
        }
        componentWillUnmount() {
          u.pU.RemoveClanImageDragListener(this.ClanImageDragListener);
        }
        ClanImageDragListener(e, t) {
          this.state.bShowDragTarget != t &&
            this.setState({ bShowDragTarget: t });
        }
        onFocus(e) {
          e && e.target.select();
        }
        InsertText(e) {
          var t;
          me.replaceSelection(
            null === (t = this.GetTextAreaRef()) || void 0 === t
              ? void 0
              : t.current,
            e,
          );
        }
        OnTextAreaDropListener(e) {
          var t;
          if (
            (e.preventDefault(),
            e.stopPropagation(),
            e.dataTransfer.items && e.dataTransfer.items[0])
          ) {
            let s = e.dataTransfer.getData("text");
            if (s && s.length > 0)
              for (let e of [g.zU.GetBaseURL(), g.zU.GetBaseURLV2()])
                if (s.startsWith(e)) {
                  let i = "[img]" + ce.lw + "/" + s.substr(e.length) + "[/img]";
                  me.replaceSelection(
                    null === (t = this.GetTextAreaRef()) || void 0 === t
                      ? void 0
                      : t.current,
                    i,
                  );
                  break;
                }
          }
        }
        GetTextAreaRef() {
          var e;
          return null === (e = this.descAutoTextAreaRef.current) || void 0 === e
            ? void 0
            : e.GetTextAreaRef();
        }
        render() {
          return (0, n.jsxs)(l.Fragment, {
            children: [
              (0, n.jsx)(Ie, {
                pathToImages:
                  z.TS.COMMUNITY_CDN_URL + "public/images/sharedfiles/guides/",
                fnTextareaRef: this.GetTextAreaRef,
                emoticonStore: this.props.emoticonStore,
                supportBBCodes: this.props.limitBBCode
                  ? this.props.limitBBCode
                  : h.Kl,
                bSupportHTMLImport: this.props.bSupportHTMLImport,
                showFormatHelp: this.props.showFormatHelp,
                bEmbeddedInDialog: this.props.bEmbeddedInDialog,
                clanSteamID: this.props.clanSteamID,
              }),
              (0, n.jsx)("div", {
                className: (0, b.A)(
                  q().DescriptionCtn,
                  q().BBCodeEditorInputStyles,
                  this.state.bShowDragTarget ? q().DragTarget : "",
                  this.props.className ? this.props.className : "",
                ),
                children: (0, n.jsx)(O, {
                  cols: 90,
                  rows: this.props.nOverridesRows || 22,
                  maxLength: 64e3,
                  className: (0, b.A)(
                    q().DefaultEditor,
                    this.props.classNameForTextArea
                      ? this.props.classNameForTextArea
                      : "",
                  ),
                  placeholder: this.props.strPlaceholder,
                  ref: this.descAutoTextAreaRef,
                  value: this.props.fnGetCurText(),
                  onChange: this.props.fnOnTextChange,
                  onDrop: this.OnTextAreaDropListener,
                  emoticonStore: this.props.emoticonStore,
                  fnSetText: this.props.fnSetText,
                  supportBBCodes: this.props.limitBBCode
                    ? this.props.limitBBCode
                    : h.Kl,
                }),
              }),
            ],
          });
        }
      };
      (0, i.Cg)([m.oI], ue.prototype, "ClanImageDragListener", null),
        (0, i.Cg)([m.oI], ue.prototype, "onFocus", null),
        (0, i.Cg)([m.oI], ue.prototype, "OnTextAreaDropListener", null),
        (0, i.Cg)([m.oI], ue.prototype, "GetTextAreaRef", null),
        (ue = (0, i.Cg)([o.PA], ue));
      class me {
        static BIsFireFox() {
          return Boolean(
            new RegExp(/Firefox\/([0-9\.]+)(?:\s|$)/i).exec(
              navigator.userAgent,
            ),
          );
        }
        static replaceSelection(e, t) {
          if (!e) return;
          let s = e.selectionStart;
          e.focus(),
            me.InsertTextAtSelect(t, e),
            e.setSelectionRange(s, s + t.length);
        }
        static getSelectedString(e) {
          return e
            ? e.value.substr(
                e.selectionStart,
                e.selectionEnd - e.selectionStart,
              )
            : "";
        }
        static wrapBBCode(e, t, s) {
          if (!s) return;
          let i = me.getSelectedString(s),
            n = "";
          (n =
            0 == i.indexOf(e) && i.lastIndexOf(t) == i.length - t.length
              ? i.substr(e.length, i.length - e.length - t.length)
              : e + i + t),
            me.replaceSelection(s, n);
        }
        static append(e, t) {
          t &&
            (t.focus(),
            t.setSelectionRange(t.value.length, t.value.length),
            me.InsertTextAtSelect(e, t));
        }
        static ClearTextArea(e) {
          if (e) {
            e.focus();
            const t = 0,
              s = e.value.length;
            if (t !== s) {
              e.setRangeText
                ? e.setRangeText("", t, s, "preserve")
                : (e.value = "");
              const i = new Event("input", { bubbles: !0 });
              e.dispatchEvent(i);
            }
            e.focus();
          }
        }
        static overwrite(e, t) {
          t && (me.ClearTextArea(t), me.InsertTextAtSelect(e, t));
        }
        static InsertTextAtSelect(e, t) {
          const s = t.selectionStart,
            i = t.selectionEnd;
          if (null !== s && null !== i) {
            t.setRangeText
              ? t.setRangeText(e, s, i, "preserve")
              : (t.value = t.value.slice(0, s) + e + t.value.slice(i));
            const n = new Event("input", { bubbles: !0 });
            t.dispatchEvent(n),
              (t.selectionStart = t.selectionEnd = s + e.length);
          }
          t.focus();
        }
      }
      let Ie = class extends l.Component {
        constructor() {
          super(...arguments), (this.m_linkPopupRef = l.createRef());
        }
        onBold() {
          var e;
          me.wrapBBCode(
            "[b]",
            "[/b]",
            null === (e = this.props.fnTextareaRef()) || void 0 === e
              ? void 0
              : e.current,
          );
        }
        onItalics() {
          var e;
          me.wrapBBCode(
            "[i]",
            "[/i]",
            null === (e = this.props.fnTextareaRef()) || void 0 === e
              ? void 0
              : e.current,
          );
        }
        onUnderline() {
          var e;
          me.wrapBBCode(
            "[u]",
            "[/u]",
            null === (e = this.props.fnTextareaRef()) || void 0 === e
              ? void 0
              : e.current,
          );
        }
        onStrikeThrough() {
          var e;
          me.wrapBBCode(
            "[strike]",
            "[/strike]",
            null === (e = this.props.fnTextareaRef()) || void 0 === e
              ? void 0
              : e.current,
          );
        }
        onHeader() {
          var e;
          me.wrapBBCode(
            "[h1]",
            "[/h1]",
            null === (e = this.props.fnTextareaRef()) || void 0 === e
              ? void 0
              : e.current,
          );
        }
        onHeader2() {
          var e;
          me.wrapBBCode(
            "[h2]",
            "[/h2]",
            null === (e = this.props.fnTextareaRef()) || void 0 === e
              ? void 0
              : e.current,
          );
        }
        onHeader3() {
          var e;
          me.wrapBBCode(
            "[h3]",
            "[/h3]",
            null === (e = this.props.fnTextareaRef()) || void 0 === e
              ? void 0
              : e.current,
          );
        }
        onUnorderedList() {
          this.handleList("list");
        }
        onOrderedList() {
          this.handleList("olist");
        }
        handleList(e) {
          var t;
          let s =
            null === (t = this.props.fnTextareaRef()) || void 0 === t
              ? void 0
              : t.current;
          if (!s) return;
          let i = "[" + e + "]\n",
            n = "[/" + e + "]";
          if (s.selectionStart == s.selectionEnd)
            me.wrapBBCode(i + "[*]", "\n" + n, s);
          else {
            let e =
              i +
              me
                .getSelectedString(s)
                .split("\n")
                .map((e) => (e.match(/\*+\s/) ? "[*]" : "[*] ") + e)
                .join("\n") +
              "\n" +
              n;
            me.replaceSelection(s, e);
          }
        }
        OnAddLink(e) {
          var t;
          const s = this.props.fnTextareaRef();
          s &&
            (0, G.HT)(
              (0, n.jsx)(ve, { textareaRef: s }),
              null !== (t = (0, P.uX)(e)) && void 0 !== t ? t : window,
            );
        }
        ShowHelpDialog(e) {
          var t;
          this.props.showFormatHelp &&
            (0, G.HT)(
              (0, n.jsx)(he, { formatType: this.props.showFormatHelp }),
              null !== (t = (0, P.uX)(e)) && void 0 !== t ? t : window,
            );
        }
        OnConvertHTMLToBBCodeDialog(e) {
          var t;
          const s = this.props.fnTextareaRef();
          if (!s) return;
          const i = null !== (t = (0, P.uX)(e)) && void 0 !== t ? t : window;
          (0, G.HT)((0, n.jsx)(we, { ownerWindow: i, textareaRef: s }), i);
        }
        OnOpenYoutubeDialog(e) {
          var t;
          const s = this.props.fnTextareaRef();
          if (!s) return;
          let i = z.TS.IMG_URL + "applications/community/";
          (0, G.HT)(
            (0, n.jsx)(_e, { textareaRef: s, pathToImages: i }),
            null !== (t = (0, P.uX)(e)) && void 0 !== t ? t : window,
          );
        }
        OnOpenImageDialog(e) {
          var t;
          const s = this.props.fnTextareaRef();
          s &&
            (0, G.HT)(
              (0, n.jsx)(fe, { textareaRef: s }),
              null !== (t = (0, P.uX)(e)) && void 0 !== t ? t : window,
            );
        }
        OnOpenSpeakerDialog(e) {
          var t;
          const s = this.props.fnTextareaRef(),
            i = this.props.clanSteamID;
          s &&
            i &&
            (0, G.pg)(
              (0, n.jsx)(xe, { clanSteamID: i, textareaRef: s }),
              null !== (t = (0, P.uX)(e)) && void 0 !== t ? t : window,
            );
        }
        OnEmoticonSelected(e, t = !1) {
          var s, i, n;
          let a = `ː${e}ː`;
          me.replaceSelection(
            null === (s = this.props.fnTextareaRef()) || void 0 === s
              ? void 0
              : s.current,
            a,
          ),
            null ===
              (n =
                null === (i = this.props.fnTextareaRef()) || void 0 === i
                  ? void 0
                  : i.current) ||
              void 0 === n ||
              n.focus();
        }
        BSupports(e) {
          return this.props.supportBBCodes.findIndex((t) => t == e) >= 0;
        }
        render() {
          const {
            showFormatHelp: e,
            bEmbeddedInDialog: t,
            bSupportHTMLImport: s,
            pathToImages: i,
          } = this.props;
          let a;
          return (
            e &&
              (a = t
                ? (0, n.jsx)("span", {
                    className: (0, b.A)("ttip", q().ActionGetHelp),
                    children: (0, n.jsx)(J.he, {
                      toolTipContent: (0, j.we)(
                        "#EventEditor_FormattingHelp_GetHelpLink",
                      ),
                      children: (0, n.jsxs)("a", {
                        href: he.GetHelpURL(e, !1),
                        target: z.TS.IN_CLIENT ? void 0 : "_blank",
                        children: [
                          (0, n.jsx)("img", { src: i + "/action_help.png" }),
                          " ",
                          (0, j.we)("#EventEditor_FormattingHelp_GetHelpLink"),
                        ],
                      }),
                    }),
                  })
                : (0, n.jsx)("span", {
                    onClick: this.ShowHelpDialog,
                    className: (0, b.A)("ttip", q().ActionGetHelp),
                    children: (0, n.jsxs)(J.he, {
                      toolTipContent: (0, j.we)(
                        "#EventEditor_FormattingHelp_GetHelpLink",
                      ),
                      children: [
                        (0, n.jsx)("img", { src: i + "/action_help.png" }),
                        " ",
                        (0, j.we)("#EventEditor_FormattingHelp_GetHelpLink"),
                      ],
                    }),
                  })),
            (0, n.jsxs)("div", {
              className: q().TextEditorToolBarContainer,
              children: [
                this.BSupports("b") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.onBold,
                    tooltip: (0, j.we)("#Editor_Bold"),
                    imgURL: this.props.pathToImages + "/format_bold.png",
                  }),
                this.BSupports("u") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.onUnderline,
                    tooltip: (0, j.we)("#Editor_Underline"),
                    imgURL: this.props.pathToImages + "/format_underline.png",
                  }),
                this.BSupports("i") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.onItalics,
                    tooltip: (0, j.we)("#Editor_Italics"),
                    imgURL: this.props.pathToImages + "/format_italic.png",
                  }),
                this.BSupports("strike") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.onStrikeThrough,
                    tooltip: (0, j.we)("#Editor_StrikeThrough"),
                    imgURL: this.props.pathToImages + "/format_strike.png",
                  }),
                Boolean(this.BSupports("url") && !t) &&
                  (0, n.jsx)(Ce, {
                    onClick: this.OnAddLink,
                    tooltip: (0, j.we)("#Editor_Link"),
                    imgURL: this.props.pathToImages + "/format_link.png",
                  }),
                this.BSupports("list") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.onUnorderedList,
                    tooltip: (0, j.we)("#Editor_Unordered"),
                    imgURL: this.props.pathToImages + "/format_bullet.png",
                  }),
                this.BSupports("olist") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.onOrderedList,
                    tooltip: (0, j.we)("#Editor_Ordered"),
                    imgURL: this.props.pathToImages + "/format_numbered.png",
                  }),
                this.BSupports("h1") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.onHeader,
                    tooltip: (0, j.we)("#Editor_Header"),
                    imgURL: this.props.pathToImages + "/format_header1.png",
                  }),
                this.BSupports("h2") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.onHeader2,
                    tooltip: (0, j.we)("#Editor_Header2"),
                    imgURL: this.props.pathToImages + "/format_header2.png",
                  }),
                this.BSupports("h3") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.onHeader3,
                    tooltip: (0, j.we)("#Editor_Header3"),
                    imgURL: this.props.pathToImages + "/format_header3.png",
                  }),
                this.BSupports("previewyoutube") &&
                  (0, n.jsx)(Ce, {
                    onClick: this.OnOpenYoutubeDialog,
                    tooltip: (0, j.we)("#EventEditor_InsertYouTube"),
                    imgURL: Y.A,
                  }),
                (0, n.jsx)("span", {
                  className: "ttip",
                  children:
                    this.props.emoticonStore &&
                    (0, n.jsx)(J.he, {
                      toolTipContent: (0, j.we)("#Editor_Emoticon"),
                      children: (0, n.jsx)(B.A, {
                        title: " ",
                        className: (0, b.A)(q().EmoteOuter),
                        disabled: !1,
                        OnEmoticonSelected: this.OnEmoticonSelected,
                        rtLastAckedNewEmoticons: Number.MAX_VALUE,
                        emoticonStore: this.props.emoticonStore,
                        useImg: this.props.pathToImages + "/format_emote.png",
                        contextOptions: {
                          bOverlapHorizontal: !0,
                          bDisablePopTop: !0,
                        },
                      }),
                    }),
                }),
                Boolean(this.BSupports("img") && !t) &&
                  (0, n.jsx)(Ce, {
                    onClick: this.OnOpenImageDialog,
                    tooltip: (0, j.we)("#EventEditor_InsertImage"),
                    imgURL: this.props.pathToImages + "/insert_img.png",
                  }),
                Boolean(
                  z.iA.is_support &&
                    this.props.clanSteamID &&
                    this.BSupports("speaker"),
                ) &&
                  (0, n.jsx)(Ce, {
                    onClick: this.OnOpenSpeakerDialog,
                    tooltip: (0, j.we)("#EventEditor_AddSpeaker"),
                    imgURL: this.props.pathToImages + "/insert_img.png",
                  }),
                Boolean(s && !t) &&
                  (0, n.jsx)(Ce, {
                    onClick: this.OnConvertHTMLToBBCodeDialog,
                    className: q().ActionImportHTML,
                    tooltip: (0, j.we)("#EventEditor_ImportFromHTML_ttip"),
                    children: (0, j.we)("#EventEditor_ImportHTML"),
                  }),
                a,
              ],
            })
          );
        }
      };
      function Ce(e) {
        return (0, n.jsx)("span", {
          onClick: e.onClick,
          className: e.className,
          children: (0, n.jsxs)(J.he, {
            toolTipContent: e.tooltip,
            className: "ttip",
            children: [
              Boolean(e.imgURL) && (0, n.jsx)("img", { src: e.imgURL }),
              e.children,
            ],
          }),
        });
      }
      (0, i.Cg)([m.oI], Ie.prototype, "onBold", null),
        (0, i.Cg)([m.oI], Ie.prototype, "onItalics", null),
        (0, i.Cg)([m.oI], Ie.prototype, "onUnderline", null),
        (0, i.Cg)([m.oI], Ie.prototype, "onStrikeThrough", null),
        (0, i.Cg)([m.oI], Ie.prototype, "onHeader", null),
        (0, i.Cg)([m.oI], Ie.prototype, "onHeader2", null),
        (0, i.Cg)([m.oI], Ie.prototype, "onHeader3", null),
        (0, i.Cg)([m.oI], Ie.prototype, "onUnorderedList", null),
        (0, i.Cg)([m.oI], Ie.prototype, "onOrderedList", null),
        (0, i.Cg)([m.oI], Ie.prototype, "OnAddLink", null),
        (0, i.Cg)([m.oI], Ie.prototype, "ShowHelpDialog", null),
        (0, i.Cg)([m.oI], Ie.prototype, "OnConvertHTMLToBBCodeDialog", null),
        (0, i.Cg)([m.oI], Ie.prototype, "OnOpenYoutubeDialog", null),
        (0, i.Cg)([m.oI], Ie.prototype, "OnOpenImageDialog", null),
        (0, i.Cg)([m.oI], Ie.prototype, "OnOpenSpeakerDialog", null),
        (0, i.Cg)([m.oI], Ie.prototype, "OnEmoticonSelected", null),
        (Ie = (0, i.Cg)([o.PA], Ie));
      let _e = class extends l.Component {
        constructor() {
          super(...arguments),
            (this.state = { youtubeInput: "", alignment: de.V2.left });
        }
        OnYoutubeInsertLink() {
          const e =
            this.state.youtubeInput && (0, c.XU)(this.state.youtubeInput);
          if (e) {
            if (this.state.alignment == de.V2.summary) {
              const t =
                "https://www.youtube.com/watch?v=" +
                e.strVideoID +
                (e.nStartSeconds ? "&t=" + e.nStartSeconds : "");
              me.wrapBBCode(t, "", this.props.textareaRef.current);
            } else {
              let t =
                "[previewyoutube=" +
                e.strVideoID +
                ";" +
                this.state.alignment +
                "]";
              me.wrapBBCode(
                t,
                "[/previewyoutube]",
                this.props.textareaRef.current,
              );
            }
            this.setState({ youtubeInput: "", alignment: de.V2.left });
          } else alert((0, j.we)("#EventEditor_InsertYouTube_NoURL"));
        }
        OnUrlChange(e) {
          this.state.youtubeInput != e.target.value &&
            this.setState({ youtubeInput: e.target.value });
        }
        OnLeftSelected() {
          this.setState({ alignment: de.V2.left });
        }
        OnRightSelected() {
          this.setState({ alignment: de.V2.right });
        }
        OnFullSelected() {
          this.setState({ alignment: de.V2.full });
        }
        OnSummarySelected() {
          this.setState({ alignment: de.V2.summary });
        }
        OnOuterDivClickPassDown(e) {}
        render() {
          return (0, n.jsx)(H.o0, {
            strTitle: (0, j.we)("#EventEditor_InsertYouTube"),
            strDescription: "",
            closeModal: this.props.closeModal,
            onCancel: this.props.closeModal,
            onOK: this.OnYoutubeInsertLink,
            strOKButtonText: (0, j.we)("#EventEditor_InsertYouTube"),
            className: q().BBCodeEditorInputStyles,
            children: (0, n.jsxs)("div", {
              className: q().YouTubeInput,
              children: [
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, j.we)("#EventEditor_InsertYouTube_URL"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          className: "DialogInput DialogTextInputBase",
                          ref: (e) => {
                            null == e || e.focus();
                          },
                          type: "text",
                          value: this.state.youtubeInput,
                          onChange: this.OnUrlChange,
                          placeholder: (0, j.we)(
                            "#EventEditor_InsertYouTube_Placholder",
                          ),
                        }),
                      }),
                    ],
                  }),
                }),
                (0, n.jsxs)("div", {
                  className: "DialogInputLabelGroup",
                  children: [
                    (0, n.jsx)("div", {
                      className: "DialogLabel",
                      children: (0, j.we)(
                        "#EventEditor_InsertYouTube_Position",
                      ),
                    }),
                    (0, n.jsxs)("div", {
                      className: q().YouTubePreviewInsertOption,
                      onClick: this.OnOuterDivClickPassDown,
                      children: [
                        (0, n.jsx)("input", {
                          type: "radio",
                          name: "YouTubePreviewInsertType",
                          id: de.V2.left,
                          value: de.V2.left,
                          checked: this.state.alignment == de.V2.left,
                          onChange: this.OnLeftSelected,
                        }),
                        (0, n.jsx)("label", {
                          htmlFor: de.V2.left,
                          children: (0, n.jsx)("span", {
                            children: (0, j.we)(
                              "#EventEditor_InsertYouTube_Left",
                            ),
                          }),
                        }),
                      ],
                    }),
                    (0, n.jsxs)("div", {
                      className: q().YouTubePreviewInsertOption,
                      onClick: this.OnOuterDivClickPassDown,
                      children: [
                        (0, n.jsx)("input", {
                          type: "radio",
                          name: "YouTubePreviewInsertType",
                          id: de.V2.right,
                          value: de.V2.right,
                          checked: this.state.alignment == de.V2.right,
                          onChange: this.OnRightSelected,
                        }),
                        (0, n.jsx)("label", {
                          htmlFor: de.V2.right,
                          children: (0, n.jsx)("span", {
                            children: (0, j.we)(
                              "#EventEditor_InsertYouTube_Right",
                            ),
                          }),
                        }),
                      ],
                    }),
                    (0, n.jsxs)("div", {
                      className: q().YouTubePreviewInsertOption,
                      onClick: this.OnOuterDivClickPassDown,
                      children: [
                        (0, n.jsx)("input", {
                          type: "radio",
                          name: "YouTubePreviewInsertType",
                          id: de.V2.full,
                          value: de.V2.full,
                          checked: this.state.alignment == de.V2.full,
                          onChange: this.OnFullSelected,
                        }),
                        (0, n.jsx)("label", {
                          htmlFor: de.V2.full,
                          children: (0, n.jsx)("span", {
                            children: (0, j.we)(
                              "#EventEditor_InsertYouTube_Full",
                            ),
                          }),
                        }),
                      ],
                    }),
                    (0, n.jsxs)("div", {
                      className: q().YouTubePreviewInsertOption,
                      onClick: this.OnOuterDivClickPassDown,
                      children: [
                        (0, n.jsx)("input", {
                          type: "radio",
                          name: "YouTubePreviewInsertType",
                          id: de.V2.summary,
                          value: de.V2.summary,
                          checked: this.state.alignment == de.V2.summary,
                          onChange: this.OnSummarySelected,
                        }),
                        (0, n.jsx)("label", {
                          htmlFor: de.V2.summary,
                          children: (0, n.jsx)("span", {
                            children: (0, j.we)(
                              "#EventEditor_InsertYouTube_Summary",
                            ),
                          }),
                        }),
                      ],
                    }),
                  ],
                }),
              ],
            }),
          });
        }
      };
      (0, i.Cg)([m.oI], _e.prototype, "OnYoutubeInsertLink", null),
        (0, i.Cg)([m.oI], _e.prototype, "OnUrlChange", null),
        (0, i.Cg)([m.oI], _e.prototype, "OnLeftSelected", null),
        (0, i.Cg)([m.oI], _e.prototype, "OnRightSelected", null),
        (0, i.Cg)([m.oI], _e.prototype, "OnFullSelected", null),
        (0, i.Cg)([m.oI], _e.prototype, "OnSummarySelected", null),
        (0, i.Cg)([m.oI], _e.prototype, "OnOuterDivClickPassDown", null),
        (_e = (0, i.Cg)([o.PA], _e));
      let ve = (pe = class extends l.Component {
        constructor() {
          super(...arguments), (this.state = { textToDisplay: "", strURL: "" });
        }
        LoadFromTextArea() {
          const { textareaRef: e } = this.props;
          if (e && e.current) {
            let t = me.getSelectedString(e.current),
              s = pe.m_regExp.exec(t);
            s
              ? this.setState({ strURL: s[1], textToDisplay: s[2] })
              : this.setState({ textToDisplay: t });
          }
        }
        componentDidMount() {
          this.LoadFromTextArea();
        }
        onLinkTitleUpdate(e) {
          this.setState({ textToDisplay: e.target.value });
        }
        onLinkURLUpdate(e) {
          this.setState({ strURL: e.target.value });
        }
        onInsertLink() {
          const { strURL: e, textToDisplay: t } = this.state;
          let s = "[url=" + e + "]" + t + "[/url]";
          me.replaceSelection(this.props.textareaRef.current, s);
        }
        render() {
          return (0, n.jsx)(H.o0, {
            strTitle: (0, j.we)("#Editor_Link"),
            strDescription: "",
            closeModal: this.props.closeModal,
            onOK: this.onInsertLink,
            onCancel: this.props.closeModal,
            strOKButtonText: (0, j.we)("#EventEditor_InsertLinkURL"),
            className: q().BBCodeEditorInputStyles,
            children: (0, n.jsxs)("div", {
              className: q().EventEditorLinkInput,
              children: [
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, j.we)("#EventEditor_LinkDescription"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          type: "text",
                          onChange: this.onLinkTitleUpdate,
                          value: this.state.textToDisplay,
                          className: "DialogInput DialogTextInputBase",
                        }),
                      }),
                    ],
                  }),
                }),
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, j.we)("#EventEditor_LinkURL"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          type: "text",
                          onChange: this.onLinkURLUpdate,
                          value: this.state.strURL,
                          className: "DialogInput DialogTextInputBase",
                        }),
                      }),
                    ],
                  }),
                }),
              ],
            }),
          });
        }
      });
      (ve.m_regExp = new RegExp(/\[url=([^\]]*)\]([^\[\]]+)\[\/url\]/i)),
        (0, i.Cg)([m.oI], ve.prototype, "onLinkTitleUpdate", null),
        (0, i.Cg)([m.oI], ve.prototype, "onLinkURLUpdate", null),
        (0, i.Cg)([m.oI], ve.prototype, "onInsertLink", null),
        (ve = pe = (0, i.Cg)([o.PA], ve));
      let fe = class extends l.Component {
        constructor() {
          super(...arguments),
            (this.refFirstInput = l.createRef()),
            (this.state = { imgURL: "", anchorURL: "" });
        }
        componentDidMount() {
          var e;
          null === (e = this.refFirstInput.current) ||
            void 0 === e ||
            e.focus();
        }
        OnImageInsert() {
          const { anchorURL: e, imgURL: t } = this.state;
          let s = "",
            i = "";
          e && e.length > 0 && ((s += "[url=" + e + "]"), (i = "[/url]" + i)),
            (s += "[img]" + t),
            (i = "[/img]" + i),
            me.wrapBBCode(s, i, this.props.textareaRef.current);
        }
        OnImageURLChange(e) {
          this.state.imgURL != e.target.value &&
            this.setState({ imgURL: e.target.value });
        }
        OnAnchorURLChange(e) {
          this.state.anchorURL != e.target.value &&
            this.setState({ anchorURL: e.target.value });
        }
        render() {
          const { imgURL: e, anchorURL: t } = this.state;
          return (0, n.jsx)(H.o0, {
            strTitle: (0, j.we)("#EventEditor_InsertImage_Title"),
            strDescription: "",
            closeModal: this.props.closeModal,
            onCancel: this.props.closeModal,
            onOK: this.OnImageInsert,
            strOKButtonText: (0, j.we)("#EventEditor_InsertImage_Title"),
            className: q().BBCodeEditorInputStyles,
            children: (0, n.jsxs)("div", {
              className: q().EventEditorLinkInput,
              children: [
                (0, n.jsx)("p", {
                  children: (0, j.we)("#EventEditor_InsertImage_Desc"),
                }),
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, j.we)("#EventEditor_InsertImage_URL"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          className: "DialogInput DialogTextInputBase",
                          type: "text",
                          value: e,
                          onChange: this.OnImageURLChange,
                          placeholder: (0, j.we)(
                            "#EventEditor_InsertImage_Placeholder",
                          ),
                          ref: this.refFirstInput,
                        }),
                      }),
                    ],
                  }),
                }),
                (0, n.jsx)("div", {
                  className: "DialogInputLabelGroup",
                  children: (0, n.jsxs)("label", {
                    children: [
                      (0, n.jsx)("div", {
                        className: "DialogLabel",
                        children: (0, j.we)("#EventEditor_InsertImage_Anchor"),
                      }),
                      (0, n.jsx)("div", {
                        className: "DialogInput_Wrapper",
                        children: (0, n.jsx)("input", {
                          className: "DialogInput DialogTextInputBase",
                          type: "text",
                          value: t,
                          onChange: this.OnAnchorURLChange,
                          placeholder: (0, j.we)(
                            "#EventEditor_InsertImage_Placeholder",
                          ),
                        }),
                      }),
                    ],
                  }),
                }),
              ],
            }),
          });
        }
      };
      (0, i.Cg)([m.oI], fe.prototype, "OnImageInsert", null),
        (0, i.Cg)([m.oI], fe.prototype, "OnImageURLChange", null),
        (0, i.Cg)([m.oI], fe.prototype, "OnAnchorURLChange", null),
        (fe = (0, i.Cg)([o.PA], fe));
      const xe = (e) => {
        const [t, s] = l.useState(""),
          [i, a] = l.useState(""),
          [r, o] = l.useState(""),
          [h, c] = l.useState(""),
          [u, m] = l.useState(void 0),
          [I, C] = l.useState(void 0),
          { data: _ } = (0, K.Dv)(),
          { isLoading: v, data: f } = (0, V.js)(
            null == I ? void 0 : I.GetAccountID(),
          ),
          x = 0 != t.trim().length && 0 != h.trim().length;
        return (0, n.jsx)(H.o0, {
          strTitle: (0, j.we)("#EventEditor_AddSpeaker"),
          strDescription: (0, j.we)("#EventEditor_AddSpeaker_Desc"),
          closeModal: e.closeModal,
          onCancel: e.closeModal,
          bOKDisabled: !x,
          onOK: () => {
            let s = `[speaker name="${t.trim()}"`;
            if (
              (i.trim().length > 0 && (s += ` title="${i}"`),
              r.trim().length > 0 && (s += ` company="${r}"`),
              u)
            ) {
              s += ` photo="${ce.lw + "/" + u.clanAccountID + "/" + g.zU.GetHashAndExt(u)}"`;
            }
            I && (s += ` steamid="${I.ConvertTo64BitString()}"`),
              (s += `]${h}[/speaker]`),
              me.replaceSelection(e.textareaRef.current, s);
          },
          className: q().BBCodeEditorInputStyles,
          children: (0, n.jsxs)("div", {
            className: q().InsertSpeakerCtn,
            children: [
              (0, n.jsx)(k.pd, {
                type: "text",
                label: (0, j.we)("#EventEditor_AddSpeaker_Name"),
                value: t,
                onChange: (e) => s(e.target.value),
                focusOnMount: !0,
              }),
              (0, n.jsxs)("div", {
                className: q().TitleGroup,
                children: [
                  (0, n.jsx)(k.pd, {
                    type: "text",
                    label: (0, j.we)("#EventEditor_AddSpeaker_Title"),
                    value: i,
                    onChange: (e) => a(e.target.value),
                  }),
                  (0, n.jsx)(k.pd, {
                    type: "text",
                    label: (0, j.we)("#EventEditor_AddSpeaker_Company"),
                    value: r,
                    onChange: (e) => o(e.target.value),
                  }),
                ],
              }),
              (0, n.jsx)(J.he, {
                toolTipContent: (0, j.we)(
                  "#EventEditor_AssociateSteamAccount_ttip",
                ),
                children: (0, n.jsxs)("div", {
                  className: "DialogLabel",
                  children: [
                    (0, j.we)("#EventEditor_AssociateSteamAccount"),
                    " (?)",
                  ],
                }),
              }),
              (0, n.jsxs)("div", {
                children: [
                  v &&
                    (0, n.jsx)(Z.t, {
                      string: (0, j.we)("#Loading"),
                      size: "small",
                    }),
                  I &&
                    f &&
                    (0, n.jsxs)("a", {
                      href:
                        z.TS.COMMUNITY_BASE_URL +
                        "profiles/" +
                        I.ConvertTo64BitString(),
                      target: "_blank",
                      children: [
                        f
                          ? (0, n.jsx)("img", {
                              style: { marginRight: "8px" },
                              src: f.avatar_url,
                            })
                          : null,
                        f ? f.m_strPlayerName : null,
                      ],
                    }),
                  (0, n.jsxs)("div", {
                    className: q().AssociateRowCtn,
                    children: [
                      (0, n.jsx)(k.$n, {
                        onClick: () => C(new d.b(z.iA.steamid)),
                        children: (0, j.we)("#EventEditor_SteamAccount_addme"),
                      }),
                      (0, n.jsx)(k.$n, {
                        onClick: (e) =>
                          (0, p.lX)(
                            (0, n.jsx)(be, {
                              friends: null != _ ? _ : [],
                              setSteamID: C,
                            }),
                            e,
                          ),
                        children: (0, j.we)(
                          "#EventEditor_SteamAccount_addfriend",
                        ),
                      }),
                      (0, n.jsx)(k.$n, {
                        onClick: () => C(void 0),
                        children: (0, j.we)("#EventEditor_SteamAccount_clear"),
                      }),
                    ],
                  }),
                ],
              }),
              (0, n.jsxs)("div", {
                className: q().PhotoCtn,
                children: [
                  (0, n.jsx)("div", {
                    className: "DialogLabel",
                    children: (0, j.we)("#EventEditor_ChoosePhoto"),
                  }),
                  (0, n.jsx)(re, {
                    clanSteamID: e.clanSteamID,
                    inputClanImage: u,
                    setImage: m,
                    nWidth: 184,
                    nHeight: 184,
                  }),
                ],
              }),
              (0, n.jsxs)("div", {
                className: q().AboutCtn,
                children: [
                  (0, n.jsx)("div", {
                    className: "DialogLabel",
                    children: (0, j.we)("#EventEditor_AddSpeaker_About"),
                  }),
                  (0, n.jsx)(k.Cl, {
                    value: h,
                    onChange: (e) => c(e.target.value),
                    rows: 8,
                    cols: 80,
                    nMinHeight: 40,
                    placeholder: (0, j.we)(
                      "#EventEditor_AddSpeaker_About_Placeholder",
                    ),
                  }),
                ],
              }),
              (0, n.jsxs)("div", {
                className: q().PreviewCtn,
                children: [
                  (0, n.jsx)("div", {
                    className: "DialogLabel",
                    children: (0, j.we)("#Button_Preview"),
                  }),
                  (0, n.jsx)(X.$k, {
                    company: r,
                    name: t,
                    title: i,
                    bioString: h,
                    photo: u ? u.url : void 0,
                  }),
                ],
              }),
            ],
          }),
        });
      };
      function be(e) {
        const { friends: t, setSteamID: s } = e;
        return (0, n.jsx)("div", {
          className: q().DropDownScroll,
          children: t.map((e) =>
            (0, n.jsx)(Se, { steamid: e, setSteamID: s }, e),
          ),
        });
      }
      function Se(e) {
        const { steamid: t, setSteamID: s } = e,
          { data: i } = (0, V.js)(t);
        return (0, n.jsx)(W.kt, {
          onSelected: () => s(new d.b(t)),
          children: (0, n.jsxs)("div", {
            style: { display: "flex", alignItems: "center" },
            children: [
              i &&
                (0, n.jsx)("img", {
                  className: F().WhitelistAvatar,
                  src: i.avatar_url,
                }),
              null == i ? void 0 : i.m_strPlayerName,
            ],
          }),
        });
      }
      let we = class extends l.Component {
        constructor(e) {
          super(e),
            (this.m_isMounted = !1),
            (this.m_bAppend = !1),
            (this.state = { bPreserveNewLines: !1, strHTMLData: "" });
        }
        componentDidMount() {
          this.m_isMounted = !0;
        }
        componentWillUnmount() {
          this.m_isMounted = !1;
        }
        OnConvertAndOverriteHTML() {
          (this.m_bAppend = !1), this.ConvertBBCode();
        }
        OnConvertAndAppendHTML() {
          (this.m_bAppend = !0), this.ConvertBBCode();
        }
        async ConvertHtmlToBBCode(e, t) {
          let s = new URLSearchParams();
          s.append("content", e), s.append("preserve_newlines", t ? "1" : "0");
          return (
            await r().post(
              z.TS.COMMUNITY_BASE_URL + "/actions/ConvertHTMLToBBCode",
              s,
            )
          ).data.content;
        }
        ConvertBBCode() {
          this.setState({ bConverting: !0 }),
            this.ConvertHtmlToBBCode(
              this.state.strHTMLData,
              this.state.bPreserveNewLines,
            )
              .then((e) => {
                this.m_isMounted &&
                  (this.m_bAppend
                    ? me.append(e, this.props.textareaRef.current)
                    : me.overwrite(e, this.props.textareaRef.current),
                  this.setState({ bConverting: !1, bFinishedConverting: !0 }));
              })
              .catch((e) => {
                (0, G.pg)(
                  (0, n.jsx)(H.KG, {
                    strTitle: (0, j.we)("#EventEditor_ConvertHTML_Error"),
                    strDescription: (0, j.we)(
                      "#EventEditor_ConvertHTML_Error_Desc",
                      e.response && e.response.data ? e.response.data.msg : e,
                    ),
                    bAlertDialog: !0,
                    bDestructiveWarning: !0,
                  }),
                  this.props.ownerWindow,
                  { strTitle: (0, j.we)("#EventEditor_ConvertHTML_Error") },
                );
              });
        }
        OnCheckboxChange(e) {
          let t = e.target.checked;
          t != this.state.bPreserveNewLines &&
            this.setState({ bPreserveNewLines: t });
        }
        OnTextAreaChange(e) {
          this.setState({ strHTMLData: e.currentTarget.value });
        }
        render() {
          const { closeModal: e } = this.props;
          return this.state.bConverting
            ? (0, n.jsx)(H.o0, {
                strTitle: (0, j.we)("#EventEditor_ImportFromHTML"),
                strDescription: (0, j.we)(
                  "#EventEditor_ImportFromHTML_ConversionInProgress",
                ),
                closeModal: e,
                bAlertDialog: !0,
                onOK: e,
                onCancel: e,
                children: (0, n.jsx)(Z.t, {}),
              })
            : this.state.bFinishedConverting
              ? (0, n.jsx)(H.o0, {
                  strTitle: (0, j.we)("#EventEditor_ImportFromHTML"),
                  strDescription: (0, j.we)(
                    "#EventEditor_ImportFromHTML_ConvertFinished",
                  ),
                  closeModal: e,
                  bAlertDialog: !0,
                  onOK: e,
                  onCancel: e,
                })
              : (0, n.jsx)(H.eV, {
                  title: (0, j.we)("#EventEditor_ImportFromHTML"),
                  onOK: this.OnConvertAndOverriteHTML,
                  onCancel: e,
                  className: q().BBCodeEditorInputStyles,
                  children: (0, n.jsxs)(k.nB, {
                    children: [
                      (0, n.jsx)(k.a3, {
                        children: (0, n.jsxs)("div", {
                          className: (0, b.A)(
                            R().FlexColumnContainer,
                            q().ImportHTMLCtn,
                          ),
                          children: [
                            (0, n.jsx)("div", {
                              className: R().FlexColumnContainer,
                              children: (0, j.PP)(
                                "#EventEditor_ImportFromHTML_ConvertDescription",
                                (0, n.jsx)("a", {
                                  target: z.TS.IN_CLIENT ? void 0 : "_blank",
                                  href: "https://partner.steamgames.com/doc/marketing/event_tools/import",
                                  children: (0, j.we)(
                                    "#EventEditor_ImportFromHTML_ConvertLearn",
                                  ),
                                }),
                              ),
                            }),
                            (0, n.jsx)("textarea", {
                              value: this.state.strHTMLData,
                              placeholder: (0, j.we)(
                                "#EventEditor_ImportFromHTML_Instruction",
                              ),
                              className: q().ImportHTMLTextArea,
                              onChange: this.OnTextAreaChange,
                              ref: (e) => {
                                null == e || e.focus();
                              },
                            }),
                            (0, n.jsxs)("div", {
                              className: q().ImportHTMLCheckBoxLine,
                              children: [
                                (0, n.jsx)("input", {
                                  id: "ImportFromHTMLNewLines",
                                  type: "checkbox",
                                  checked: this.state.bPreserveNewLines,
                                  onChange: this.OnCheckboxChange,
                                }),
                                (0, n.jsxs)("label", {
                                  htmlFor: "ImportFromHTMLNewLines",
                                  children: [
                                    (0, j.we)(
                                      "#EventEditor_ImportFromHTML_PreserveNewlines",
                                    ),
                                    (0, n.jsx)(ge.o, {
                                      tooltip: (0, j.we)(
                                        "#EventEditor_ImportFromHTML_PreserveNewlines_Hint",
                                      ),
                                    }),
                                  ],
                                }),
                              ],
                            }),
                            (0, n.jsx)("div", {
                              children: (0, j.we)(
                                "#EventEditor_ImportFromHTML_ConvertToBBCode",
                              ),
                            }),
                          ],
                        }),
                      }),
                      (0, n.jsx)(k.wi, {
                        children: (0, n.jsx)(k.VQ, {
                          onCancel: () => (null == e ? void 0 : e()),
                          strOKText: (0, j.we)("#Button_Overwrite"),
                          onUpdate: this.OnConvertAndAppendHTML,
                          strUpdateText: (0, j.we)("#Button_Append"),
                        }),
                      }),
                    ],
                  }),
                });
        }
      };
      (0, i.Cg)([m.oI], we.prototype, "OnConvertAndOverriteHTML", null),
        (0, i.Cg)([m.oI], we.prototype, "OnConvertAndAppendHTML", null),
        (0, i.Cg)([m.oI], we.prototype, "OnCheckboxChange", null),
        (0, i.Cg)([m.oI], we.prototype, "OnTextAreaChange", null),
        (we = (0, i.Cg)([o.PA], we));
    },
    29347: (e, t, s) => {
      "use strict";
      s.d(t, { r: () => u, z: () => d });
      var i = s(7850),
        n = s(90626),
        a = s(68255),
        r = s(84811),
        o = s(9154),
        l = s(61859),
        h = s(22797),
        p = s(17618),
        c = s(86355);
      const d = (e) => {
          const { clanSteamID: t, fnImageSelectCallBack: s } = e,
            [d, u] = (0, n.useState)(""),
            m = (0, c.mr)(e.clanSteamID.GetAccountID()),
            I = () => e.closeModal && e.closeModal(),
            C = c.pU.GetFilteredClanImages(t, d),
            _ = (e) => {
              s(e), I();
            };
          return (0, i.jsx)(r.tH, {
            children: (0, i.jsx)(o.x_, {
              onEscKeypress: I,
              children: (0, i.jsxs)(a.UC, {
                children: [
                  (0, i.jsx)(a.Y9, {
                    children: (0, l.we)("#ClanImageChooser_Title"),
                  }),
                  (0, i.jsx)(a.nB, {
                    children: (0, i.jsxs)(a.a3, {
                      children: [
                        (0, i.jsx)("p", {
                          children: (0, l.we)("#ClanImageChooser_Desc"),
                        }),
                        (0, i.jsx)(a.pd, {
                          placeholder: (0, l.we)("#ClanImageChooser_Search"),
                          value: d,
                          onChange: (e) => u(e.currentTarget.value),
                        }),
                        (0, i.jsx)("div", {
                          className: p.ImagesOuterContainer,
                          children: m
                            ? (0, i.jsx)(h.t, {
                                size: "medium",
                                string: (0, l.we)("#Loading"),
                              })
                            : Boolean(C.length > 0)
                              ? C.map((e) =>
                                  (0, i.jsx)(
                                    g,
                                    {
                                      clanImage: e,
                                      searchStringHilight: d,
                                      fnImageClick: _,
                                    },
                                    "ci" + e.image_hash,
                                  ),
                                )
                              : Boolean(0 == d.trim().length)
                                ? (0, i.jsx)("div", {
                                    children: (0, l.we)(
                                      "#ClanImageChooser_None",
                                    ),
                                  })
                                : (0, i.jsx)("div", {
                                    children: (0, l.we)(
                                      "#EventCalendar_GameSearch_NoneFound",
                                    ),
                                  }),
                        }),
                      ],
                    }),
                  }),
                  (0, i.jsx)(a.wi, {
                    children: (0, i.jsx)(a.$n, {
                      onClick: I,
                      children: (0, l.we)("#Button_Cancel"),
                    }),
                  }),
                ],
              }),
            }),
          });
        },
        g = (e) => {
          const { clanImage: t, searchStringHilight: s, fnImageClick: n } = e;
          let a = t.file_name ? t.file_name : "",
            r = u(s, a, String(t.imageid), p.Hilight);
          return (0, i.jsxs)("div", {
            className: p.ImageContainer,
            children: [
              (0, i.jsx)("div", {
                className: p.Image,
                style: { backgroundImage: `url( '${t.thumb_url}' )` },
                onDoubleClick: () => n(t),
              }),
              (0, i.jsx)("div", {
                className: p.ImageFilename,
                title: a,
                children: r,
              }),
            ],
          });
        };
      function u(e, t, s, n) {
        let a = [];
        if (e.length > 0) {
          let r = t.toLocaleLowerCase();
          for (let o = 0; o < t.length; ) {
            let l = r.indexOf(e, o);
            if (l < 0) {
              a.push(
                (0, i.jsx)(
                  "span",
                  { children: t.substring(o) },
                  s + "_" + String(o),
                ),
              );
              break;
            }
            o < l &&
              a.push(
                (0, i.jsx)(
                  "span",
                  { children: t.substring(o, l) },
                  s + "_" + String(o),
                ),
              ),
              a.push(
                (0, i.jsx)(
                  "span",
                  { className: n, children: t.substr(l, e.length) },
                  s + "_" + String(o),
                ),
              ),
              (o = l + e.length);
          }
        } else a.push((0, i.jsx)("span", { children: t }, s + "_null"));
        return a;
      }
    },
    1909: (e, t, s) => {
      "use strict";
      s.d(t, { Ng: () => I });
      var i = s(34629),
        n = s(7850),
        a = s(75844),
        r = s(90626),
        o = s(22837),
        l = s(2160),
        h = (s(63556), s(95695)),
        p = s.n(h),
        c = s(52038),
        d = s(61859),
        g = s(91675),
        u = s(73745),
        m = s(32754);
      let I = class extends r.Component {
        GenerateLanguageOptions() {
          let e = [];
          const {
            fnFilterLanguage: t,
            fnLangHasData: s,
            fnLastUpdateRTime: i,
            fnIsLangSupported: a,
          } = this.props;
          this.props.bAllowUnsetOption &&
            e.push(
              (0, n.jsx)(
                "option",
                {
                  value: o.xPp,
                  children: (0, d.we)("#language_selection_none"),
                },
                "langpicker_unset",
              ),
            );
          let r = new Array();
          const h = this.props.realms || [l.TU.k_ESteamRealmGlobal];
          for (const e of d.A0.GetLanguageListForRealms(h)) {
            if (t && !t(e)) continue;
            const s = (0, o.LgB)(e),
              i = (0, d.we)("#Language_" + s),
              n = !(!a || !a(e));
            r.push({ eLang: e, sLocName: i, bSupported: n });
          }
          r.sort((e, t) =>
            e.bSupported != t.bSupported
              ? e.bSupported
                ? -1
                : 1
              : e.sLocName.localeCompare(t.sLocName),
          );
          let u = !1;
          for (const t of r) {
            t.bSupported != u &&
              (e.push(
                (0, n.jsx)(
                  "option",
                  {
                    className: p().SupportedGroupLabel,
                    disabled: !0,
                    children: (0, d.we)(
                      t.bSupported
                        ? "#LanguageGroup_Supported"
                        : "#LanguageGroup_Unsupported",
                    ),
                  },
                  t.bSupported ? "SupportedGroup" : "UnsupportedGroup",
                ),
              ),
              (u = t.bSupported));
            const a = s && s(t.eLang),
              r = i && i(t.eLang);
            let o = t.sLocName;
            r &&
              0 !== r &&
              ((o += " "),
              (o += (0, d.we)(
                "#Language_Last_Update",
                (0, d.$z)(r) + " @ " + (0, g.KC)(r, { bForce24HourClock: !1 }),
              ))),
              e.push(
                (0, n.jsx)(
                  "option",
                  {
                    value: t.eLang,
                    className: (0, c.A)(
                      { [p().LanguageWithContent]: a },
                      t.bSupported
                        ? p().SupportedLanguage
                        : p().UnsupportedLanguage,
                    ),
                    children: o,
                  },
                  "langpicker" + t.eLang + (a ? "_hasdata" : ""),
                ),
              );
          }
          return e;
        }
        OnLanguageChange(e) {
          const { fnOnLanguageChanged: t, selectedLang: s } = this.props;
          let i = Number.parseInt(e.currentTarget.value);
          i != s && t && t(i);
        }
        render() {
          const { selectedLang: e, bDisabled: t, strTooltip: s } = this.props;
          let i = this.GenerateLanguageOptions();
          return (0, n.jsx)(m.he, {
            toolTipContent: s,
            children: (0, n.jsx)("select", {
              value: e,
              onChange: this.OnLanguageChange,
              disabled: t,
              children: i,
            }),
          });
        }
      };
      (0, i.Cg)([u.oI], I.prototype, "OnLanguageChange", null),
        (I = (0, i.Cg)([a.PA], I));
    },
    66331: (e, t, s) => {
      "use strict";
      s.d(t, { q: () => I, t: () => C });
      var i = s(34629),
        n = s(7850),
        a = s(90626),
        r = s(73744),
        o = s(43261),
        l = s(68255),
        h = s(81416),
        p = s(9154),
        c = s(52038),
        d = s(61859),
        g = s(73745),
        u = s(50666),
        m = s(56011);
      class I extends a.Component {
        constructor() {
          super(...arguments),
            (this.state = {
              region: {
                xPosPct: 0,
                yPosPct: 0,
                widthPct:
                  (this.GetDestWidth() / this.props.uploadFile.width) * 100,
                heightPct:
                  (this.GetDestHeight() / this.props.uploadFile.height) * 100,
              },
            });
        }
        async OnCrop() {
          var e, t;
          const s = this.props.uploadFile.GetCanvasImageSource();
          s &&
            (await (async function (e, t, s, i, n, a, r, l, h) {
              return new Promise((p, c) => {
                const d = C(h);
                if (!d) return void c("Invalid format provided");
                const g = document.createElement("canvas");
                (g.width = r), (g.height = l);
                const u = 0,
                  I = 0,
                  _ = g.getContext("2d");
                null == _ || _.drawImage(t, s, i, n, a, u, I, r, l),
                  g.toBlob((t) => {
                    const s = g.toDataURL(d);
                    h !== o.bg.dU && s.startsWith("data:image/png")
                      ? c("Unable to encode into the requested file format")
                      : t
                        ? ((e.file = (0, m.pE)(t, e.filename)),
                          (e.width = r),
                          (e.height = l),
                          (e.dataUrl = s),
                          (e.uploadTime = Date.now()),
                          (e.bCropped = !0),
                          p())
                        : c("Unable to apply crop into image");
                  });
              });
            })(
              this.props.uploadFile,
              s,
              (this.state.region.xPosPct / 100) * this.props.uploadFile.width,
              (this.state.region.yPosPct / 100) * this.props.uploadFile.height,
              (this.state.region.widthPct / 100) * this.props.uploadFile.width,
              (this.state.region.heightPct / 100) *
                this.props.uploadFile.height,
              this.GetDestWidth(),
              this.GetDestHeight(),
              this.props.fileType,
            )),
            null === (t = (e = this.props).closeModal) ||
              void 0 === t ||
              t.call(e);
        }
        UpdateCrop(e, t) {
          this.setState({ region: t });
        }
        GetDestWidth() {
          const { uploadFile: e, forceResolution: t } = this.props;
          if (t) return t.width;
          const s = e.GetCurrentImageOption();
          if (!s) return 0;
          const i = r.Fj[s.artworkType].width;
          return s ? (0, r.qj)(i) : 0;
        }
        GetDestHeight() {
          const { uploadFile: e, forceResolution: t } = this.props;
          if (t) return t.width;
          const s = e.GetCurrentImageOption();
          if (!s) return 0;
          const i = r.Fj[s.artworkType].height;
          return s ? (0, r.qj)(i) : 0;
        }
        GetLargestBoxThatFits(e, t, s, i) {
          let n = s,
            a = (n * t) / Math.max(e, 1);
          return (
            a > i && ((a = i), (n = (a * e) / Math.max(t, 1))),
            { width: n, height: a }
          );
        }
        GetPreviewWindowStyle() {
          const { region: e } = this.state,
            t = this.GetLargestBoxThatFits(
              this.GetDestWidth(),
              this.GetDestHeight(),
              500,
              150,
            ),
            s = t.width,
            i = t.height,
            n = 1 / Math.max(e.widthPct / 100, 1e-4),
            a = 1 / Math.max(e.heightPct / 100, 1e-4),
            r = (this.props.uploadFile.width * e.xPosPct) / 100,
            o = (this.props.uploadFile.height * e.yPosPct) / 100;
          return {
            width: s,
            height: i,
            backgroundPosition: `${-r * ((s * n) / this.props.uploadFile.width)}px ${-o * ((i * a) / this.props.uploadFile.height)}px`,
            backgroundSize: `${100 * n}% ${100 * a}%`,
            backgroundImage: `url(${this.props.uploadFile.dataUrl})`,
          };
        }
        render() {
          const e = (this.GetDestWidth() / this.props.uploadFile.width) * 100,
            t = (this.GetDestHeight() / this.props.uploadFile.height) * 100,
            s = this.GetLargestBoxThatFits(
              this.props.uploadFile.width,
              this.props.uploadFile.height,
              800,
              500,
            );
          return (0, n.jsx)(p.x_, {
            onEscKeypress: this.props.closeModal,
            bDisableBackgroundDismiss: !0,
            children: (0, n.jsxs)("div", {
              className: (0, c.A)("DialogContent", "_DialogCenterVertically"),
              children: [
                (0, n.jsx)(l.iK, {
                  children: (0, d.we)(
                    "#ImageUpload_CropModalTitleDims",
                    this.GetDestWidth(),
                    this.GetDestHeight(),
                  ),
                }),
                (0, n.jsx)("div", {
                  className: (0, c.A)("DialogBodyText"),
                  children: (0, d.we)("#ImageUpload_CropModalDescription"),
                }),
                (0, n.jsxs)("div", {
                  className: u.CropImage,
                  style: { width: s.width, height: s.height },
                  children: [
                    (0, n.jsx)("img", {
                      style: {
                        maxWidth: "100%",
                        maxHeight: "100%",
                        objectFit: "contain",
                      },
                      src: this.props.uploadFile.dataUrl,
                    }),
                    (0, n.jsx)(h.I, {
                      bLockAspectRatio: !0,
                      bDisableLink: !0,
                      index: 0,
                      updateFn: this.UpdateCrop,
                      xPosPct: 0,
                      yPosPct: 0,
                      widthMinPct: e,
                      heightMinPct: t,
                      widthPct: e,
                      heightPct: t,
                    }),
                  ],
                }),
                (0, n.jsxs)("div", {
                  className: u.CropPreviewGroup,
                  children: [
                    (0, n.jsx)("div", {
                      className: u.CropPreviewLabel,
                      children: (0, d.we)("#ImageUpload_CropPreview"),
                    }),
                    (0, n.jsx)("div", { style: this.GetPreviewWindowStyle() }),
                  ],
                }),
                (0, n.jsx)(l.jn, {
                  onClick: this.OnCrop,
                  children: (0, d.we)("#ImageUpload_CropAndContinue"),
                }),
              ],
            }),
          });
        }
      }
      function C(e) {
        switch (e) {
          case o.bg.dU:
            return "image/png";
          case o.bg.iS:
            return "image/jpeg";
        }
      }
      (0, i.Cg)([g.oI], I.prototype, "OnCrop", null),
        (0, i.Cg)([g.oI], I.prototype, "UpdateCrop", null);
    },
    44483: (e, t, s) => {
      "use strict";
      s.d(t, { A: () => i });
      const i =
        "data:image/png;base64,";
    },
  },
]);
